...
** 4.5      25.09.00 OJ    Fixed bug in findRepresentativeOilStdRate. Replaced parameter in call to
**         EcBp_Performance_Curve.findReprYByParameterCode with
**        performance_curve.curve_parameter_code
** 4.5      26.09.00 OJ     Added : findRepresentativeChokeSizeNat
...
**     23-11-2012 musaamah ECPD-20665: Modified getCurveParamValue by changing the ELSIF condition from EcDp_Curve_Parameter.BH_PRESS to EcDp_Curve_Parameter.DH_PRESS
**     14-06-2013 musthram ECPD-22763: Modified function populateCurveParameterValues, getWaterCutPct, getGasOilRatio
**     04-09-2013 abdulmaw ECPD-25181: Modified function populateCurveParameterValues by adding EcDp_ProductionDay.getProductionDay
**      22.11.2013 kumarsur ECPD-18576: Modified populateCurveParameterValues added phase_current,ac_frequency and intake_press.
...
**     24-01-2014 leongwen ECPD-26671: Modified the functions of getCondensateGasRatio, getGasOilRatio, getWaterOilRatio, getWaterCutPct and getWaterGasRatio to return value when the rate is NOT zero or NULL,
**                                     and return zero when the rate is zero, and return NULL when the rate is NULL.
**     05-06-2014 dhavaalo ECPD-27333: Modified EcDp_Well_Theoretical.populateCurveParameterValues function to calculate Theoretical Water and Gas values correctly for day offset for Daily and sub-daily screen.
**     18-06-2014 dhavaalo ECPD-27333: Modified EcDp_Well_Theoretical.populateCurveParameterValues,getCondensateGasRatio, getGasOilRatio, getWaterOilRatio, getWaterCutPct and getWaterGasRatio function to calculate Theoretical Water and Gas values correctly for day offset for Daily and sub-daily screen.
**      12.04.2016 jainnraj ECPD-33875: Modified populateCurveParameterValues to modify all the curve methods using pwel_day_status.avg_annulus_press to pwel_day_status.avg_annulus
**     17.03.2017 mishrdha ECPD-36124: Modified getCurveParamValue to support Curve Parameter AVG_DH_PUMP_POWER
**     16.08.2017 shindani ECPD-31708: Modified getCurveParamValue to support Curve Parameter AVG_DP_VENTURI.
*****************************************************************/
...
BEGIN
  ...
  ln_gasrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,ecdp_phase.GAS,lr_params,p_calc_method);

  IF ln_gasrate > 0 THEN
    ln_condrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,ecdp_phase.CONDENSATE,lr_params,p_calc_method);
...
BEGIN
   ...
   ln_oilrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION, p_phase,lr_params,p_calc_method);
...
BEGIN
   ...
   ln_oilrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,ecdp_phase.OIL,lr_params,p_calc_method);

   IF ln_oilrate > 0 THEN
     ln_waterrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,ecdp_phase.WATER,lr_params,p_calc_method);
...
BEGIN
    ...
    ln_liqrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,p_phase,lr_params,p_calc_method);
    ln_waterrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,ecdp_phase.WATER,lr_params,p_calc_method);
...
BEGIN
   ...
   ln_gasrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,p_phase,lr_params,p_calc_method);
   ln_waterrate := Ecdp_Well_Theoretical.getCurveStdRate(p_object_id,EcDp_ProductionDay.getProductionDay('WELL', p_object_id, p_daytime),EcDp_Curve_Purpose.PRODUCTION,ecdp_phase.WATER,lr_params,p_calc_method);
...
BEGIN
   ...
   ELSIF p_param_code=EcDp_Curve_Parameter.AVG_DH_PUMP_POWER THEN
	  ln_param_value:=p_param_values.avg_dh_pump_power;
   ELSIF p_param_code=EcDp_Curve_Parameter.AVG_DP_VENTURI THEN
      ln_param_value:=p_param_values.avg_dp_venturi;
   ELSE
      ln_param_value:=NULL;
   END IF;
...
BEGIN
   ...
   lr_pwel_day_status_rec := ec_pwel_day_status.row_by_pk(p_object_id,ld_prod_daytime);
     ...
     lr_params.annulus_press     := lr_pwel_day_status_rec.annulus_press;
     ...
     lr_params.avg_dh_pump_power := lr_pwel_day_status_rec.avg_dh_pump_power;

   ELSIF p_class_name = 'PWEL_SUB_DAY_STATUS' THEN
     lr_pwel_sub_day_status_rec := ec_pwel_sub_day_status.row_by_pk(p_object_id,p_daytime);
     ...
     lr_params.avg_dh_pump_power := lr_pwel_sub_day_status_rec.avg_dh_pump_power;
