...
--Round trip time using single RTT column
...
-- Description    : Route days for carrier utilisation chart
--
-- Preconditions  :
-- Postconditions :
--
...
--
---------------------------------------------------------------------------------------------------
FUNCTION getRouteDays(p_carrier_id VARCHAR2, p_daytime DATE, p_storage_id VARCHAR2, p_port_id VARCHAR2)
RETURN NUMBER
IS

lv_round_trip_time NUMBER;

BEGIN

  SELECT cpa.round_trip_time
    INTO lv_round_trip_time
    FROM carrier_port_acc cpa
   WHERE cpa.object_id = p_carrier_id
     AND cpa.port_id = p_port_id
     AND ec_stor_version.op_fcty_class_1_id(p_storage_id, p_daytime, '<=') = cpa.fcty_class_1_id
     AND daytime <= p_daytime
     AND nvl(end_date, p_daytime + 1 ) >= p_daytime;

  RETURN lv_round_trip_time;
...
-- Description    : Returns the carrier_id
--
-- Preconditions  :
-- Postconditions :
--
...
--
---------------------------------------------------------------------------------------------------
FUNCTION get_carrier_id(p_carrier_name VARCHAR2)
RETURN VARCHAR2
IS
...
SELECT min(object_id) INTO lv_carrier_id FROM ov_carrier WHERE name = p_carrier_name;

RETURN lv_carrier_id;
