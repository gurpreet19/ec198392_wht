...
** 22.08.2014   mulaysul-ECPD-24242-added function getClassNonKeysMandatory,getClassKeys
** 11.03.2016   jainnraj-ECPD-30283-Modified function getGroupModelDepPopupSortOrder to add sorting of Collection Point
** 26.07.2016   austbhan-ECPD-37540-Added RefreshMViews procedure
**************************************************************/
...
SELECT class_cnfg.db_object_name, class_cnfg.time_scope_code, class_attribute_cnfg.db_sql_syntax
FROM   class_cnfg, class_attribute_cnfg
WHERE  class_attribute_cnfg.class_name = class_cnfg.class_name
AND    class_cnfg.class_name = p_class_name
AND    class_attribute_cnfg.attribute_name = p_property_name
UNION ALL
SELECT class_cnfg.db_object_name, class_cnfg.time_scope_code, class_relation_cnfg.db_sql_syntax
FROM   class_cnfg, class_relation_cnfg
WHERE  class_relation_cnfg.to_class_name = class_cnfg.class_name
AND    class_cnfg.class_name = p_class_name
AND    class_relation_cnfg.role_name = substr(p_property_name,1, length(p_property_name) - 3);

CURSOR c_db_revinfo IS
SELECT class_cnfg.db_object_name, class_cnfg.time_scope_code
FROM   class_cnfg
WHERE  class_cnfg.class_name = p_class_name;
...
lv2_return_val class_cnfg.owner_class_name%TYPE;
...
IS
CURSOR c_attr IS
  SELECT NVL(Ecdp_Classmeta_Cnfg.isMandatory(class_name, attribute_name), 'N') as is_mandatory
  FROM class_attribute_cnfg
  WHERE attribute_name = p_property_name
  AND class_name = p_class_name
UNION
    SELECT NVL(Ecdp_Classmeta_Cnfg.isMandatory(r.from_class_name, r.to_class_name, r.role_name), 'N') as is_mandatory
  FROM class_relation_cnfg r
  WHERE r.from_class_name = p_class_name -- TODO: Should this be to_class_name?
    AND r.role_name = substr(p_property_name,1, length(p_property_name) - 3)
    AND r.multiplicity IN ('1:1', '1:N')
;
...
IS
BEGIN
    RETURN Ecdp_Classmeta_Cnfg.getJournalRuleDbSyntax(p_class_name);
END;

--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- function       : getPropertyValueFromClass
-- Description    : Find the Property Value with Property Code and Class Name
--
-- Preconditions  :
-- Postcondition  :
-- Using Tables   : class_property_cnfg, class_cnfg
--
-- Using functions:
--
-- Configuration
...
CURSOR c_classJournal (cp_class_name VARCHAR2, p_property_code VARCHAR2, p_presentation_cntx VARCHAR2) IS
  with class_property_max as
   (select class_name, property_code, property_value
      from class_property_cnfg p1
     where p1.presentation_cntx = p_presentation_cntx
       and owner_cntx =
  ...
  select cast(p.property_value as varchar2(128)) as rev_text_db_syntax
    from class_cnfg c
    left join class_property_max p
      on (c.class_name = p.class_name and
         p.property_code = p_property_code)
   where c.class_name = cp_class_name;

  lv2_Rev_Text_DB_Syntax VARCHAR2(128);
  lv2_presentation_cntx VARCHAR2(250) := CASE WHEN ecdp_classmeta_cnfg.isViewlayerProperty(ecdp_classmeta_cnfg.CLASS_PROPERTY_CNFG, p_property_code) = 'Y' THEN '/' ELSE '/EC' END;
BEGIN
    FOR curClass IN c_classJournal(p_class_name, p_property_code, lv2_presentation_cntx) LOOP

      lv2_Rev_Text_DB_Syntax := curClass.rev_text_db_syntax;

    END LOOP;

    RETURN lv2_Rev_Text_DB_Syntax;
...
CURSOR c_classAttribute (p_class_name VARCHAR2,p_attribute_name VARCHAR2) IS
  SELECT is_key, EcDp_ClassMeta_Cnfg.isMandatory(class_name, attribute_name) AS is_mandatory
  FROM class_attribute_cnfg ca
...
CURSOR c_classRelation (p_class_name VARCHAR2,p_attribute_name VARCHAR2) IS
  SELECT EcDp_ClassMeta_Cnfg.isMandatory(cr.from_class_name, cr.to_class_name, cr.role_name) AS is_mandatory
...
IS
BEGIN
  RETURN Ecdp_Classmeta_Cnfg.getLabel(upper(p_class_name), upper(p_attribute_name));
END;
...
IS
BEGIN
  RETURN Ecdp_Classmeta_Cnfg.getStaticPresentationSyntax(p_class_name, p_attribute_name);
END;
...
IS
 ...
 CURSOR c_group_model_pres (p_group_type VARCHAR2, p_object_type VARCHAR2, p_parent_group_type VARCHAR2, p_parent_object_type VARCHAR2) IS
  SELECT Ecdp_Classmeta_Cnfg.getLabel(c.from_class_name, c.to_class_name, c.role_name) AS label
  FROM class_relation_cnfg c
  WHERE c.from_class_name = p_parent_object_type
  AND c.to_class_name = p_object_type
  AND c.group_type = p_group_type
  AND Ecdp_Classmeta_Cnfg.isDisabled(c.from_class_name, c.to_class_name, c.role_name) = 'N'
  ;
...
IS
BEGIN
  RETURN ecdp_classmeta_cnfg.getUomCode(UPPER(p_class_name), UPPER(p_attribute_name));
END getUomCode;
...
IS
    ...
    lv2_db_sql_syntax           class_attribute_cnfg.db_sql_syntax%TYPE;
...
IS
    ...
    lv2_db_sql_syntax           class_relation_cnfg.db_sql_syntax%TYPE;
...
IS
    ...
    lv2_DB_MAPPING_TYPE         class_relation_cnfg.DB_MAPPING_TYPE%TYPE;
...
IS
BEGIN
  RETURN EcDp_ClassMeta_Cnfg.isReadOnly(p_class_name);
END IsReadOnlyClass;
...
IS
  ...
  from class_relation_cnfg r, class_relation_cnfg rp
  where rp.group_type (+) = r.group_type
  and rp.to_class_name (+) = r.from_class_name
  AND r.group_type is not null
  AND EcDp_ClassMeta_Cnfg.isDisabled(rp.from_class_name, rp.to_class_name,  rp.role_name) = 'N'
...
IS
BEGIN
  RETURN Ecdp_Classmeta_Cnfg.getEnsureRevTextOnUpdate(p_class_name);
...
IS
BEGIN
  RETURN ec_class_attribute_cnfg.data_type(p_class_name, p_attribute_name) IS NOT NULL;
...
-- Description    : Returns a concatenated string of names of class keys
--
--
--
-- Preconditions  :
...
CURSOR c_class_keys IS
SELECT attribute_name
FROM class_attribute_cnfg
WHERE class_name = p_class_name
AND Nvl(is_key,'N') = 'Y'
order by ecdp_classmeta_cnfg.getDbSortOrder(class_name, attribute_name);

lv2_string VARCHAR2(1000);

BEGIN


      FOR cur_rec IN c_class_keys LOOP

         IF c_class_keys%ROWCOUNT = 1 THEN
            lv2_string := cur_rec.attribute_name;
         ELSE
            lv2_string := lv2_string || ', ' || cur_rec.attribute_name;
         END IF;
...
CURSOR c_class_keys IS
  ...
  WHERE class_name = p_class_name
  AND Nvl(is_key,'N') = 'N'
  AND Nvl(Ecdp_Classmeta_Cnfg.isMandatory(class_name, attribute_name), 'N') = 'Y'
  order by Ecdp_Classmeta_Cnfg.getDbSortOrder(class_name, attribute_name);
...
--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- function       : RefreshMViews
-- Description    : Refresh materialized class meta views
--
-- Preconditions  :
--
-- Postcondition  :
