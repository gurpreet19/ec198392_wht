...
**  16-Feb-2018    Zho   	ECPD-52529: fix wrong role_name to get class relation.
****************************************************************/
--</HEAD>
  ...
  lt_all_roles_available Varchar32L_t:= Varchar32L_t();
...
--</EC-DOC>
  CURSOR c_acl_relations IS
     ...
     ,      r.db_mapping_type
     ,      r.db_sql_syntax
     ,      EcDp_ClassMeta_Cnfg.getAccessControlMethod(r.from_class_name, r.to_class_name, r.role_name) AS access_control_method
     ,      EcDp_ClassMeta_Cnfg.isMandatory(r.from_class_name, r.to_class_name, r.role_name) AS is_mandatory
     ,      r.is_key
     ,      EcDp_ClassMeta_Cnfg.isReportOnly(r.from_class_name, r.to_class_name, r.role_name) AS report_only_ind
     ,      t.db_object_attribute
     ,      t.db_object_name
     ...
     ,      class_cnfg f
     ,      class_cnfg t
     WHERE  NVL(r.db_mapping_type,'FUNCTION') IN ('ATTRIBUTE','COLUMN')
     AND    EcDp_ClassMeta_Cnfg.getAccessControlMethod(r.from_class_name, r.to_class_name, r.role_name) IN ('TO_CLASS', 'FROM_CLASS')
     AND    r.from_class_name=f.class_name
     AND    r.to_class_name=t.class_name
     AND    EcDp_ClassMeta_Cnfg.getAccessControlInd(r.from_class_name) = 'Y'
     AND    EcDp_ClassMeta_Cnfg.getAccessControlInd(r.to_class_name) = 'Y'
...
--</EC-DOC>
  ...
  CURSOR c_partition(p_role_id VARCHAR2, p_class_name VARCHAR2) IS
    ...
    ,      t_basis_access a
    where  EcDp_ClassMeta_Cnfg.getAccessControlInd(p_class_name) = 'Y'
  ...
  lv_object_table class_cnfg.db_object_name%TYPE;
  lv2_class_where class_cnfg.db_where_condition%TYPE;
...
BEGIN
   ...
   lv_object_table := ec_class_cnfg.db_object_name(p_class_name);
   lv2_class_where := ec_class_cnfg.db_where_condition(p_class_name);
...
--</EC-DOC>
  CURSOR c_t_basis_access IS
    ...
    WHERE EcDp_ClassMeta_Cnfg.getAccessControlInd(a.class_name) = 'Y'
    and a.t_basis_access_id=p.t_basis_access_id
    ORDER BY role_id, class_name;
BEGIN
	 SELECT role_id
	 BULK COLLECT INTO lt_all_roles_available
	 FROM t_basis_role;
...
--</EC-DOC>
  ...
  lv_ref_object_id  VARCHAR2(32);
  lv_ref_class_name VARCHAR2(24);
  lt_relation_list  RelationL_t;
...
BEGIN
  ...
  IF lt_rel_id_list.COUNT != 0 THEN
...
--</EC-DOC>
  ...
  CURSOR c_interface_rec2(p_object_id VARCHAR2, p_class_name VARCHAR2) IS
    ...
    AND    o.class_name=c.class_name
    AND    Ecdp_Classmeta_Cnfg.getAccessControlInd(p.class_name)='Y'
    AND    Ecdp_Classmeta_Cnfg.getAccessControlInd(c.class_name)='N'
    AND    NVL(p_object_id,o.object_id)=o.object_id
    AND    NVL(p_class_name,o.class_name)=o.class_name;
...
BEGIN
     ...
     FOR cur_obj IN c_interface_rec2(p_object_id, p_class_name) LOOP
       FOR r IN 1..lt_all_roles_available.COUNT LOOP
           INSERT INTO acl_objects (object_id,class_name,role_id) VALUES (cur_obj.object_id,cur_obj.parent_class,lt_all_roles_available(r));
       END LOOP;
     END LOOP;
