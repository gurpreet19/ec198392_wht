...
**  07.08.2017  jainnraj ECPD-46835: Modified procedure checkIfEventOverlaps,verifyObjectGrpConn to correct the error message.
****************************************************************/
    ...
    IF p_end_date IS NOT NULL THEN
      FOR cur_Validate1 IN c_validate1 LOOP
        IF cur_Validate1.end_date IS NOT NULL  AND cur_Validate1.end_date > p_end_date THEN
          Raise_Application_Error(-20217,'End Date must be equal to or after the End Date of the child records.');
          Exit;
        END IF;
    ...
    FOR cur_Validate1 IN c_validate1 LOOP
       IF p_daytime IS NOT NULL THEN
         ...
         IF (cur_Validate1.end_date IS NOT NULL and cur_Validate1.end_date < p_daytime ) THEN
           Raise_Application_Error(-20221,'Start Date must be before the End Date of the parent.');
         END IF;
       END IF;
       ...
       IF p_end_date IS NOT NULL THEN
         ...
         IF cur_Validate1.end_date IS NOT NULL and  p_end_date > cur_Validate1.end_date THEN
           Raise_Application_Error(-20219,'End Date must be equal to or before the End Date of the parent.');
         END IF;
       END IF;
    END LOOP;
  END IF;
...
--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- Function       : verifyEqpmConn
-- Description    : This procedure checks validity of the eqpm conn period.
--
-- Preconditions  :
--
-- Postconditions :
-- Using tables   : EQPM_CONN
--
-- Using functions:
--
--
...
IS
  CURSOR c_validate1 IS
    SELECT object_id, eqpm_conn_name, daytime, end_date
      FROM EQPM_CONN
     WHERE object_id = p_object_id
       AND eqpm_conn_name = p_eqpm_conn_name
...
BEGIN
    ...
    FOR cur_Validate1 IN c_validate1 LOOP
      Raise_Application_Error(-20121, 'Start Date overlaps with another equipment connection period.');
      Exit;
    END LOOP;

    FOR cur_Validate2 IN c_validate2 LOOP
      Raise_Application_Error(-20121, 'End Date overlaps with another equipment connection period.');
      Exit;
    END LOOP;
