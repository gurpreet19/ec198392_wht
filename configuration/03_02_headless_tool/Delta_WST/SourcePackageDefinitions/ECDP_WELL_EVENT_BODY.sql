...
** 08.01.2015  dhavaalo		 ECPD-25537-Added new functions getLastWellEventSingleDaytime() and getNextWellEventSingleDaytime()
** 26.02.2015  dhavaalo 	 ECPD-29953-Modified getNextWellEventSingleDaytime to calculate theoretical values correctly.
** 28.05.2015  abdulmaw    ECPD-30716: Updated getChildClassName
*****************************************************************/
...
BEGIN
  ...
  IF ld_return IS NULL THEN
    ld_return := to_date(to_char(Ecdp_Timestamp.getCurrentSysdate,'yyyy/mm/dd hh:mi:am'), 'yyyy/mm/dd hh:mi:am');
  END IF;
...
CURSOR c_well_single_evt(cp_object_id well.object_id%TYPE, cp_daytime DATE) IS
 ...
 AND we.object_id = cp_object_id
 AND we.daytime > cp_daytime
 AND we.daytime < TRUNC(cp_daytime) + 1
 ORDER BY we.daytime ASC;
