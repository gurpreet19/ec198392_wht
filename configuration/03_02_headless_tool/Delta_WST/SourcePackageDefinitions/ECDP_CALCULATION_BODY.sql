...
IS
   ...
   lv2_user_id VARCHAR2(30) := Nvl(EcDp_User_Session.getUserSessionParameter('USERNAME'), User);
BEGIN
   lv2_sql := 'UPDATE calculation_version v ' ||
              'SET v.name=:p_new_name, v.comments=:p_new_comments, v.last_updated_by=:lv2_user_id ' ||
              'WHERE v.object_id=:p_impl_calc_id ' ||
              'AND v.daytime=:p_daytime ' ||
              'AND (v.name<>:p_new_name OR NVL(v.comments,''--XDUMMYX--'')<>NVL(:p_new_comments,''--XDUMMYX--'')) ' ||
              'AND EXISTS (SELECT 1 FROM calculation WHERE object_id = v.object_id AND calc_scope = ''PRIVATE_SUB'')';
...
BEGIN
   ...
   IF EcDp_ClassMeta_Cnfg.getAccessControlInd('CALCULATION')='Y' THEN
...
BEGIN
   IF lt_calculation.calc_scope  = 'MAIN' OR lt_calculation.calc_scope  = 'PUBLIC_SUB' THEN
...
IS
  ...
  mainJobId Calculation_version.MAIN_CALCULATION_ID%TYPE;
  calcType Calculation.Calc_Type%TYPE;
BEGIN
  ...
  IF lr_log_record.job_id IS NOT NULL THEN
    ...
    IF calcType IS NOT NULL AND calcType = 'PROXY' THEN
        select main_calculation_id into mainJobId from calculation_version where object_id=lr_log_record.job_id and daytime <= lr_log_record.daytime and lr_log_record.daytime  < nvl(end_date, lr_log_record.daytime + 1) ;
...
IS
  ...
  lv2_name_token   VARCHAR2(2000);
  lv2_value_token  VARCHAR2(2000);
  lv2_param_names  VARCHAR2(2000);
  lv2_param_values VARCHAR2(2000);
  engine_params    PARAM_MAP;

BEGIN
   --Create map of engine parameters :
    lv2_param_names := p_engineparams;
    lv2_param_values := p_engineparamvalues;
    LOOP
      lv2_name_token := EcDp_Utilities.getNextToken(lv2_param_names, ';');
      lv2_value_token := EcDp_Utilities.getNextToken(lv2_param_values, ';');
      EXIT WHEN lv2_name_token IS NULL OR lv2_value_token IS NULL;
      engine_params(lv2_name_token) := lv2_value_token;
   END LOOP ;
	return ue_calc_engine.calcObjAttrFilter(p_startdate, p_enddate, p_object_type, p_attr_name, p_attr_value,engine_params);
