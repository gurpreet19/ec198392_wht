...
BEGIN
	...
	lv_delpnt_uom := Nvl(EcDp_Unit.GetUnitFromLogical(nvl(EcDp_ClassMeta_Cnfg.getUomCode('DP_SUB_DAY_TARGET_EXIT', 'TARGET_QTY'),'')), '');
...
BEGIN
   ...
   lv_delpnt_uom := Nvl(EcDp_Unit.GetUnitFromLogical(nvl(EcDp_ClassMeta_Cnfg.getUomCode(p_class_name, 'TARGET_QTY'),'')), '');
