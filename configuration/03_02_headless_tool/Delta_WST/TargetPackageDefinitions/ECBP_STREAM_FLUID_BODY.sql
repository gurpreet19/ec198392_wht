...
**		   07.05.2012 limmmchu  ECPD-20876: Modified findStdDens to add new method, SAMPLE_ANALYSIS
**		   15.06.2012 kumarsur  ECPD-21278: Modified findGrsStdMass added OR aggregate flag = 'Y'
**         16.07.2012 leongwen  ECPD-21488: Modified findStdDens to include the check on 'CO2'
**		   30.07.2012 abdulmaw  ECPD-21630: Modified findGrsStdVol, findNetStdVol, findGrsStdMass and findNetStdMass to fix the formula calculation for Monthly
**		   20.11.2012 musthram  ECPD-22576: Modified findWatMass to support method Gross - Net
**         05.01.2013 hismahas  ECPD-22994: Added User_Exit in
**                                        - findGrsStdVol
**                                        - findNetStdVol
** 		    21.03.2013 kumarsur ECPD-23650: Modified findGrsStdVol(),findNetStdVol() and added calcShrunkVol(), calcDiluentVol().
** 			30.04.2013 musthram ECPD-24081: Modified findNetStdVol()
...
**          27.09.2013 rajarsar ECPD-25582: Updated function findGrsStdVol to support method = 'AGA' correctly with the passing of correct class name 'PERIOD_GAS_STREAM_DATA_AGA' instead of 'PERIOD_GAS_STREAM_DATA'.
**          22.10.2013 musthram ECPD-25803: Updated function findGrsStdMass, findGrsStdVol, findGrsStdVol and findNetStdVol to fix formula calculation for Monthly
**			07.04.2014 kumarsur ECPD-27240: Modified findStdDens and findNetStdMass functions.
*****************************************************************/
...
SELECT ste.grs_vol, ste.grs_vol_adj,ste.net_vol,ste.event_no, ste.water_vol ,ste.vcf
FROM strm_transport_event ste
WHERE ((object_id = cp_object_id) OR (stream_id = cp_object_id))
AND production_day between cp_fromday AND cp_today
UNION ALL
SELECT ote.grs_vol, ote.grs_vol_adj, ote.net_vol,ote.event_no, ote.water_vol,ote.vcf
...
/* Cursors to be used by a number of functions involved with processing table strm_transport_event for mass */
CURSOR c_strm_transport_event_mass(cp_object_id VARCHAR2, cp_fromday DATE, cp_today DATE) IS
SELECT *
...
--                  ECDP_STREAM_DERIVED.GETGRSSTDMASS
...
--                2. 'DERIVED': User exit using hardcoded derived stream package
--                3. 'NET_MASS_WATER'
--                4. 'FORMULA': Find gross mass based on a formula
--                5. 'NET_MASS': Gross mass equals net mass
--                6. 'VOLUME_DENSITY': Find gross mass as (volume * density)
--                7. 'RUNTIME_RATE': Find gross mass as stream ref value mass rate times on strm fraction
--                8. 'OPEN_CLOSE_WEIGHT'
--                9. 'USER_EXIT'
--               10. 'TOTALIZER_DAY'
---------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.DERIVED) THEN

    ln_return_val := EcDp_Stream_Derived.getGrsStdMass(
                                                       p_object_id,
                                                       ld_fromday,
                                                       ld_today);
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.VOLUME_DENSITY) THEN
    ...
    IF lv2_strm_meter_freq = 'MTH' THEN
      ...
      ln_return_val := EcBp_Stream_Fluid.findGrsStdVol(p_object_id,p_fromday,p_fromday) *
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.MEASURED_TRUCKED) THEN
    ...
    IF ld_today IS NULL THEN -- indicates we are looking for single event value
      ln_return_val := NULL;  -- this is not possible, use EcBp_Stream_TruckTickets.findGrsStdMass() instead but it cannot be called from here.

    ELSE -- looking for sum of event values over a given period

      lb_first_iteration:= TRUE;

      FOR curEvent2 IN c_strm_transport_event_mass (lv2_stream_object_id, ld_fromday, ld_today) LOOP
        IF lb_first_iteration THEN
          lb_first_iteration := FALSE;
          ln_return_val :=  curEvent2.grs_mass_adj;

        ELSE
          ln_return_val :=  ln_return_val + curEvent2.grs_mass_adj;
  ...
  ELSIF (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ...
    ln_return_val := ue_stream_fluid.getGrsStdMass(p_object_id, p_fromday);
...
--                  ECDP_STREAM_DERIVED.GETGRSSTDVOL
--                  ECDP_STREAM_DERIVED.GETNETSTDVOL
...
--                 10.  'TOTALIZER_EVENT'
--                 11.'NET_VOL_WATER'
--                 12.'MEASURED_TRUCKED' Get grs_vol from truck ticket data
--                 13.'TOTALIZER_DAY'
--                 14.'USER_EXIT'
---------------------------------------------------------------------------------------------------
FUNCTION findGrsStdVol (
...
BEGIN
  ...
  -- Find grs_vol using data from strm_day_stream only (or derived).
  IF (lv2_method = EcDp_Calc_Method.MEASURED) THEN
  ...
  ELSIF (lv2_method = Ecdp_Calc_Method.AGA) THEN
    ...
    IF lv2_strm_meter_method = 'PERIOD' THEN
      ...
      FOR mycur IN c_daytime LOOP
        ...
        lecd_ProductionDayStart := EcDp_ProductionDay.getProductionDayStartTime('STREAM',p_object_id, ld_fromday);
        ld_prod_day_start := lecd_ProductionDayStart.daytime;
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.DERIVED) THEN
    ...
    ln_return_val := EcDp_Stream_Derived.getGrsStdVol(
                                                      p_object_id,
                                                      ld_fromday,
                                                      ld_today);
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.MEASURED_TRUCKED) THEN
    ...
    ln_return_val := 0;
    IF ld_today IS NULL THEN -- indicates we are looking for single event value
      ln_return_val := NULL;  -- this is not possible, use EcBp_Stream_TruckTickets.findGrsStdVol() instead but it cannot be called from here.
    ...
    ELSE -- looking for sum of event values over a given period
      ...
      FOR curEvent2 IN c_strm_transport_event (lv2_stream_object_id, ld_fromday, ld_today) LOOP
        IF lb_first_iteration THEN
          lb_first_iteration := FALSE;
          ln_return_val :=  curEvent2.grs_vol_adj;

        ELSE
          ln_return_val :=  ln_return_val + curEvent2.grs_vol_adj;
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.MEASURED_TRUCKED_LOAD) THEN
    ...
    ELSE -- looking for sum of event values over a given period
      ...
      FOR curEvent2 IN c_strm_transport_event (lv2_stream_object_id, ld_fromday, ld_today) LOOP
        IF lb_first_iteration THEN
          lb_first_iteration := FALSE;
          ln_return_val :=  curEvent2.grs_vol;

        ELSE
          ln_return_val :=  ln_return_val + curEvent2.grs_vol;
        END IF;
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.ENERGY_GCV) THEN
    ln_return_val := 0;
    IF lv2_strm_meter_freq = 'MTH' THEN
      ln_energy := EcBp_Stream_Fluid.findEnergy(p_object_id, p_fromday, p_fromday);
  ...
  ELSIF (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ...
    ln_return_val := ue_stream_fluid.getGrsStdVol(p_object_id, p_fromday);
...
--                  ECDP_STREAM_DERIVED.GETNETSTDMASS
...
--                2. 'DERIVED': Derived formula
--                3. 'ALLOCATED': Get data from strm_day_alloc
--                4. 'GRS_MASS'
--                5. 'VOLUME_DENSITY'
--                6. 'GROSS_BSW' : Calculate net mass based on gross mass and BSW weight frac
--                7. 'FORMULA'
--                8. 'TANK_DUAL_DIP'
--                9. 'GRS_MASS_WDF'
--               10. 'GRS_MASS_MINUS_WATER'
--               11. 'USER_EXIT'
--               12. 'TOTALIZER_DAY'
---------------------------------------------------------------------------------------------------
FUNCTION findNetStdMass (
...
BEGIN
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.DERIVED) THEN
    ...
    ln_return_val := EcDp_Stream_Derived.getNetStdMass(
                                                       p_object_id,
                                                       ld_fromday,
                                                       ld_today);
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.VOLUME_DENSITY) THEN
    ...
    IF lv2_strm_meter_method = 'EVENT' THEN
      ...
      IF ld_today IS NULL THEN
        ...
        FOR curEvent IN c_strm_event_single(lv2_stream_object_id, ld_fromday) LOOP
          ...
          END IF;
        END LOOP;
        -- if well_event didn't return data, then try truck ticket table
        IF ln_return_val IS NULL THEN
          FOR curEvent IN c_strm_transport_event_single(lv2_stream_object_id, ld_fromday) LOOP
            ln_net_std_vol := EcBp_Stream_Fluid.findNetStdVol(curEvent.Object_id,curEvent.daytime);
      ...
      ELSE
        ...
        FOR curEvent IN c_strm_event(lv2_stream_object_id, ld_fromday, ld_today) LOOP
          ...
          ln_net_std_vol := EcBp_Stream_Fluid.findNetStdVol(curEvent.Object_id,curEvent.daytime,curEvent.daytime);
          --if net volume = 0 then return 0
          IF ln_net_std_vol = 0 THEN
			IF (ln_return_val IS NULL) THEN
				ln_return_val := ln_return_val;
			END IF;
        ...
        IF ln_return_val IS NULL THEN
          ...
          FOR curEvent IN c_strm_transport_event(lv2_stream_object_id, ld_fromday, ld_today) LOOP
            ...
            ln_net_std_vol := EcBp_Stream_TruckTicket.findNetStdVol(curEvent.EVENT_NO);
            IF ln_net_std_vol = 0 THEN
				IF (ln_return_val IS NULL) THEN
					ln_return_val := ln_return_val;
				END IF;
    ...
    ELSE
      ...
      IF lv2_strm_meter_freq = 'MTH' THEN
        ...
        ln_net_std_vol := EcBp_Stream_Fluid.findNetStdVol(p_object_id,p_fromday,p_fromday);
        IF ln_net_std_vol = 0 THEN
          ln_return_val := 0;
        ELSE
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.GROSS_BSW) THEN
    ...
    IF lv2_strm_meter_method = 'EVENT' THEN
      ...
      IF(lv2_grs_method = EcDp_Calc_Method.MEASURED_TRUCKED) THEN
        ...
        -- test if it is a single record or sum over production days to be returned
        IF ld_today IS NULL THEN
          ln_return_val := NULL;  -- this is not possible, use EcBp_Stream_TruckTickets.findNetStdMass() instead but it cannot be called from here.

        ELSE -- looking for sum of event values over a given period
          lb_first_iteration:= TRUE;
		  ln_return_val := 0;
          FOR curEvent2 IN c_strm_transport_event (lv2_stream_object_id, ld_fromday, ld_today) LOOP
            ln_bsw_wt := EcBp_Stream_TruckTicket.getBswWeightFrac(curEvent2.event_no);
            ln_grs_mass := EcBp_Stream_TruckTicket.findGrsStdMass(curEvent2.event_no);
            ln_net_mass :=  ln_grs_mass * (1 - ln_bsw_wt);
    ...
    ELSE   -- not (lv2_strm_meter_method = 'EVENT')
      ...
      IF lv2_strm_meter_freq = 'MTH' THEN
		...
		ln_grs_mass := findGrsStdMass(p_object_id,p_fromday,p_fromday);
        IF ln_grs_mass = 0 THEN
          ln_return_val := 0;
        ELSE
  ...
  ELSIF (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ...
    ln_return_val := ue_stream_fluid.getNetStdMass(p_object_id, p_fromday);
...
--                 3.  'DERIVED': Derived stream
--                 5.  'FORMULA': Value based on a formula using stream formula editor
--                 5.  'GROSS_BSW': Find net_vol using data from gross volume and bsw
--                 6.  'GRS_VOL': equal to grs_vol
--                 7.  'MASS_DENSITY': find net_vol using data from net_mass and density
--                 8.  'NA': Not Applicable , return NULL
--                 9.  'GROSS_FACTOR': Calculate based on gross volume, bsw, meter factor and shrinkage factor
--                 10. 'TANK_DUAL_DIP': Calculate based on tank double dips
--                 11. 'BATCH_API':
--                 12. 'GRS_VOL_WDF'
--                 13. 'TOTALIZER_DAY'
--				         14. 'TOTALIZER_EVENT'
...
BEGIN
  ...
  lv2_method := Nvl(p_method,
                     ec_strm_version.NET_VOL_METHOD(
                                                    p_object_id,
                                                    p_fromday,
                                                    '<='));
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.DERIVED) THEN
    ...
    ln_return_val := EcDp_Stream_Derived.getNetStdVol(
                                                      p_object_id,
                                                      ld_fromday,
                                                      ld_today);
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.GROSS_BSW) THEN
    ...
    IF lv2_strm_meter_method = 'EVENT' THEN
      ...
      IF(lv2_grs_method = EcDp_Calc_Method.MEASURED_TRUCKED) THEN
        ...
        ELSE -- looking for sum of event values over a given period
          ...
          FOR curEvent2 IN c_strm_transport_event (lv2_stream_object_id, ld_fromday, ld_today) LOOP
            ln_bsw := EcBp_Stream_TruckTicket.getBswVolFrac(curEvent2.event_no);
            ln_grs_vol := EcBp_Stream_TruckTicket.findGrsStdVol (curEvent2.event_no);
            ln_vcf := ec_strm_transport_event.vcf(curEvent2.event_no);
            ln_net_vol :=  ln_grs_vol * (1 - ln_bsw)* nvl(ln_vcf,1);
      ...
      ELSIF(lv2_grs_method = EcDp_Calc_Method.MEASURED_TRUCKED_LOAD) THEN
        ...
        ELSE -- looking for sum of event values over a given period
          ...
          FOR curEvent2 IN c_strm_transport_event (lv2_stream_object_id, ld_fromday, ld_today) LOOP
            ln_net_vol :=  Ecbp_Stream_Truckticket.getNetVol(curEvent2.Event_No);
    ...
    ELSE -- non event based stream
       ...
       IF lv2_strm_meter_freq = 'MTH' THEN
		 ...
		 ln_grs_vol := EcBp_Stream_Fluid.findGrsStdVol(lv2_stream_object_id,p_fromday,p_fromday);
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.MASS_DENSITY) THEN
    ...
    lv_net_mass_method := Nvl(p_method,
                          ec_strm_version.NET_MASS_METHOD(
                          p_object_id,
                          ld_fromday,
                          '<='));
  ...
  ELSIF (lv2_method = EcDp_Calc_Method.TOTALIZER_DAY) THEN
     ...
     lv2_phase := Ec_Strm_Version.stream_phase(p_object_id,ld_fromday, '<=');
   ...
   ELSIF (lv2_method = EcDp_Calc_Method.TOTALIZER_EVENT) THEN
      ...
      IF p_today IS NULL THEN
        ...
        END LOOP;
            ...
            ln_net_vol := EcBp_Stream_Fluid.findGrsStdVol(p_object_id, p_fromday, p_fromday, 'TOTALIZER_EVENT') * (1 - ln_bsw);
      ...
      ELSE -- p_today IS NOT NULL
        ...
        FOR mycur IN c_system_days LOOP
	      ...
	      ln_bsw := NVL(curTotalizer.bs_w,0);
              ln_net_vol := ln_net_vol + nvl(EcBp_Stream_Fluid.findGrsStdVol(p_object_id, curTotalizer.daytime, curTotalizer.daytime , 'TOTALIZER_EVENT'),0) * (1 - ln_bsw);
	  END LOOP;
 ...
 -- User Exit
   ELSIF (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ...
    ln_return_val := ue_stream_fluid.getNetStdVol(p_object_id, p_fromday);
...
BEGIN
    ...
    IF lv2_strm_meter_method = 'EVENT' THEN
      ...
      IF (lv2_grs_method = EcDp_Calc_Method.MEASURED_TRUCKED) THEN
        ...
        IF ld_today IS NULL THEN -- test if it is a single event record value to be returned
          ...
          ln_return_val := NULL;  -- this is not possible, use EcBp_Stream_TruckTickets.findWatMass() instead but it cannot be called from here.
        ...
        ELSE -- looking for sum of event values over a given period
           ...
           FOR curEvent2 IN c_strm_transport_event (p_object_id, ld_fromday, ld_today) LOOP
              ...
              ln_bsw_wt := EcBp_Stream_TruckTicket.getBswWeightFrac(curEvent2.event_no);
              ln_grs_mass := EcBp_Stream_TruckTicket.findGrsStdMass (curEvent2.event_no);
              ln_net_mass :=  ln_grs_mass * ln_bsw_wt;
  ...
  ELSIF  (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ...
    ln_return_val := ue_stream_fluid.getWatMass(p_object_id, p_fromday);
...
BEGIN
    ...
    IF lv2_strm_meter_method = 'EVENT' THEN
      ...
      ELSIF(lv2_grs_method = EcDp_Calc_Method.MEASURED_TRUCKED_LOAD) THEN
        ...
        ELSE -- looking for sum of event values over a given period
          ...
          FOR curEvent2 IN c_strm_transport_event (lv2_stream_object_id, ld_fromday, ld_today) LOOP
            ln_wat_vol :=  ecbp_stream_truckticket.getWaterVol(curEvent2.Event_No);
            ...
            ELSE
              ln_return_val := ln_return_val + ln_wat_vol;
      ...
      ELSIF(lv2_grs_method = EcDp_Calc_Method.MEASURED_TRUCKED_UNLOAD) THEN
        ...
        ELSE -- looking for sum of event values over a given period
          ...
          FOR curEvent2 IN c_strm_transport_event (lv2_stream_object_id, ld_fromday, ld_today) LOOP
            ln_wat_vol :=  nvl(curEvent2.Water_Vol,0);
  ...
  ELSIF  (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ...
    ln_return_val := ue_stream_fluid.getWatVol(p_object_id, p_fromday);
...
BEGIN
   ...
   ELSIF  (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
       ...
       ln_return_val := ue_stream_fluid.getCondVol(p_object_id, p_fromday);
...
ln_ret_val NUMBER;
BEGIN
     SELECT SUM(ecbp_stream_truckticket.getNetVol(ste.event_no)) INTO ln_ret_val
     FROM strm_transport_event ste
     WHERE ste.object_id = p_object_id
     AND ste.production_day = p_production_day
     AND ste.profit_centre_id = p_profit_centre_id;
...
ln_ret_val NUMBER;
BEGIN
     SELECT SUM(ecbp_stream_truckticket.getWaterVol(ste.event_no)) INTO ln_ret_val
     FROM strm_transport_event ste
     WHERE ste.object_id = p_object_id
     AND ste.production_day = p_production_day
     AND ste.profit_centre_id = p_profit_centre_id;
...
BEGIN
  ...
  if (lr_strm_event.static_chart_type in ('LINEAR', 'PERCENT') AND lr_strm_event.static_chart_rating > 0) then
    ...
    ln_return_value := Ecdp_Unit.convertValue(ln_return_value,
                                                  ...
                                                  Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(p_class_name,'STATIC_PRESS')));
...
BEGIN
  ...
  if (lr_strm_event.diff_chart_type in ('LINEAR', 'PERCENT') AND lr_strm_event.diff_chart_rating > 0) then
    ...
    ln_return_value := Ecdp_Unit.convertValue(ln_return_value,
                                                  ...
                                                  Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(p_class_name,'DIFF_PRESS')));
...
BEGIN
   ...
   IF lv2_phase IN ('GAS','LNG') THEN
      ...
      ELSIF lv2_method = 'ENERGY_DIV_VOLUME' THEN
         ...
         ln_return_val := 0;
         IF lv2_strm_meter_freq = 'MTH' THEN
           ln_energy := EcBp_Stream_Fluid.findEnergy(p_object_id, p_daytime, p_daytime);
      ...
      substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT THEN
          ln_return_val := ue_stream_fluid.getGCV(p_object_id, p_daytime);
...
--                3. 'DERIVED': Get derived value from userdefined function.
--                4. 'ALLOCATED': Access the STRM_DAY_ALLOC.ALLOC_ENERGY field directly.
--                5. 'VOLUME_REF_MBTU'
--                6. 'USER_EXIT'
--          7. 'TOTALIZER_DAY'
--
...
BEGIN
   ...
   ELSIF (lv2_method = EcDp_Calc_Method.VOLUME_GCV) THEN
    ...
    -- METHOD= 'DERIVED' ( Only derived values.)
   ELSIF (lv2_method = EcDp_Calc_Method.DERIVED) THEN

      ln_return_val := EcDp_Stream_Derived.getEnergy(
                                                     p_object_id,
                                                     p_fromday,
                                                     p_today);
   ...
   ELSIF (lv2_method = EcDp_Calc_Method.FORMULA) THEN
      ...
      ln_return_val := EcDp_Stream_Formula.getEnergy(p_object_id, p_fromday, p_today);

   -- Method == 'USER_EXIT'
   ELSIF  (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN

      ln_return_val := ue_stream_fluid.getEnergy(p_object_id, p_fromday);
...
ln_ret_val NUMBER;
BEGIN
     SELECT SUM(ecbp_stream_truckticket.getNetMass(ste.event_no)) INTO ln_ret_val
     FROM strm_transport_event ste
     WHERE ste.object_id = p_object_id
     AND ste.production_day = p_production_day
     AND ste.profit_centre_id = p_profit_centre_id;
...
BEGIN
   ...
   ELSIF (lv2_method = EcDp_Calc_Method.VOLUME_GCV) THEN
      ...
      FOR mycur IN c_daytime LOOP
          ...
          ln_return_val := EcBp_Stream_Fluid.findNetStdVol(p_object_id,mycur.daytime,mycur.daytime) *
                                                          EcBp_Stream_Fluid.findGCV(p_object_id,mycur.daytime);
   ...
   */
   ...
   ELSIF (lv2_method = EcDp_Calc_Method.DERIVED) THEN
      ...
      ln_return_val := EcDp_Stream_Derived.getPowerConsumption(
                                                     p_object_id,
                                                     p_fromday,
                                                     p_today);
   ...
   ELSIF (lv2_method = EcDp_Calc_Method.ALLOCATED) THEN
      ...
      FOR c_strm_day_alloc in c_alloc(p_object_id, p_fromday) LOOP
          ...
          ln_return_val := c_strm_day_alloc.alloc_energy; --?? change to alloc_powerconsumption??
   ...
   ELSIF (lv2_method = EcDp_Calc_Method.VOLUME_REF_MBTU) THEN
     ...
     IF p_today IS NULL THEN -- access single record only
       ln_return_val := ecbp_stream_fluid.findNetStdVol(p_object_id, p_fromday, p_today) * nvl(ec_strm_reference_value.mbtu_factor(p_object_id, p_fromday, '<='),1);
     ELSE -- access from day to day
       lb_first_iteration:= TRUE;
       FOR mycur IN c_daytime LOOP
         IF lb_first_iteration THEN
           lb_first_iteration:= FALSE;
           ln_return_val := ecbp_stream_fluid.findNetStdVol(p_object_id, mycur.daytime, mycur.daytime) * nvl(ec_strm_reference_value.mbtu_factor(p_object_id, p_fromday, '<='),1);
         END IF;
         ln_return_val := ln_return_val + (ecbp_stream_fluid.findNetStdVol(p_object_id, mycur.daytime, mycur.daytime) *nvl(ec_strm_reference_value.mbtu_factor(p_object_id, p_fromday, '<='),1));
       END LOOP;
     END IF;
     */
...
BEGIN
  ...
  FOR mycur1 IN c_daytime LOOP
    ...
    ELSIF (substr(lv2_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
      ln_return_val := ln_return_val + Ue_Stream_Fluid.findOnStreamHours(p_object_id,mycur1.daytime);
    ELSE
