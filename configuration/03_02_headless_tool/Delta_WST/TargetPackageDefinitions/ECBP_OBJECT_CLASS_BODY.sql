...
**  19.04.2012 rajarsar ECPD-20643: Updated procedure StreamObject to ensure correct stream set lists are inserted during copy object.
**  13.09.2013 abdulmaw ECPD-24980: Added new function  getObjName
**  14.05.2015 abdulmaw ECPD-31018: Updated CopyObject function to support Month lock correctly
****************************************************************/
--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- function       : CopyObject
...
BEGIN
      ...
      IF ec_class.lock_ind(p_class_name) = 'Y' THEN
...
IS
   ...
   CURSOR c_wb_col IS
          SELECT property_name FROM dao_meta
                     ...
                     ELSE
                        ...
                        AND nvl(END_DATE,p_daytime+1) > p_daytime;
                            ...
                            OPEN c_wb_split_col ('WELL_BORE_SPLIT_FACTOR');
                             ...
                             lv_result1 := EcDp_Utilities.executeStatement(lv_sql1);
                           ...
                           FOR cur_webo_int IN c_webo_interval (lv_id, p_daytime, p_new_date) LOOP
                             ...
                             CLOSE c_wb_int_col;
                                     ...
                                     lv_result2 := EcDp_Utilities.executeStatement(lv_sql2);
                                     ...
                                     ELSE
                                        ...
                                        AND nvl(END_DATE,p_daytime+1) > p_daytime;
                                            ...
                                            OPEN c_wb_split_col ('WELL_BORE_INTERVAL_SPLIT');
                                             ...
                                             lv_result1 := EcDp_Utilities.executeStatement(lv_sql3);
                                           ...
                                           FOR cur_perf_interval IN c_perf_interval (lv_oriCode1, p_daytime, p_new_date) LOOP
                                              ...
                                              OPEN c_perf_int_col;
                                               ...
                                               lv_result4 := EcDp_Utilities.executeStatement(lv_sql4);
                                                  ...
                                                  AND nvl(END_DATE,p_daytime+1) > p_daytime;
                                                      ...
                                                      OPEN c_perf_int_split_col;
                                                       ...
                                                       CLOSE c_perf_int_split_col;
                                                             ...
                                                             lv_result5 := EcDp_Utilities.executeStatement(lv_sql6);
          ...
          FOR cur_strm_set_list IN c_strm_set_list (p_ori_id, p_new_date) LOOP
              ...
              OPEN c_strm_set_list_col;
                  ...
                  CLOSE c_strm_set_list_col;
                     ...
                     lv_value := lv_value || ' FROM TV_STREAM_SET_LIST WHERE STREAM_ID='''|| p_ori_id ||''' AND CODE='''|| lv_code ||''' AND FROM_DATE>=to_date('''||to_char(p_daytime,'yyyy-mm-dd')||''',''yyyy-mm-dd''))';
