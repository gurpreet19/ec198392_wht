...
BEGIN
    ...
    FOR runs IN run_nos LOOP
        v_loading_time := v_loading_time + NVL(EC_Lifting_Activity.to_daytime(p_cargo_no, v_loading_end_code, runs.run_no,'LOAD'),
                                               EC_Lifting_Activity.from_daytime(p_cargo_no, v_loading_end_code, runs.run_no,'LOAD'))
                                         - EC_Lifting_Activity.from_daytime(p_cargo_no, v_loading_start_code, runs.run_no,'LOAD');
    END LOOP;
...
IS
    ...
    AND from_daytime >= ec_lifting_activity.from_daytime(p_cargo_no, GetDemurrageActivityCode(p_cargo_no, p_demurrage_type, 'START'), 1,'LOAD')
    AND to_daytime <= ec_lifting_activity.from_daytime(p_cargo_no, GetDemurrageActivityCode(p_cargo_no, p_demurrage_type, 'END'), 1,'LOAD');
    ...
    AND from_daytime >= CASE WHEN ec_lifting_activity.from_daytime(p_cargo_no, 'LNG_NOR_EFFECTIVE', 1,'LOAD') > nvl(ec_cargo_transport.DATE_1(p_cargo_no), ue_ct_cargo_info.getprat(p_cargo_no))
                             THEN ec_lifting_activity.from_daytime(p_cargo_no, 'LNG_NOR_EFFECTIVE', 1,'LOAD')
                             ELSE nvl(ec_cargo_transport.DATE_1(p_cargo_no), ue_ct_cargo_info.getprat(p_cargo_no)) END
    AND to_daytime <= ec_lifting_activity.from_daytime(p_cargo_no, 'LNG_FIRST_LINE', 1,'LOAD');
