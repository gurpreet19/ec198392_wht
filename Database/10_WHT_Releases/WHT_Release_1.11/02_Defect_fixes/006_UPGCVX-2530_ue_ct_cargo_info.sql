        ...
        FOR item IN stream_events LOOP
          ...
          ELSIF p_measurement = 'GRS_MASS' THEN -- Include the net change in free water in the gross mass. Use unit conversions to ensure that the water density is used correctly.
            v_return_value := v_return_value + item.std_grs_liq_mass + EcDp_Unit.convertvalue(EcDp_Unit.convertValue((EcBp_Tank.FindFreeWaterVol(item.export_tank, 'DUAL_DIP_OPENING', item.daytime)
                                                                                               - EcBp_Tank.FindFreeWaterVol(item.export_tank, 'DUAL_DIP_CLOSING', item.daytime)), EcDp_Unit.getunitfromlogical(ecdp_classmeta_cnfg.getUomCode('TANK_OIL_BATCH_EXP_DET', 'FREE_WAT_VOL')), v_wat_vol_unit)
                                                                                               * EcDp_System.getWaterDensity,
                                                                                              v_wat_mass_unit, EcDp_Unit.getunitfromlogical(ecdp_classmeta_cnfg.getUomCode('TANK_OIL_BATCH_EXP', 'STD_GRS_LIQ_MASS')));
...
--by : tlxt@ 11-Nov-2013
  ...
  IS
    ...
    v_target_unit VARCHAR2(32) := EcDp_Unit.GetUnitFromLogical(ecdp_classmeta_cnfg.getUomCode(p_class_name,p_target_attribute));
  BEGIN
