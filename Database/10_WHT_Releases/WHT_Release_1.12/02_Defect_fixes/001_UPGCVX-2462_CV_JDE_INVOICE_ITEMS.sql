CREATE OR REPLACE VIEW "CV_JDE_INVOICE_ITEMS" ("INVOICE_TYPE", "CARGO_NO", "CARGO_HARBOUR_DUES_NO", "TRANSPORTER_ADDRESS", "SERVICE_DATE", "INVOICE_ADDRESS", "LINE_ITEM_CODE", "QTY", "UNIT", "INVOICE_AMOUNT", "CURRENCY_CODE", "LIFTING_NUMBER", "LIFTER_CARGO_NAME", "DESCRIPTION", "JDE_ACCOUNT_CODE", "LIFTER_ADDRESS", "JDE_BATCH_NO","JDE_BATCH_TIMESTAMP", "INVOICE_APPROVED", "SPLIT_PCT", "PARCEL_NO", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT", "APPROVAL_STATE", "APPROVAL_BY", "APPROVAL_DATE", "REC_ID") AS 
  (SELECT 'EC-RPC' AS INVOICE_TYPE,
           ...
           DECODE (DUES.UNIT_UOM,
		   --begin Item_129174 FR -- GEDV: ISWR02739: Starts - Updated the column to use function to calculate amount.
                     'HRS', dues.LINE_ITEM_COST*NVL (EC_STORAGE_LIFT_NOMINATION.VALUE_11 (NOM.PARCEL_NO), 100)/100,
                     UE_CT_CARGO_INFO.getInvoiceAmount(dues.LINE_ITEM_COST, dues.CARGO_NO, nom.PARCEL_NO))
		   --end Item_129174 FR -- GEDV: ISWR02739: Ends
              AS INVOICE_AMOUNT,
	   ...
	   EC_CARRIER_VERSION.NAME(NOM.CARRIER_ID, NOM.NOM_DATE , '<=') as DESCRIPTION,
           ...
           EC_COMPANY_VERSION.ADDRESS_8 (
              ...
              AS LIFTER_ADDRESS,
           dues.JDE_BATCH_NO,
		   dues.JDE_BATCH_TIMESTAMP, -- Item 131306
...
create or replace TRIGGER IUV_CV_JDE_INVOICE_ITEMS
    INSTEAD OF INSERT OR UPDATE OR DELETE ON CV_JDE_INVOICE_ITEMS
    FOR EACH ROW
DECLARE
BEGIN
    IF INSERTING OR DELETING THEN
        RAISE_APPLICATION_ERROR(-20101, 'Cannot insert into or delete from this view!');
    END IF;

    IF UPDATING('JDE_BATCH_NO') THEN
        IF :old.INVOICE_TYPE = 'EC-RPC' THEN

            UPDATE DV_CARGO_HARBOUR_DUES
        ...
        ELSIF :old.INVOICE_TYPE = 'EC-REBC' THEN
            ...
            UPDATE DV_DEMURRAGE
				SET JDE_BATCH_TIMESTAMP = :new.JDE_BATCH_NO, -- Item 131306
					JDE_BATCH_NO = ue_ct_cargo_info.getJDEBatchNo ( :old.CARGO_NO, :new.JDE_BATCH_NO) -- Item 131306
            WHERE 	CARGO_NO = :old.CARGO_NO
            AND 	DEMURRAGE_TYPE IN ('COND_EBO', 'EBO');
