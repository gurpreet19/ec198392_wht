delete from t_basis_access where object_id in (select object_id from t_basis_object where object_name = '/com.ec.sale.co.screens/processing_unit' )
and  role_id in ('PA.OBJ.READ','PA.OBJ.WRITE'); -- processing  unit

delete from t_basis_access where object_id in (select object_id from t_basis_object where object_name = '/com.ec.sale.co.screens/processing_unit' )
and  role_id = 'SYST.ADM' and level_id = '40'; -- processing  unit

delete from t_basis_access where object_id in (select object_id from t_basis_object where object_name = '/com.ec.prod.po.screens/daily_tank_status_1' )
and  role_id in ('CP.ORIGINAL.READ','PA.A.DATA.WRITE', 'PA.DATA.READ', 'PA.P.DATA.WRITE', 'PA.V.DATA.WRITE'); -- tank daily
