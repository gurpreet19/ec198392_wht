...
** 13-Aug-2018 GEDV  128872: INC000017771458: Updated the procedure to calculate the Split Pct at the end of BoL Calc.
** 24-Jul-2019 WVIC	 Item 132473: ISWR03114: Added call to UE_CT_CARGO_INFO.transferCargoAnalysis and transferCargoLoaded at the end of BoL Calc
********************************************************************/
...
--FOR COND
    ...
    END IF;
    -- GEDV: Item 128872: INC000017771458 - Starts:
    IF v_job_code = 'CT_WST_BL'
	THEN
        DECLARE
           v_total           NUMBER;
           ...
           p_response_text   VARCHAR2 (2000);
		   -- Item 132473 End

           CURSOR c_Parcels (cp_parcel VARCHAR2)
           IS
              SELECT DISTINCT parcel_no
                FROM STORAGE_LIFT_NOMINATION
               WHERE cargo_no IN (SELECT DISTINCT cargo_no
                                    FROM STORAGE_LIFT_NOMINATION
                                   WHERE PARCEL_NO = cp_parcel);

        BEGIN
           v_parcel_no :=
              SUBSTR (
                 SUBSTR (p_extra_params, INSTR (p_extra_params, 'parcel_no')),
                   INSTR (SUBSTR (p_extra_params, INSTR (p_extra_params, 'parcel_no')), '=') + 1,
                   INSTR (SUBSTR (p_extra_params, INSTR (p_extra_params, 'parcel_no')), ';')
                 - INSTR (SUBSTR (p_extra_params, INSTR (p_extra_params, 'parcel_no')), '=') - 1);

           SELECT SUM (load_value)
             INTO v_total
             FROM dv_STOR_DAY_LIFTING_ALLOC
            WHERE     parcel_no IN
                         (SELECT DISTINCT parcel_no
                            FROM STORAGE_LIFT_NOMINATION
                           WHERE cargo_no IN
                  ...
                  AND ITEM_CODE IN ('LIFT_NET_M3', 'LIFT_COND_VOL_NET');

           FOR PARCEL_NOS IN c_Parcels (v_parcel_no)
           LOOP
              v_loaded := 0;
                     ...
                     AND PRODUCT_MEAS_NO IN
                            (SELECT PRODUCT_MEAS_NO
                              ...
                              WHERE     LIFTING_EVENT = 'LOAD'
                                    AND MEAS_ITEM IN ('LIFT_NET_M3', 'LIFT_COND_VOL_NET'));

              IF v_total <> 0
              THEN
                 UPDATE STORAGE_LIFT_NOMINATION
                    SET VALUE_11 = ROUND ( (v_loaded * 100 / v_total), 6)
                  WHERE PARCEL_NO = v_parcel_no;
              END IF;
           END LOOP;
           ...
           EcDp_DynSql.writetemptext ('UE_CALC_ENGINE', 'Executing transferCargoAnalysis ' || v_parcel_no);
           UE_CT_CARGO_INFO.transferCargoAnalysis (v_parcel_no, p_response_code, p_response_text);
           EcDp_DynSql.writetemptext ('UE_CALC_ENGINE', 'Finished transferCargoAnalysis with p_response_text = ' || p_response_text);

           IF p_response_code <> 0
           THEN
              RAISE_APPLICATION_ERROR (-20998, 'transferCargoAnalysis: ' || p_response_text);
           END IF;

           p_response_code := 0;
           p_response_text := NULL;

           EcDp_DynSql.writetemptext ('UE_CALC_ENGINE', 'Executing transferCargoLoaded ' || v_parcel_no);
           UE_CT_CARGO_INFO.transferCargoLoaded (v_parcel_no, p_response_code, p_response_text);
           EcDp_DynSql.writetemptext ('UE_CALC_ENGINE', 'Finished transferCargoLoaded with p_response_text = ' || p_response_text);

           IF p_response_code <> 0
           THEN
              RAISE_APPLICATION_ERROR (-20998, 'transferCargoLoaded: ' || p_response_text);
           END IF;
		   -- Item 132473 End
        END;
    END IF;

    -- GEDV: Item 128872: INC000017771458 - Ends.
