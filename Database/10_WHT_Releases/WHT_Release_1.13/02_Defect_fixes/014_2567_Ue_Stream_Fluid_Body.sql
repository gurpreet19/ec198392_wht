...
** 20.05.2014  udfd     Changes to getNetStdMass
** 15.10.2014  udfd     Changes to getStdDens
** 19.02.2015  udfd     Changes to getStdDens
** 11.03.2015  udfd     Changes to getNetStdMass
** 23.03.2015  udfd     Changes to getNetStdVol
** 11.06.2015  tlxt     updated getGrsStdMass
** 03.07.2015  udfd     Changes to getStdDens
** 18.02.2016  lbfk     Synching up with Gorgon latest package where applicable
** 08.03.2018  joey		Item_126915: calculate density for SWR_GP_COND_PROD
** 20.04.2018  eseq     Item 127686: Added logic to handle net mass/net Volume SW_GP_BOG_INERT_IMPORT depnding on the new stream SW_GP_LNG_PURGE
** 30.05.2018  joey     ISWR02542: Item_127929 calculate density for SWR_GP_COND_PROD (for rounding purpose)
** 19.10.2018  kajy     ITEM 129549: ISWR02772: Logic correction to calculate Vol for SW_GP_LNG_LOAD for overlapping cargos
** 19.08.2019  gedv     Updated NetVol calc for SW_GP_BOG_INERT_IMPORT
  ...
  code      := EcDp_Objects.GetObjCode(p_object_id);
    ...
    IF lv2_phase = 'COND' THEN
		...
		IF ECDP_OBJECTS.GETOBJCODE(p_object_id) = 'SWR_GP_COND_PROD' THEN
			...
			lv_delta_vol	:= 0;
			lv_load_vol	:= 0;
			-- ISWR02542: Item_127929 Load Volume Incorrectly Updated: Begin
			-- round mass and volume to 2 decimal places and round calculated density to 3 decimal places
