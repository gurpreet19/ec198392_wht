...
** 04-OCT-2019 KDTJ  Item 133196: ADO641091: WHS LC - SDS Load Start Defect.:Modified getSingleStorageLevel() to handle Null forecoast id.
*****************************************************************/
...
ELSE  IF lv2_forecast_source IS NULL OR lv2_forecast_source = 'TRAN_PCTR_FCAST' THEN
    ...
    ELSE
        ...
        ln_lifted_qty := ec_fcst_stream_day.MATH_VALUE_43(ln_fcst_scen_no,lv2_storage_stream,p_daytime,p_daytime);
        --ln_lifted_qty := EC_STOR_DAY_FCST_FCAST.MATH_FORECAST_QTY(p_storage_id,p_forecast_id,p_daytime,p_daytime);
    END IF;
...
BEGIN
    ...
    IF v_return_row.storage_level IS NULL THEN
		-- Item 133196 : ADO641091 - WHS LC - SDS Load Start Defect.:Modified getSingleStorageLevel() to handle Null forecoast id.
		 IF v_return_row.forecast_id is null   THEN
			Null;
		 ELSE
