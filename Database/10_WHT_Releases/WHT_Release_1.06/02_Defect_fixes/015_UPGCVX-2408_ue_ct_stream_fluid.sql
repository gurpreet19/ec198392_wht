...
** 26.11.2019  kolleadr  Bulk upgrade : UPGCVX-2408 ecbp_vcf.calcVCFstdDensity_SI got new parameter list in EC12.1
*****************************************************************/
...
BEGIN
    ...
    IF lv2_stream_phase = 'COND' THEN
        ...
        IF NVL(ln_density15C,0) <> 0 THEN
            -- Bulk upgrade UPGCVX-2408 ----- Begin
            --ln_vcf := ecbp_vcf.calcVCFstdDensity_SI(ln_density15C, 15, 15.556);
            ln_vcf := ecbp_vcf.calcVCFstdDensity_SI(ln_density15C, 15, null, null, 15.556);
            -- Bulk upgrade UPGCVX-2408 ----- End
