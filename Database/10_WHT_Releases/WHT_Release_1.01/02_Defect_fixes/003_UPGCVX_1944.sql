...
END UE_CT_GENERATE;
/
CREATE OR REPLACE PACKAGE BODY UE_CT_GENERATE IS
/*********************************************************************************************
...
** 18-Fed-2019  SUNDAR    Old class tables are changed to new class tables
*********************************************************************************************/
...
---              d1.property_name attribute_name,
          DECODE(d1.property_name, 'OP_SUB_PRODUCTIONUNIT_CODE', 'OP_SUB_PROD_UNIT_CODE', 'OP_SUB_PRODUCTIONUNIT_ID', 'OP_SUB_PROD_UNIT_ID','RESV_BLOCK_FORMATION_CODE', 'RBF_CODE', 'RESV_BLOCK_FORMATION_ID', 'RBF_ID',d1.property_name) AS attribute_name,
--- SUKF END
          d1.role_name,
          d1.db_mapping_type,
          d1.db_sql_syntax,
...
---              cr.role_name||'_ID' attribute_name,
	       DECODE(cr.role_name, 'RESV_BLOCK_FORMATION', 'RBF',cr.role_name) ||'_ID' attribute_name,
---              cr.role_name,
	       DECODE(cr.role_name, 'RESV_BLOCK_FORMATION', 'RBF',cr.role_name) as role_name,
--- SUKF END
          cr.db_mapping_type,
          cr.db_sql_syntax,
          'N' is_group,
...
IS
 ...
 --   select max(owner_cntx) as owner_cntx,property_code from class_property_cnfg WHERE class_name =upper(p_object_class_name)  group by property_code;
  ...
  --  and owner_cntx=0;
    ...
    cursor c_null is
    select from_class_name,to_class_name,role_name from class_rel_property_cnfg where to_class_name = p_object_class_name and property_code = 'DISABLED_IND' and property_value IS NULL;
...
BEGIN
  ...
  v_class_name := p_ct_class_name_override;
    ...
    SELECT class_type INTO v_description FROM class_cnfg WHERE class_name = upper(p_object_class_name);
