...
procedure Calc_MUSP_VentFlare(p_object_id IN  stream.object_id%TYPE, p_daytime   IN  DATE);

FUNCTION PopulatePassword RETURN VARCHAR2;

FUNCTION SetPasswordExpiry RETURN VARCHAR2;

PROCEDURE GenericScheduledAction(p_action_name VARCHAR2, p_parameter_1 VARCHAR2, p_parameter_2 VARCHAR2, p_date_parameter DATE);

PROCEDURE CreateAssociatedCalcSample(p_analysis_no NUMBER);

PROCEDURE SynchronizeCalcSample(p_gc_analysis_no NUMBER, p_calc_analysis_no NUMBER, p_spot_analysis_no NUMBER, p_component_no VARCHAR2 DEFAULT NULL);

PROCEDURE HandleGCSampleDelete(p_gc_sample_no NUMBER);

PROCEDURE HandleSpotSampleDelete(p_spot_sample_date DATE, p_object_id VARCHAR2, p_spot_sample_no NUMBER);

PROCEDURE CleanStagedAnalysis(p_stage_sample_number NUMBER);

PROCEDURE ImportStagedAnalysis(p_stage_sample_number NUMBER);

FUNCTION getAnalysis_no RETURN VARCHAR2;

PROCEDURE ImportStagedFlare(p_object_id VARCHAR2, p_daytime DATE);

PROCEDURE ImportStagedWellAnalysis(p_stage_sample_number NUMBER);

-- disabled for testing purposes
--PROCEDURE SynchronizeCntrMsgDistr(p_contract_id VARCHAR2);

PROCEDURE CalcAdjMass(p_object_id IN stream.object_id%TYPE, p_daytime IN DATE, p_profit_centre_id VARCHAR2 DEFAULT NULL, p_CORR_METHOD VARCHAR2, p_CORR_FACTOR NUMBER);

PROCEDURE ResetNomValidFlag(p_parcel_number NUMBER, p_forecast_id VARCHAR2);

PROCEDURE ResetAllNomValidFlags(p_forecast_id VARCHAR2);

PROCEDURE CheckEBOCDemurrageStatus(p_cargo_id NUMBER);

PROCEDURE SyncCargoAnalComp(p_analysis_no NUMBER);

PROCEDURE CheckOOSFlag(p_daytime date);

--Item 127684: Begin
PROCEDURE cfwd_tdds(p_daytime DATE, p_object_id varchar2);
--Item 127684: End
...
BEGIN
    ...
    if (p_object_id = v_mus_flare_stream_id) then
        ...
        update STRM_DAY_STREAM set GRS_MASS = n_wi_mus_flare_mass where object_id = v_wi_mus_flare_stream_id and daytime = p_daytime;
        update STRM_DAY_STREAM set GRS_MASS = n_jb_mus_flare_mass where object_id = v_jb_mus_flare_stream_id and daytime = p_daytime;
    end if;
...
PROCEDURE UpdateStatusAutonomous(p_table_name VARCHAR2, p_from_status VARCHAR2, p_to_status VARCHAR2, p_and_other VARCHAR2 DEFAULT NULL)
IS
...
IS
    ...
    SELECT class_name, stage_comp_analysis_no
...
--COMP STAGING
        ...
        FOR item IN staging_analysis LOOP
            BEGIN
            ...
            EXCEPTION WHEN OTHERS THEN
                v_error_string := 'Error at import all'||substr(v_error_string || '///' || SQLERRM, 1, 20000);
            END;
        END LOOP;
...
--FLARE STAGING
        ...
        FOR item IN staging_flare LOOP
            BEGIN
                ImportStagedFlare(item.object_id, item.daytime);
            EXCEPTION WHEN OTHERS THEN
                v_error_string := 'Error at import flare'|| substr(v_error_string || '///' || SQLERRM, 1, 20000);
            END;
        END LOOP;
        ...
        FOR staging_item IN staging_flare LOOP
            ...
            FOR flaring IN flare_parent(staging_item.object_id) LOOP
                ...
                EXCEPTION WHEN OTHERS THEN
                    v_error_string := 'Error at import flare (Save and Update)'|| substr(v_error_string || '///' || SQLERRM, 1, 20000);
                END;
            END LOOP;
        END LOOP;
...
PROCEDURE CleanNormalize(p_analysis_no NUMBER)
IS
    v_wt_factor NUMBER;
    v_mol_factor NUMBER;
    SUM_WT_PCT NUMBER;
...
SELECT SUM(wt_pct) into SUM_WT_PCT FROM FLUID_ANALYSIS_COMPONENT WHERE analysis_no = p_analysis_no ;

    IF (NVL(SUM_WT_PCT,0)<>0 AND SUM_WT_PCT<>100) THEN
      SELECT 100 / SUM(wt_pct) INTO v_wt_factor
      FROM FLUID_ANALYSIS_COMPONENT
      WHERE analysis_no = p_analysis_no;
...
SELECT SUM(mol_pct) INTO SUM_MOL_PCT FROM FLUID_ANALYSIS_COMPONENT WHERE analysis_no = p_analysis_no;

    IF (NVL(SUM_MOL_PCT,0)<>0 AND SUM_MOL_PCT<>100) THEN
      SELECT 100 / SUM(mol_pct) INTO v_mol_factor
      FROM FLUID_ANALYSIS_COMPONENT
      WHERE analysis_no = p_analysis_no;
...
-- added by UDFD
PROCEDURE MolToWt (p_analysis_no NUMBER)
IS

CURSOR components_in_analysis(cp_analysis_no NUMBER) IS
    SELECT *
    FROM fluid_analysis_component
...
BEGIN
   ...
   FOR comp_item IN components_in_analysis(p_analysis_no) LOOP
...
BEGIN
    ...
    FOR item IN analysis_samples LOOP
        ...
        INSERT INTO DV_STRM_ANALYSIS (ANALYSIS_NO, OBJECT_ID, DAYTIME, ANALYSIS_TYPE, SAMPLING_METHOD, PHASE, VALID_FROM_DATE, ANALYSIS_STATUS, DENSITY, GC_ANALYSIS_NO)
             VALUES (v_new_analysis_no, item.object_id, item.daytime, item.analysis_type, 'CALC_COMP', item.phase, item.valid_from_Date, 'APPROVED', item.density, p_analysis_no);
        ...
        UPDATE dv_strm_analysis SET gc_analysis_no = v_new_analysis_no WHERE analysis_no = p_analysis_no;

    END LOOP;

END CreateAssociatedCalcSample;

FUNCTION ValidateCalcSample(p_sample_no NUMBER) RETURN VARCHAR2
IS
    CURSOR components_in_analysis(cp_comp_set_code VARCHAR2) IS
...
BEGIN
    ...
    IF ec_object_fluid_analysis.analysis_status(p_sample_no) = 'REJECTED' OR
       (ec_object_fluid_analysis.sampling_method(p_sample_no) = 'SPOT' AND ec_object_fluid_analysis.analysis_status(p_sample_no) <> 'APPROVED') THEN
       RETURN 'N';
    END IF;

    FOR item IN components_in_analysis(ec_strm_version.comp_set_code(v_stream_id, v_daytime, '<=')) LOOP
...
IS
    ...
    CURSOR single_analysis_info(cp_analysis_no NUMBER) IS
    SELECT *
    FROM dv_strm_analysis
    WHERE analysis_no = cp_analysis_no;

    CURSOR gc_analysis_in_range(cp_from_date DATE, cp_to_date DATE, cp_object_id VARCHAR2) IS
    SELECT *
    FROM dv_strm_analysis
    ...
    CURSOR spot_analysis_prior(cp_gc_analysis_no NUMBER) IS
    SELECT analysis_no
    FROM dv_strm_analysis
    WHERE valid_from_date <= ec_object_fluid_analysis.valid_from_date(cp_gc_analysis_no)
    AND sampling_method = 'SPOT'
    AND object_id = (select x.object_id from dv_strm_analysis x where x.analysis_no = cp_gc_analysis_no)
    ...
    TYPE comp_factor_table IS TABLE OF NUMBER INDEX BY VARCHAR2(8);
    TYPE comp_value_table IS TABLE OF VARCHAR2(1) INDEX BY VARCHAR2(8);
...
BEGIN
    ...
    IF v_spot_analysis_no IS NOT NULL AND v_calc_analysis_no IS NULL AND v_gc_analysis_no IS NULL THEN
        FOR spot_item IN single_analysis_info(v_spot_analysis_no) LOOP
            SELECT nvl(min(valid_from_date), sysdate + 2*365) INTO v_next_spot_analysis_date
            FROM dv_strm_analysis
            ...
            FOR update_item IN gc_analysis_in_range(spot_item.valid_from_date, v_next_spot_analysis_date, spot_item.object_id) LOOP
        ...
        SET spot_analysis_no = v_spot_analysis_no, analysis_status = CASE WHEN analysis_status = 'REJECTED' THEN 'REJECTED' ELSE 'APPROVED' END  --106940
        WHERE analysis_no IN (v_calc_analysis_no);

        UPDATE dv_strm_analysis
        SET spot_analysis_no = v_spot_analysis_no, analysis_status = CASE WHEN NVL(analysis_status,'N') = 'N' THEN 'APPROVED' ELSE analysis_status END  --if it is null, it means it is inserted from staging and it needs to be updated as approved
    ...
    IF v_spot_analysis_no IS NOT NULL AND v_calc_analysis_no IS NOT NULL AND v_gc_analysis_no IS NOT NULL THEN
        IF v_spot_analysis_no = -1 THEN -- 106940: -1 is a flag indicating that there were no APPROVED spot analysis samples prior to loading this from the GC.
        ...
        ELSE
            ...
            IF NVL(p_component_no, 'C9+') IN ('IC5','NC5','NC6','NC7','NC8','C9+') THEN
                    ...
                    IF comp_item.component_no IN ('IC5','NC5','NC6','NC7','NC8','C9+') THEN
                        ...
                        ELSE
                            ...
                            IF NVL(comp_item.mol_pct, 0) > 0 THEN
                                ...
                                --comp_values.delete();
								comp_values.delete(comp_item.component_no); /* REQ3*/
                ...
                IF v_final_component <> 'C9+' THEN
                    ...
                    FOR comp_item IN components_in_analysis(v_spot_analysis_no) LOOP
        ...
        END IF;
            ...
            UPDATE dv_strm_analysis
            SET spot_analysis_no = v_spot_analysis_no, analysis_status = 'INFO'
            ...
            SELECT DENSITY INTO v_density FROM dv_strm_analysis WHERE analysis_no = v_gc_analysis_no;

            UPDATE dv_strm_analysis
            SET density = v_density, spot_analysis_no = v_spot_analysis_no, analysis_status = CASE WHEN analysis_status = 'REJECTED' THEN 'REJECTED' ELSE 'APPROVED' END  --106940:  If it's rejected, leave it rejected (but continue to update it)
            WHERE analysis_no = v_calc_analysis_no;
...
BEGIN
    ...
    IF v_calc_sample_no IS NULL THEN
        FOR ANALYSIS_ITEM IN (SELECT ANALYSIS_NO FROM OBJECT_FLUID_ANALYSIS WHERE TEXT_1 = p_gc_sample_no) LOOP
            DELETE FROM dv_strm_analysis WHERE analysis_no = ANALYSIS_ITEM.ANALYSIS_NO;
            DELETE FROM fluid_analysis_component WHERE analysis_no = ANALYSIS_ITEM.ANALYSIS_NO;
            DELETE FROM object_fluid_analysis WHERE analysis_no = ANALYSIS_ITEM.ANALYSIS_NO;
        END LOOP;
    ELSE
...
IS
    ...
    AND (p_object_id IS NULL OR p_object_id = OBJECT_ID)
    AND sampling_method = 'SPOT'
    AND ANALYSIS_STATUS = 'APPROVED'
    ORDER BY valid_from_date DESC;
...
BEGIN
    ...
    SELECT object_id INTO v_stream_id
    ...
    FOR attribute_item IN attributes_to_clean LOOP
        ...
        IF v_intermediate IS NOT NULL AND length(v_intermediate) > 0 THEN
            ...
            v_sql := 'UPDATE ct_stage_comp_analysis SET ANALYSIS_DATE = TO_DATE(''' || v_first || ''', ''DD-MON-YY HH24:MI:SS''), ' ||
                              'TEXT_1 = ''' || v_third || ''', ' || ec_class_attribute_cnfg.db_sql_syntax('CT_STAGE_COMP_ANALY_TXT', attribute_item.attribute_name) || ' = ''' || v_second || ''' WHERE ' ||
    ...
    UPDATE ct_stage_comp_analysis SET analysis_type = v_analysis_type, sampling_method = v_sampling_method, phase = v_phase
    WHERE stage_comp_analysis_no = p_stage_sample_number;
...
IS
    ...
    SELECT * FROM CT_STAGE_COMP_ANALYSIS
    ...
    CURSOR lookup_cargos(cp_cargo_ref VARCHAR2) IS
    SELECT *
    FROM dv_cargo_info
   ...
   /* OR cargo_no IN
    ...
    CURSOR lookup_analyses(cp_object_id VARCHAR2, cp_analysis_date DATE) IS
    SELECT analysis_no, sampling_method
    FROM dv_strm_analysis
    ...
    CURSOR existing_cargo_analysis(cp_cargo_no NUMBER, cp_official_ind VARCHAR2, cp_analysis_type VARCHAR2, cp_sample_source VARCHAR2 DEFAULT NULL) is
    SELECT analysis_no
    FROM dv_cargo_analysis
    WHERE cargo_no = cp_cargo_no
    AND (nvl(OFFICIAL_IND, 'N') = nvl(cp_official_ind, 'N') OR 'ALL' = cp_official_ind)
...
BEGIN
    ...
    FOR item IN analysis_rows LOOP
       ...
       FOR cargo_item IN lookup_cargos(item.text_1) LOOP
            ...
            select nvl(COUNT(RUN_NO),0) into v_parcel_count from tv_CALC_TRAN_TO_LOG
            WHERE EXIT_STATUS='SUCCESS' AND EVENT_TYPE='LOAD'
            AND PARCEL_NO IN (SELECT PARCEL_NO FROM STORAGE_LIFT_NOMINATION WHERE CARGO_NO=cargo_item.cargo_no );
            ...
            IF ec_stream.object_code(item.object_id) = 'SW_GP_LNG_CARGO' OR ec_stream.object_code(item.object_id) = 'SW_GP_COND_CARGO' THEN
                ...
                FOR analy_item IN existing_cargo_analysis(cargo_item.cargo_no, 'Y', null) LOOP
                ...
                IF v_existing_analysis_no IS NULL THEN
                    ...
                    RAISE_APPLICATION_ERROR(-20101, 'There is no existing official cargo analysis record for this particular cargo!');
                END IF;
                -- Set the basic info
                --Item 113805: Begin
                /*
                ...
                */
                UPDATE dv_cargo_analysis_basic SET analysis_value = item.text_10 WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'H2S';
                --Item 113805: End
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.text_9, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'MERCAPTAN';
                --Item 113805: Begin
                /*
                ...
                */
                UPDATE dv_cargo_analysis_basic SET analysis_value = item.text_8 WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'SULFUR';
                --Item 113805: End
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.text_7, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'HG';
                ...
                */
                UPDATE dv_cargo_analysis_basic SET analysis_value = item.co2_mol WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'CO2';
                --Item 113805: End

                --Item 113805: Begin
                UPDATE dv_cargo_analysis_basic SET analysis_value = item.text_11 WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'H2S_UOP212';
                UPDATE dv_cargo_analysis_basic SET analysis_value = item.text_12 WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'CO2_ISO';
                UPDATE dv_cargo_analysis_basic SET analysis_value = item.text_13 WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'SULFUR_D6667';
                --Item 113805: End

                -- added by udfd - needed for Condensate Analysis
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.rvp, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'RVP';
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.bs_w, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'BS_W';
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.density, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'DENSITY';
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.text_2, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'SALT';
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.text_7, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'MERCURY';
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.text_6, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'API';
                UPDATE dv_cargo_analysis_basic SET analysis_value = NVL(item.text_8, 0) WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'COND_SULFUR';
                ...
                IF ec_cargo_analysis.analysis_type(v_existing_analysis_no) = 'WST_LAB' THEN -- Assume this means that the official record should receive data from the lab
                   ...
                   IF v_total_comps>0 THEN
                    ...
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.C1_MOL),20), 0), analysis_value_raw = NVL(to_number(item.c1_mol), 0) WHERE analysis_item_code = 'C1' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.C2_MOL),20), 0), analysis_value_raw = NVL(to_number(item.c2_mol), 0) WHERE analysis_item_code = 'C2' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.C3_MOL),20), 0), analysis_value_raw = NVL(to_number(item.c3_mol), 0) WHERE analysis_item_code = 'C3' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.IC4_MOL),20), 0), analysis_value_raw = NVL(to_number(item.ic4_mol), 0) WHERE analysis_item_code = 'IC4' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.NC4_MOL),20), 0), analysis_value_raw = NVL(to_number(item.nc4_mol), 0) WHERE analysis_item_code = 'NC4' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.IC5_MOL),20), 0), analysis_value_raw = NVL(to_number(item.ic5_mol), 0) WHERE analysis_item_code = 'IC5' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.NC5_MOL),20), 0), analysis_value_raw = NVL(to_number(item.nc5_mol), 0) WHERE analysis_item_code = 'NC5' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.NC6_MOL),20), 0), analysis_value_raw = NVL(to_number(item.Nc6_mol), 0) WHERE analysis_item_code = 'NC6' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.NC7_MOL),20), 0), analysis_value_raw = NVL(to_number(item.Nc7_mol), 0) WHERE analysis_item_code = 'NC7' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.NC8_MOL),20), 0), analysis_value_raw = NVL(to_number(item.Nc8_mol), 0) WHERE analysis_item_code = 'NC8' AND analysis_no = v_existing_analysis_no;
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.C9_PLUS_MOL),20), 0), analysis_value_raw = NVL(to_number(item.c9_PLUS_mol), 0) WHERE analysis_item_code = 'C9+' AND analysis_no = v_existing_analysis_no;
                    ...
                    UPDATE dv_cargo_analysis_component SET analysis_value = NVL(round(to_number(item.N2_MOL),20), 0), analysis_value_raw = NVL(to_number(item.n2_mol), 0) WHERE analysis_item_code = 'N2' AND analysis_no = v_existing_analysis_no;
                    -- 125197 end

                    -- Normalize components
                    SELECT SUM(NVL(analysis_value,0)) INTO v_total_components
                    ...
                    WHERE analysis_no = v_existing_analysis_no
                    AND analysis_item_code = (SELECT x1.analysis_item_code
                                              FROM (SELECT x2.analysis_item_code, RANK() OVER (PARTITION BY x2.analysis_no ORDER BY x2.analysis_value DESC, x2.analysis_item_code ASC) rankx
                                                    FROM dv_cargo_analysis_component x2
                                                    WHERE x2.analysis_no = v_existing_analysis_no) x1
                                              WHERE x1.rankx = 1);
                   END IF;
                     -- Item 127684:ISWR02329 :End: IF condition ends.

                    -- udfd Calculate C3+, C4+, C5+
                    SELECT SUM(NVL(analysis_value,0)) INTO v_propane_plus
                    FROM dv_cargo_analysis_component
                    WHERE analysis_no = v_existing_analysis_no and analysis_item_code in ('C3','IC4', 'NC4', 'IC5', 'NC5', 'NC6', 'NC7', 'NC8', 'C9+');

                    SELECT SUM(NVL(analysis_value,0)) INTO v_butane_plus
                    FROM dv_cargo_analysis_component
                    WHERE analysis_no = v_existing_analysis_no and analysis_item_code in ('IC4', 'NC4', 'IC5', 'NC5', 'NC6', 'NC7', 'NC8', 'C9+');

                    SELECT SUM(NVL(analysis_value,0)) INTO v_pentane_plus
                    FROM dv_cargo_analysis_component
                    WHERE analysis_no = v_existing_analysis_no and analysis_item_code in ('IC5', 'NC5', 'NC6', 'NC7', 'NC8', 'C9+');

                    UPDATE dv_cargo_analysis_basic SET analysis_value = v_propane_plus WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'C3+';
                    UPDATE dv_cargo_analysis_basic SET analysis_value = v_butane_plus WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'C4+';
                    UPDATE dv_cargo_analysis_basic SET analysis_value = v_pentane_plus WHERE analysis_no = v_existing_analysis_no AND NVL(analysis_value,0) = 0  AND analysis_item_code = 'C5+';

                END IF;

                -- Update the Wheatstone Lab record as well
                FOR analy_item IN existing_cargo_analysis(cargo_item.cargo_no, 'N', 'WST_LAB') LOOP
                    v_existing_analysis_no := analy_item.analysis_no;
                END LOOP;
            ...
            IF ec_stream.object_code(item.object_id) = 'SW_GP_LNG_CARGO_LOC_GC'
                OR ec_stream.object_code(item.object_id) = 'SW_GP_LNG_CARGO_LOC_GC_B'
                OR ec_stream.object_code(item.object_id) = 'SW_GP_COND_CARGO_LOC_GC' THEN

                SELECT CASE
                    WHEN ec_stream.object_code(item.object_id) = 'SW_GP_LNG_CARGO_LOC_GC' THEN 'Online GC A'
                    WHEN ec_stream.object_code(item.object_id) = 'SW_GP_LNG_CARGO_LOC_GC_B' THEN 'Online GC B'
                    ELSE NULL END INTO v_sample_source FROM DUAL;
...
--tlxt: 10-oct-2016: when receive CONd analysis, it should be inserted directly into PA
        IF v_cargo = 'Y' AND (ec_stream.object_code(item.object_id) = 'SW_GP_COND_CARGO' OR ec_stream.object_code(item.object_id) = 'SW_GP_COND_CARGO_LOC_GC') AND v_parcel_count=0 THEN -- Item 127684:ISWR02329: v_parcel_count ADDED
            ...
            SELECT MAX(PARCEL_NO) INTO p_parcel_no FROM DV_STORAGE_LIFT_NOMINATION WHERE CARGO_NO = item.text_1;
       ...
       IF v_cargo = 'N' THEN
            FOR lookup_item IN lookup_analyses(item.object_id, item.analysis_date) LOOP
               ...
               IF    (    (ITEM.CLASS_NAME = 'CT_STAGE_COMP_ANALYSIS')
                      AND (lookup_item.SAMPLING_METHOD = 'GC'))
                  OR (    (ITEM.CLASS_NAME = 'CT_STAGE_COMP_ANALY_TXT')
                      AND (lookup_item.SAMPLING_METHOD = 'SPOT'))
               THEN
...
--tlxt: group the staging data into single row for the same object that splits by seconds
            FOR SAME_ITEM IN analysis_rows_GROUP LOOP
                ...
                IF p_lockInd<>'Y' and EcDp_month_lock.lockedMonthAffected(SAME_ITEM.analysis_date, lr_next_analysis.valid_from_date) is null
				   -- Item 127925: ISWR02513: Begin
                   and (trunc(sysdate-15)<=trunc(SAME_ITEM.analysis_date))/* REQ2*/
				   -- Item 127925: ISWR02513: End
				THEN
                    ...
                    IF v_existing_analysis_no IS NULL THEN
                        ...
                        -- Insert Base
                        INSERT INTO dv_strm_analysis(analysis_no, object_id, daytime, analysis_type, sampling_method, phase, sample_press, sample_temp, valid_from_date, density, rvp, bsw_wt)
                        VALUES (v_existing_analysis_no, SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, SAME_ITEM.sample_pressure,
                                SAME_ITEM.sample_temp, SAME_ITEM.analysis_date, SAME_ITEM.density, SAME_ITEM.rvp, SAME_ITEM.value_10);

                        -- Insert Analysis
                        INSERT INTO dv_strm_comp_analysis(object_id, daytime, analysis_type, sampling_method, phase, analysis_no, wt_pct, mol_pct, component_no)
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.c1_wt, 0), NVL(SAME_ITEM.c1_mol, 0), 'C1' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'C1') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.c2_wt, 0), NVL(SAME_ITEM.c2_mol, 0), 'C2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'C2') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.c3_wt, 0), NVL(SAME_ITEM.c3_mol, 0), 'C3' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'C3') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.ic4_wt, 0), NVL(SAME_ITEM.ic4_mol, 0), 'IC4' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'IC4') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.nc4_wt, 0), NVL(SAME_ITEM.nc4_mol, 0), 'NC4' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'NC4') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.ic5_wt, 0), NVL(SAME_ITEM.ic5_mol, 0), 'IC5' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'IC5') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.nc5_wt, 0), NVL(SAME_ITEM.nc5_mol, 0), 'NC5' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'NC5') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.Nc6_wt, 0), NVL(SAME_ITEM.Nc6_mol, 0), 'NC6' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'NC6') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.Nc7_wt, 0), NVL(SAME_ITEM.Nc7_mol, 0), 'NC7' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'NC7') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.Nc8_wt, 0), NVL(SAME_ITEM.Nc8_mol, 0), 'NC8' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'NC8') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.c9_PLUS_wt, 0), NVL(SAME_ITEM.c9_PLUS_mol, 0), 'C9+' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'C9+') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.o2_wt, 0), NVL(SAME_ITEM.o2_mol, 0), 'O2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'O2') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.n2_wt, 0), NVL(SAME_ITEM.n2_mol, 0), 'N2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'N2') UNION ALL
                        SELECT SAME_ITEM.object_id, SAME_ITEM.analysis_date, SAME_ITEM.analysis_type, SAME_ITEM.sampling_method, SAME_ITEM.phase, v_existing_analysis_no, NVL(SAME_ITEM.co2_wt, 0), NVL(SAME_ITEM.co2_mol, 0), 'CO2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_STRM_VERSION.comp_set_code(SAME_ITEM.object_id, SAME_ITEM.analysis_date, '<=') AND COMPONENT_NO = 'CO2');
    ...
    IF v_value = 0 THEN -- "Down Record"
        ...
        IF v_previous_value > 0 AND NVL(v_previous_processed, 'N') = 'N' THEN
            FOR item IN parent_object LOOP
                INSERT INTO dv_strm_day_nr_other (CLASS_NAME, OBJECT_ID, DAYTIME, START_DAYTIME, END_DAYTIME, ASSET_ID, ASSET_TYPE, PROD_DAY_RELEASE)
                VALUES ('STRM_DAY_NR_OTHER', item.object_id, ecdp_productionday.getproductionday(null, item.object_id, v_previous_daytime, 'N'), v_previous_daytime, p_daytime, item.child_obj_id, 'STREAM', v_previous_value);
            END LOOP;

            UPDATE ct_stage_stream_event SET text_1 = 'Y' WHERE object_id = p_object_id AND (daytime = p_daytime OR daytime = v_previous_daytime) AND class_name = 'CT_STAGE_FLARE_EVENT';
...
IS
    ...
    -- Assumption made here that valid_from_date is a valid key field (as opposed to daytime)
    CURSOR lookup_analyses(cp_object_id VARCHAR2, cp_analysis_date DATE, cp_phase VARCHAR2) IS
    SELECT analysis_no
...
BEGIN
    ...
    FOR item IN analysis_rows LOOP
            ...
            FOR lookup_item IN lookup_analyses(item.object_id, item.analysis_date, item.phase) LOOP
                v_existing_analysis_no := lookup_item.analysis_no;
            END LOOP;
            ...
            IF v_existing_analysis_no IS NULL THEN
                ...
                IF item.phase='OIL' then
                    INSERT INTO dv_well_analysis(analysis_no, object_id, daytime, analysis_type, analysis_status, sampling_method, phase, sample_press, sample_temp, valid_from_date, density)
                    VALUES (v_existing_analysis_no, item.object_id, item.analysis_date, item.analysis_type, 'APPROVED', item.sampling_method, item.phase, item.sample_pressure,
                            item.sample_temp, item.analysis_date, item.density);

                ELSE

                    INSERT INTO dv_well_analysis(analysis_no, object_id, daytime, analysis_type, analysis_status, sampling_method, phase, sample_press, sample_temp, valid_from_date, gas_density)
                    VALUES (v_existing_analysis_no, item.object_id, item.analysis_date, item.analysis_type, 'APPROVED', item.sampling_method, item.phase, item.sample_pressure,
                            item.sample_temp, item.analysis_date, item.density);
                ...
                -- Insert Analysis
                INSERT INTO dv_well_comp_analysis(object_id, daytime, analysis_type, sampling_method, phase, analysis_no, wt_pct, mol_pct, component_no)
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.c1_wt, 0), NVL(item.c1_mol, 0), 'C1' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'C1') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.c2_wt, 0), NVL(item.c2_mol, 0), 'C2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'C2') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.c3_wt, 0), NVL(item.c3_mol, 0), 'C3' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'C3') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.ic4_wt, 0), NVL(item.ic4_mol, 0), 'IC4' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'IC4') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.nc4_wt, 0), NVL(item.nc4_mol, 0), 'NC4' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'NC4') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.ic5_wt, 0), NVL(item.ic5_mol, 0), 'IC5' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'IC5') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.nc5_wt, 0), NVL(item.nc5_mol, 0), 'NC5' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'NC5') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.Nc6_wt, 0), NVL(item.Nc6_mol, 0), 'NC6' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'NC6') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.Nc7_wt, 0), NVL(item.Nc7_mol, 0), 'NC7' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'NC7') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.Nc8_wt, 0), NVL(item.Nc8_mol, 0), 'NC8' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'NC8') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.c9_PLUS_wt, 0), NVL(item.c9_PLUS_mol, 0), 'C9+' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'C9+') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.o2_wt, 0), NVL(item.o2_mol, 0), 'O2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'O2') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.n2_wt, 0), NVL(item.n2_mol, 0), 'N2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'N2') UNION ALL
                SELECT item.object_id, item.analysis_date, item.analysis_type, item.sampling_method, item.phase, v_existing_analysis_no, NVL(item.co2_wt, 0), NVL(item.co2_mol, 0), 'CO2' FROM DUAL WHERE EXISTS (SELECT 1 FROM TV_COMPONENT_SET_LIST WHERE CODE = EC_WELL_VERSION.comp_gas_code(item.object_id, item.analysis_date, '<=') AND COMPONENT_NO = 'CO2');
            ...
            ELSE
                ...
                END IF;
                 ...
                 -- Update analysis
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.c1_wt, 0), mol_pct = NVL(item.c1_mol, 0) WHERE component_no = 'C1' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.c2_wt, 0), mol_pct = NVL(item.c2_mol, 0) WHERE component_no = 'C2' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.c3_wt, 0), mol_pct = NVL(item.c3_mol, 0) WHERE component_no = 'C3' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.ic4_wt, 0), mol_pct = NVL(item.ic4_mol, 0) WHERE component_no = 'IC4' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.nc4_wt, 0), mol_pct = NVL(item.nc4_mol, 0) WHERE component_no = 'NC4' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.ic5_wt, 0), mol_pct = NVL(item.ic5_mol, 0) WHERE component_no = 'IC5' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.nc5_wt, 0), mol_pct = NVL(item.nc5_mol, 0) WHERE component_no = 'NC5' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.Nc6_wt, 0), mol_pct = NVL(item.Nc6_mol, 0) WHERE component_no = 'NC6' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.Nc7_wt, 0), mol_pct = NVL(item.Nc7_mol, 0) WHERE component_no = 'NC7' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.Nc8_wt, 0), mol_pct = NVL(item.Nc8_mol, 0) WHERE component_no = 'NC8' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.c9_PLUS_wt, 0), mol_pct = NVL(item.c9_PLUS_mol, 0) WHERE component_no = 'C9+' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.co2_wt, 0), mol_pct = NVL(item.co2_mol, 0) WHERE component_no = 'CO2' AND analysis_no = v_existing_analysis_no;
                 UPDATE fluid_analysis_component SET wt_pct = NVL(item.n2_wt, 0), mol_pct = NVL(item.n2_mol, 0) WHERE component_no = 'N2' AND analysis_no = v_existing_analysis_no;
...
BEGIN
 ...
 IF p_CORR_METHOD = 'CORR_FACTOR' THEN
   ...
   IF p_profit_centre_id IS NULL THEN
       ...
       UPDATE CT_STRM_DAY_ADJ SET  TOTAL_MASS_ADJ = ln_total_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  C1_MASS_ADJ = ln_c1_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  C2_MASS_ADJ = ln_c2_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  C3_MASS_ADJ = ln_c3_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  IC4_MASS_ADJ = ln_ic4_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  NC4_MASS_ADJ = ln_nc4_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  IC5_MASS_ADJ = ln_ic5_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  NC5_MASS_ADJ = ln_nc5_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  NC6_MASS_ADJ = ln_nc6_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  NC7_MASS_ADJ = ln_nc7_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  NC8_MASS_ADJ = ln_nc8_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  C9P_MASS_ADJ = ln_c9p_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  CO2_MASS_ADJ = ln_co2_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_ADJ SET  N2_MASS_ADJ = ln_n2_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND DAYTIME = p_DAYTIME;
   ...
   IF p_profit_centre_id IS NOT NULL THEN
       ...
       -- Update table with adjusted mass
       UPDATE CT_STRM_DAY_PC_ADJ SET  TOTAL_MASS_ADJ = ln_total_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  C1_MASS_ADJ = ln_c1_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  C2_MASS_ADJ = ln_c2_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  C3_MASS_ADJ = ln_c3_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  IC4_MASS_ADJ = ln_ic4_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  NC4_MASS_ADJ = ln_nc4_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  IC5_MASS_ADJ = ln_ic5_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  NC5_MASS_ADJ = ln_nc5_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  NC6_MASS_ADJ = ln_nc6_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  NC7_MASS_ADJ = ln_nc7_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  NC8_MASS_ADJ = ln_nc8_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  C9P_MASS_ADJ = ln_c9p_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  CO2_MASS_ADJ = ln_co2_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
       UPDATE CT_STRM_DAY_PC_ADJ SET  N2_MASS_ADJ = ln_n2_mass_adj WHERE OBJECT_ID = p_OBJECT_ID AND PROFIT_CENTRE_ID = p_PROFIT_CENTRE_ID AND DAYTIME = p_DAYTIME;
...
BEGIN
    ...
    IF(p_parcel_number IS NOT NULL AND p_forecast_id IS NOT NULL) THEN
        ...
        UPDATE DV_FCST_STOR_LIFT_NOM SET NOM_VALID = NULL, VALIDATION_ISSUES = NULL WHERE PARCEL_NO = p_parcel_number AND FORECAST_ID = p_forecast_id AND (RECORD_STATUS = 'P' OR RECORD_STATUS IS NULL);
...
BEGIN
    ...
    IF(p_forecast_id IS NOT NULL) THEN
        ...
        UPDATE DV_FCST_STOR_LIFT_NOM SET NOM_VALID = NULL, VALIDATION_ISSUES = NULL WHERE FORECAST_ID = p_forecast_id AND (RECORD_STATUS = 'P' OR RECORD_STATUS IS NULL);
...
BEGIN
  ...
  IF EC_PROSTY_CODES.ALT_CODE(lv_status,'TRAN_CARGO_STATUS') = 'D'
    THEN RAISE_APPLICATION_ERROR(-20854, 'Demurrage or EBOC worksheets can only be created for cargos with cargo status other than Cancelled');
  END IF;
...
BEGIN
    SELECT SUM(NVL(analysis_value,0)) INTO v_propane_plus
    FROM dv_cargo_analysis_component
    WHERE analysis_no = p_analysis_no and analysis_item_code in ('C3','IC4', 'NC4', 'IC5', 'NC5', 'NC6', 'NC7', 'NC8', 'C9+');

    SELECT SUM(NVL(analysis_value,0)) INTO v_butane_plus
    FROM dv_cargo_analysis_component
    WHERE analysis_no = p_analysis_no and analysis_item_code in ('IC4', 'NC4', 'IC5', 'NC5', 'NC6', 'NC7', 'NC8', 'C9+');

    SELECT SUM(NVL(analysis_value,0)) INTO v_pentane_plus
    FROM dv_cargo_analysis_component
    WHERE analysis_no = p_analysis_no and analysis_item_code in ('IC5', 'NC5', 'NC6', 'NC7', 'NC8', 'C9+');

    UPDATE dv_cargo_analysis_basic SET analysis_value = v_propane_plus WHERE analysis_no = p_analysis_no AND analysis_item_code = 'C3+';
    UPDATE dv_cargo_analysis_basic SET analysis_value = v_butane_plus WHERE analysis_no = p_analysis_no AND analysis_item_code = 'C4+';
    UPDATE dv_cargo_analysis_basic SET analysis_value = v_pentane_plus WHERE analysis_no = p_analysis_no AND analysis_item_code = 'C5+';
...
BEGIN
  ...
  IF lv_count =2
    THEN RAISE_APPLICATION_ERROR(-20854, 'Out Of Service Flag can either be set on Wet or Dry Flare but not both at the same time');
  END IF;
...
PROCEDURE cfwd_tdds(p_daytime DATE, p_object_id varchar2)
      IS
        ...
        IS
            SELECT OBJECT_CODE, DAYTIME, WAT_DIP_LEVEL, BSW_VOL, CORR_DENSITY
        ...
        IS
            SELECT
            (SELECT MAX(DENSITY) AS DENSITY FROM OBJECT_FLUID_ANALYSIS where
            daytime =(select max(daytime) from OBJECT_FLUID_ANALYSIS where object_id=AB.OBJECT_ID and daytime<=AB.DAYTIME AND analysis_status='APPROVED')
            and  object_id=AB.OBJECT_ID
            and analysis_status='APPROVED') AS DENSITY,
            EcBp_Stream_Fluid.findNetStdMass (AB.object_id, AB.daytime) AS NET_MASS_OIL
            FROM strm_day_stream AB WHERE OBJECT_ID IN
            (select DISTINCT OBJECT_ID from ov_stream where to_node_CODE='FC1_WS1_STL_2300' and end_date is null AND ALLOC_PERIOD <> 'NONE' AND ALLOC_DATA_FREQ IS NOT NULL)
            and DAYTIME=(to_date(p_daytime));


    BEGIN
		    SELECT COUNT(ANALYSIS_NO) INTO v_cargo_anlysis_flag FROM DV_CARGO_ANALYSIS WHERE TRUNC(DAYTIME)=TRUNC(p_daytime)
			AND OFFICIAL_IND='Y' AND PRODUCT_CODE='COND' AND ec_cargo_analysis_item.analysis_value(analysis_no, 'DENSITY') IS NOT NULL;

			IF v_cargo_anlysis_flag=0 THEN

			   Select count(object_id) into p_count from ov_stream where to_node_CODE='FC1_WS1_STL_2300' and end_date is null AND ALLOC_PERIOD <> 'NONE'
			   AND ALLOC_DATA_FREQ IS NOT NULL and object_id=p_object_id;
			   ...
			   IF p_count>0 THEN
					...
					LOOP
						...
						SELECT nvl(sum(STD_NET_VOL_OIL),0), nvl(sum(CALC_NET_MASS),0) into p_tot_vol_tank, p_tot_mass_tank
						FROM DV_TANK_DAY_DIP_STATUS
						WHERE DAYTIME = p_daytime-1
						AND OBJECT_CODE IN(
						SELECT CODE
