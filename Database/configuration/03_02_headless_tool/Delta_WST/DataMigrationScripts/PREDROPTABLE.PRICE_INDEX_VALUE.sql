...
FOR CUR IN (SELECT OWNER, CONSTRAINT_NAME , TABLE_NAME 	FROM USER_CONSTRAINTS 	WHERE  TABLE_NAME = 'PRICE_IN_ITEM_VALUE') 
LOOP
...
FROM PRICE_INDEX_VALUE ;
	...
	SELECT COUNT(*) INTO HASENTRY FROM USER_TRIGGERS 
		WHERE TRIGGER_NAME = 'IU_PRICE_IN_ITEM_VALUE'; 
		IF HASENTRY > 0 THEN 
		EXECUTE IMMEDIATE 'ALTER TRIGGER IU_PRICE_IN_ITEM_VALUE DISABLE';
		END IF;

		UPDATE PRICE_IN_ITEM_VALUE PIIV 
	...
	SELECT COUNT(*) INTO HASENTRY FROM USER_TRIGGERS 
		WHERE TRIGGER_NAME = 'IU_PRICE_IN_ITEM_VALUE'; 
		IF HASENTRY > 0 THEN 
		EXECUTE IMMEDIATE 'ALTER TRIGGER IU_PRICE_IN_ITEM_VALUE ENABLE';
		END IF;
