...
END;
--~^UTDELIM^~--
-- ECPD27282_CALCULATION_UPGRADE_SCRIPT_2...
DECLARE 
HASENTRY NUMBER;
BEGIN
	SELECT COUNT(*) INTO HASENTRY FROM USER_TRIGGERS 
		WHERE TRIGGER_NAME = 'IU_OV_CALCULATION'; 
		IF HASENTRY > 0 THEN 
		EXECUTE IMMEDIATE 'ALTER TRIGGER IU_OV_CALCULATION DISABLE';
		END IF;
		
	UPDATE OV_CALCULATION C SET C.VERSION_BACK_COMPAT = 'Y' WHERE C.CALC_SCOPE = 'MAIN';
  
  	SELECT COUNT(*) INTO HASENTRY FROM USER_TRIGGERS 
		WHERE TRIGGER_NAME = 'IU_OV_CALCULATION'; 
		IF HASENTRY > 0 THEN 
		EXECUTE IMMEDIATE 'ALTER TRIGGER IU_OV_CALCULATION ENABLE';
		END IF;
EXCEPTION
   WHEN OTHERS 
     THEN
