...
BEGIN
  ...
  SELECT COUNT(*) INTO HASENTRY FROM USER_TRIGGERS 
		WHERE TRIGGER_NAME = 'IU_FCST_STREAM_MTH_JN'; 
		IF HASENTRY > 0 THEN 
		EXECUTE IMMEDIATE 'ALTER TRIGGER IU_FCST_STREAM_MTH_JN DISABLE';
		END IF;
		
	UPDATE FCST_STREAM_MTH_JN SET SCENARIO_ID = LV2_OBJECT_ID_CURSOR WHERE SCENARIO_ID IS NULL;
	
	SELECT COUNT(*) INTO HASENTRY FROM USER_TRIGGERS 
		WHERE TRIGGER_NAME = 'IU_FCST_STREAM_MTH_JN'; 
		IF HASENTRY > 0 THEN 
		EXECUTE IMMEDIATE 'ALTER TRIGGER IU_FCST_STREAM_MTH_JN ENABLE';
		END IF;
