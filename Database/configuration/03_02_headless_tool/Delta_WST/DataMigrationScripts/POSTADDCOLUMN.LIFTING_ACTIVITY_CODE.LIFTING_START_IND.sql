-- move data 
--	UPDATE LIFTING_ACTIVITY_CODE SET LIFTING_START_IND = LIFTING_START;
DECLARE 
HASENTRY NUMBER;
BEGIN
	...
	FOR X IN ( SELECT COUNT(*) CNT FROM DUAL WHERE EXISTS 
		( SELECT 1 FROM USER_TAB_COLUMNS WHERE TABLE_NAME = 'LIFTING_ACTIVITY_CODE' AND COLUMN_NAME = 'LIFTING_START' ) ) LOOP
		IF ( X.CNT = 1 ) THEN
		EXECUTE IMMEDIATE 'UPDATE LIFTING_ACTIVITY_CODE SET LIFTING_START_IND = LIFTING_START';
		END IF;
	END LOOP;
