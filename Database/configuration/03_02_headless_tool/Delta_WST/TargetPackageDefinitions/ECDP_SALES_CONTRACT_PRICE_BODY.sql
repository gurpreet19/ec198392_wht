...
lrec_pp_value := ec_product_price_value.row_by_pk(p_object_id,p_price_concept_code,p_price_element_code,p_daytime);
 ...
 WHERE object_id = p_object_id
   ...
   AND price_element_code = c_val.price_element_code;
    ...
    IF  (ln_inserted = 0)  THEN
      ...
      INSERT
      INTO     product_price_value (object_id,price_concept_code,price_element_code,daytime,calc_price_value,adj_price_value,comments,created_by)
      VALUES   (p_object_id,p_price_concept_code, c_val.price_element_code,p_daytime,lrec_pp_value.calc_price_value,lrec_pp_value.adj_price_value,lrec_pp_value.comments,lrec_pp_value.created_by);
    END IF;
END LOOP;
...
CURSOR c_prod_price_value (cp_object_id VARCHAR2, cp_price_concept_code VARCHAR2, cp_daytime DATE) IS
SELECT price_element_code
  FROM product_price_value
...
FOR c_val IN c_price_elements (p_price_Concept_code) LOOP
    ...
    FOR c_ppv IN c_prod_price_value(p_object_id,p_price_concept_code,p_daytime) LOOP
