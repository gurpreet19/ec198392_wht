...
IS
  ...
  SELECT attribute_name FROM class_attribute ca
...
BEGIN
  ...
  IF Ecdp_classmeta.getClassType(p_class_name) = 'OBJECT' THEN
    lv2_table_name := ec_class_db_mapping.db_object_attribute(p_class_name);
    lv2_main_table := ec_class_db_mapping.db_object_name(p_class_name);
  ELSE
    lv2_table_name := ec_class_db_mapping.db_object_name(p_class_name);
  END IF;
  ...
  IF p_class_name IS NOT NULL AND p_rec_id IS NOT NULL THEN
    ...
    ELSIF ecdp_classmeta.getclasstype(p_class_name) = 'DATA'
      ...
      lv2_owner_main_ec_pck := 'EC_' || ec_class_db_mapping.db_object_name(ec_class.owner_class_name(p_class_name));
      lv2_owner_attr_ec_pck := 'EC_' || ec_class_db_mapping.db_object_attribute(ec_class.owner_class_name(p_class_name));
    ...
    ELSE
      ...
      FOR curAttr IN c_class_key LOOP
        ...
        IF curAttr.attribute_name = 'OBJECT_ID' THEN
            ...
            lv2_col := 'ecdp_objects.getObjName(OBJECT_ID,sysdate)';
