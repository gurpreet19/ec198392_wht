...
** 10.3     14-05-13   leeeewei ECPD-24195 Added missing forecast Id in checkValidNom
** 10.3     18-11-13   leeeewei	ECPD-26087 Added missing forecast id in connectNomToCargo
********************************************************************/
...
BEGIN
  IF p_copy_method = 'INSERT' THEN
 	...
 	lv_laytime := EcBp_Cargo_Transport.getCarrierLaytimeDate(p_carrier_id, trunc(sysdate));
...
BEGIN
  ...
  checkValidNom(p_parcels,p_cargo_no, p_forecast_id,lv_carrier_id);
	...
	IF p_cargo_no IS null THEN
		...
		-- get cargo name
		ls_cargoName := EcBp_Cargo_Transport.getCargoName(ln_assign_id, p_parcels);
...
BEGIN
	...
	lv_result := EcBp_Cargo_Transport.getCarrierLaytimeDate(p_carrier_id, NVL(ld_daytime, trunc(sysdate)));
	RETURN lv_result;
