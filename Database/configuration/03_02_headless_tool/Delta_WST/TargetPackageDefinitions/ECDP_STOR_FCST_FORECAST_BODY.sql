...
BEGIN
	...
	FOR curExists IN c_exists (p_storage_id, p_pc_id, p_forecast_id, p_date) LOOP
  		lb_exists := TRUE;
	END LOOP;
	...
	LOOP
		...
		IF Lcntr = ln_diff THEN
			    lv_sql := 'INSERT INTO DV_FCST_STOR_DAY_PC_FCAST (object_id, profit_centre_id, forecast_id, daytime, forecast_qty, created_by) VALUES ('''||p_storage_id||''', '''||p_pc_id||''', '''||p_forecast_id||''', '''||ln_date||''', '||(ln_day + ln_rest)||', ecdp_context.getAppUser)';
			ELSE
			    lv_sql := 'INSERT INTO DV_FCST_STOR_DAY_PC_FCAST (object_id, profit_centre_id, forecast_id, daytime, forecast_qty, created_by) VALUES ('''||p_storage_id||''', '''||p_pc_id||''', '''||p_forecast_id||''', '''||ln_date||''', '||ln_day||' , ecdp_context.getAppUser)';
			END IF;

		ELSE
			IF Lcntr = ln_diff THEN
			    lv_sql := 'UPDATE DV_FCST_STOR_DAY_PC_FCAST set forecast_qty = '||(ln_day + ln_rest)||', last_updated_by = ecdp_context.getAppUser WHERE object_id = '''||p_storage_id||''' AND profit_centre_id = '''|| p_pc_id||''' AND forecast_id = '''|| p_forecast_id||''' AND daytime = '''||ln_date||'''';
			ELSE
			    lv_sql := 'UPDATE DV_FCST_STOR_DAY_PC_FCAST set forecast_qty = '||ln_day||', last_updated_by = ecdp_context.getAppUser WHERE object_id = '''||p_storage_id||''' AND profit_centre_id = '''|| p_pc_id||''' AND forecast_id = '''|| p_forecast_id||''' AND daytime = '''||ln_date||'''';
			END IF;
		END IF;
    ...
    EXECUTE IMMEDIATE lv_sql;
		...
		/*IF p_plan = 'PLAN' THEN
			IF Lcntr = ln_diff THEN
				ecdp_storage_forecast.aiuStorPcForecast(p_storage_id, p_pc_id, ln_date, NULL, (ln_day + ln_rest), ecdp_context.getAppUser);
			ELSE
				ecdp_storage_forecast.aiuStorPcForecast(p_storage_id, p_pc_id, ln_date, NULL, ln_day, ecdp_context.getAppUser);
			END IF;
		END IF;
		*/
	          ...
	          LOOP
			        ...
			        EXIT WHEN cv%NOTFOUND;
	          		...
	          		IF lv_la_id IS NOT NULL THEN
	          			...
	          			FOR curE IN c_exist (lv_la_id, p_forecast_id, p_daytime) LOOP
	          				lv_insert := 'N';
	          			END LOOP;

	          			IF lv_insert = 'N' THEN
						UPDATE LIFT_ACC_DAY_FCST_FCAST SET forecast_qty = p_new_qty * (lv_share/100), last_updated_by = p_user
	          				WHERE object_id = lv_la_id AND forecast_id = p_forecast_id AND daytime = p_daytime;
	          			ELSE
						INSERT INTO LIFT_ACC_DAY_FCST_FCAST (object_id, forecast_id, daytime, forecast_qty, created_by)
	          		...
	          		--  all profit centre values should be aggregated to the account
						...
						lv_sql := lv_sql || ' FROM STOR_DAY_FCST_PC_FCAST s, '||lv_from||', equity_share e';
						lv_sql := lv_sql || ' WHERE '||lv_from||'.commercial_entity_id = e.object_id';
						lv_sql := lv_sql || ' AND s.profit_centre_id = '||lv_from||'.object_id';
						lv_sql := lv_sql || ' AND s.forecast_id = '''||p_forecast_id||'''';
						...
						--EcDp_DynSql.WriteTempText('aiuStorPcForecast',lv_sql);
	          			...
	          			IF lv_insert = 'N' THEN
	          				UPDATE LIFT_ACC_DAY_FCST_FCAST SET forecast_qty = ln_qty, last_updated_by = p_user
	          				WHERE object_id = lv_la_id AND forecast_id = p_forecast_id AND daytime = p_daytime;
	          			ELSE
	          				INSERT INTO LIFT_ACC_DAY_FCST_FCAST (object_id, forecast_id, daytime, forecast_qty, created_by)
