...
IS
   ...
   CURSOR c_variable_attribute_key (cp_class_name VARCHAR2) IS
      SELECT ca.class_name,
             ca.attribute_name,
             ec_class_attr_db_mapping.sort_order( ca.class_name, ca.attribute_name) as sort_order,
             ca.data_type,
      ...
      FROM class_attribute ca, class c
      ...
      SELECT cr.to_class_name as class_name,
             ...
             ec_class_rel_db_mapping.sort_order(cr.from_class_name, cr.to_class_name, cr.role_name) as sort_order,
             'STRING' as data_type,
...
BEGIN
   select ec_class_attr_presentation.label(p_class_name, p_attr_name) into lv2_retval from dual;

   IF (lv2_retval IS null) THEN
     select max(label||' (ID)') into lv2_retval  from class_rel_presentation crp where crp.to_class_name = p_class_name and substr(p_attr_name,1,instr(p_attr_name,'_ID') -1) = crp.role_name;
   END IF;

   IF (lv2_retval IS null) THEN
     select max(label||' (Code)') into lv2_retval  from class_rel_presentation crp where crp.to_class_name = p_class_name and substr(p_attr_name,1,instr(p_attr_name,'_CODE') -1) = crp.role_name;
   END IF;
...
IS
 ln_retval NUMBER;
 BEGIN
   select ec_class_attr_db_mapping.sort_order(p_class_name, p_attr_name) into ln_retval from dual;

   IF (ln_retval IS null) THEN
     select max(crp.sort_order) into ln_retval  from class_rel_db_mapping crp where crp.to_class_name = p_class_name and substr(p_attr_name,1,instr(p_attr_name,'_ID') -1) = crp.role_name;
   END IF;

   IF (ln_retval IS null) THEN
     select max(crp.sort_order) into ln_retval  from class_rel_db_mapping crp where crp.to_class_name = p_class_name and substr(p_attr_name,1,instr(p_attr_name,'_CODE') -1) = crp.role_name;
   END IF;
...
BEGIN
  ...
  SELECT c.class_type INTO lv_class_type FROM class c WHERE c.class_name = p_object_type_code;
  IF lv_class_type IN ('OBJECT','INTERFACE') THEN
    lv_key_attr := 'OBJECT_ID';
  ELSE
    SELECT COUNT(*) INTO ln_num_keys FROM class_attribute a WHERE a.class_name = p_object_type_code AND a.is_key='Y';
    IF ln_num_keys = 1 THEN
      SELECT a.attribute_name INTO lv_key_attr FROM class_attribute a WHERE a.class_name = p_object_type_code AND a.is_key='Y';
    END IF;
  END IF;
  ...
  -- Code
  --
  SELECT COUNT(*) INTO ln_code_exist FROM class_attribute WHERE class_name = p_object_type_code AND attribute_name = 'CODE';
  IF (lv_class_type IN ('OBJECT','INTERFACE')) AND (ln_code_exist > 0) AND (NVL(ec_class_attribute.disabled_ind(p_object_type_code,'CODE'),'N')<>'Y') THEN
    INSERT INTO calc_object_attribute(object_id,object_type_code,name,value_object_type_code,calc_attr_level_code,created_by)
  ...
  -- Name
  --
  SELECT COUNT(*) INTO ln_name_exist FROM class_attribute WHERE class_name = p_object_type_code AND attribute_name = 'NAME';
  IF (lv_class_type IN ('OBJECT','INTERFACE')) AND (ln_name_exist > 0) AND(NVL(ec_class_attribute.disabled_ind(p_object_type_code,'NAME'),'N')<>'Y') THEN
    INSERT INTO calc_object_attribute(object_id,object_type_code,name,value_object_type_code,calc_attr_level_code,created_by)
...
BEGIN
   ...
   IF lr_obj_type.CALC_OBJ_TYPE_CATEGORY = 'DB' THEN
      RETURN NVL(lr_obj_type.label_override, NVL(ec_class.label(p_object_type_code), p_object_type_code));
   END IF;
...
IS
  ...
  CURSOR c_date_props IS
    select
      (select decode(count(*),0,'N','Y') from class_attribute ca where ca.class_name=c.class_name and attribute_name='DAYTIME') has_daytime,
      (select decode(count(*),0,'N','Y') from class_attribute ca where ca.class_name=c.class_name and attribute_name='END_DATE') has_end_date,
      (select decode(count(*),0,'N','Y') from class_attribute ca where ca.class_name=c.class_name and attribute_name='SUMMER_TIME') has_summertime,
      (select decode(count(*),0,'N','Y') from class_attribute ca where ca.class_name=c.class_name and attribute_name='PRODUCTION_DAY') has_production_day
    from class c
...
BEGIN
  ...
  SELECT NVL(min(ca.data_type),'STRING') into lv2_retval FROM calc_var_key_read_mapping cvr, class_attribute ca where cvr.object_id = p_object_id and cvr.calc_var_signature = p_calc_var_signature
  and cvr.calc_dataset = p_calc_dataset and cvr.cls_name = p_cls_name and cvr.calc_dim_mapping_code = 'SCREEN_PARAMETER'
...
IS
  lv2_retval VARCHAR2(100);
BEGIN
  SELECT min(ca.data_type) into lv2_retval FROM calc_var_key_write_mapping cvr, class_attribute ca where cvr.object_id = p_object_id and cvr.calc_var_signature = p_calc_var_signature
  and cvr.calc_dataset = p_calc_dataset and cvr.cls_name = p_cls_name and cvr.calc_dim_mapping_code = 'SCREEN_PARAMETER'
  and ca.class_name = cvr.cls_name and ca.attribute_name = cvr.sql_syntax;
