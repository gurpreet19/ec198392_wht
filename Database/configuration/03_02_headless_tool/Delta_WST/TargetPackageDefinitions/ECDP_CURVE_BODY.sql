...
**			19.04.2012 limmmchu ECPD-20614: Modified error message ORA number in procedure moveCurvePoints, moveCurve and addCurvePoint
**			25.09.2012 limmmchu ECPD-21952: Modified getYfromXGraph to pass correct value for draw graph.
**			11.07.2013 limmmchu ECPD-24817: Modified moveCurve to change the formula of INV_POLYNOM_2.
*****************************************************************/
...
BEGIN
   ...
   IF lv2_formula_type=EcDp_Curve_Constant.LINEAR THEN
      ...
      OPEN points FOR select x_value as x,y_value as y from curve_point where curve_id=p_curve_id;
   ...
   ELSIF lv2_formula_type=EcDp_Curve_Constant.INV_POLYNOM_2 THEN
      ...
      OPEN points FOR select y_value as x,x_value as y from curve_point where curve_id=p_curve_id;
      EcBp_Mathlib.curveFitPoly2LS(points,ln_c0,ln_c1,ln_c2,err);
   ELSIF lv2_formula_type=EcDp_Curve_Constant.POLYNOM_3 THEN
      OPEN points FOR select x_value as x,y_value as y from curve_point where curve_id=p_curve_id;
      EcBp_Mathlib.curveFitPoly3LS(points,ln_c0,ln_c1,ln_c2,ln_c3,err);
      -- If formula_type is Polynom 3, then raise an error
      -- RAISE_APPLICATION_ERROR(-20000,'Cannot calculate coefficients for 3. Order Polynomial functions');
   END IF;
...
BEGIN
   ...
   ELSIF lv2_formula_type=EcDp_Curve_Constant.POLYNOM_2 THEN
      ...
      RETURN lr_curve.c0 + lr_curve.c1*p_x_value + lr_curve.c2*p_x_value*p_x_value;
   ELSIF lv2_formula_type=EcDp_Curve_Constant.POLYNOM_3 THEN
      -- Polynom 3 curve, use c0, c1, c2 and c3
      RETURN lr_curve.c0 + lr_curve.c1*p_x_value + lr_curve.c2*p_x_value*p_x_value + lr_curve.c3*p_x_value*p_x_value*p_x_value;
   ELSIF lv2_formula_type=EcDp_Curve_Constant.INV_POLYNOM_2 THEN
   ...
   ELSIF lv2_formula_type=EcDp_Curve_Constant.POLYNOM_2 THEN
	  ...
	  OPEN points FOR select NVL(ecdp_unit.convertValue(x_value,lv2_dbunit_x,lv2_screenunit_x),x_value) as x,ecdp_unit.convertValue(y_value,lv2_dbunit_y,lv2_screenunit_y) as y from curve_point where curve_id=p_curve_id;
	    ...
	    ELSE
         ...
         ln_c2 := lr_curve.c2;
	    END IF;
      RETURN ln_c0 + ln_c1*p_x_value + ln_c2*p_x_value*p_x_value;
    ...
    OPEN points FOR select NVL(ecdp_unit.convertValue(x_value,lv2_dbunit_x,lv2_screenunit_x),x_value) as x,ecdp_unit.convertValue(y_value,lv2_dbunit_y,lv2_screenunit_y) as y from curve_point where curve_id=p_curve_id;
      IF lr_curve.c0 IS NULL OR lr_curve.c1 IS NULL OR lr_curve.c2 IS NULL OR lr_curve.c3 IS NULL THEN
	    ...
	    ELSE
         ...
         ln_c3 := lr_curve.c3;
	    END IF;
      RETURN ln_c0 + ln_c1*p_x_value + ln_c2*p_x_value*p_x_value + ln_c3*p_x_value*p_x_value*p_x_value;
...
--</EC-DOC>
  IS
    ...
    lv2_c1_old curve.C0%TYPE;
    lv2_c2_old curve.C0%TYPE;
    lv2_c0 curve.C0%TYPE;
    lv2_c1 curve.C0%TYPE;
    lv2_c2 curve.C0%TYPE;
    lv2_pic VARCHAR2(32);
    ln_y_old NUMBER:= getYfromX(p_curve_id, p_x);
    ln_ratio NUMBER:= p_y / ln_y_old;
  ...
  BEGIN
    ...
    select PLANE_INTERSECT_CODE into lv2_pic from PERFORMANCE_CURVE pc where PERF_CURVE_ID in (select PERF_CURVE_ID from CURVE c where c.CURVE_ID = p_curve_id);
      ...
      END IF;
      IF lv2_formula_type = 'INV_POLYNOM_2' THEN
      ...
      END IF;
      IF lv2_formula_type = 'POLYNOM_2' THEN
        --handles concave function where coeff value c2 (2nd constant) < 0
  ...
  BEGIN
   ...
   --to get the measurement type for y axis
     ...
     ELSIF ec_curve.phase(p_curve_id)=ecdp_phase.WATER then
        lv2_measurementtype := 'WATER_RATE';
