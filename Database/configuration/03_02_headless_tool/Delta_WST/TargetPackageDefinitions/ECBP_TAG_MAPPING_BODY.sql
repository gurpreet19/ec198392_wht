...
IS
 BEGIN
    IF p_object_id is null THEN RAISE_APPLICATION_ERROR (-20999, 'Object Name is not set');
    END IF;
    IF p_new_object_id is null THEN RAISE_APPLICATION_ERROR (-20999, 'New Mapping Object Name is not set');
    END IF;
    IF p_last_transfer is null THEN RAISE_APPLICATION_ERROR (-20999, 'Last Transfer Date is not set');
    END IF;
    IF p_object_id = p_new_object_id THEN RAISE_APPLICATION_ERROR (-20999, 'Object Name cannot be the same as New Mapping Object Name');
    END IF;
