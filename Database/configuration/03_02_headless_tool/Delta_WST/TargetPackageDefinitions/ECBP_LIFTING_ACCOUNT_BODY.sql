...
BEGIN
	...
	FOR curAccount IN c_account(p_storage_id, p_company_id, p_profit_centre_id, p_daytime) LOOP
		ln_count := ln_count + 1;
...
BEGIN
	...
	IF ln_count > 1 THEN
		Raise_Application_Error(-20308,'More than one lifting account for storage and company');
	END IF;
...
BEGIN
	...
	IF ln_count > 1 THEN
		Raise_Application_Error(-20328,'There is already a lifting account for this contract and storage');
	END IF;
...
IS
	...
	WHERE 	company_id = cp_company_id
	AND 	storage_id = cp_storage_id
	AND     Nvl(profit_centre_id, cp_profit_centre_id) = cp_profit_centre_id
...
BEGIN
	...
	IF p_company_id IS NULL THEN
		Raise_Application_Error(-20329,'A lifting account must have a company');
	END IF;

	IF p_profit_centre_id IS NULL THEN
		FOR curVal IN c_validate2(p_object_id, p_storage_id, p_company_id) LOOP
			Raise_Application_Error(-20308,'Not allowed to have more than one account for a Storage, Lifter (Company) and Profit Centre');
			EXIT;
		END LOOP;
	ELSE
		FOR curVal IN c_validate1(p_object_id, p_storage_id, p_company_id, p_profit_centre_id) LOOP
			Raise_Application_Error(-20308,'Not allowed to have more than one account for a Storage, Lifter (Company) and Profit Centre');
			EXIT;
		END LOOP;
	END IF;
...
BEGIN
  ...
  IF lv_message is not null THEN
    RAISE_APPLICATION_ERROR(-20336, 'An lifting account connection must not overlaps with existing connection period.');
  END IF;
...
BEGIN
  ...
  FOR sumAccountShare IN c_account_share LOOP
    ln_count := ln_count + 1;
    ln_account_share := ln_account_share + ROUND(Nvl(sumAccountShare.account_share,0),9);
  END LOOP;
...
BEGIN
    ...
    ld_production_day := null;
     ...
     IF EcDp_Date_Time.interceptsWinterAndSummerTime(p_daytime, lv_pday_object_id) = 'N' OR lv_summer_time IS NULL THEN
