...
*****************************************************************/
  ...
  CURSOR c_acl_class(p_class_name VARCHAR2) IS
        ...
        FROM   class
        WHERE  NVL(access_control_ind,'N')='Y' AND (class_type='OBJECT' OR class_type='INTERFACE')
      ...
      UNION ALL
        ...
        AND    NVL(f.access_control_ind,'N')='Y'
        AND    NVL(r.access_control_method,'NA')='ACL_LOOKUP'
  ...
  TYPE Relation_t IS RECORD (
      id                    BINARY_INTEGER,
      from_class_name       class.class_name%TYPE,
      to_class_name         class.class_name%TYPE,
      role_name             class_relation.role_name%TYPE,
      access_control_method class_relation.access_control_method%TYPE,
      db_mapping_type       class_rel_db_mapping.db_mapping_type%TYPE,
      db_sql_syntax         class_rel_db_mapping.db_sql_syntax%TYPE,
      is_key                class_relation.is_key%TYPE,
      is_mandatory          class_relation.is_mandatory%TYPE,
      report_only_ind       class_relation.report_only_ind%TYPE,
      db_object_attribute   class_db_mapping.db_object_attribute%TYPE,
      db_object_name        class_db_mapping.db_object_name%TYPE
