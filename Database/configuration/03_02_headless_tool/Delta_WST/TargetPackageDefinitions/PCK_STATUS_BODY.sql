...
** 17.09.2013 wonggkai ECPD-25579: Modified procedure set_analysis_rs change p_daytime to valid_from_date
** 27.11.2013 musthram ECPD-26206: Removed Commit statement from procedure set_alloc_netw_log_status_rs, set_cargo_rs, set_analysis_rs, set_aga_analysis_rs and set_deferment_rs to work in JMS mode
**************************************************************************************************************/
--
--------------------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  IF access_to_process(  p_process_id, p_user_id) THEN
    ...
    IF ((lrec_status_process.process_interval = 'MTH' AND TRUNC(EcDp_Date_Time.getCurrentSysdate,'MM') >= ld_to_daytime) OR
        (lrec_status_process.process_interval <> 'MTH' AND EcDp_Date_Time.getCurrentSysdate > ld_from_daytime)) THEN

      -- Loop through all child processes
      FOR mycur IN cur_status_process(p_process_id) LOOP
        run_child_process(mycur.process_id, lrec_status_process.from_rs_level, lrec_status_process.to_rs_level, ld_from_daytime, ld_to_daytime, ln_total_rows_upd, p_user_id);
      END LOOP;

      -- Execute the tasks for p_process_id
      run_tasks(p_process_id, lrec_status_process.from_rs_level, lrec_status_process.to_rs_level, ld_from_daytime, ld_to_daytime, ln_total_rows_upd, p_user_id);
...
BEGIN
	...
	FOR child_cur IN cur_status_process(p_process_id) LOOP
		run_child_process(child_cur.process_id, p_from_rs_level, p_to_rs_level, p_from_daytime, p_to_daytime, ln_current_rows_upd, p_user_id);
	END LOOP;

	-- Execute the tasks for p_process_id
	run_tasks(p_process_id, p_from_rs_level, p_to_rs_level, p_from_daytime, p_to_daytime, ln_current_rows_upd, p_user_id);
...
CURSOR cur_tasks(pid VARCHAR2) IS
	...
	AND table_id NOT IN ('CTRL_CHECK_LOG','WEATHER','PROD_TEST_RESULT','PROD_TEST_RESULT_SINGLE'); -- these classes have spesial handling
...
mysql VARCHAR2(2000);
lv_owner_class_name VARCHAR2(100);
lv_owner_class_type VARCHAR2(100);
...
BEGIN
    ...
    -- mycur.table_id is the data class name. Use the view DV_<dataclass name>
        	...
        	mysql := mysql||', d.rev_text = '''|| 'Updated by status process at '||to_char(sysdate,'yyyy-mm-dd hh24:mi:ss')||'''';
    	END IF;
      ...
      FOR myClass IN cur_sub_daily(mycur.table_id) LOOP
  	    ...
  	    lv_datewhere := ' WHERE o.daytime <= d.production_day AND (o.end_date > d.production_day OR o.end_date IS NULL)';
      END LOOP;
      ...
      FOR myClass IN cur_daily(mycur.table_id) LOOP
  	    ...
  	    mysql := mysql||' AND EXISTS ';
        lv_datewhere := ' WHERE o.daytime <= d.daytime AND (o.end_date > d.daytime OR o.end_date IS NULL)';
      END LOOP;

	    -- sub select to find correct records
	    -- use owner class name from class
	    FOR mycur2 IN cur_owner_class(mycur.table_id) LOOP
	      lv_owner_class_name := mycur2.class_name;
	      lv_owner_class_type := mycur2.class_type;
	    END LOOP;
	    mysql := mysql||' ( SELECT 1 ';
	    IF lv_owner_class_type = 'INTERFACE' THEN
	      mysql := mysql||' FROM iv_'||lv_owner_class_name ||' o';
	    ELSIF lv_owner_class_type = 'OBJECT' THEN
	      mysql := mysql||' FROM ov_'||lv_owner_class_name ||' o';
	    ELSE
	      mysql := mysql||' FROM iv_facility' ||' o'; -- facility is default
	    END IF;

	    mysql := mysql || lv_datewhere;
  ...
  -- spesial handling of ctrl_check_log as this is not a class and is not displayed in any screens
	...
	FOR mycur IN cur_tasks_data_class(p_process_id, 'CTRL_CHECK_LOG') LOOP
      ...
      -- mycur.table_id is the data class name. Use the view TV_<dataclass name> as chem_injection_status is a Table class
        ...
        IF ln_from_level_id IS NOT NULL THEN
           mysql := mysql||' AND ec_ctrl_record_status_level.level_id(d.record_status)='||ln_from_level_id;
        END IF;
  		mysql := mysql||' AND daytime BETWEEN to_date('||chr(39)||to_char(p_from_daytime,'DD.MM.YYYY HH24:MI')||chr(39)|| ','||chr(39)||'DD.MM.YYYY HH24:MI'||chr(39)||')'||' AND to_date('||chr(39)||to_char(p_to_daytime-(1/1440),'DD.MM.YYYY HH24:MI')||chr(39)|| ','||chr(39)||'DD.MM.YYYY HH24:MI'||chr(39)||')';
      mysql := mysql||' AND '||mycur.where_clause;
  ...
  -- needs to lookup weather site to find facility conection
    ...
    -- mycur.table_id is the data class name. Use the view TV_<dataclass name> as chem_injection_status is a Table class
		...
		mysql := mysql||'     AND to_date('||chr(39)||to_char(p_from_daytime,'DD.MM.YYYY HH24:MI')||chr(39)|| ','||chr(39)||'DD.MM.YYYY HH24:MI'||chr(39)||') BETWEEN i.daytime and nvl(i.end_date-(1/1440),sysdate)';
		IF mycur.where_clause IS NOT NULL THEN
			mysql := mysql||'     AND '||mycur.where_clause;
		END IF;
		mysql := mysql||') ';
	...
	END LOOP;
	  ...
	  -- spesial handling of PROD_TEST_RESULT and PROD_TEST_RESULT_SINGLE
 	FOR mycur IN cur_tasks_data_class_prod_test(p_process_id) LOOP
		...
		mysql := mysql||'     AND to_date('||chr(39)||to_char(p_from_daytime,'DD.MM.YYYY HH24:MI')||chr(39)|| ','||chr(39)||'DD.MM.YYYY HH24:MI'||chr(39)||') BETWEEN ov.daytime and nvl(ov.end_date-(1/1440),sysdate)';
		IF mycur.where_clause IS NOT NULL THEN
			mysql := mysql||'     AND '||mycur.where_clause;
		END IF;
		mysql := mysql||') ';
...
lv_new_status VARCHAR2(1);

BEGIN
	lv_new_status := process_day_rs(p_process_id, p_from_daytime, p_to_daytime);
	-- Only update record_status_level if any records have been updated, else use the record status from prev. run!!!
	IF (lv_new_status<>p_to_rs_level AND p_rows_updated>0) THEN
		lv_new_status := p_to_rs_level;
	END IF;

	INSERT INTO stat_process_status (process_id, record_status_level, daytime, run_daytime, end_date, rows_updated, created_by)
	VALUES (p_process_id, lv_new_status, p_from_daytime, EcDp_Date_Time.getCurrentSysdate, p_to_daytime, p_rows_updated, p_user_id);
...
BEGIN
	FOR mycur IN cur_stat_process_status(p_process_id, p_from_day, p_to_day) LOOP
		lv_status := mycur.record_status_level;
	END LOOP;

	RETURN nvl(lv_status,'P');
...
BEGIN
   ...
   lv_record_status := ec_prosty_codes.alt_code(ec_cargo.cargo_status(p_cargo_no),'CARGO_STATUS');
   IF ec_cargo.record_status(p_cargo_no) <> lv_record_status THEN
...
BEGIN
  ...
  END IF;
   ...
   lv_record_status := ec_prosty_codes.alt_code( ec_alloc_job_log.accept_status(p_run_no),
                                                'ALLOC_NETWORK_LOG_STATUS');
   IF ec_alloc_job_log.record_status(p_run_no) <> lv_record_status THEN
      -- update alloc_network_log
   	UPDATE alloc_job_log
   	SET record_status = lv_record_status, last_updated_by = NVL(p_user_id, NVL(EcDp_User_Session.getUserSessionParameter('USERNAME'), User))
...
BEGIN
   ...
   IF lr_analysis.record_status <> lv_record_status THEN
      ...
      -- update fluid_analysis_component
   	UPDATE fluid_analysis_component
   	SET record_status = lv_record_status, last_updated_by = p_user_id, last_updated_date = Ecdp_Date_Time.getCurrentSysdate
   	WHERE analysis_no = lr_analysis.analysis_no;
...
BEGIN
   ...
   lv_record_status := ec_prosty_codes.alt_code(lr_objects_def_event.status,'EVENT_STATUS');

   IF lr_objects_def_event.record_status <> lv_record_status THEN
...
-----------------------------------------------------------------
-- PROCEDURE: set_deferment_record_status - Set record status for all records for a production deferment
-- Note that this is for tables fcty_deferment_event and well_deferment_event
-----------------------------------------------------------------
PROCEDURE set_deferment_record_status(p_event_no VARCHAR2)
IS
lv_record_status fcty_deferment_event.status%TYPE;
lr_objects_def_event fcty_deferment_event%ROWTYPE;
BEGIN
   lr_objects_def_event := ec_fcty_deferment_event.row_by_pk(p_event_no);
   lv_record_status := ec_prosty_codes.alt_code(lr_objects_def_event.status,'EVENT_STATUS');
   ...
   IF lr_objects_def_event.record_status <> lv_record_status THEN
     ...
     SET record_status = lv_record_status
     WHERE event_no = lr_objects_def_event.event_no
      AND record_status<>'V';
...
END set_deferment_record_status;
