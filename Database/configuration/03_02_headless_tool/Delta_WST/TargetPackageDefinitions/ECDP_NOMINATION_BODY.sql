...
IS
    ...
    SELECT SUM(decode(p_nom_type, 'TRAN_INPUT', ADJUSTED_IN_QTY, 'TRAN_OUTPUT', ADJUSTED_OUT_QTY)) ADJ_QTY
...
BEGIN
    ...
    FOR curAdjQty IN c_adjusted_qty LOOP
		    ln_aggregated_qty := curAdjQty.ADJ_QTY;
    END LOOP;
...
IS
    ...
    SELECT SUM(decode(p_nom_type, 'TRAN_INPUT', SCHEDULED_IN_QTY, 'TRAN_OUTPUT', SCHEDULED_OUT_QTY)) SCHED_QTY
    FROM objloc_day_nomination
    WHERE object_id = p_object_id
    AND daytime = p_daytime;
...
BEGIN
    ...
    FOR curSchedQty IN c_scheduled_qty LOOP
		    ln_aggregated_qty := curSchedQty.SCHED_QTY;
    END LOOP;
