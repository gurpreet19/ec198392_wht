...
** ------   -----      --------------------------------------
** 18.05.2001  ØJ      Added method for potential calculation in findOilProducionPotential, ->INTERPOLATE_WT
...
** 24-05-2012 rajarsar ECPD-21043: Updated findConProductionPotential,findOilProductionPotential,findGasProductionPotential,findWatProductionPotential,findGasLiftPotential and findDiluentPotential.
** 22.11.2013 kumarsur ECPD-26104: Updated getCurveRate to support new parameters added phase_current,ac_frequency and intake_press.
** 02.12.2013 makkkkam ECPD-25991: Updated getCurveRate to support new parameters added mpm_oil_rate,mpm_gas_rate,mpm_water_rate and mpm_cond_rate.
*****************************************************************/
...
BEGIN
  ...
  ELSIF (lv2_potential_method = Ecdp_Calc_Method.FORECAST) THEN
   ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
   ln_return_val := ec_prod_well_forecast.cond_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');

   -- method is candidate for decline calculations
   IF lv2_decline_flag = 'Y' THEN
     ld_from_date := ec_prod_forecast.daytime(Ecbp_ProductionForecast.getRecentProdForecastNo(p_object_id, p_daytime));
     ln_return_val := EcBp_Well_Theoretical.declineResult(p_object_id, p_daytime, ld_from_date, 'CONDENSATE', ln_return_val);
   END IF;
  ...
  IF (ec_ctrl_system_attribute.attribute_text(p_daytime, 'ADJUST_POTENTIAL_DST','<=')= 'Y') THEN
    ln_return_val := ln_return_val/24 * ecdp_date_time.getNumHours('WELL', p_object_id,p_daytime);
  END IF;
...
BEGIN
  ...
  ELSIF (lv2_potential_method = Ecdp_Calc_Method.FORECAST) THEN
    ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
    ln_return_val := ec_prod_well_forecast.gas_inj_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
  ...
  ELSIF (lv2_potential_method = Ecdp_Calc_Method.FORECAST) THEN
    ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
    ln_return_val := ec_prod_well_forecast.net_oil_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
  ...
  ELSIF (lv2_potential_method = EcDp_Calc_Method.FORECAST) THEN
   ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
   ln_return_val := ec_prod_well_forecast.gas_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
  ...
  ELSIF (lv2_potential_method = EcDp_Calc_Method.FORECAST) THEN
    ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
    ln_return_val := ec_prod_well_forecast.water_inj_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
  ...
  IF (lv2_potential_method = EcDp_Calc_Method.FORECAST) THEN
    ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
    ln_return_val := ec_prod_well_forecast.steam_inj_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
   ...
   IF (lv2_potential_method = EcDp_Calc_Method.FORECAST) THEN
     ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
     ln_return_val := ec_prod_well_forecast.CO2_INJ_RATE(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
  ...
  ELSIF (lv2_potential_method = EcDp_Calc_Method.FORECAST) THEN
   ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
   ln_return_val := ec_prod_well_forecast.water_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
   ...
   ELSIF (lv2_potential_method = EcDp_Calc_Method.FORECAST) THEN
    ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
    ln_return_val := ec_prod_well_forecast.gas_lift_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
  ...
  ELSIF (lv2_potential_method = EcDp_Calc_Method.FORECAST) THEN
    ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
    ln_return_val := ec_prod_well_forecast.diluent_rate(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
   ...
   ELSIF (lv2_potential_mass_method = Ecdp_Calc_Method.FORECAST) THEN
     ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
     ln_return_val := ec_prod_well_forecast.net_oil_rate_mass(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
   ...
   ELSIF (lv2_potential_mass_method = Ecdp_Calc_Method.FORECAST) THEN
     ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
     ln_return_val := ec_prod_well_forecast.gas_rate_mass(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
   ...
   ELSIF (lv2_potential_mass_method = Ecdp_Calc_Method.FORECAST) THEN
     ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
     ln_return_val := ec_prod_well_forecast.cond_rate_mass(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
BEGIN
   ...
   ELSIF (lv2_potential_mass_method = Ecdp_Calc_Method.FORECAST) THEN
     ln_fcst_scen_no := Ecbp_Productionforecast.getRecentProdForecastNo(p_object_id,p_daytime);
     ln_return_val := ec_prod_well_forecast.water_rate_mass(ln_fcst_scen_no,p_object_id,p_daytime,'<=');
...
--   IF (ln_def_version = 'PD.0001.01') THEN
--     ln_return_val := ecdp_objects_event.calcWellProdLossDay(p_object_id, p_daytime, p_phase);

   IF (ln_def_version = 'PD.0001.02') THEN
     ln_return_val := ecdp_deferment_event.calcWellProdLossDay(p_object_id, p_daytime, p_phase);

   ELSIF (ln_def_version = 'PD.0001') THEN
    ln_return_val := ecdp_objects_deferment_event.calcWellProdLossDay(p_object_id, p_daytime, p_phase);

   ELSIF (ln_def_version = 'PD.0006') THEN
    ln_return_val := ecbp_well_eqpm_deferment.calcWellProdLossDay(p_object_id, p_daytime, p_phase);
...
--     ln_return_val := ecdp_objects_event.calcWellProdLossDay(p_object_id, p_daytime, p_phase);
   ...
   IF (ln_def_version = 'PD.0001.02') THEN
     ln_return_val := ecdp_deferment_event.calcWellProdLossDayMass(p_object_id, p_daytime, p_phase);
