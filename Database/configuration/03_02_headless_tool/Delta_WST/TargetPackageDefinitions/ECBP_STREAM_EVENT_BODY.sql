...
** 23.05.2012  kumarsur  ECPD-21024: Period Oil Lact Stream (PO.0021) - make validate period configurable
** 18.05.2015  hismahas  ECPD-31001: Updated ORA error code for validatePeriod
**************************************************************************************************/
  ...
  IF UPPER(p_within_mth) = 'YES' THEN
      IF trunc(EcDp_ProductionDay.getProductionDay('STREAM', p_object_id, p_daytime),'MM') <> TRUNC(EcDp_ProductionDay.getProductionDay('STREAM', p_object_id, p_end_date),'MM') THEN
        IF p_end_date > Ecdp_Productionday.getProductionDayStart('STREAM', p_object_id, p_daytime+1)
     		THEN
        		RAISE_APPLICATION_ERROR(-20668, 'An event must not span production month boundaries');
     	END IF;
      END IF;
  END IF;
...
BEGIN
  ...
  FOR mycur IN c_exists(p_object_id, p_event_type, p_daytime, p_end_date) LOOP
    RAISE_APPLICATION_ERROR(-20000, 'This event overlaps with existing period for the stream');
  END LOOP;
