...
BEGIN
   ...
   lv2_db_unit := ecdp_unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(lv2_class_name,p_attr_name));
...
IS
	...
	CURSOR c_has_from_rel (cp_class_name VARCHAR2, cp_object_id VARCHAR2) IS
		select qc.rep_class_name, qc.class_alias, r.role_name, db.db_sql_syntax
		  from qb_class qc, class_relation r, class_rel_db_mapping db
		 where qc.object_id = cp_object_id
		   and qc.rep_class_name != cp_class_name
		   and r.from_class_name = qc.rep_class_name
		   and r.to_class_name = cp_class_name
		   and nvl(r.disabled_ind, 'N') != 'Y'
		   and r.from_class_name = db.from_class_name
		   and r.to_class_name = db.to_class_name
		   and r.role_name = db.role_name
		   and qc.class_alias not in (select r.from_class_alias
										from qb_obj_relation r, qb_class qb
									   where r.to_class_alias = qb.class_alias
										 and qb.rep_class_name = cp_class_name
...
IS
	CURSOR c_attr (cp_class_name VARCHAR2) IS
		select a.attribute_name from class_attribute a
...
BEGIN
	...
	SELECT date_handeling INTO lv_date_handling from dao_meta where class_name = lv_class_name and rownum = 1;
