...
--		dbms_output.put_line('max line_number: ' || lv_max_number);
	END LOOP;
...
BEGIN
	...
	FOR curPeriods IN c_get_period_type(p_from_forecast_id, p_new_forecast_id) LOOP
		dbms_output.put_line('forecast_id: ' || curPeriods.object_id);
	...
	dbms_output.put_line('lv_from_period_type: ' || lv_from_period_type);
	dbms_output.put_line('lv_to_period_type: ' || lv_to_period_type);
