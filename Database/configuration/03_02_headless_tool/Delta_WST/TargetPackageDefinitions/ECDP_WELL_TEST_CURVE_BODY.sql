...
** 07.12.2010  limmmchu  ECPD-22788: Modified analyzeTrend() to support include todate.
** 01.09.2014  shindani  ECPD-28517: Modified function trendSegmentRowCount() , removed trend_analysis_ind flag condition.
********************************************************************/
  ...
  FOR c_trend_curve IN trendCurve_cur(ld_trend_start) LOOP
    ...
    UPDATE trend_curve tc
      SET tc.c0=ln_C0,
          ...
          tc.last_updated_date=Ecdp_Date_Time.getCurrentSysdate
    ...
    OPEN prev_trend_curve_cur(ld_trend_start);
      LOOP
        ...
        FOR i IN 1..l_trend_curve.COUNT LOOP
          ...
          l_trend_curve(i).last_updated_date := Ecdp_Date_Time.getCurrentSysdate;
        END LOOP;
...
BEGIN
  ld_last_updated_date := EcDp_Date_Time.getCurrentSysdate;

  UPDATE ptst_result SET status = 'ACCEPTED', record_status = 'A', trend_analysis_ind = 'Y', last_updated_by = p_user, last_updated_date = ld_last_updated_date WHERE result_no = p_result_no;
  UPDATE pwel_result SET status = 'ACCEPTED', record_status = 'A', trend_analysis_ind = 'Y', last_updated_by = p_user, last_updated_date = ld_last_updated_date WHERE result_no = p_result_no;
...
BEGIN
  ...
  UPDATE ptst_result SET status = 'REJECTED', record_status = 'V', trend_analysis_ind = 'N', last_updated_by = p_user, last_updated_date = ld_last_updated_date WHERE result_no = p_result_no;
  UPDATE pwel_result SET status = 'REJECTED', record_status = 'V', trend_analysis_ind = 'N', last_updated_by = p_user, last_updated_date = ld_last_updated_date WHERE result_no = p_result_no;
