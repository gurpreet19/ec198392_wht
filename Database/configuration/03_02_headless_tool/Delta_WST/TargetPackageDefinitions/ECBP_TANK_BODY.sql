...
** 			22.01.2013  limmmchu  ECPD-23172: Enhance findClosingNetVol to support event closing
...
**		    02.04.2014  musthram  ECPD-27161: Modified findOpeningGrsVol, findOpeningGrsMass, findClosingGrsVol, findClosingGrsMass, findOpeningDiluentVol, findClosingDiluentVol, findOpeningNetVol, 	findClosingNetVol, findOpeningNetMass, findClosingNetMass, findOpeningWatVol, findClosingWatVol, findOpeningEnergy, findClosingEnergy, findClosingGrsStdOilVol, add criteria for meas_event_type
*****************************************************************/
...
BEGIN
  ...
  ELSIF (substr(lv2_calc_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ...
    ln_retval := NULL; --to do, include in Ue_Tank
...
BEGIN
  ...
  ln_grs_vol           := Nvl(EcBp_Tank.findGrsVol(p_tank_object_id, p_meas_event_type, p_daytime, p_method, lv2_resolve_loop),0);
...
BEGIN
  ...
  IF (lv2_calc_method IN ('GRS_BSW_DILUENT','GRS_VOL_BSW','API_BLEND_SHRINKAGE') OR lv2_calc_method IS NULL) THEN
    ...
    IF lv2_calc_method = 'GRS_BSW_DILUENT' THEN
      ...
      ln_retval := ln_retval * (1 - ((ln_bitumen_dens - ln_tank_dens) / (ln_bitumen_dens - ln_diluent_dens)));
    ...
    END IF;
  ELSIF lv2_calc_method = 'USER_EXIT' THEN
    ln_retval := NULL;-- not yet
  ELSE -- unknown method
    ln_retval := NULL;
  END IF;
...
BEGIN
  ...
  IF lv2_calc_method = 'DENSITY_CALC' THEN
    ...
    ln_retval := ln_retval * ((ln_bitumen_dens - ln_tank_dens) / (ln_bitumen_dens - ln_diluent_dens));
  ELSIF lv2_calc_method = 'API_BLEND_SHRINKAGE' THEN
    ln_retval := findNetStdOilVol(p_tank_object_id, p_meas_event_type, p_daytime, 'GRS_VOL_BSW', lv2_resolve_loop);
    ln_retval := (ln_retval + findShrunkVol(p_tank_object_id,p_meas_event_type,p_daytime)) *
...
--                 ec_tank_measurement.diluent_cut(p_tank_object_id, p_meas_event_type, p_daytime);
  ELSIF lv2_calc_method LIKE 'USER_EXIT%' THEN
    ln_retval := NULL; -- user_exit
  ELSE -- unknown method
    ln_retval := NULL;
  END IF;
