...
** 31.10.2013  leeeewei		ECPD-25899: Modified getAdjustmentsDay, getAdjustmentsMth and calcEstClosingBalanceDay to read use_sub_day from system settings
******************************************************************/
...
FUNCTION calcEstClosingBalanceDay(p_object_id  VARCHAR2, p_daytime DATE, p_xtra_qty NUMBER DEFAULT 0)
RETURN NUMBER
--<EC-DOC>
IS
		...
		ELSE
			IF ln_firstday_opening_bal IS NOT NULL THEN
	        	...
	        	FOR cur_rec IN c_sub_day_actual(p_object_id, ld_from_date) LOOP
              		...
              		END IF;
	        		ln_firstday_opening_bal := calcEstClosingBalanceSubDay(p_object_id, ld_day_end, ecdp_date_time.summertime_flag(ld_day_end), p_xtra_qty);
				    --check if latest actual is for requested date?
				    IF ld_from_date = p_daytime THEN
				    	ln_closing_balance := ln_firstday_opening_bal;
				        lb_actual := true;
...
FUNCTION calcEstClosingBalanceSubDay(p_object_id  VARCHAR2, p_daytime DATE, p_summer_time VARCHAR2, p_xtra_qty NUMBER DEFAULT 0)
RETURN NUMBER
--<EC-DOC>
IS
  ...
  END IF;
	...
	ELSE
		...
		IF ln_closing_balance IS NULL THEN
			...
			ELSE
				ln_closing_balance := nvl(calcEstClosingBalanceDay(p_object_id, ld_production_day - 1, p_xtra_qty), 0);
			END IF;
		END IF;
	END IF;
...
--TODO Must set balance2
  ...
  END IF;
   ...
   SET balance = p_balance, last_updated_by = Nvl(p_user,USER), rev_no = rev_no + 1
   WHERE object_id = p_object_id
   AND daytime = p_daytime;
  ...
  SET    record_status = 'V', last_updated_by = Nvl(p_user,USER)
  WHERE   object_id = p_object_id
