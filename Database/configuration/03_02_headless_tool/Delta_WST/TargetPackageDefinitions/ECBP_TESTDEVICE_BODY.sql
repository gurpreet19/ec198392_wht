...
**                                  Bugfix in CalcShrinkageVolume when method is Formula. The code call ec_eqpm_result() function, but used object_id from test_device.
...
** 27.04.2012 syazwnur  ECPD-20750: Removed IF statements for user exit in findNetRate function.
** 25.02.2013 kumarsur  ECPD-23386:Added new function getMeterText, getOilMeterText, getGasMeterText and getWaterMeterText
** 02.07.2013 limmmchu  ECPD-24781: Modified getDefaultTdev
...
IS
  ...
  CURSOR c_attribute(cp_class_name VARCHAR2, cp_column_name VARCHAR2) IS
    SELECT * FROM class_attr_db_mapping cm
    WHERE cm.class_name = cp_class_name AND
          cm.db_sql_syntax = cp_column_name;

BEGIN
  lv_class_name := ec_eqpm_result.data_class_name(p_object_id,p_result_no);
...
BEGIN
  ...
  lv_std_net_rate_method := nvl(ec_eqpm_version.std_net_rate_method(lv_object_id,ld_daytime,'<='), 'MEASURED');
  lv_grs_gas_rate_method := nvl(ec_eqpm_version.grs_gas_rate_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_std_net_rate_method = 'MEASURED' THEN

    IF p_phase = Ecdp_Phase.OIL THEN
      ln_result := ec_eqpm_result.net_oil_rate_adj(lv_object_id,p_result_no);

    ELSIF p_phase = Ecdp_Phase.GAS THEN
      ln_result := ec_eqpm_result.gas_rate_adj(lv_object_id,p_result_no);

    ELSIF p_phase = Ecdp_Phase.CONDENSATE THEN
      ln_result := ec_eqpm_result.net_cond_rate_adj(lv_object_id,p_result_no);

    ELSIF p_phase = Ecdp_Phase.WATER THEN
      ln_result := ec_eqpm_result.tot_water_rate_adj(lv_object_id,p_result_no);
...
BEGIN
  ...
  lv_net_rate_method := nvl(ec_eqpm_version.net_rate_method(lv_object_id,ld_daytime,'<='), 'MEASURED');
  ...
  ELSE
    ...
    IF lv_net_rate_method = 'MEASURED' THEN
      ...
      IF p_phase = Ecdp_Phase.OIL THEN
        ln_result := nvl(ec_eqpm_result.net_oil_rate_flc(lv_object_id,p_result_no), 0) *
                       nvl(getMeterFactor(lv_object_id, EcDp_phase.OIL, ld_daytime, p_result_no),1);

      ELSIF p_phase = Ecdp_Phase.GAS THEN
        ln_result := ec_eqpm_result.gas_rate_flc(lv_object_id,p_result_no);

      ELSIF p_phase = Ecdp_Phase.CONDENSATE THEN
        ln_result := nvl(ec_eqpm_result.net_cond_rate_flc(lv_object_id,p_result_no), 0) *
                       nvl(getMeterFactor(lv_object_id, EcDp_phase.OIL, ld_daytime, p_result_no),1);

      ELSIF p_phase = Ecdp_Phase.WATER THEN
        ln_result := nvl(ec_eqpm_result.tot_water_rate_flc(lv_object_id,p_result_no), 0) *
...
BEGIN
  ...
  lv_grs_liq_rate_method := nvl(ec_eqpm_version.grs_liq_rate_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_grs_liq_rate_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.oil_out_rate_raw(lv_object_id,p_result_no);
...
BEGIN
  ...
  IF lv_grs_gas_rate_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.gas_out_rate_raw(lv_object_id,p_result_no);
...
BEGIN
  ...
  IF lv_prod_method = 'GL' THEN
    -- determine grs rate method
    lv_grs_gl_rate_method := nvl(ec_eqpm_version.grs_gl_rate_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

    IF lv_grs_gl_rate_method = 'MEASURED' THEN
      ln_result := ec_eqpm_result.gas_lift_rate_raw(lv_object_id,p_result_no);
    ...
    ELSIF lv_grs_gl_rate_method = 'RAWGAS_MINUS_GAS' THEN
      ln_result := calcShrinkageVolume(p_result_no, ec_eqpm_result.gas_out_rate_raw(lv_object_id,p_result_no), 'GAS', lv_object_id, ld_daytime) -
...
BEGIN
  ...
  -- determine grs rate method
  lv_grs_water_rate_method := nvl(ec_eqpm_version.grs_water_rate_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_grs_water_rate_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.water_out_rate_raw(lv_object_id,p_result_no);
...
BEGIN
  ...
  lv_grs_gas_vol_method := nvl(ec_eqpm_version.grs_gas_vol_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_grs_gas_vol_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.gas_out_vol_raw(lv_object_id,p_result_no);

  ELSIF lv_grs_gas_vol_method = 'ORIFICE_AGA' THEN
    ln_result := ec_eqpm_result.gas_vol_out_aga(lv_object_id,p_result_no) * nvl(findDuration(p_result_no), 0);
...
BEGIN
  ...
  lv_grs_lift_vol_method := nvl(ec_eqpm_version.grs_gl_vol_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_grs_lift_vol_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.gas_lift_vol_raw(lv_object_id,p_result_no);

  ELSIF lv_grs_lift_vol_method = 'ORIFICE_AGA' THEN
    ln_result := ec_eqpm_result.gas_lift_vol_out_aga(lv_object_id,p_result_no) * nvl(findDuration(p_result_no), 0);
...
BEGIN
  ...
  lv_grs_liq_vol_method := nvl(ec_eqpm_version.grs_liq_vol_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_grs_liq_vol_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.oil_out_vol_raw(lv_object_id,p_result_no);
...
BEGIN
  ...
  lv_grs_water_vol_method := nvl(ec_eqpm_version.grs_water_vol_method(lv_object_id, ld_daytime, '<='), 'MEASURED');

  IF lv_grs_water_vol_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.water_out_vol_raw(lv_object_id, p_result_no);
...
BEGIN
  ...
  -- determine GORMethod
  lv_gor_method := nvl(ec_eqpm_version.gor(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_gor_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.gor(lv_object_id,p_result_no);
...
BEGIN
  ...
  lv_cgr_method := nvl(ec_eqpm_version.cgr(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_cgr_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.cgr(lv_object_id,p_result_no);
...
BEGIN
  ...
  lv_bsw_vol_method := nvl(ec_eqpm_version.bsw_vol_method(lv_object_id,ld_daytime,'<='), 'MEASURED');

  --Method : MEASURED
  IF lv_bsw_vol_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.water_in_oil_out(lv_object_id,p_result_no);
...
BEGIN
  ...
  -- determine WORMethod
  lv_wor_method := nvl(ec_eqpm_version.wor(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_wor_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.wor(lv_object_id,p_result_no);
...
BEGIN
  ...
  -- determine GLRMethod
  lv_glr_method := nvl(ec_eqpm_version.glr(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_glr_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.glr(lv_object_id,p_result_no);
...
BEGIN
  ...
  -- determine WGRMethod
  lv_wgr_method := nvl(ec_eqpm_version.wgr(lv_object_id,ld_daytime,'<='), 'MEASURED');

  IF lv_wgr_method = 'MEASURED' THEN
    ln_result := ec_eqpm_result.wgr(lv_object_id,p_result_no);
...
BEGIN
  ...
  IF ln_gas_std_rate > 0 THEN
    ln_result := 2.856 *
                 ...
                 ln_gas_std_rate) +
                 ec_eqpm_result.gas_sp_grav(lv_object_id,p_result_no);
  ELSE
    ln_result := NULL;
  END IF;
...
BEGIN
  ...
  lr_eqpm_result := ec_eqpm_result.row_by_pk(lv_object_id,p_result_no);
  ...
  IF p_phase = Ecdp_Phase.OIL OR p_phase = Ecdp_Phase.CONDENSATE THEN
    ...
    ln_oil_in_water := nvl(findGrsWaterRate(p_result_no, lv_object_id, ld_daytime), 0) * (nvl(lr_eqpm_result.oil_in_water_out, 0) / 1000000); -- deduct any oil in water
  ...
  ELSIF p_phase = Ecdp_Phase.GAS THEN
    ...
    IF ec_eqpm_version.grs_gas_rate_method(lv_object_id,ld_daytime,'<=') = 'LIQUID_GOR' THEN
       RETURN 0;
    ELSE
      ln_flcPress := Ecdp_Unit.convertValue(lr_eqpm_result.tdev_press,
                                            Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(lr_eqpm_result.data_class_name, 'TDEV_PRESS')),
                                            'MBAR');
      ln_flcTemp := Ecdp_Unit.convertValue(lr_eqpm_result.tdev_temp,
                                           Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(lr_eqpm_result.data_class_name, 'TDEV_TEMP')),
                                           'K');
  ...
  ELSIF p_phase = Ecdp_Phase.WATER THEN
    ln_oil_in_water := nvl(lr_eqpm_result.oil_in_water_out, 0) / 1000000;  -- deduct any oil in the water
    ...
    ln_power_water_used := nvl(lr_eqpm_result.power_water_flc, 0);  -- deduct any power water used
...
BEGIN
  ...
  FOR cur_rst IN c_result(p_result_no) LOOP
    ...
    lv_well_object_id := cur_rst.well_object_id;
  END LOOP;
  -- get eqpm_result row, it will be used several times in this function
  lr_eqrt := ec_eqpm_result.row_by_pk(lv_object_id, p_result_no);
  ln_tdev_press := nvl(lr_eqrt.tdev_press, 0);
  ln_tdev_temp := nvl(lr_eqrt.tdev_temp, 0);
  -- determine shrinkage method
  lv_shrinkage_method := ec_eqpm_version.shrinkage_method(lv_object_id, ld_daytime, '<=');
  --
  ...
  ELSIF lv_shrinkage_method = 'TABLE_LOOKUP' THEN
    ...
    -- lr_eqrt variable is already fetch in the beginning, it holds the eqpm_result record.
    -- get temp and press from eqpm result row
    -- get eqpm_result and pwel_result row

    ln_pwr_temp := lr_eqrt.tdev_temp;
    ln_pwr_press := lr_eqrt.tdev_press;
    -- get density from eqpm result row
    IF p_phase = Ecdp_Phase.OIL THEN
...
IS
  ...
  --ln_temp_factor                  NUMBER; -- do we need this?
  --ln_gravity_factor               NUMBER;  -- do we need this?
  ln_vcf                          NUMBER;
  ln_bsw                          NUMBER;
  ln_adj_vol                      NUMBER;
...
BEGIN
  ...
  IF lv_code_exist <> 'Y' THEN
    ...
    FOR cr IN c_result(p_result_no) LOOP
      lv_eqpm_obj_id := cr.tdev_object_id;
    ...
    IF ec_well_version.isOilProducer(p_well_object_id, p_daytime,'<=') = ECDP_TYPE.IS_TRUE THEN
      ...
      ln_net_oil_rate_adj := findStdNetRate(p_result_no, Ecdp_Phase.OIL, lv_eqpm_obj_id, p_daytime);
      ln_gas_rate_adj := findStdNetRate(p_result_no, Ecdp_Phase.GAS, lv_eqpm_obj_id, p_daytime);
      ln_tot_water_rate_adj := findStdNetRate(p_result_no, Ecdp_Phase.WATER, lv_eqpm_obj_id, p_daytime);
      ln_gl_rate := findGrsGasLiftRate(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_gor := findGOR(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_glr := findGLR(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_water_cut := calcWaterCut(p_result_no, NULL, ec_ptst_result.daytime(p_result_no));
      ln_net_oil_mass_rate_adj := findNetRate(p_result_no, Ecdp_Phase.OIL, lv_eqpm_obj_id, ec_ptst_result.daytime(p_result_no)) * ec_eqpm_result.Net_oil_density_flc(lv_eqpm_obj_id, p_result_no);
      ln_diluent_rate := findDiluentRate(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_wor := findWOR(p_result_no, lv_eqpm_obj_id, p_daytime);
    ...
    ELSIF ec_well_version.isGasProducer(p_well_object_id, p_daytime,'<=') = ECDP_TYPE.IS_TRUE THEN
      ln_net_cond_rate_adj := findStdNetRate(p_result_no, Ecdp_Phase.CONDENSATE, lv_eqpm_obj_id, p_daytime);
      ln_gas_rate_adj := findStdNetRate(p_result_no, Ecdp_Phase.GAS, lv_eqpm_obj_id, p_daytime);
      ln_tot_water_rate_adj := findStdNetRate(p_result_no, Ecdp_Phase.WATER, lv_eqpm_obj_id, p_daytime);
      ln_gl_rate := findGrsGasLiftRate(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_cgr := findCGR(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_wgr := findWGR(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_wet_dry_gas_ratio := findWetDryFactor(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_wet_gas_grav := findWetGasGravity(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_gas_sp_grav := ec_eqpm_result.gas_sp_grav(lv_eqpm_obj_id, p_result_no);
      ln_net_cond_mass_rate_adj := findNetRate(p_result_no, Ecdp_Phase.CONDENSATE, lv_eqpm_obj_id, ec_ptst_result.daytime(p_result_no)) * ec_eqpm_result.net_cond_density_flc(lv_eqpm_obj_id, p_result_no);
      ln_fws_rate := findWellWetGasRate(p_result_no, lv_eqpm_obj_id, p_daytime);
      ln_diluent_rate := findDiluentRate(p_result_no, lv_eqpm_obj_id, p_daytime);
...
BEGIN
  --update pwel_result
  UPDATE pwel_result pw SET pw.rate_source = 'MANUAL', pw.last_updated_by = Nvl(p_user, ecdp_context.getAppUser())
...
BEGIN
  ...
  IF p_phase = 'GAS' THEN
    lv_meter_run_id := ec_eqpm_result.ovr_gas_meter_run_id(lv_object_id, p_result_no);
  ...
  ELSIF p_phase = 'GAS_LIFT' THEN
    lv_meter_run_id := ec_eqpm_result.ovr_gl_meter_run_id(lv_object_id, p_result_no);
    IF lv_meter_run_id IS NULL THEN
...
BEGIN
  ...
  IF p_phase = 'GAS' THEN
    lv_orifice_plate_id := ec_eqpm_result.ovr_gas_orifice_plate_id(lv_object_id, p_result_no);
    IF lv_orifice_plate_id IS NULL THEN
      lv_orifice_plate_id := ec_tdev_reference_value.orifice_plate_id(lv_object_id, ld_daytime, '<=');
    END IF;

  ELSIF p_phase = 'GAS_LIFT' THEN
    lv_orifice_plate_id := ec_eqpm_result.ovr_gl_orifice_plate_id(lv_object_id,p_result_no);
    IF lv_orifice_plate_id IS NULL THEN
...
--
---------------------------------------------------------------------------------------------------
FUNCTION agaStaticPress(p_result_no eqpm_result.result_no%TYPE,
...
BEGIN
  ...
  lv_class_name := lr_eqpm_result.data_class_name;
  ...
  FOR my_result IN c_result LOOP
    ...
    FOR my_tdev_ref IN c_tdev_ref(my_result.daytime) LOOP
      -- use values in eqpm_result, but if null default from tdev_reference_value

      IF p_phase='GAS' THEN -- gas
        ln_static_chart_scale := nvl(lr_eqpm_result.gas_static_chart_scale, my_tdev_ref.gas_stat_press_max_scale);
        ln_static_chart_rating := nvl(lr_eqpm_result.gas_static_chart_rating, my_tdev_ref.gas_stat_press_spg_rate);
        lv_attr_name := getAttributeName(p_result_no,p_object_id,'GAS_STATIC_PRESS_READ');
        ln_static_press_read := lr_eqpm_result.gas_static_press_read;
        ...
        IF (my_tdev_ref.gas_stat_press_chart_type in ('LINEAR', 'PERCENT') AND ln_static_chart_rating>0) THEN
          ...
          ln_return_value := Ecdp_Unit.convertValue(ln_return_value,
                                                  ...
                                                  Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(lv_class_name,'GAS_STATIC_PRESS')));
    ...
    ELSIF (my_tdev_ref.gas_stat_press_chart_type = 'SQUARE_ROOT' AND ln_static_chart_rating>0) THEN
      ...
      ELSE -- gas lift
        ln_static_chart_scale := nvl(lr_eqpm_result.gl_static_chart_scale, my_tdev_ref.gl_static_press_max_scale);
        ln_static_chart_rating := nvl(lr_eqpm_result.gl_static_chart_rating, my_tdev_ref.gl_static_press_spg_rate);
        lv_attr_name := getAttributeName(p_result_no,p_object_id,'GL_STATIC_PRESS_READ');
    ln_static_press_read := lr_eqpm_result.gl_static_press_read;
        ...
        IF (my_tdev_ref.gl_static_press_chart_type in ('LINEAR', 'PERCENT') AND ln_static_chart_rating>0) THEN
          ...
          ln_return_value := Ecdp_Unit.convertValue(ln_return_value,
                                                  nvl(lv_static_chart_unit,'PSIG'),
                                                  Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(lv_class_name,'GL_STATIC_PRESS')));
...
--
---------------------------------------------------------------------------------------------------
FUNCTION agaDiffPress(p_result_no eqpm_result.result_no%TYPE,
                      p_object_id VARCHAR2,
                      p_phase VARCHAR2)
RETURN number
--</EC-DOC>
...
BEGIN
  ...
  FOR my_result IN c_result LOOP
    ...
    FOR my_tdev_ref IN c_tdev_ref(my_result.daytime) LOOP
      ...
      IF p_phase='GAS' THEN -- gas
        ln_diff_chart_scale := nvl(lr_eqpm_result.gas_diff_chart_scale, my_tdev_ref.gas_diff_press_max_scale);
        ln_diff_chart_rating := nvl(lr_eqpm_result.gas_diff_chart_rating, my_tdev_ref.gas_diff_press_spg_rate);
        lv_attr_name := getAttributeName(p_result_no,p_object_id,'GAS_DIFF_PRESS_READ');
        ln_diff_press_read := lr_eqpm_result.gas_diff_press_read;
        ...
        IF (my_tdev_ref.gas_diff_press_chart_type in ('LINEAR', 'PERCENT') AND ln_diff_chart_rating>0) THEN
          ...
          ln_return_value := Ecdp_Unit.convertValue(ln_return_value,
                                                  ...
                                                  Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(lv_class_name,'GAS_DIFF_PRESS')));
      ...
      ELSE -- gas lift
        ln_diff_chart_scale := nvl(lr_eqpm_result.gl_diff_chart_scale, my_tdev_ref.gl_diff_press_max_scale);
        ln_diff_chart_rating := nvl(lr_eqpm_result.gl_diff_chart_rating, my_tdev_ref.gl_diff_press_spg_rate);
        lv_attr_name := getAttributeName(p_result_no,p_object_id,'GL_DIFF_PRESS_READ');
        ln_diff_press_read := lr_eqpm_result.gl_diff_press_read;
        ...
        IF (my_tdev_ref.gl_diff_press_chart_type in ('LINEAR', 'PERCENT') AND ln_diff_chart_rating>0) THEN
          ...
          ln_return_value := Ecdp_Unit.convertValue(ln_return_value,
                                                  nvl(lv_diff_chart_unit,'INH2O'),
                                                  Ecdp_Unit.GetUnitFromLogical(ec_class_attr_presentation.uom_code(lv_class_name,'GL_DIFF_PRESS')));
...
lv_meter_factor_method := ec_eqpm_version.meter_factor_method(p_object_id, p_daytime, '<=');
IF lv_meter_factor_method = 'COPY_FORWARD' THEN
   IF p_phase = EcDp_phase.OIL THEN
      ln_return_value := nvl(ec_eqpm_result.override_oil_mtr_factor(p_object_id,p_result_no), 1);
   ELSIF p_phase = EcDp_phase.WATER THEN
      ln_return_value := nvl(ec_eqpm_result.override_wat_mtr_factor(p_object_id,p_result_no), 1);
   END IF;

ELSIF lv_meter_factor_method = 'TDEV_REFERENCE' THEN
   IF p_phase = EcDp_phase.OIL THEN
      ln_return_value := nvl(ec_tdev_reference_value.oil_meter_factor(p_object_id, p_daytime, '<='), nvl(ec_eqpm_result.override_oil_mtr_factor(p_object_id,p_result_no),1));
   ELSIF p_phase = EcDp_phase.WATER THEN
      ln_return_value := nvl(ec_tdev_reference_value.water_meter_factor(p_object_id, p_daytime, '<='), nvl(ec_eqpm_result.override_wat_mtr_factor(p_object_id,p_result_no),1));
   END IF;
...
IF  lv_meter_factor_method = 'COPY_FORWARD' THEN
    FOR c_last_testdevice IN c_pwel_result(p_object_id, p_result_no, p_daytime) LOOP
        ...
        lr_eqpm_result := ec_eqpm_result.row_by_pk(p_object_id, c_last_testdevice.result_no);

        UPDATE EQPM_RESULT ER
        SET ER.Override_Oil_Mtr_Factor = lr_eqpm_result.override_oil_mtr_factor,
        ER.Override_Wat_Mtr_Factor = lr_eqpm_result.override_wat_mtr_factor
...
BEGIN
  ...
  lv_diluent_rate_method := nvl(ec_eqpm_version.diluent_rate_method(lv_object_id, ld_daytime,'<='), 'MEASURED');

  IF lv_diluent_rate_method = 'MEASURED' THEN
     ln_result := ec_eqpm_result.est_diluent_rate_flc(lv_object_id, p_result_no);

  ELSIF lv_diluent_rate_method = 'VOLUME_DURATION' THEN
     ln_duration := findDuration(p_result_no);

     IF ln_duration > 0 THEN
        ln_result := ec_eqpm_result.est_diluent_vol_flc(lv_object_id, p_result_no) / ln_duration * 24;
     END IF;
...
BEGIN
  ...
  lv_well_wet_gas_rate_method := nvl(ec_eqpm_version.well_wet_gas_rate_method(lv_object_id, ld_daytime,'<='), 'MEASURED');

  IF lv_well_wet_gas_rate_method = 'MEASURED' THEN
     ln_result := ec_eqpm_result.well_wet_gas_rate(lv_object_id, p_result_no);

  ELSIF lv_well_wet_gas_rate_method = 'VOLUME_DURATION' THEN
     ln_duration := findDuration(p_result_no);

     IF ln_duration > 0 THEN
        ln_result := ec_eqpm_result.well_wet_gas_vol(lv_object_id, p_result_no) / ln_duration * 24;
     END IF;
...
---------------------------------------------------------------------------------------------------
-- Function      : findRefShrinkageFactor
-- Description    : Find the correct Shrinkage method = TDEV_FACTOR or WELL_FACTOR or WELL_TDEV_FACTOR
--
--
-- Preconditions  :
--
...
BEGIN
 ...
 IF ln_result = 1 THEN
  ...
  lv_result:='Test Device '||ln_name||' has already been set as a default for current Facility.';
   RAISE_APPLICATION_ERROR(-20643, lv_result);
 ...
 ELSIF ln_result > 1 THEN
  ...
  END LOOP;
    ...
    lv_tot_result:='Test Device '||lv_result||' has already been set a default for current Facility.';
     ...
     RAISE_APPLICATION_ERROR(-20643, lv_tot_result);
 ELSE
  NULL;
 END IF;
