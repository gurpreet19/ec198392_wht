...
**  30.07.12 Jimmy ECPD-21382: Add if statement to function GetObjName if the return value is null from the original implementation, the logic will return the name of the object based on its start date.
****************************************************************/
...
IS
	...
	CURSOR c_object IS
	 ...
	 WHERE (class_name = UPPER(p_class_name) OR
	 		  class_name IN (SELECT child_class FROM class_dependency WHERE parent_class = UPPER(p_class_name) and dependency_type = 'IMPLEMENTS'))
	 AND code = p_code;
...
IS
	...
	lv2_return_val class.class_name%TYPE;
...
IS
	...
	lv2_class_name 		class.class_name%TYPE := EcDp_Objects.GetObjClassName(p_object_id);
...
BEGIN
   ...
   IF p_daytime > ld_earliest_date_use THEN
   	Raise_Application_Error(-20109,'New object start_date can not be after '||TO_CHAR(ld_earliest_date_use,'dd.mm.yyyy')||
   ...
   IF lv2_rel_object_id IS NOT NULL THEN
   	Raise_Application_Error(-20109,'New object start_date cannot be before '||TO_CHAR(EcDp_Objects.GetObjStartDate(lv2_rel_object_id),'dd.mm.yyyy')||
...
IS
	...
	lv2_class_name			class.class_name%TYPE := GetObjClassName(p_object_id);
   ld_latest_use			DATE;
	lv2_rel_object_id		VARCHAR2(32);
...
BEGIN
   ...
   IF p_daytime < ld_latest_use THEN
		Raise_Application_Error(-20110,'New object end_date can not be before '||
...
IS
   CURSOR c_child IS
    ...
    SELECT c.class_name, c.class_type, cd.DB_OBJECT_OWNER, cd.DB_OBJECT_NAME,  cd.DB_WHERE_CONDITION , 'OBJECT_ID' DB_SQL_SYNTAX, 'Y' use_daytime
    FROM class c, class_db_mapping cd
    ...
    SELECT cr.to_class_name, c.class_type, cd.DB_OBJECT_OWNER, cd.DB_OBJECT_NAME,  cd.DB_WHERE_CONDITION ,cr.DB_SQL_SYNTAX, 'Y' use_daytime
    FROM class_rel_db_mapping cr, class_db_mapping cd, class c
    ...
    SELECT db.class_name, c.class_type, db.db_object_owner, db.db_object_name, db.db_where_condition, cr.db_sql_syntax, 'N' use_daytime
    ...
    SELECT db.class_name, c.class_type, db.db_object_owner, db.db_object_attribute db_object_name, db.db_where_condition, cr.db_sql_syntax, 'Y' use_daytime
    FROM class_rel_db_mapping cr, class c, class_db_mapping db
    WHERE cr.to_class_name = c.class_name
    AND   cr.to_class_name = db.class_name
    AND   cr.from_class_name = p_class_name
    ...
    SELECT c.class_name, c.class_type, cd.DB_OBJECT_OWNER, cd.DB_OBJECT_NAME,  cd.DB_WHERE_CONDITION , 'OBJECT_ID' DB_SQL_SYNTAX, 'N' use_daytime
    FROM class c, class_db_mapping cd, class_dependency cy
    WHERE c.class_name  = cd.class_name
...
IS
	...
	CURSOR c_relation IS
	 ...
	 FROM v_group_level v, class_rel_db_mapping crdm, class_db_mapping cdm
	...
	UNION -- Non versioned relations
	 SELECT crdm.db_sql_syntax, cdm.db_object_name,'MAIN' table_mode
	 FROM class_rel_db_mapping crdm, class_relation cr,class_db_mapping cdm
	 WHERE crdm.from_class_name = cr.from_class_name
	 AND crdm.to_class_name = cr.to_class_name
	 AND crdm.role_name = cr.role_name
	 AND crdm.from_class_name = cdm.class_name
	 AND cr.group_type is null
	 AND crdm.db_mapping_type = 'COLUMN'
	 AND crdm.to_class_name = p_class_name
	UNION -- Versioned relations
	 SELECT crdm.db_sql_syntax, cdm.db_object_name,'VERSION' table_mode
	 FROM class_rel_db_mapping crdm, class_relation cr,class_db_mapping cdm
	 WHERE crdm.from_class_name = cr.from_class_name
	 AND crdm.to_class_name = cr.to_class_name
	 AND crdm.role_name = cr.role_name
	 AND crdm.from_class_name = cdm.class_name
	 AND cr.group_type is null
	 AND crdm.db_mapping_type = 'ATTRIBUTE'
	 AND crdm.to_class_name = p_class_name;
...
BEGIN
	...
	FOR curRel IN c_relation LOOP
		...
		lv2_sql := lv2_sql||' SELECT start_date, object_id'||CHR(10);
...
BEGIN
	...
	FOR curRel IN c_relation LOOP
		...
		lv2_sql := lv2_sql||' SELECT nvl(end_date,EcDp_System_Constants.FUTURE_DATE)), object_id'||CHR(10);
		lv2_sql := lv2_sql||' FROM '||curRel.db_object_name||' o'||CHR(10);
		lv2_sql := lv2_sql||' WHERE EXISTS('||CHR(10);
		lv2_sql := lv2_sql||'   SELECT 1 FROM '||lv2_table||CHR(10);
		lv2_sql := lv2_sql||'   WHERE object_id = :p_object_id'||CHR(10);
...
IS
    ...
    UNION ALL
    -- All data classes with a relation to p_class_name
    SELECT cr.to_class_name, c.class_type, cd.DB_OBJECT_OWNER, cd.DB_OBJECT_NAME,  cd.DB_WHERE_CONDITION ,cr.DB_SQL_SYNTAX
    FROM class_rel_db_mapping cr, class_db_mapping cd, class c
    WHERE cr.to_class_name = c.class_name
    AND   cr.to_class_name = cd.class_name
    AND   cr.from_class_name = p_class_name
    ...
    UNION ALL
    -- All object classes with a non versioned relation to p_class_name
    SELECT db.class_name, c.class_type, db.db_object_owner, db.db_object_name, db.db_where_condition, cr.db_sql_syntax
    FROM class_rel_db_mapping cr, class c, class_db_mapping db
    WHERE cr.to_class_name = c.class_name
    AND   cr.to_class_name = db.class_name
    AND   cr.from_class_name = p_class_name
...
BEGIN
   ...
   AND   p_object_start_date <> p_daytime THEN
     Raise_Application_Error(-20109,'On insert DAYTIME cannot be different from OBJECT_START_DATE.');
   ...
   IF p_object_end_date IS NOT NULL AND p_object_end_date <= ld_daytime THEN
     Raise_Application_Error(-20110,'On insert OBJECT_END_DATE must be greater than OBJECT_START_DATE.');
   END IF;
