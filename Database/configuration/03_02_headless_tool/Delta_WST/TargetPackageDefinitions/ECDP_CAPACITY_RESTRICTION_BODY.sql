...
** 2012-07-02  sharawan  ECPD-21426: Modified function getCapacityUom and getDesignCapacity to query from iv_operational_restriction.
...
--</EC-DOC>
 IS
  ...
  SELECT SUM(VOL_QTY) sum_vol_qty
...
BEGIN
	...
	DELETE FROM oploc_daily_restriction p
	...
	WHILE ld_daytime < ld_period_last_date + 1 LOOP
		...
		IF ld_period_first_date = ld_period_last_date THEN
			...
			ln_hrs_total   := Ecdp_Date_Time.getNumHours(NULL, p_object_id, ld_period_first_date);
		...
		ELSE
			...
			IF ld_daytime = ld_period_first_date THEN
				ln_hrs_total  := Ecdp_Date_Time.getNumHours(NULL, p_object_id, ld_daytime);
		...
		INSERT INTO oploc_daily_restriction
			(object_id, daytime, restricted_capacity, restriction_type, created_by)
		VALUES
			(p_object_id, ld_daytime, ln_total_cap, lv2_rest_type, ecdp_context.getAppUser);
