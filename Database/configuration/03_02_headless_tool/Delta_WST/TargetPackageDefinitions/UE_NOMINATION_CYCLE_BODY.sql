...
BEGIN
	IF p_daytime IS NULL THEN
		ld_date := ecdp_date_time.getCurrentSysdate;
	ELSE
		ld_date := p_daytime;
	END IF;

	-- if navigator day is the same as current day
	IF TRUNC(ld_date, 'DD') = TRUNC(ecdp_date_time.getCurrentSysdate, 'DD') THEN
	...
	ELSIF TRUNC(ld_date, 'DD') > TRUNC(ecdp_date_time.getCurrentSysdate, 'DD') THEN
		FOR curMin IN c_min LOOP
			ln_min_offset := curMin.min_offset;
		END LOOP;

		ln_diff := TRUNC(ecdp_date_time.getCurrentSysdate, 'DD') - TRUNC(ld_date, 'DD');
