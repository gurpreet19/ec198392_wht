...
** 2006-05-18   Stian Skjørestad    TD#6059  Modified function aggregateAllSubDaily, added "distinct" keyword to query in for-loop.
...
** 2012-04-12   rajarsar            ECPD-20520: Modified function aggregateAllSubDaily to include check on aggegation flag and end date
** 2013-04-09   limmmchu            ECPD-23847: Modified aggregateAllSubDaily
** 2013-07-16	choooshu			ECPD-24455: Included missing well types for aggregateAllSubDaily
** 2013-07-27   musthram            ECPD-24737: Modified aggregateAllSubDaily to support access level checking
** 2013-08-15   rajarsar            ECPD-25172: Updated the portion when UOM is equals to %RATE%, %FRAC%, %PCT% in aggregateSubDaily() function to ensure null rates are not assumed as 0.
**************************************************************/
...
BEGIN
  ...
  END IF;
  ln_numrows := EcDp_Utilities.executeSingleRowNumber(lv2_sql);
  ...
  LOOP
    ...
    IF EcDp_ClassMeta.c_intersect_columns%NOTFOUND THEN
      IF p_aggr_datetime IS NULL THEN
        ...
        lv2_aggr_datetime := to_char(ecdp_date_time.getCurrentSysdate,'yyyy-mm-dd"T"hh24:mi:ss');
      ELSE
...
IS
  ...
  lv2_aggr_datetime   VARCHAR2(32) := to_char(ecdp_date_time.getCurrentSysdate,'yyyy-mm-dd"T"hh24:mi:ss');
  lv2_inj_type        VARCHAR2(2);
  ld_end_date         DATE;
...
BEGIN
   ...
   lv2_obj_ver_tab := ec_class_db_mapping.db_object_attribute(EcDp_ClassMeta.OwnerClassName(p_from_class_name));
   ld_end_date := p_daytime + 1;

   IF lv2_obj_ver_tab = 'WEBO_INTERVAL_VERSION' THEN

      cur_sql := 'SELECT wbi.OBJECT_ID '||
                 'FROM WEBO_INTERVAL wbi, WEBO_INTERVAL_VERSION wbiv, WEBO_BORE wb, well_version wellver '||
   ...
   ELSIF lv2_obj_ver_tab = 'WELL_VERSION' THEN
      ...
      CASE p_from_class_name
         WHEN 'PWEL_SUB_DAY_STATUS' THEN
            lv2_well_type := '(''OP'', ''GP'', ''CP'', ''OPGI'', ''GPI'', ''OPSI'',''GP2'')';
      ...
      cur_sql := 'SELECT OBJECT_ID FROM '||lv2_obj_ver_tab||
                 ...
                 ' AND DAYTIME = (SELECT MAX(DAYTIME) FROM '||lv2_obj_ver_tab||
                                   ' WHERE OBJECT_ID = wv.OBJECT_ID'||
                                   ' AND DAYTIME < '''||p_daytime_tomorrow||''')'||
                   ' AND OBJECT_ID NOT IN (SELECT OBJECT_ID FROM IWEL_PERIOD_STATUS iwps '||
                                           'WHERE NVL(ec_iwel_period_status.active_well_status(object_id, '''||p_daytime_tomorrow||''', '''||lv2_inj_type||''', ''EVENT'', ''<''),''OPEN'') = ''CLOSED_LT'' '||
                                           'UNION '||
                                           'SELECT OBJECT_ID FROM PWEL_PERIOD_STATUS pwps '||
                                           'WHERE NVL(ec_pwel_period_status.active_well_status(object_id, '''||p_daytime_tomorrow||''', ''EVENT'', ''<''),''OPEN'') = ''CLOSED_LT'') '||
                   ' AND DAYTIME = (SELECT MAX(DAYTIME) FROM '||lv2_obj_ver_tab||
                                   ' WHERE OBJECT_ID = wv.OBJECT_ID'||
                                   ' AND OP_FCTY_CLASS_1_ID = '''||p_facility_id||
                                   ''' AND DAYTIME <= '''||p_daytime||''')';
