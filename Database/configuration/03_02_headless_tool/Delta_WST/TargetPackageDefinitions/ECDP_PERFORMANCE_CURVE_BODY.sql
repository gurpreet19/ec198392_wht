...
** 25.09.00   ØJ    	Bug fix in : addUpdatePerfCurveExtraXValue in-argument ln_curve_id in
...
** 21.11.2012 limmmchu ECPD-22594: Modified getWellPerformanceCurveId, getNextWellPerformanceCurveId and getPrevWellPerformanceCurveId
** 18.12.2012 musthram ECPD-22894: Modified getCurveIdPhase
** 10.05.2013 wonggkai ECPD-24185: Modified getCurveIdPhase
*************************************************************************************************/
...
BEGIN
  ...
  IF ecdp_objects.GetObjClassName(p_curve_object_id) = 'WELL' THEN
    ...
    ELSIF (p_phase=EcDp_Phase.WATER and p_curve_purpose=EcDp_Curve_Purpose.PRODUCTION) THEN
      ...
      ELSIF ec_well_version.well_meter_freq(p_curve_object_id, p_daytime, '<=') IN ('2H', '1H', '30M','15M','10M','5M', '1M') THEN
        ...
        END IF;
      END IF;
    /*ELSIF (p_phase=EcDp_Phase.CONDENSATE and p_curve_purpose=EcDp_Curve_Purpose.PRODUCTION) THEN
      IF ec_well_version.well_meter_freq(p_curve_object_id, p_daytime, '<=') IN ('YR', 'MTH', 'DAY') THEN
        IF nvl(p_calc_method,ec_well_version.calc_cond_method(p_curve_object_id, p_daytime, '<=')) = EcDp_Calc_Method.CURVE THEN
          lv2_phase := EcDp_Phase.CONDENSATE;
        ELSIF nvl(p_calc_method,ec_well_version.calc_cond_method(p_curve_object_id, p_daytime, '<=')) = EcDp_Calc_Method.CURVE_LIQUID THEN
          lv2_phase := EcDp_Phase.CONDENSATE;
        ELSE
          lv2_phase := EcDp_Phase.GAS;
        END IF;
      ELSIF ec_well_version.well_meter_freq(p_curve_object_id, p_daytime, '<=') IN ('2H', '1H', '30M','15M','10M','5M', '1M') THEN
        IF nvl(p_calc_method,ec_well_version.calc_sub_day_cond_method(p_curve_object_id, p_daytime, '<=')) = EcDp_Calc_Method.CURVE THEN
          lv2_phase := EcDp_Phase.CONDENSATE;
        ELSIF nvl(p_calc_method,ec_well_version.calc_sub_day_cond_method(p_curve_object_id, p_daytime, '<=')) = EcDp_Calc_Method.CURVE_LIQUID THEN
          lv2_phase := EcDp_Phase.CONDENSATE;
        ELSE
          lv2_phase := EcDp_Phase.GAS;
        END IF;
  ...
  ELSIF ecdp_objects.GetObjClassName(p_curve_object_id) = 'FLOWLINE' THEN
    IF (p_phase=EcDp_Phase.OIL and p_curve_purpose=EcDp_Curve_Purpose.PRODUCTION) THEN
      IF ec_flwl_version.flw_calc_oil_mtd(p_curve_object_id, p_daytime, '<=') = EcDp_Calc_Method.CURVE THEN
        lv2_phase := EcDp_Phase.OIL;
      ELSE
        lv2_phase := NULL;
      END IF;
    ELSIF (p_phase=EcDp_Phase.GAS and p_curve_purpose=EcDp_Curve_Purpose.PRODUCTION) THEN
      IF ec_flwl_version.flw_calc_gas_mtd(p_curve_object_id, p_daytime, '<=') = EcDp_Calc_Method.CURVE THEN
        lv2_phase := EcDp_Phase.GAS;
      ELSE
        lv2_phase := NULL;
      END IF;
    ELSIF (p_phase=EcDp_Phase.WATER and p_curve_purpose=EcDp_Curve_Purpose.PRODUCTION) THEN
      IF ec_flwl_version.flw_calc_water_mtd(p_curve_object_id, p_daytime, '<=') = EcDp_Calc_Method.CURVE THEN
        lv2_phase := EcDp_Phase.WATER;
      ELSE
        lv2_phase := NULL;
      END IF;
    ELSIF (p_phase=EcDp_Phase.CONDENSATE and p_curve_purpose=EcDp_Curve_Purpose.PRODUCTION) THEN
      IF ec_flwl_version.flw_calc_cond_mtd(p_curve_object_id, p_daytime, '<=') = EcDp_Calc_Method.CURVE THEN
        lv2_phase := EcDp_Phase.CONDENSATE;
      ELSE
        lv2_phase := NULL;
      END IF;
...
BEGIN
  ...
  ELSE
    IF ecdp_objects.GetObjClassName(p_curve_object_id) = 'WELL' THEN
      ...
      END IF;
    ELSIF ecdp_objects.GetObjClassName(p_curve_object_id) = 'FLOWLINE' THEN
      lv2_phase := getCurveIdPhase(p_curve_object_id,
                                     p_daytime,
                                     p_curve_purpose,
                                     p_phase,
                                     p_calc_method);
...
BEGIN
   ...
   ld_daytime:=trunc(EcDp_Date_Time.getCurrentSysdate,'dd');
   ...
   FOR r_curve IN c_curves LOOP
      ...
      INSERT INTO curve (curve_id,perf_curve_id,formula_type,phase,z_value,c0,c1,c2,y_valid_from,y_valid_to)
      (
         SELECT ln_dest_curve_id,ln_dest_perf_curve_id,formula_type,phase,z_value,c0,c1,c2,y_valid_from,y_valid_to
         FROM curve
