...
-- Using tables   :
--
-- Using functions: EcBp_Cargo_Numbering.getCargoName
--
-- Configuration
-- required       :
--
...
BEGIN
	...
	-- customer specific cargo name
	 ls_cargo_name := ls_cargo_name || EcBp_Cargo_Numbering.getCargoName(p_cargo_no, p_parcels, EcDp_Date_Time.getCurrentSysdate);
...
BEGIN
  ...
  to_Char(ecdp_date_time.getCurrentSysdate(),'dd-mm-yyyy hh24:mi:ss')|| '. cargo_no=' || p_cargo_no || ' / offset=' || p_day_offset || ' / fcast=' || p_fcast_id );
  */
...
BEGIN
  IF p_copy_method = 'INSERT' THEN
 	...
 	lv_laytime := getCarrierLaytimeDate(p_carrier_id, trunc(sysdate));
...
BEGIN
  ...
  checkValidNom(p_parcels,p_cargo_no,lv_carrier_id);
	...
	lv_sql := 'UPDATE storage_lift_nomination SET cargo_no = ' || ln_assign_id ||', LAST_UPDATED_BY = '''||p_user||''' WHERE parcel_no IN ' || p_parcels;
...
BEGIN
  ...
  ELSE
    RETURN ecdp_objects.GetObjName(p_carrier_id,ecdp_date_time.getCurrentSysdate);
  END IF;
...
BEGIN
	...
	lv_result := getCarrierLaytimeDate(p_carrier_id, NVL(ld_daytime, trunc(sysdate)));
	RETURN lv_result;
