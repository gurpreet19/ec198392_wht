...
BEGIN
    ...
    IF m.pk_val_1 IS NOT NULL THEN
    	lv2_retval := nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_1), m.pk_attr_1);
    END IF;

    IF m.pk_val_2 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_2), m.pk_attr_2);
    END IF;

    IF m.pk_val_3 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_3), m.pk_attr_3);
    END IF;

    IF m.pk_val_4 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_4), m.pk_attr_4);
    END IF;

    IF m.pk_val_5 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_5), m.pk_attr_5);
    END IF;

    IF m.pk_val_6 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_6), m.pk_attr_6);
    END IF;

    IF m.pk_val_7 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_7), m.pk_attr_7);
    END IF;

    IF m.pk_val_8 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_8), m.pk_attr_8);
    END IF;

    IF m.pk_val_9 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_9), m.pk_attr_9);
    END IF;

    IF m.pk_val_10 IS NOT NULL THEN
    	lv2_retval := lv2_retval ||','|| nvl(Ec_Class_Attr_Db_Mapping.db_sql_syntax(m.data_class,m.pk_attr_10), m.pk_attr_10);
    END IF;
