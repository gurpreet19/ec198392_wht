...
** 21.06.2012 limmmchu ECPD-21304: Modified createCompSetForPeriodAnalysis function in EcBp_Fluid_Analysis.getPeriodCompSet.
** 06.02.2013 makkkkam ECPD-23303: Modified convCompMolToEnergy.
** 05.08.2013 wonggkai ECPD-25025: Add new method getAnalysisNo, getAnalysisNoPeriodAn and getNextAnalysisSamplePeriodAn, Modified normalizeCompTo100 and normalizeCompTo100PeriodAn, change procedure called to get lr_analysis.
** 24.09.2013 wonggkai ECPD-25579: Fix logic and wrong table reference in method convCompBetweenMolWt and convCompBetweenMolWtPeriodAn, replaced p_daytime with valid_from_date in getLastAnalysisSample() and NextAnalysisSample()
...
IS
  ...
  -- Cursor using the sample method
  CURSOR c_analyse(
    cp_object_id        stream.object_id%TYPE,
    cp_analysis_type    VARCHAR2,
    cp_sampling_method  VARCHAR2,
  ...
  -- Cursor that not using the sample method
  CURSOR c_analyse_noSample(
    cp_object_id        stream.object_id%TYPE,
    cp_analysis_type    VARCHAR2,
    cp_daytime          DATE,
    cp_phase            VARCHAR2
...
BEGIN
  ...
  -- If p_sampling_method is NOT NULL, use cursor with sampling method
  IF p_sampling_method IS NOT NULL THEN

    FOR cur_rec IN c_analyse(lv2_analysis_ref_id, p_analysis_type, p_sampling_method, p_daytime, lv2_phase) LOOP

      lr_analysis_sample := cur_rec;

    END LOOP;

  ELSE -- If p_sampling_method is NULL, use cursor without sampling method

    FOR cur_rec IN c_analyse_noSample(lv2_analysis_ref_id, p_analysis_type, p_daytime, lv2_phase) LOOP
...
BEGIN
  ...
  FOR cur_rec IN c_analyse(p_object_id, p_analysis_type, p_sampling_method, p_daytime, lv2_phase) LOOP
...
IF EcDp_Date_Time.interceptsWinterAndSummerTime(p_daytime, lv_pday_object_id) = 'N' THEN
...
BEGIN
   ...
   -- lock test
     p_analysis_no := EcDp_Fluid_analysis.getAnalysisNo(p_object_id,p_analysis_type,p_sampling_method,p_daytime);
     lr_analysis := ec_object_fluid_analysis.row_by_pk(p_analysis_no);
...
IS
   ...
   p_analysis_no      VARCHAR2(32);
   lr_analysis_pre   object_fluid_analysis%ROWTYPE;

BEGIN

   -- lock test
  p_analysis_no := EcDp_Fluid_analysis.getAnalysisNoPeriodAn(p_object_id,p_analysis_type,p_sampling_method,p_daytime);
  lr_analysis := ec_strm_analysis_event.row_by_pk(to_number(p_analysis_no));
   ...
   ln_analysis_no := EcDp_Fluid_Analysis.getPeriodAnalysisNumberByUK(p_object_id, p_analysis_type, p_daytime,p_sampling_method,p_daytime_summer_time);
...
BEGIN
  ...
  LOOP
    ...
    FOR comp_rec IN c_comp(ana_rec.analysis_no) LOOP
      ...
      FOR class_rec IN (SELECT DISTINCT d.class_name, d.source_name
                          FROM dao_meta d
                         WHERE d.class_name = p_class_name) LOOP
        lv_view_name := class_rec.class_name;

        j := 0;
        lv_sql := 'UPDATE ' || class_rec.source_name || ' SET ';

        FOR attr_rec IN (SELECT a.attribute_name, d.db_sql_syntax, a.disabled_ind
                           FROM class_attribute a, class_attr_db_mapping d
                          WHERE d.class_name = a.class_name
                            AND d.attribute_name = a.attribute_name
                            AND a.class_name = lv_view_name
                            AND NVL(a.disabled_IND, 'N') = 'N'
                            AND D.DB_SQL_SYNTAX IN ('MOL_PCT', 'WT_PCT')) LOOP
          j := j + 1;
      ...
      FOR class_rec IN (SELECT DISTINCT d.class_name, d.source_name
        ...
        FOR attr_rec IN (SELECT a.attribute_name, d.db_sql_syntax, a.disabled_ind
                          ...
                          WHERE d.class_name = a.class_name
                            ...
                            AND a.class_name = lv_view_name
                            AND NVL(a.disabled_IND, 'N') = 'N'
                            AND D.DB_SQL_SYNTAX IN ('WT_PCT')) LOOP
          j := j + 1;
