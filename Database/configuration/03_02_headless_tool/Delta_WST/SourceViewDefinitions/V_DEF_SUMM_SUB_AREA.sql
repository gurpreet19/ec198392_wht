CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_DEF_SUMM_SUB_AREA" ("SUMMARY_TYPE", "DAYTIME", "OBJECT_ID", "OIL", "GAS", "COND", "WATER", "WATER_INJ", "GAS_INJ", "STEAM_INJ", "SORT_ORDER", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  (
----------------------------------------------------------------------------------------------------
-- File name: V_DEF_SUMM_SUB_AREA.sql
-- View name: V_DEF_SUMM_SUB_AREA
--
-- $Revision: 1.0 $
--
-- Purpose  : This view is used to find total planned, actual, assigned deferment and unassigned deferment for sub-area at given date.
--
...
GROUP BY p.code_text, sd.daytime, oa.op_sub_area_id
UNION
SELECT
p.code_text summary_type,
sd.daytime daytime,
