CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_FCST_SUMM_REASON_MTH" ("REASON_CODE_1", "MONTH", "FORECAST_ID", "SCENARIO_ID", "OIL_LOSS_VOLUME", "GAS_LOSS_VOLUME", "COND_LOSS_VOLUME", "WATER_LOSS_VOLUME", "WATER_INJ_LOSS_VOLUME", "STEAM_INJ_LOSS_VOLUME", "GAS_INJ_LOSS_VOLUME", "DILUENT_LOSS_VOLUME", "GAS_LIFT_LOSS_VOLUME", "CO2_INJ_LOSS_VOLUME", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  (
----------------------------------------------------------------------------------------------------
-- File name: v_fcst_summ_reason_mth.sql
-- View name: V_FCST_SUMM_REASON_MTH
--
-- $Revision: 1.0 $
--
...
SELECT we.reason_code_1  AS REASON_CODE_1,
       TRUNC(wec.daytime, 'month') AS MONTH,
       we.forecast_id,
       we.SCENARIO_ID,
