CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_DASH_DOC_CNT_BP_CA" ("SORT_ORDER", "IS_DEFAULT", "PERIOD_CHAR", "PERIOD_DATE", "DOC_COUNT", "STATUS", "INC_BOOKED", "DASH_HEADER", "CA_ID") AS 
  select max(m.sort_order) as sort_order,
       ...
       'Document Validation Count by Booking Period for Contract Area '''
       ||max(m.ca_name)
       ||'''' as DASH_HEADER,
       m.CA_ID as CA_ID
