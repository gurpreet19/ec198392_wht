CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_WELL_RESV_BLOCK_FORMATION" ("OBJECT_ID", "WELL_ID", "DAYTIME", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  (
-------------------------------------------------------------------------------------
...
NULL AS REV_NO,
NULL AS REV_TEXT
FROM well_version wv, well w, system_days sd, webo_bore wb ,webo_interval wbi, perf_interval pi, perf_interval_version piv, resv_block_formation rbf, rbf_version rbfv
 ...
 AND sd.daytime BETWEEN piv.daytime AND Nvl(piv.end_date-1,sd.daytime+1)
)