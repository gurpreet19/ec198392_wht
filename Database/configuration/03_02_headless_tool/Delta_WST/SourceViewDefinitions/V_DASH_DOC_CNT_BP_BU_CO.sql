CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_DASH_DOC_CNT_BP_BU_CO" ("SORT_ORDER", "IS_DEFAULT", "PERIOD_CHAR", "PERIOD_DATE", "DOC_COUNT", "STATUS", "INC_BOOKED", "DASH_HEADER", "CO_ID", "BU_ID") AS 
  select max(m.sort_order) as sort_order,
       ...
       'Document Validation Count by Booking Period for Company/Business Unit '''
       ||max(m.co_name)
       ||''''
       ||'/'''
       ||max(m.bu_name)
  ...
  left join cont_document x
    on (m.daytime = trunc(nvl(x.booking_period,
                               decode(ec_ctrl_system_attribute.attribute_text(x.daytime,
                                      ...
                                      x.document_date)),
                           'MM') and
       m.code = x.document_level_code and m.CO_ID = x.owner_company_id and
       m.bu_id = ec_contract_area_version.business_unit_id(ec_contract_area.object_id_by_uk(x.CONTRACT_AREA_CODE), x.daytime,'<='))
