CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_FCST_SUMM_VAR_CPR" ("FORECAST_ID_1", "OBJECT_ID_1", "FORECAST_ID_2", "OBJECT_ID_2", "VARIABLE_CODE", "VARIABLE_NAME", "SORT_ORDER", "OIL_1", "OIL_2", "OIL_DIFF", "GAS_1", "GAS_2", "GAS_DIFF", "WAT_1", "WAT_2", "WAT_DIFF", "COND_1", "COND_2", "COND_DIFF", "GL_1", "GL_2", "GL_DIFF", "DL_1", "DL_2", "DL_DIFF", "WI_1", "WI_2", "WI_DIFF", "GI_1", "GI_2", "GI_DIFF", "SI_1", "SI_2", "SI_DIFF", "CI_1", "CI_2", "CI_DIFF", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  SELECT
       ...
       NULL AS REV_NO,
       NULL AS REV_TEXT
FROM  (SELECT FORECAST_ID, OBJECT_ID, FROM_MONTH AS FROM_MONTH_1, TO_MONTH AS TO_MONTH_1, VARIABLE_CODE AS VARIABLE_CODE_1, VARIABLE_NAME AS VARIABLE_NAME_1, SORT_ORDER AS SORT_ORDER_1, OIL AS OIL_1, GAS AS GAS_1, WAT AS WAT_1, COND AS COND_1, GL AS GL_1, DL AS DL_1, WI AS WI_1, GI AS GI_1, SI AS SI_1, CI AS CI_1
       FROM TABLE(EcBp_Forecast_Prod.getGrpFcstVarDataGroup('SCENARIO_1'))) s1
FULL OUTER JOIN
      (SELECT FORECAST_ID, OBJECT_ID, FROM_MONTH AS FROM_MONTH_2, TO_MONTH AS TO_MONTH_2, VARIABLE_CODE AS VARIABLE_CODE_2, VARIABLE_NAME AS VARIABLE_NAME_2, SORT_ORDER AS SORT_ORDER_2, OIL AS OIL_2, GAS AS GAS_2, WAT AS WAT_2, COND AS COND_2, GL AS GL_2, DL AS DL_2, WI AS WI_2, GI AS GI_2, SI AS SI_2, CI AS CI_2
       FROM TABLE(EcBp_Forecast_Prod.getGrpFcstVarDataGroup('SCENARIO_2'))) s2
