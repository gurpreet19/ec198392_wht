CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_RE_HCB_BETW_NODES_DAY_AC" ("DAYTIME", "BALANCE_CODE", "BALANCE_NAME", "BALANCE_CODE_NAME", "MASS_UOM_CODE", "VOLUME_UOM_CODE", "ENERGY_UOM_CODE", "NET_ENERGY", "NET_MASS", "NET_VOLUME", "IN_OUT", "BETWEEN_NODE1_CODE", "BETWEEN_NODE1_NAME", "BETWEEN_NODE2_CODE", "BETWEEN_NODE2_NAME", "AT_NODE_CODE", "AT_NODE_NAME", "FROM_NODE_CODE", "FROM_NODE_NAME", "TO_NODE_CODE", "TO_NODE_NAME", "FIELD_CODE", "FIELD_NAME", "STREAM_CATEGORY_CODE", "STREAM_ITEM_CODE", "STREAM_ITEM_NAME", "MASTER_UOM_GROUP", "STATUS", "NET_ENERGY_JO", "NET_ENERGY_TH", "NET_ENERGY_WH", "NET_ENERGY_BE", "NET_MASS_MA", "NET_MASS_MV", "NET_MASS_UA", "NET_MASS_UV", "NET_VOLUME_BI", "NET_VOLUME_BM", "NET_VOLUME_SF", "NET_VOLUME_NM", "NET_VOLUME_SM", "PRODUCT_CODE") AS 
  (
----------------------------------------------------------------------------------------------------
--
-- File name: v_re_hcb_betw_nodes_day_ac.sql
-- View name: v_re_hcb_betw_nodes_day_ac
--
-- $Revision: 1.5 $
--
-- Purpose  : This report contains values for a hydrocarbon balance between connected nodes (actual version)
--
...
FROM    balance b                                                 -- BALANCE
        ...
        ,stream_item si
        ,STIM_DAY_ACTUAL stim                                     -- STIM_DAY_ACTUAL
        ,v_re_between_node    BETWEEN_NODE1
        ,v_re_between_node    BETWEEN_NODE2
        ,stim_mth_value  smv
--        ,objects_attribute  sic_so
WHERE    b.object_id = b_si.object_id                              -- BALANCE -> STREAM_ITEM
...
AND     smv.daytime = between_node1.daytime                      -- STIM_DAY_ACTUAL -> ACROSS_NODE
AND     b.object_id = between_node1.b_object_id                   -- BALANACE->STREAM_ITEM.ATTRIBUTE -> BETWEEN_NODE1
--      join in between_node2
AND     smv.daytime = between_node2.daytime                      -- STIM_DAY_ACTUAL -> ACROSS_NODE
AND     b.object_id = between_node2.b_object_id                   -- BALANACE->STREAM_ITEM.ATTRIBUTE -> BETWEEN_NODE2
--      only include if the given stream_item is between the to and from node given here
--      to find that out use the infrastructure replication in STIM_MTH_VALUE
AND     stim.object_id = smv.object_id
AND     si.object_id = smv.object_id
