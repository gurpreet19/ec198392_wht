CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_SYSTEM_MTH_STATUS_BOOK" ("DAYTIME", "COUNTRY_ID", "COMPANY_ID", "SALE", "PURCHASE", "TA_INCOME", "TA_COST", "QUANTITIES", "INVENTORY", "JOU_ENT", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT", "RECORD_STATUS") AS 
  SELECT DISTINCT
sms.DAYTIME,
sms.COUNTRY_ID,
sms.COMPANY_ID,
(select t.closed_book_date FROM system_mth_status t
WHERE t.daytime = sms.daytime AND t.country_id = sms.country_id AND t.company_id = sms.company_id AND t.booking_area_code = 'SALE'
AND t.closed_book_date <= Ecdp_Timestamp.getCurrentSysdate)
AS SALE,
(select t.closed_book_date FROM system_mth_status t
