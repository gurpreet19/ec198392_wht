CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_PERIOD_TRANSACTION_LEVEL" ("TRANS_TEMP_ID", "PRODUCT_ID", "PRODUCT_CODE", "PRICE_OBJECT_ID", "PRICE_OBJECT_CODE", "DELIVERY_POINT_ID", "DELIVERY_POINT_CODE", "CUSTOMER_ID", "PRICE_CONCEPT_CODE", "QTY_STATUS", "DOC_STATUS", "UOM1_CODE", "UNIQUE_KEY_1", "UNIQUE_KEY_2", "IFAC_TT_CONN_CODE", "PERIOD_START_DATE", "PERIOD_END_DATE", "PROCESSING_PERIOD", "CONTRACT_ID", "CONTRACT_CODE", "DOC_SETUP_CODE", "DOC_SETUP_ID", "TRANS_TEMP_CODE", "PRECEDING_DOC_KEY", "PRECEDING_TRANS_KEY", "IGNORE_IND", "TRANSACTION_KEY", "ALLOC_NO_MAX_IND", "TRANS_KEY_SET_IND", "TRANS_TEMP_ID_OVRD", "TRANS_TEMP_CODE_OVRD", "LINE_ITEM_BASED_TYPE", "REV_TEXT", "REV_NO", "LAST_UPDATED_DATE", "LAST_UPDATED_BY", "CREATED_DATE", "CREATED_BY", "RECORD_STATUS", "SAMPLE_SOURCE_ENTRY_NO", "TYPE") AS 
  SELECT  trans_temp_id
                          ...
                          WHERE
                             ...
                             AND source_entry_no in(SELECT MAX(source_entry_no)
                            FROM ifac_sales_qty
                                 WHERE customer_id = isq.customer_id
                                   ...
                                   AND nvl(trans_temp_id_ovrd,'xxx') = nvl(isq.trans_temp_id_ovrd,'xxx')
