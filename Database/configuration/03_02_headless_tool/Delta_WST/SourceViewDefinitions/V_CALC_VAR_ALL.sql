CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_CALC_VAR_ALL" ("OBJECT_ID", "DAYTIME", "CALC_VAR_SIGNATURE", "NAME", "CALC_CONTEXT_ID", "DIM1_OBJECT_TYPE_CODE", "DIM2_OBJECT_TYPE_CODE", "DIM3_OBJECT_TYPE_CODE", "DIM4_OBJECT_TYPE_CODE", "DIM5_OBJECT_TYPE_CODE", "DESCRIPTION", "SCOPE_NAME", "INHERITED_FROM_ID", "INHERITED_FROM_NAME", "MAPPINGS_IND", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  (
----------------------------------------------------------------------------------------------------
--
...
where NVL(v.LIBRARY_INHERITED_IND,'N') <> 'Y'
UNION ALL
-- Variables inherited from library interface/caller
select v.object_id,
       v.daytime,
       v.calc_var_signature,
       v.name,
       ...
       v.REV_TEXT
from calc_variable_local v
where v.LIBRARY_INHERITED_IND = 'Y'
UNION ALL
