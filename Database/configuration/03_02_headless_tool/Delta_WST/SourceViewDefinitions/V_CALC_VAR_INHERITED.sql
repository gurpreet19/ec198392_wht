CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_CALC_VAR_INHERITED" ("OBJECT_ID", "DAYTIME", "CALC_VAR_SIGNATURE", "NAME", "CALC_CONTEXT_ID", "DIM1_OBJECT_TYPE_CODE", "DIM2_OBJECT_TYPE_CODE", "DIM3_OBJECT_TYPE_CODE", "DIM4_OBJECT_TYPE_CODE", "DIM5_OBJECT_TYPE_CODE", "DESCRIPTION", "SCOPE_NAME", "INHERITED_FROM_ID", "INHERITED_FROM_NAME", "MAPPINGS_IND", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  (
----------------------------------------------------------------------------------------------------
--
-- Purpose  : Lists all variables available to a calculation that are inherited from the context
--            or parent calculations.
--
----------------------------------------------------------------------------------------------------
...
UNION ALL
-- Inherited from library calculation caller
select cv.object_id,
       cv.daytime,
       v.calc_var_signature,
       v.name,
       ...
       'Inherited' as scope_name,
       v.object_id as inherited_from_id,
       cv.name as inherited_from_name,
       NULL as mappings_ind,
       v.RECORD_STATUS,
       v.CREATED_BY,
       v.CREATED_DATE,
...
from calc_variable_local v, calculation_version cv
where v.daytime = cv.daytime
and v.object_id = cv.object_id
and v.LIBRARY_INHERITED_IND = 'Y'
)