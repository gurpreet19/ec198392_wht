CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_IS_SOURCE_TAG_LIST" ("SOURCE_ID", "TAG_ID", "NEXT_READ", "END_TIME", "SOURCE_INTERVAL", "SOURCE_FUNCTION", "SOURCE_DELAY", "SOURCE_DATATYPE", "TIME_ZONE_REGION", "TARGET_TIME_CORRECTION", "TARGET_FUNCTION", "SHIFT_TIME_TO_PERIOD_ST", "FROM_TIME", "TO_TIME") AS 
  SELECT
m.source_id
,m.tag_id
,Nvl(st.next_read,
...
,decode(m.from_time, null, to_date(null), ecdp_date_time.local2utc(m.from_time)) as from_time
,decode(m.to_time, null, to_date(null), ecdp_date_time.local2utc(m.to_time)) as to_time
...
AND   m.source_id = tt.target_sourceid
AND   m.active = 'Y'
AND   m.valid = 'Y'
