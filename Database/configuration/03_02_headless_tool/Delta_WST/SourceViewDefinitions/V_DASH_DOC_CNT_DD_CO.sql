CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_DASH_DOC_CNT_DD_CO" ("SORT_ORDER", "IS_DEFAULT", "PERIOD_CHAR", "PERIOD_DATE", "DOC_COUNT", "STATUS", "INC_BOOKED", "DASH_HEADER", "CO_ID") AS 
  select max(m.sort_order) as sort_order,
       ...
       'Document Validation Count by Document Date for Company '''
       ||max(m.co_name)
       ||'''' as DASH_HEADER,
       m.CO_ID as CO_ID
