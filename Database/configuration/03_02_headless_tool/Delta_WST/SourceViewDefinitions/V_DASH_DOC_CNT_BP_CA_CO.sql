CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_DASH_DOC_CNT_BP_CA_CO" ("SORT_ORDER", "IS_DEFAULT", "PERIOD_CHAR", "PERIOD_DATE", "DOC_COUNT", "STATUS", "INC_BOOKED", "DASH_HEADER", "CO_ID", "CA_ID") AS 
  select max(m.sort_order) as sort_order,
       ...
       'Document Validation Count by Booking Period for Company/Contrac Area '''
       ||max(m.co_name)
       ||''''
       ||'/'''
       ||max(m.ca_name)
  ...
  left join cont_document x
    on (m.daytime = trunc(nvl(x.booking_period,
       ...
       m.CA_ID = ec_contract_area.object_id_by_uk(x.CONTRACT_AREA_CODE))
