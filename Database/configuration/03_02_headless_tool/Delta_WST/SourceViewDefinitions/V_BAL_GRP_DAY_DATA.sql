CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_BAL_GRP_DAY_DATA" ("DAYTIME", "TOTAL_VOL", "NET_ADJ_VOL", "DILUENT_VOL", "SHRINKAGE_VOL", "BITUMEN_VOL", "ALLOC_ENERGY", "NET_ADJ_ALLOC_ENERGY", "NET_MASS", "OBJECT_CODE", "NAME", "OBJECT_ID", "OP_FCTY_CLASS_1_ID", "PRODUCT_ID", "PRODUCT_BALANCE", "STRM_BAL_CATEGORY", "ACTIVITY_CODE", "PRODUCT_BALANCE_GROUP", "BALANCE_GROUP_CODE", "IN_OUT_FLAG", "FROM_NODE_ID", "TO_NODE_ID", "PARENT_OBJECT_ID", "URL", "SCREEN_NAME", "SORT_ORDER", "TRUCK_TICKET_FLAG", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  SELECT sda.daytime daytime,
       sda.net_vol total_vol,
       sda.net_vol net_adj_vol,
       sda.net_diluent_vol diluent_vol,
       sda.net_shrinkage_vol shrinkage_vol,
       sda.net_bitumen_vol bitumen_vol,
       sda.alloc_energy alloc_energy,
       sda.alloc_energy net_adj_alloc_energy,
       sda.net_mass net_mass,
  ...
  LEFT OUTER JOIN (SELECT DISTINCT ssl.object_id,
                    ...
                    WHERE (tv.component_ext_name LIKE b.url || '?%' OR
                      ...
                      AND ssl.balancing_usage LIKE '%PO.0125%') strm_set_list
    ON strm_set_list.object_id = strm_data.object_id
 WHERE sda.object_id = strm_data.object_id
   AND strm_data.daytime <= sda.daytime
   AND (strm_data.end_date > sda.daytime OR strm_data.end_date IS NULL)
UNION ALL
SELECT daytime,
       ...
       rev_no,
       rev_text
  FROM (SELECT TRUNC(ote.daytime, 'DD') daytime -- Get Net Vol for all product and Grs Vol for Water product
              ,
         ...
         GROUP BY trunc(ote.daytime, 'DD'),
                  ...
                  DECODE(ote.object_type,
                         ...
                         ecgp_group.findparentobjectid('operational',
                                                       ...
                                                       ote.production_day))
        UNION ALL
        SELECT TRUNC(ote.daytime, 'DD') daytime -- Get Water Volume for non water products
              ,
         ...
         GROUP BY trunc(ote.daytime, 'DD'),
                  ...
                  DECODE(ote.to_object_type,
                         ...
                         ecgp_group.findparentobjectid('operational',
                                                       ...
                                                       ote.production_day))
        UNION ALL
        SELECT trunc(ote.daytime, 'DD') daytime -- Get Water Vol for Non Water Products
              ,
               SUM(ecbp_truck_ticket.findwatvol(ote.event_no)) total_vol,
               NULL diluent_vol,
               NULL shrinkage_vol,
