CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_DASH_DOC_CNT_BP_BU" ("SORT_ORDER", "IS_DEFAULT", "PERIOD_CHAR", "PERIOD_DATE", "DOC_COUNT", "STATUS", "INC_BOOKED", "DASH_HEADER", "BU_ID") AS 
  select max(m.sort_order) as sort_order,
       ...
       'Document Validation Count by Booking Period for Business Unit '''
       ||max(m.bu_name)
       ||'''' as DASH_HEADER,
       m.BU_ID as BU_ID
  ...
  left join cont_document x
    on (m.daytime = trunc(nvl(x.booking_period,
                               decode(ec_ctrl_system_attribute.attribute_text(x.daytime,
                                      ...
                                      x.document_date)),
                           'MM') and
       m.code = x.document_level_code and
       m.BU_ID = ec_contract_area_version.business_unit_id(ec_contract_area.object_id_by_uk(x.CONTRACT_AREA_CODE),
                                                  x.daytime,
                                                  '<='))
