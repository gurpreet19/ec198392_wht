CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_TRANS_INV_LI_PR_OVER" ("SOURCE", "PRORATE_LINE", "PRORATE_IND", "PROJECT", "OBJECT_ID", "LINE_TAG", "PERIOD", "PRODUCT_GROUP_ID", "PRODUCT_ID", "COST_TYPE", "PROD_SET_ITEM_ID", "SORT_ORDER", "PRICE_COLUMN", "PRODUCT_COLUMN", "COUNTER_PRODUCT_IND", "REBAL_PRODUCT_IND", "SUM_COST_IND", "DAYTIME", "END_DATE", "DESCRIPTION", "SEQ_NO", "EXEC_ORDER", "VAL_EXEC_ORDER", "QUANTITY_SOURCE_METHOD", "VALUE_METHOD", "TRANS_DEF_DIMENSION", "PRICE_INDEX_ID", "VAL_EXTRACT_TAG", "QTY_EXTRACT_TAG", "VAL_EXTRACT_TYPE", "QTY_EXTRACT_TYPE", "EXTR_VAL_NET_ZERO", "EXTRACT_REVRS_VAL", "EXTRACT_REVRS_QTY", "PRORATE_DIM_TO_PROD_ID", "ID", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  SELECT 'PRODUCT_OVERRIDE' SOURCE,
       tilpo.PRORATE_LINE,
...
SELECT 'COST_OVERRIDE' SOURCE,
       ...
       tilo.TAG,
       tilo.period,
       pgs.object_id TRANS_PROD_SET_id,
       pgs.product_id,
       pgc.cost_type,
...
SELECT 'PRODUCT_TEMPLATE' SOURCE,
       tilp.PRORATE_LINE,
     ...
     tilp.EXTRACT_REVRS_QTY,
       ...
       tilp.REV_NO  ,
       tilp.REV_TEXT
 FROM  V_TRANS_INV_LINE_OVERRIDE tilo,
       PRODUCT_GROUP_SETUP pgs,
       contract_attribute ca,
       trans_inv_prod_stream tip,
       trans_inv_li_product tilp,
       trans_inventory_version t
WHERE  t.object_id = tip.inventory_id -- prod stream connected to  non-template
       ...
       AND nvl(t.config_template,t.object_id) = tilp.OBJECT_ID --product is unknown if template or not
       --AND tilp.object_id = tip.inventory_id
       --tilo.object_id = tip.inventory_id
       AND ca.daytime <= tilo.period
       and nvl(ca.end_Date,tilo.period+1) > tilo.period
       AND pgs.daytime <= tilo.period
       and nvl(pgs.end_Date,tilo.period+1) > tilo.period
