CREATE OR REPLACE FORCE EDITIONABLE VIEW "V_FCST_LIQUID_MEMBER_QTY" ("MEMBER_NO", "OBJECT_ID", "OBJECT_START_DATE", "CODE", "FIELD_ID", "FIELD_NAME", "PRODUCT_ID", "UOM", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT", "INV_OPENING_POS_QTY", "PLAN_QTY", "COMM_ADJ_QTY", "SWAP_ADJ_QTY", "AVAIL_SALE_QTY", "SALE_QTY", "INV_MOV_QTY", "INV_CLOSING_POS_QTY", "INV_CLOSING_VALUE") AS 
  (
SELECT fm.member_no,
       ...
       ec_field_version.name(fm.field_id, fc.start_date, '<=') FIELD_NAME,
       ...
       SUM(ecdp_revn_forecast.getCommAdjQtyByMember(fm.member_no, fc.start_date, 'SUM_YEAR_BY_MTH')) COMM_ADJ_QTY,
       ...
       SUM(NVL(ecdp_revn_forecast.getInventoryMovQtyLiqByMember(fm.member_no, fc.start_date, 'SUM_YEAR_BY_MTH'),0)) INV_MOV_QTY,
...
WHERE fv.object_id = fc.object_id
  ...
  GROUP BY fc.OBJECT_ID,
       ...
       fc.object_code,
       fm.field_id,
       ec_field_version.name(fm.field_id, fc.start_date, '<='),
       fm.product_id,
