CREATE OR REPLACE EDITIONABLE TRIGGER "IUD_STOR_FCST_SUB_DAY_LIFT_NOM" 
  FOR INSERT OR UPDATE OR DELETE ON STOR_FCST_SUB_DAY_LIFT_NOM
  COMPOUND TRIGGER

  TYPE REC_STOR_FCST_SUB_DAY_LIFT_NOM IS RECORD(
    parcel_no          STOR_FCST_SUB_DAY_LIFT_NOM.parcel_no%TYPE,
    production_day     STOR_FCST_SUB_DAY_LIFT_NOM.production_day%TYPE,
    object_id          STOR_FCST_SUB_DAY_LIFT_NOM.object_id%TYPE,
    forecast_id        STOR_FCST_SUB_DAY_LIFT_NOM.forecast_id%TYPE
  );

  TYPE T_STOR_FCST_SUB_DAY_LIFT_NOM IS TABLE OF REC_STOR_FCST_SUB_DAY_LIFT_NOM;
  ROW_STOR_FCST_SUB_DAY_LIFT_NOM T_STOR_FCST_SUB_DAY_LIFT_NOM := T_STOR_FCST_SUB_DAY_LIFT_NOM();

  --Executed before each row change- :NEW, :OLD are available
  BEFORE EACH ROW IS
  BEGIN
         -- Original IU trigger for BEFORE INSERT OR UPDATE ON STOR_FCST_SUB_DAY_LIFT_NOM
         IF Inserting THEN
  ...
  AFTER EACH ROW IS
  BEGIN
		  ROW_STOR_FCST_SUB_DAY_LIFT_NOM.EXTEND();
		  ROW_STOR_FCST_SUB_DAY_LIFT_NOM(ROW_STOR_FCST_SUB_DAY_LIFT_NOM.COUNT).parcel_no := nvl(:NEW.PARCEL_NO, :OLD.PARCEL_NO);
		  ROW_STOR_FCST_SUB_DAY_LIFT_NOM(ROW_STOR_FCST_SUB_DAY_LIFT_NOM.COUNT).production_day := nvl(:NEW.PRODUCTION_DAY, :OLD.PRODUCTION_DAY);
		  ROW_STOR_FCST_SUB_DAY_LIFT_NOM(ROW_STOR_FCST_SUB_DAY_LIFT_NOM.COUNT).object_id := nvl(:NEW.OBJECT_ID, :OLD.OBJECT_ID);
		  ROW_STOR_FCST_SUB_DAY_LIFT_NOM(ROW_STOR_FCST_SUB_DAY_LIFT_NOM.COUNT).forecast_id := nvl(:NEW.FORECAST_ID, :OLD.FORECAST_ID);
  END AFTER EACH ROW;

  --Executed after DML statement
  AFTER STATEMENT IS
  BEGIN
   FOR I IN 1 .. ROW_STOR_FCST_SUB_DAY_LIFT_NOM.COUNT() LOOP
       MERGE INTO STOR_FCST_SUBDAY_NOM_SUM o
        USING ( SELECT parcel_no, production_day, object_id, forecast_id,
               ...
               WHERE parcel_no = ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).PARCEL_NO
                 AND production_day = ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).production_day
                 AND object_id = ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).object_id
                 AND forecast_id = ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).forecast_id
               GROUP BY parcel_no, production_day, object_id, forecast_id) f
               ON ( o.parcel_no = f.parcel_no
                   AND o.production_day = f.production_day
                   AND o.object_id = f.object_id
                   AND o.forecast_id = f.forecast_id )
        WHEN MATCHED THEN
         ...
         WHEN NOT MATCHED THEN
              ...
              VALUES
                (ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).PARCEL_NO,
			           ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).object_id,
			           ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).forecast_id,
			           ROW_STOR_FCST_SUB_DAY_LIFT_NOM(I).production_day,
                 f.grs_vol_requested,
   ...
   DELETE FROM STOR_FCST_SUBDAY_NOM_SUM sumtb
    WHERE NOT EXISTS (SELECT 1 FROM STOR_FCST_SUB_DAY_LIFT_NOM nomtb
                       WHERE nomtb.parcel_no = sumtb.parcel_no
                         AND nomtb.object_id  = sumtb.object_id
                         AND nomtb.forecast_id = sumtb.forecast_id
