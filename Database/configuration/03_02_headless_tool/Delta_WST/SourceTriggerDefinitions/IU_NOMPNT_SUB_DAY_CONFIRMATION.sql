CREATE OR REPLACE EDITIONABLE TRIGGER "IU_NOMPNT_SUB_DAY_CONFIRMATION" 
BEFORE INSERT OR UPDATE ON NOMPNT_SUB_DAY_CONFIRMATION
FOR EACH ROW
BEGIN
    -- Common
    -- $Revision: 1.1 $
    ...
    IF Inserting THEN
        ...
        IF :new.CONFIRMATION_SEQ IS NULL THEN
            EcDp_System_Key.assignNextNumber('NOMPNT_SUB_DAY_CONFIRMATION', :new.CONFIRMATION_SEQ);
        END IF;
        ...
        EcDp_Timestamp_Utils.syncUtcDate('CONTRACT', :NEW.contract_id, :NEW.utc_daytime, :NEW.time_zone, :NEW.daytime, :NEW.summer_time);
        EcDp_Timestamp_Utils.setProductionDay('CONTRACT', :NEW.contract_id, :NEW.utc_daytime, :NEW.production_day);
