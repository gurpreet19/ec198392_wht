CREATE OR REPLACE EDITIONABLE TRIGGER "IU_PFLW_PERIOD_STATUS" 
BEFORE INSERT OR UPDATE ON PFLW_PERIOD_STATUS
FOR EACH ROW
BEGIN
    -- Basis
    IF Inserting THEN
      :new.day := trunc(:new.daytime,'DD');

      :new.record_status := nvl(:new.record_status, 'P');
      EcDp_Timestamp_Utils.syncUtcDate(:NEW.object_id, :NEW.utc_daytime, :NEW.daytime, :NEW.summer_time);
      :NEW.day := EcDp_Timestamp.getProductionDayFromLocal(:NEW.object_id, :NEW.daytime);
