CREATE OR REPLACE EDITIONABLE TRIGGER "IUV_DT_TAG_EVENT_STATUS_2" 
 INSTEAD OF INSERT OR UPDATE OR DELETE ON TV_DT_TAG_EVENT_STATUS_2
FOR EACH ROW
...
BEGIN
  ...
  IF n_PK_VAL_2 IS NULL THEN Raise_Application_Error(-20103,'Missing value for PK_VAL_2'); END IF;
   ...
   IF INSERTING THEN
      ...
      INSERT INTO ECKERNEL_EC12SRC.V_TRANS_CONFIG(DATA_CLASS, ATTRIBUTE, PK_VAL_1, PK_ATTR_1, MAPPING_NO, SOURCE_ID, TAG_ID, TEMPLATE_CODE, FROM_UNIT, TO_UNIT, LAST_TRANSFER, ACTIVE, PK_ATTR_2, PK_VAL_2, FROM_TIME, TO_TIME, CREATED_BY, CREATED_DATE, LAST_UPDATED_BY, LAST_UPDATED_DATE, REV_NO, REV_TEXT, RECORD_STATUS)
      VALUES(n_DATA_CLASS, n_ATTRIBUTE, n_PK_VAL_1, n_PK_ATTR_1, n_MAPPING_NO, n_SOURCE_ID, n_TAG_ID, n_TEMPLATE_CODE, n_FROM_UNIT, n_TO_UNIT, n_LAST_TRANSFER, n_ACTIVE, n_PK_ATTR_2, n_PK_VAL_2, n_FROM_TIME, n_TO_TIME, n_CREATED_BY, n_CREATED_DATE, n_LAST_UPDATED_BY, n_LAST_UPDATED_DATE, n_REV_NO, n_REV_TEXT, n_RECORD_STATUS);
   ELSIF UPDATING THEN
      ...
      SET DATA_CLASS = n_DATA_CLASS, ATTRIBUTE = n_ATTRIBUTE, PK_VAL_1 = n_PK_VAL_1, PK_ATTR_1 = n_PK_ATTR_1, MAPPING_NO = n_MAPPING_NO, SOURCE_ID = n_SOURCE_ID, TAG_ID = n_TAG_ID, TEMPLATE_CODE = n_TEMPLATE_CODE, FROM_UNIT = n_FROM_UNIT, TO_UNIT = n_TO_UNIT, LAST_TRANSFER = n_LAST_TRANSFER, ACTIVE = n_ACTIVE, PK_ATTR_2 = n_PK_ATTR_2, PK_VAL_2 = n_PK_VAL_2, FROM_TIME = n_FROM_TIME, TO_TIME = n_TO_TIME, CREATED_BY = n_CREATED_BY, CREATED_DATE = n_CREATED_DATE , LAST_UPDATED_BY = n_LAST_UPDATED_BY,LAST_UPDATED_DATE = n_LAST_UPDATED_DATE ,REV_NO = n_rev_no, REV_TEXT = n_REV_TEXT,RECORD_STATUS = n_RECORD_STATUS
