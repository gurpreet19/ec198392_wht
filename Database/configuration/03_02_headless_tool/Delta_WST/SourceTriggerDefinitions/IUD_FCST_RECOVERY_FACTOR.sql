CREATE OR REPLACE EDITIONABLE TRIGGER "IUD_FCST_RECOVERY_FACTOR" 
...
BEGIN
 ...
 IF INSERTING THEN
  ...
  -- Start Insert relation block ---------------------------------------
    ...
    INSERT INTO  V_FCST_RECOVERY_FACTOR(CLASS_NAME, OBJECT_ID, FORECAST_ID, DAYTIME, COMPONENT_NO, COMPONENT_NAME, FRAC, SORT_ORDER,  CREATED_BY, CREATED_DATE, LAST_UPDATED_BY, LAST_UPDATED_DATE, REV_NO, REV_TEXT, RECORD_STATUS, REC_ID)
  ...
  ELSIF UPDATING THEN 
    ...
    UPDATE  V_FCST_RECOVERY_FACTOR SET OBJECT_ID = n_OBJECT_ID, FORECAST_ID = n_FORECAST_ID, DAYTIME = n_DAYTIME, COMPONENT_NO = n_COMPONENT_ID, COMPONENT_NAME = n_COMPONENT_LABEL, FRAC = n_COMPONENT_VALUE, SORT_ORDER = n_COMPONENT_SORT_ORDER, CREATED_BY = n_CREATED_BY, CREATED_DATE = n_CREATED_DATE , LAST_UPDATED_BY = n_LAST_UPDATED_BY,LAST_UPDATED_DATE = n_LAST_UPDATED_DATE ,REV_NO = n_rev_no, REV_TEXT = n_REV_TEXT, RECORD_STATUS = n_RECORD_STATUS, REC_ID = n_rec_id
    WHERE CLASS_NAME='FCST_RECOVERY_FACTOR' AND OBJECT_ID= o_OBJECT_ID AND FORECAST_ID= o_FORECAST_ID AND DAYTIME= o_DAYTIME AND COMPONENT_NO= o_COMPONENT_ID;

  ELSE -- Deleting 

     DELETE FROM  V_FCST_RECOVERY_FACTOR
     WHERE CLASS_NAME='FCST_RECOVERY_FACTOR' AND OBJECT_ID= o_OBJECT_ID AND FORECAST_ID= o_FORECAST_ID AND DAYTIME= o_DAYTIME AND COMPONENT_NO= o_COMPONENT_ID;
