CREATE OR REPLACE TRIGGER "IUD_TRNL_EVENT_PROD_CP_FAC" 
 INSTEAD OF INSERT OR UPDATE OR DELETE ON DV_TRNL_EVENT_PROD_CP_FAC
 FOR EACH ROW
...
DECLARE
  ...
  n_PRODUCT_ID VARCHAR2(4000) := EcDB_Utils.ConditionNVL(NOT Updating('PRODUCT_ID'),:NEW.PRODUCT_ID,:OLD.PRODUCT_ID);
  o_PRODUCT_ID VARCHAR2(4000) := :OLD.PRODUCT_ID;
...
BEGIN
 ...
 IF INSERTING OR UPDATING THEN 
   ...
   IF n_class_name IS NOT NULL AND rtrim(upper(n_class_name)) <> rtrim(upper('TRNL_EVENT_PROD_CP_FAC')) THEN Raise_Application_Error(-20104,'Given class_name do not correspond to view class name.'); END IF;
 ...
 IF INSERTING THEN
    ...
    IF n_PRODUCT_ID IS NULL THEN Raise_Application_Error(-20103,'Missing value for PRODUCT_ID'); END IF;
  ...
  -- Start Insert relation block ---------------------------------------
    IF ecdp_objects.isValidOwnerReference('TRNL_EVENT_PROD_CP_FAC',n_OBJECT_ID) = 'N'  THEN 
    ...
    INSERT INTO ECKERNEL_WST.V_OBJ_EVENT_DIM1_CP_TRAN_FAC(CLASS_NAME,OBJECT_ID,DIM1_KEY,DAYTIME,COMPONENT_NO,COMPONENT_NAME,FRAC,SORT_ORDER,CREATED_BY,CREATED_DATE,LAST_UPDATED_BY,LAST_UPDATED_DATE,REV_NO,REV_TEXT,RECORD_STATUS,REC_ID)
    VALUES('TRNL_EVENT_PROD_CP_FAC',n_OBJECT_ID,n_PRODUCT_ID,n_DAYTIME,n_COMPONENT_ID,n_COMPONENT_LABEL,n_COMPONENT_VALUE,n_COMPONENT_SORT_ORDER,n_CREATED_BY,n_CREATED_DATE,n_LAST_UPDATED_BY,n_LAST_UPDATED_DATE,n_REV_NO,n_REV_TEXT,n_RECORD_STATUS,n_rec_id) ;
  ...
  ELSIF UPDATING THEN 
    ...
    UPDATE ECKERNEL_WST.V_OBJ_EVENT_DIM1_CP_TRAN_FAC SET OBJECT_ID = n_OBJECT_ID,DIM1_KEY = n_PRODUCT_ID,DAYTIME = n_DAYTIME,COMPONENT_NO = n_COMPONENT_ID,COMPONENT_NAME = n_COMPONENT_LABEL,FRAC = n_COMPONENT_VALUE,SORT_ORDER = n_COMPONENT_SORT_ORDER,CREATED_BY = n_CREATED_BY, CREATED_DATE = n_CREATED_DATE , LAST_UPDATED_BY = n_LAST_UPDATED_BY,LAST_UPDATED_DATE = n_LAST_UPDATED_DATE ,REV_NO = n_rev_no, REV_TEXT = n_REV_TEXT,RECORD_STATUS = n_RECORD_STATUS,REC_ID = n_rec_id
    WHERE OBJECT_ID= o_OBJECT_ID AND DIM1_KEY= o_PRODUCT_ID AND DAYTIME= o_DAYTIME AND COMPONENT_NO= o_COMPONENT_ID;

  ELSE -- Deleting 

     DELETE FROM ECKERNEL_WST.V_OBJ_EVENT_DIM1_CP_TRAN_FAC
     WHERE OBJECT_ID= o_OBJECT_ID AND DIM1_KEY= o_PRODUCT_ID AND DAYTIME= o_DAYTIME AND COMPONENT_NO= o_COMPONENT_ID;
 ...
 --*************************************************************************************
  ...
  END; -- TRIGGER IUD_TRNL_EVENT_PROD_CP_FAC
