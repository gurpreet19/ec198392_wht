CREATE OR REPLACE TRIGGER "IUD_CALC_OBJ_TYPE_IMPL_CLS" 
 INSTEAD OF INSERT OR UPDATE OR DELETE ON DV_CALC_OBJ_TYPE_IMPL_CLS
 FOR EACH ROW
...
DECLARE
  ...
  n_EXCLUDED_IND VARCHAR2(4000) := EcDB_Utils.ConditionNVL(NOT Updating('EXCLUDED_IND'),:NEW.EXCLUDED_IND,:OLD.EXCLUDED_IND);
  o_EXCLUDED_IND VARCHAR2(4000) := :OLD.EXCLUDED_IND;
  ...
  n_CLASS_LABEL VARCHAR2(4000) := EcDB_Utils.ConditionNVL(NOT Updating('CLASS_LABEL'),:NEW.CLASS_LABEL,:OLD.CLASS_LABEL);
  o_CLASS_LABEL VARCHAR2(4000) := :OLD.CLASS_LABEL;
...
BEGIN
 ...
 IF INSERTING THEN
  ...
  -- Start Insert relation block ---------------------------------------
    IF ecdp_objects.isValidOwnerReference('CALC_OBJ_TYPE_IMPL_CLS',n_OBJECT_ID) = 'N'  THEN 
    ...
    INSERT INTO ECKERNEL_WST.V_CALC_OBJ_TYPE_IMPL_CLS(OBJECT_ID,OBJECT_TYPE_CODE,EXCLUDED_IND,CLASS_NAME,CLASS_LABEL,CREATED_BY,CREATED_DATE,LAST_UPDATED_BY,LAST_UPDATED_DATE,REV_NO,REV_TEXT,RECORD_STATUS)
    VALUES(n_OBJECT_ID,n_OBJECT_TYPE_CODE,n_EXCLUDED_IND,n_CLASS_NAME,n_CLASS_LABEL,n_CREATED_BY,n_CREATED_DATE,n_LAST_UPDATED_BY,n_LAST_UPDATED_DATE,n_REV_NO,n_REV_TEXT,n_RECORD_STATUS) ;
  ...
  ELSIF UPDATING THEN 
    ...
    UPDATE ECKERNEL_WST.V_CALC_OBJ_TYPE_IMPL_CLS SET OBJECT_ID = n_OBJECT_ID,OBJECT_TYPE_CODE = n_OBJECT_TYPE_CODE,EXCLUDED_IND = n_EXCLUDED_IND,CLASS_NAME = n_CLASS_NAME,CLASS_LABEL = n_CLASS_LABEL,CREATED_BY = n_CREATED_BY, CREATED_DATE = n_CREATED_DATE , LAST_UPDATED_BY = n_LAST_UPDATED_BY,LAST_UPDATED_DATE = n_LAST_UPDATED_DATE ,REV_NO = n_rev_no, REV_TEXT = n_REV_TEXT,RECORD_STATUS = n_RECORD_STATUS
    WHERE OBJECT_ID= o_OBJECT_ID AND OBJECT_TYPE_CODE= o_OBJECT_TYPE_CODE AND EXCLUDED_IND= o_EXCLUDED_IND AND CLASS_NAME= o_CLASS_NAME AND CLASS_LABEL= o_CLASS_LABEL;

  ELSE -- Deleting 

     DELETE FROM ECKERNEL_WST.V_CALC_OBJ_TYPE_IMPL_CLS
     WHERE OBJECT_ID= o_OBJECT_ID AND OBJECT_TYPE_CODE= o_OBJECT_TYPE_CODE AND EXCLUDED_IND= o_EXCLUDED_IND AND CLASS_NAME= o_CLASS_NAME AND CLASS_LABEL= o_CLASS_LABEL;
 ...
 --*************************************************************************************
  ...
  END; -- TRIGGER IUD_CALC_OBJ_TYPE_IMPL_CLS
