CREATE OR REPLACE TRIGGER "IUV_CARGO_DOCUMENT" 
...
BEGIN
  ...
  IF n_RUN_DATE IS NULL THEN Raise_Application_Error(-20103,'Missing value for RUN_DATE'); END IF;

   IF INSERTING THEN

      INSERT INTO ECKERNEL_WST.V_CARGO_DOCUMENT(REPORT_NO,PARCEL_NO,DOC_CODE,RECEIVER,ORIGINAL,RUN_DATE,UPLOAD,FORMAT_CODE,CREATED_BY,CREATED_DATE,LAST_UPDATED_BY,LAST_UPDATED_DATE,REV_NO,REV_TEXT,RECORD_STATUS)
   ...
   END IF;
      ...
      UPDATE ECKERNEL_WST.V_CARGO_DOCUMENT
      ...
      WHERE REPORT_NO= o_REPORT_NO AND RUN_DATE= o_RUN_DATE AND UPLOAD= o_UPLOAD;

   ELSE -- deleting

     DELETE FROM ECKERNEL_WST.V_CARGO_DOCUMENT
     WHERE REPORT_NO= o_REPORT_NO AND RUN_DATE= o_RUN_DATE AND UPLOAD= o_UPLOAD;
