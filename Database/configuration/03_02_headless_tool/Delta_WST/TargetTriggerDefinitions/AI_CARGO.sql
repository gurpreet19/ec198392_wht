CREATE OR REPLACE TRIGGER "AI_CARGO" 
...
SELECT activity_code, activity_name, sort_order
 ...
 WHERE from_date <= Nvl(:new.start_loading, Ecdp_Date_Time.getCurrentSysdate)
   AND (end_date > Nvl(:new.start_loading, Ecdp_Date_Time.getCurrentSysdate) OR end_date IS NULL);
