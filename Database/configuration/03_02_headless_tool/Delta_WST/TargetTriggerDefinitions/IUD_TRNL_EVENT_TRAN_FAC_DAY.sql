CREATE OR REPLACE TRIGGER "IUD_TRNL_EVENT_TRAN_FAC_DAY" 
 INSTEAD OF INSERT OR UPDATE OR DELETE ON DV_TRNL_EVENT_TRAN_FAC_DAY
 FOR EACH ROW
...
BEGIN
 ...
 IF INSERTING THEN
  ...
  -- Start Insert relation block ---------------------------------------
    ...
    INSERT INTO ECKERNEL_WST.V_OBJ_EVENT_CP_TRAN_FACTOR(DAYTIME,CLASS_NAME,OBJECT_ID,CREATED_BY,CREATED_DATE,LAST_UPDATED_BY,LAST_UPDATED_DATE,REV_NO,REV_TEXT,RECORD_STATUS,REC_ID)
    VALUES(n_DAYTIME,n_CLASS_NAME,n_NOM_LOC_ID,n_CREATED_BY,n_CREATED_DATE,n_LAST_UPDATED_BY,n_LAST_UPDATED_DATE,n_REV_NO,n_REV_TEXT,n_RECORD_STATUS,n_rec_id) ;
  ...
  ELSIF UPDATING THEN 
    ...
    UPDATE ECKERNEL_WST.V_OBJ_EVENT_CP_TRAN_FACTOR SET DAYTIME = n_DAYTIME,CLASS_NAME = n_CLASS_NAME,OBJECT_ID = n_NOM_LOC_ID,CREATED_BY = n_CREATED_BY, CREATED_DATE = n_CREATED_DATE , LAST_UPDATED_BY = n_LAST_UPDATED_BY,LAST_UPDATED_DATE = n_LAST_UPDATED_DATE ,REV_NO = n_rev_no, REV_TEXT = n_REV_TEXT,RECORD_STATUS = n_RECORD_STATUS,REC_ID = n_rec_id
    WHERE DAYTIME= o_DAYTIME AND CLASS_NAME= o_CLASS_NAME;

  ELSE -- Deleting 

     DELETE FROM ECKERNEL_WST.V_OBJ_EVENT_CP_TRAN_FACTOR
     WHERE DAYTIME= o_DAYTIME AND CLASS_NAME= o_CLASS_NAME;
 ...
 --*************************************************************************************
  ...
  END; -- TRIGGER IUD_TRNL_EVENT_TRAN_FAC_DAY
