CREATE OR REPLACE TRIGGER "IUD_FCST_RECOVERY_FACTOR" 
 INSTEAD OF INSERT OR UPDATE OR DELETE ON DV_FCST_RECOVERY_FACTOR
 FOR EACH ROW
...
BEGIN
 ...
 IF INSERTING OR UPDATING THEN 
   ...
   IF n_class_name IS NOT NULL AND rtrim(upper(n_class_name)) <> rtrim(upper('FCST_RECOVERY_FACTOR')) THEN Raise_Application_Error(-20104,'Given class_name do not correspond to view class name.'); END IF;
 ...
 IF INSERTING THEN
  ...
  -- Start Insert relation block ---------------------------------------
    IF ecdp_objects.isValidOwnerReference('FCST_RECOVERY_FACTOR',n_OBJECT_ID) = 'N'  THEN 
    ...
    INSERT INTO ECKERNEL_WST.V_FCST_RECOVERY_FACTOR(CLASS_NAME,OBJECT_ID,FORECAST_ID,DAYTIME,COMPONENT_NO,COMPONENT_NAME,FRAC,SORT_ORDER,CREATED_BY,CREATED_DATE,LAST_UPDATED_BY,LAST_UPDATED_DATE,REV_NO,REV_TEXT,RECORD_STATUS,REC_ID)
    VALUES('FCST_RECOVERY_FACTOR',n_OBJECT_ID,n_FORECAST_ID,n_DAYTIME,n_COMPONENT_ID,n_COMPONENT_LABEL,n_COMPONENT_VALUE,n_COMPONENT_SORT_ORDER,n_CREATED_BY,n_CREATED_DATE,n_LAST_UPDATED_BY,n_LAST_UPDATED_DATE,n_REV_NO,n_REV_TEXT,n_RECORD_STATUS,n_rec_id) ;
  ...
  ELSIF UPDATING THEN 
    ...
    UPDATE ECKERNEL_WST.V_FCST_RECOVERY_FACTOR SET OBJECT_ID = n_OBJECT_ID,FORECAST_ID = n_FORECAST_ID,DAYTIME = n_DAYTIME,COMPONENT_NO = n_COMPONENT_ID,COMPONENT_NAME = n_COMPONENT_LABEL,FRAC = n_COMPONENT_VALUE,SORT_ORDER = n_COMPONENT_SORT_ORDER,CREATED_BY = n_CREATED_BY, CREATED_DATE = n_CREATED_DATE , LAST_UPDATED_BY = n_LAST_UPDATED_BY,LAST_UPDATED_DATE = n_LAST_UPDATED_DATE ,REV_NO = n_rev_no, REV_TEXT = n_REV_TEXT,RECORD_STATUS = n_RECORD_STATUS,REC_ID = n_rec_id
    WHERE OBJECT_ID= o_OBJECT_ID AND FORECAST_ID= o_FORECAST_ID AND DAYTIME= o_DAYTIME AND COMPONENT_NO= o_COMPONENT_ID;

  ELSE -- Deleting 

     DELETE FROM ECKERNEL_WST.V_FCST_RECOVERY_FACTOR
     WHERE OBJECT_ID= o_OBJECT_ID AND FORECAST_ID= o_FORECAST_ID AND DAYTIME= o_DAYTIME AND COMPONENT_NO= o_COMPONENT_ID;
 ...
 --*************************************************************************************
  ...
  END; -- TRIGGER IUD_FCST_RECOVERY_FACTOR
