CREATE OR REPLACE TRIGGER "IUD_V_TRANS_CONFIG" 
...
IF INSERTING THEN
  ld_last_transfer          := Nvl(:New.last_transfer,trunc(sysdate));
  lv2_force_read            := 'Y';
END IF;
...
IF UPDATING THEN
  ...
  IF Nvl(lv2_mapping_no,-999999999) <> Nvl(:Old.mapping_no,-999999999)
  	...
  	OR Nvl(:New.description,'Dummy') <> Nvl(:Old.description,'Dummy') THEN
  ...
  IF :New.last_transfer <> :Old.last_transfer THEN
  	UPDATE trans_mapping
       SET active = active
--     WHERE tag_id = :New.tag_id;
     WHERE mapping_no = :New.mapping_no;
  END IF;
