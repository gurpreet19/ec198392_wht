CREATE OR REPLACE PACKAGE BODY EcDp_Nomination_Cntr_Loc_Bal IS
/******************************************************************************
** Package        :  EcDp_Nomination_Cntr_Loc_Bal, body part
**
** $Revision: 1.4 $
**
...
** ------      	-----       -----------------------------------------------------------------------------------
** 17.04.2013  	meisihil    ECPD-23623: Added additional parameter: p_supplier_nom_only to getTotalPrContract functions
** 19.04.2013	muhammah	ECPD-23451: added p_location_id, p_transfer_service to functions:getTotalReqTransfQtyPrContract, getTotalAccTransfQtyPrContract, getTotalAdjTransfQtyPrContract, getTotalSchTransfQtyPrContract
**************************************************************************************************************/
...
FUNCTION getTotalReqQtyPrContract(
  ...
  p_date                   DATE,
  p_supplier_nom_only VARCHAR2 DEFAULT 'Y'
  )
RETURN NUMBER
--</EC-DOC>
IS
          ...
          CURSOR c_contract_nom_cycle IS
			  ...
			  and nomination_type=p_nom_type and nom_status != 'REJ' and nvl(supplier_nompnt_id, 'NA') = decode(p_supplier_nom_only,'Y',supplier_nompnt_id,'NA')
          ...
          CURSOR c_contract_nom_cycle_oper IS
			  ...
			  and nom_status != 'REJ' and nvl(supplier_nompnt_id, 'NA') = decode(p_supplier_nom_only,'Y',supplier_nompnt_id,'NA')
			  and p_location_id IN (ec_nomination_point.entry_location_id(object_id),ec_nomination_point.exit_location_id(object_id))
			  and ((ec_nomination_point.entry_location_id(object_id) is not null and ec_nomination_point.exit_location_id(object_id) is null) or (ec_nomination_point.exit_location_id(object_id) is not null and ec_nomination_point.entry_location_id(object_id) is null));
...
IS
          ...
          CURSOR c_contract_nom_cycle_oper IS
		      ...
		      FROM nompnt_day_nomination
		      WHERE contract_id = p_contract_id AND nom_cycle_code = p_nom_cycle AND daytime = p_date
			  and nomination_type=p_nom_type and nvl(oper_nom_ind,'N')<>'Y' and nom_status != 'REJ' and nvl(supplier_nompnt_id, 'NA') = decode(p_supplier_nom_only,'Y',supplier_nompnt_id,'NA')
			  and p_location_id IN (ec_nomination_point.entry_location_id(object_id),ec_nomination_point.exit_location_id(object_id))
			  and ((ec_nomination_point.entry_location_id(object_id) is not null and ec_nomination_point.exit_location_id(object_id) is null) or (ec_nomination_point.exit_location_id(object_id) is not null and ec_nomination_point.entry_location_id(object_id) is null));
...
END EcDp_Nomination_Cntr_Loc_Bal;
