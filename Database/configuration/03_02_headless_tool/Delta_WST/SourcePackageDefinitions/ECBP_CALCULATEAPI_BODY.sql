...
** 07.11.2012   rajarsar    ECPD-19864: Modified the function of calcApiTruckTicket, calcApiNotApprovedTruckTicket, getVCFForTruckTicket and getStdDensityForTruckTicket.
**                                       Added 2 new procedures calcNetVol and calcNetVolNotApproved
** 28.05.2013   rajarsar	ECPD-21876: Updated procedures calcNetVol and calcNetVolNotApproved
** 01.12.2013   rajarsar	ECPD-19074: Updated procedures calcApiVal and calcApiNotApprVal to support units correctly.
** 02.04.2014   musthram	ECPD-27129: Updated procedure calcApiNotApprVal to support additional bf_usage for event tank status screen.
** 03.04.2014   dhavaalo  ECPD-27142: ECBP_CalculateApi.CalcApiVal does not consider measurement_event_type for VCF calc
** 17.07.2014   dhavaalo  ECPD-28080: Replace EQPM_RESULT with TEST_DEVICE_RESULT
** 10.10.2014	abdulmaw  ECPD-28941: Updated CalculateApiVal to support correct calculation for strm_sample_analysis
** 19.02.2016   abdulmaw  ECPD-28636: Updated calcApiVal, calcApiNotApprVal, calcApiSinglePWEL to support pressure correction for API
** 19.10.2016   singishi  ECPD-32618: Change the UPDATE statement for EQPM_RESULT table to include both last_update_by and last_update_date for revision info
** 27.03.2017   leongwen  ECPD-41165: Added the user exit function for calcApiVal and calcApiNotApprVal functions.
*****************************************************************/
...
BEGIN
   ...
   IF lv2_class_name = 'TANK' THEN
      ...
      ln_temp_base := ec_ctrl_system_attribute.attribute_value(p_daytime, 'REF_TEMP_BASE', '<=');
      ...
      --pressure to calculate VCF
      lv_press_unit := ecdp_unit.GetUnitFromLogical('PRESS_OBS');
      ln_VCF_press := EcBp_VCF.convertPressAPI(lr_measurement_type.PRESS, lv_press_unit, 'PSIG');
      ...
      IF (lv2_vcf_method = 'API')  AND (substr(lv2_OBS_Dens_method,1,3) = 'API') THEN
         ...
         IF (lv_corr_gravity_method != 'MEASURED' OR lv_corr_gravity_method is null) THEN
            ln_corr_API := ue_VCF_calculation.calcAPIGravityCorr(ln_API_obs, ln_temp, ln_VCF_press, p_daytime);
            IF ln_corr_API IS NULL THEN
              ln_corr_API := Ecbp_Vcf.calcAPIGravityCorr(ln_API_obs, ln_temp, ln_VCF_press, p_daytime);
            END IF;
            ln_VCF := ue_VCF_calculation.calcVCFstdAPI(ln_corr_API, ln_VCF_temp, ln_VCF_press, p_daytime);
            IF ln_VCF IS NULL THEN
              ln_VCF := Ecbp_Vcf.calcVCFstdAPI(ln_corr_API, ln_VCF_temp, ln_VCF_press, p_daytime);
            END IF;
         ...
         ELSE
            ln_VCF := ue_VCF_calculation.calcVCFstdAPI(lr_measurement_type.corr_api, ln_VCF_temp, ln_VCF_press, p_daytime);
            IF ln_VCF IS NULL THEN
              ln_VCF := Ecbp_Vcf.calcVCFstdAPI(lr_measurement_type.corr_api, ln_VCF_temp, ln_VCF_press, p_daytime);
            END IF;
            UPDATE tank_measurement
            SET vcf              = ln_VCF,
                last_updated_by  = p_user
      ...
      ELSIF (lv2_vcf_method = 'API') AND (substr(lv2_OBS_Dens_method,1,3) != 'API') THEN
         IF (lv_corr_density_method = 'CALCULATED' OR lv_corr_density_method is null) THEN
            IF lv_temp_unit = 'C' AND lv_density_unit = 'KGPERM3' THEN
               ln_corr_Dens := Ecbp_Vcf.calcDensityCorr_SI(ln_Dens_obs, ln_temp, ln_VCF_press, p_daytime, ln_temp_base);
                     ln_VCF := Ecbp_Vcf.calcVCFstdDensity_SI(ln_corr_Dens, ln_VCF_temp, ln_VCF_press, p_daytime, ln_temp_base);
            ELSIF lv_temp_unit ='F' THEN
               ln_corr_Dens := Ecbp_Vcf.calcDensityCorr(ln_Dens_obs, ln_temp, ln_VCF_press, p_daytime);
                     ln_VCF := Ecbp_Vcf.calcVCFstdDensity(ln_corr_Dens, ln_VCF_temp, ln_VCF_press, p_daytime);
            END IF;
            UPDATE tank_measurement
	       ...
	       ELSIF (lv_corr_density_method = 'STREAM_SAMPLE_ANALYSIS') THEN
            ln_Dens_obs := ecbp_tank.findStdDens(p_object_id, p_event_type, p_daytime, lv_corr_density_method);
            IF lv_temp_unit = 'C' AND lv_density_unit = 'KGPERM3' THEN
               ln_corr_Dens := Ecbp_Vcf.calcDensityCorr_SI(ln_Dens_obs, ln_temp, ln_VCF_press, p_daytime, ln_temp_base);
		           ln_VCF := Ecbp_Vcf.calcVCFstdDensity_SI(ln_corr_Dens, ln_VCF_temp, ln_VCF_press, p_daytime, ln_temp_base);
            ELSIF lv_temp_unit ='F' THEN
         ...
         ELSE
            IF lv_temp_unit = 'C' AND lv_density_unit = 'KGPERM3' THEN
               ln_VCF := Ecbp_Vcf.calcVCFstdDensity_SI(lr_measurement_type.corr_density, ln_VCF_temp, ln_VCF_press, p_daytime, ln_temp_base);
            ELSIF lv_temp_unit ='F' THEN
               ln_VCF := Ecbp_Vcf.calcVCFstdDensity(lr_measurement_type.corr_density, ln_VCF_temp, ln_VCF_press, p_daytime);
            END IF;
            UPDATE tank_measurement
            SET vcf              = ln_VCF,
                last_updated_by  = p_user
   ...
   ELSIF lv2_class_name = 'STREAM' THEN
      ...
      --pressure to calculate VCF
      lv_press_unit := ecdp_unit.GetUnitFromLogical('PRESS_OBS');
      ln_VCF_press := EcBp_VCF.convertPressAPI(lr_strm_measurement_type.AVG_PRESS, lv_press_unit, 'PSIG');
...
-- this cursor is to select all records with status is not approve
  ...
  b.api, b.corr_api, b.obs_density, b.corr_density, b.PRESS
...
BEGIN
   ...
   FOR cur_rec IN c_calc_not_appr(lv_event_type) LOOP
      ...
      --pressure to calculate VCF
      lv_press_unit := ecdp_unit.GetUnitFromLogical('PRESS_OBS');
      ln_VCF_press := EcBp_VCF.convertPressAPI(cur_rec.PRESS, lv_press_unit, 'PSIG');
      --to get the corrected gravity and density method whether measured or not
      ...
      IF (cur_rec.vcf_method = 'API') AND (substr(cur_rec.obs_dens_method,1,3)='API') THEN
         ...
         END IF;
         IF (lv_corr_gravity_method != 'MEASURED' OR lv_corr_gravity_method is null) THEN
            ln_corr_API := ue_VCF_calculation.calcAPIGravityCorr(cur_rec.api, ln_temp, ln_VCF_press, p_daytime);
            IF ln_corr_API IS NULL THEN
              ln_corr_API := Ecbp_Vcf.calcAPIGravityCorr(cur_rec.api, ln_temp, ln_VCF_press, p_daytime);
            END IF;
            ln_VCF := ue_VCF_calculation.calcVCFstdAPI(ln_corr_API, ln_VCF_temp, ln_VCF_press, p_daytime);
            IF ln_VCF IS NULL THEN
              ln_VCF := Ecbp_Vcf.calcVCFstdAPI(ln_corr_API, ln_VCF_temp, ln_VCF_press, p_daytime);
            ...
            AND measurement_event_type = p_event_type;
         ELSE
            ln_VCF := ue_VCF_calculation.calcVCFstdAPI(cur_rec.corr_api, ln_VCF_temp, ln_VCF_press, p_daytime);
            IF ln_VCF IS NULL THEN
              ln_VCF := Ecbp_Vcf.calcVCFstdAPI(cur_rec.corr_api, ln_VCF_temp, ln_VCF_press, p_daytime);
            END IF;
      ...
      ELSIF (cur_rec.vcf_method = 'API') AND (substr(cur_rec.obs_dens_method,1,3) != 'API') THEN
         IF (lv_corr_density_method = 'CALCULATED' OR lv_corr_density_method is null) THEN
            IF lv_temp_unit = 'C' AND lv_density_unit = 'KGPERM3' THEN
               ln_corr_Dens := Ecbp_Vcf.calcDensityCorr_SI(cur_rec.obs_density, ln_temp, ln_VCF_press, p_daytime, ln_temp_base);
		           ln_VCF := Ecbp_Vcf.calcVCFstdDensity_SI(ln_corr_Dens, ln_VCF_temp, ln_VCF_press, p_daytime, ln_temp_base);
            ELSIF lv_temp_unit ='F' THEN
               ln_corr_Dens := Ecbp_Vcf.calcDensityCorr(cur_rec.obs_density, ln_temp, ln_VCF_press, p_daytime);
		           ln_VCF := Ecbp_Vcf.calcVCFstdDensity(ln_corr_Dens,ln_VCF_temp, ln_VCF_press, p_daytime);
            END IF;
            UPDATE tank_measurement
            SET corr_density     = ln_corr_Dens,
            ...
            AND measurement_event_type = p_event_type;
		     ELSIF (lv_corr_density_method = 'STREAM_SAMPLE_ANALYSIS') THEN
            ln_Dens_obs := ecbp_tank.findStdDens(cur_rec.object_id,p_event_type,p_daytime,lv_corr_density_method);
            IF lv_temp_unit = 'C' AND lv_density_unit = 'KGPERM3' THEN
               ln_corr_Dens := Ecbp_Vcf.calcDensityCorr_SI(ln_Dens_obs, ln_temp, ln_VCF_press, p_daytime, ln_temp_base);
               ln_VCF := Ecbp_Vcf.calcVCFstdDensity_SI(ln_corr_Dens,ln_VCF_temp, ln_VCF_press, p_daytime, ln_temp_base);
            ELSIF lv_temp_unit ='F' THEN
         ...
         ELSE
            IF lv_temp_unit = 'C' AND lv_density_unit = 'KGPERM3' THEN
               ln_VCF := Ecbp_Vcf.calcVCFstdDensity_SI(cur_rec.corr_density, ln_VCF_temp, ln_VCF_press, p_daytime, ln_temp_base);
            ELSIF lv_temp_unit ='F' THEN
               ln_VCF := Ecbp_Vcf.calcVCFstdDensity(cur_rec.corr_density,ln_VCF_temp, ln_VCF_press, p_daytime);
            END IF;
            UPDATE tank_measurement
            SET vcf              = ln_VCF,
                last_updated_by  = p_user
...
IS
  ...
  WHERE a.production_day between p_start_date and p_end_date
...
BEGIN
	 ...
	 ln_API_obs := lr_test_device_result.observed_gravity;
	 ln_run_temp := lr_test_device_result.run_temp;
   --temperature to calculate corrected Gravity and Density
	 ln_obs_temp := lr_test_device_result.observed_temp;
   --get the run temp if exists to calculate the VCF
	 ln_VCF_temp := NVL(ln_run_temp,ln_obs_temp);
	 ln_temp := ln_obs_temp;
   --temperature unit in db
   ...
   --pressure to calculate VCF
   lv_press_unit := ecdp_unit.GetUnitFromLogical('PRESS_OBS');
   ln_VCF_press := EcBp_VCF.convertPressAPI(lr_test_device_result.OBSERVED_PRESS, lv_press_unit, 'PSIG');
   --to convert the temperature to C if in F
	 IF lv_temp_unit = 'C' THEN
	    ln_temp := ecdp_unit.convertValue(ln_obs_temp,'C','F');
	    ln_VCF_temp := ecdp_unit.convertValue(ln_VCF_temp,'C','F');
	 END IF;
	 IF ec_test_device_version.grs_liq_vol_method(p_object_id,p_daytime,'<=') = 'TOTALIZER' or ec_test_device_version.grs_water_vol_method(p_object_id,p_daytime,'<=') =  'TOTALIZER' THEN
	    ln_corr_API := Ecbp_Vcf.calcAPIGravityCorr(ln_API_obs, ln_temp, ln_VCF_press, p_daytime);
	    ln_VCF := Ecbp_Vcf.calcVCFobsAPI(ln_corr_API, ln_VCF_temp, ln_VCF_press, p_daytime);
