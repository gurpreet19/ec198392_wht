...
** 25.07.2013 musthram   ECPD-23999: Updated aggregateSubDaily and aggregateAllSubDaily
** 02.12.2013 musthram   ECPD-26178: Updated aggregateSubDaily to prompt message unable to aggregate when well has been closed long term
...
BEGIN
	...
	IF Ecdp_Well.IsWellNotClosedLT(p_well_object_id, p_daytime) = 'N' THEN
      RAISE_APPLICATION_ERROR(-20661, 'Unable to aggregate this data. The well has been closed long term.');
    END IF;
...
--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- Procedure       : getLastStoredValue
-- Description    : This function gets last stored value for an attribute
--
-- Preconditions  :
-- Postcondition  :
-- Using Tables   :
...
BEGIN
  ...
  v_query_str := 'SELECT '|| p_attribute || ' FROM DV_PWEL_SUB_DAY_STATUS ' ||
    'WHERE object_id = :p_object_id and daytime = (' ||
    'SELECT MAX(daytime) FROM DV_PWEL_SUB_DAY_STATUS WHERE object_id = :p_object_id AND daytime < :p_daytime and ' || p_attribute ||' is not null)';

    EXECUTE IMMEDIATE v_query_str INTO ln_number USING p_object_id, p_object_id, p_daytime;
...
BEGIN
  ...
  FOR cur_r IN  ln_from_date..ln_to_date LOOP
    EcBp_Well_SubDaily.aggregateSubDaily(
      ld_loop_date,
			p_well_object_id,
			p_subdaily_class,
			p_daily_class,
			...
			ld_loop_date := EcDp_ProductionDay.getProductionDayStart('WELL', p_well_object_id, ld_loop_date);
	END LOOP;
...
BEGIN
  ...
  FOR cur_r IN  ln_from_date..ln_to_date LOOP
    EcBp_Well_SubDaily.aggregateAllSubDaily(
      ld_loop_date,
			p_well_object_id,
			p_facility_id,
			p_subdaily_class,
