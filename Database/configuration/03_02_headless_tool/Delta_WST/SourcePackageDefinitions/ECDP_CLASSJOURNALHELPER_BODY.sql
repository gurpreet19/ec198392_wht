...
IS
   ...
   lv2_rev_text_db_syntax   VARCHAR2(32000);
   lv2_journal_if           CLASS_PROPERTY_CNFG.PROPERTY_VALUE%TYPE;
...
begin
   ...
   lv2_journal_if := Ecdp_Classmeta_Cnfg.getJournalRuleDbSyntax(p_class_name);

   IF lv2_journal_if IS NOT NULL THEN

     IF ec_ctrl_system_attribute.attribute_text(TRUNC(Ecdp_Timestamp.getCurrentSysdate),'JOUR_USER_EXCL_OLD_IND','<=') = 'Y' THEN
       lv2_jour_usr_excl_old  := ec_ctrl_system_attribute.attribute_text(TRUNC(Ecdp_Timestamp.getCurrentSysdate),'JOUR_USER_EXCL_OLD','<=');
     END IF;

     IF ec_ctrl_system_attribute.attribute_text(TRUNC(Ecdp_Timestamp.getCurrentSysdate),'JOUR_USER_EXCL_NEW_IND','<=') = 'Y' THEN
       lv2_jour_usr_excl_new  := ec_ctrl_system_attribute.attribute_text(TRUNC(Ecdp_Timestamp.getCurrentSysdate),'JOUR_USER_EXCL_NEW','<=');
     END IF;
     ...
     IF EcDp_ClassMeta.IsRevTextMandatory(p_class_name) = 'Y' THEN
         lv2_journal_section := lv2_journal_section || '   IF UPDATING AND '|| lv2_build_if_rev_text || '   AND NOT UPDATING(''DAYTIME'')';
         lv2_rev_text_db_syntax := EcDp_ClassMeta.getPropertyValueFromClass(p_class_name, 'REV_TEXT_DB_SYNTAX');

         IF lv2_rev_text_db_syntax IS NOT NULL THEN
             lv2_journal_section := lv2_journal_section || ' AND ' || lv2_rev_text_db_syntax;
         END IF;
         lv2_journal_section := lv2_journal_section ||' THEN -- Check rev_text, controlled by class.ensure_rev_text_on_upd and REV_TEXT_DB_SYNTAX' || CHR(10);
...
begin
   ...
   IF lv2_journal_if IS NOT NULL THEN
     ...
     IF EcDp_ClassMeta.IsRevTextMandatory(p_class_name) = 'Y' THEN
         lv2_journal_section := '   IF UPDATING AND '|| lv2_build_if_rev_text || ' AND NOT UPDATING(''DAYTIME'')';
         lv2_rev_text_db_syntax := EcDp_ClassMeta.getPropertyValueFromClass(p_class_name, 'REV_TEXT_DB_SYNTAX');
