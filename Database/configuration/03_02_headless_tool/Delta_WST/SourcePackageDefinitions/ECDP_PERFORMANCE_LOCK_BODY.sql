...
** 15.07.2016 leongwen ECPD-34049: Added the function chkIfOnlyAnlysResetIndUpd and use it at the procedure CheckLockProductionTest
*************************************************************************************************/
...
--                  tables, PTST_RESULT, TEST_DEVICE_RESULT, PWEL_RESULT, PFLW_RESULT. It is the procedure CheckLockProductionTest
...
--
--                  Operations on TEST_DEVICE_RESULT, PWEL_RESULT, PFLW_RESULT
--                  ---------------------------------------------------------
  ...
  IF NOT ( ln_n_result_no IS NULL AND ln_o_result_no IS NULL )  THEN -- ELSE This is a performance test definition with no active result, so no locking needed
    ...
    IF  (ld_n_valid_from IS NOT NULL AND  lv2_n_status = 'ACCEPTED' AND lv2_n_obj_id IS NOT NULL)
      ...
      ELSIF p_operation = 'UPDATING' THEN
        ...
        IF l_new('CLASS_NAME').column_name = 'TEST_RESULT_TREND' THEN
          lb_ifOnlyIndChg := EcDp_Performance_Lock.chkIfOnlyAnlysResetIndUpd(l_new);
          IF lb_ifOnlyIndChg = FALSE THEN -- ECPD-34049 - If lb_ifOnlyIndChg IS TRUE, just proceed and without executing the EcDp_month_lock.validatePeriodForLockOverlap check below.
            EcDp_month_lock.validatePeriodForLockOverlap(p_operation, ld_n_valid_from, ld_n_valid_from, lv2_id||' direct lock detected', lv2_n_obj_id);
      ...
      IF l_new('CLASS_NAME').column_name = 'TEST_RESULT_TREND' THEN
        IF lb_ifOnlyIndChg = FALSE THEN -- ECPD-34049 - If lb_ifOnlyIndChg IS TRUE, just proceed and without executing the DetectDependentlock check below.
...
-- Description    :
--
-- Preconditions  : Checks if only the attributes of 'TREND_ANALYSIS_IND' AND 'TREND_RESET_IND' were updated in the first data-section and no other attributes were modified.
--                  So that the modified attributes above can be saved even when it is in a locked month.
-- Postconditions :
--
-- Using tables   :
--
...
--
---------------------------------------------------------------------------------------------------
FUNCTION chkIfOnlyAnlysResetIndUpd(p_col_list EcDp_month_lock.column_list) RETURN BOOLEAN
--</EC-DOC>
IS
lv2_index VARCHAR2(64);
lb_AnlysNResetINDChanged BOOLEAN := FALSE;
lb_othersChanged BOOLEAN := FALSE;
BEGIN
  IF p_col_list.count > 0 THEN
    ...
    LOOP
      ...
      IF Nvl(p_col_list(lv2_index).is_checked,'N') <> 'Y' AND Nvl(p_col_list(lv2_index).is_changed,'N') = 'Y' THEN
        IF p_col_list(lv2_index).column_name in ('TREND_ANALYSIS_IND', 'TREND_RESET_IND') THEN
         lb_AnlysNResetINDChanged := TRUE;
        ELSE -- Need to check if any additional attributes added at the project besides the EC shipped attributes
          lb_othersChanged := TRUE;
        END IF;
      END IF;
      lv2_index := p_col_list.next(lv2_index);
    END LOOP;
  END IF;
  IF lb_AnlysNResetINDChanged = TRUE AND lb_othersChanged = FALSE THEN
    RETURN TRUE;
  ELSE
    RETURN FALSE;
  END IF;
