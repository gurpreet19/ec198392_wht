...
** 23.04.2013  wonggkai  ECPD-23396: Removed distinct keyword in getWellRBFPhaseFraction()
** 14.05.2014  deshpadi  ECPD-26763: Modified getFirstWellPerfFaultBlock,getFirstWellPerfFormation,getResvBlockPhaseFracFromPfInt,getWellRBFPhaseFraction
                                     getWellCoEntPhaseFraction to access RESB_BLOCK_FORMATION_ID, that has been moved to
                                     PERF_INTERVAL_VERSION table from PERF_INTERVAL table.
** 24.10.2016  xxbrattr  ECPD-36856: Modified getRefQualityStream to also check for well_reference_object.
** 10.03.2017  shindani  ECPD-40494: Modified cursor in getBlockFormationStream() function to return exact stream_id.
** 28.04.2017  jainnraj  ECPD-44610: Modified cursor in function getWellRBFPhaseFraction to include condition on daytime of perf_interval_version
...
--                                                                                               --
-- Behaviour      : This function returns reference quality stream from a block/reservoir        --
--                  formation. As this function returns only stream_id and no other attribute    --
--                  hence version check from strm_version and rbfv_version is not required.        --
--                                                                                               --
---------------------------------------------------------------------------------------------------
FUNCTION getBlockFormationStream (
   p_stream_phase VARCHAR2,
...
FUNCTION getRefQualityStream(
   p_well_id VARCHAR2,
   p_phase   VARCHAR2,
   p_daytime DATE DEFAULT TRUNC(Ecdp_Timestamp.getCurrentSysdate) )
...
SELECT wv.fluid_quality, wv.well_reference_obj_id
...
BEGIN
   ...
   FOR cur_fluid_quality IN c_fluid_quality(p_well_id,p_daytime)  LOOP
      ...
      ELSIF cur_fluid_quality.well_reference_obj_id IS NOT NULL THEN
         lv2_ref_stream  := getRefQualityStream(cur_fluid_quality.well_reference_obj_id, NULL, p_daytime);  --Recursive call
    ELSE
...
SELECT pi.*, f.resv_formation_id, f.resv_block_id
  FROM perf_interval pi, webo_interval wbi, resv_block_formation f, perf_interval_gor pg, perf_interval_version piv
 WHERE pi.webo_interval_id = wbi.object_id
   AND wbi.well_bore_id = p_well_bore_id
   --AND pi.resv_block_formation_id = f.object_id
   AND piv.object_id = pi.object_id
   AND piv.resv_block_formation_id = f.object_id
...
BEGIN
   ...
   IF EcDp_ClassMeta_Cnfg.getLockInd('WELL_BORE_INTERVAL_SPLIT') = 'Y' THEN
...
CURSOR well_rbfCur IS
 ...
 FROM well w, webo_bore wb ,webo_interval wbi,system_days sd, webo_split_factor wbs, webo_interval_gor wbis, resv_block_formation rbf, perf_interval pi, perf_interval_gor pis, perf_interval_version piv
 WHERE wb.well_id = w.object_id
       ...
       AND (piv.end_date is null OR piv.end_date > sd.daytime)
...
CURSOR well_rbfCur IS
 SELECT DISTINCT wb.well_id, rb.object_id resv_block_id, rf.object_id resv_formation_id
  FROM resv_block_formation rbf,
       ...
       webo_bore wb,
       webo_interval wbi,
       perf_interval pi,perf_interval_version piv
...
BEGIN
   ...
   IF EcDp_ClassMeta_Cnfg.getLockInd('PERF_INTERVAL_SPLIT') = 'Y' THEN
