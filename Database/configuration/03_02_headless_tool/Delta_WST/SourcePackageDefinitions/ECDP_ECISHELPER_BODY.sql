...
-- Description    : Returns the number of tag mappings attached to the given template
--
-- Preconditions  :
-- Postconditions :
--
...
-- required       :
--
-- Behaviour      : Scans trans_mapping and finds all mappings with the given template id.
---------------------------------------------------------------------------------------------------
FUNCTION getNumberOfAttachedMappings(p_template_id VARCHAR2)
RETURN NUMBER
--</EC-DOC>
IS
    ln_number_of_mappings NUMBER;

BEGIN
    select count(*) into ln_number_of_mappings from trans_mapping tm where tm.template_no = p_template_id;
    RETURN ln_number_of_mappings;
END;
...
-- Description    : Returns object_code followed by the non-empty values of the PK columns if the first attribute is the object_id
--                  otherwise it returns the non-empty values of the PK columns
--
-- Preconditions  :
-- Postconditions :
--
-- Using tables   :
--
-- Using functions: ecdp_objects.GetObjCode
--
-- Configuration
-- required       :
--
...
--</EC-DOC>
IS
    object_ref VARCHAR2(1000);

BEGIN

    IF LTRIM(RTRIM(UPPER(p_pk_attr_1))) = 'OBJECT_ID' THEN

        object_ref := ecdp_objects.GetObjCode(p_pk_val_1);

    ELSE
        object_ref := p_pk_val_1;

    END IF;

    IF NVL(p_pk_val_2, ' ') != ' ' THEN
        object_ref := object_ref || ',' || p_pk_val_2;
        IF NVL(p_pk_val_3, ' ') != ' ' THEN
            object_ref := object_ref || ',' || p_pk_val_3;
            IF NVL(p_pk_val_4, ' ') != ' ' THEN
                object_ref := object_ref || ',' || p_pk_val_4;
                IF NVL(p_pk_val_5, ' ') != ' ' THEN
                    object_ref := object_ref || ',' || p_pk_val_5;
                    IF NVL(p_pk_val_6, ' ') != ' ' THEN
                        object_ref := object_ref || ',' || p_pk_val_6;
                        IF NVL(p_pk_val_7, ' ') != ' ' THEN
                            object_ref := object_ref || ',' || p_pk_val_7;
                            IF NVL(p_pk_val_8, ' ') != ' ' THEN
                                object_ref := object_ref || ',' || p_pk_val_8;
                                IF NVL(p_pk_val_9, ' ') != ' ' THEN
                                    object_ref := object_ref || ',' || p_pk_val_9;
                                    IF NVL(p_pk_val_10, ' ') != ' ' THEN
                                        object_ref := object_ref || ',' || p_pk_val_10;
                                    END IF;
                                END IF;
                            END IF;
                        END IF;
