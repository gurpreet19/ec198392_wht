...
**
** Created  : 17.01.2000  Carl-Fredrik Soerensen
**
...
** 07.05.2012 musthram ECPD-20125: Updated updateEndDateOnChildObjects and updatDateOnConnectedObjects.
** 26.07.2012 rajarsar ECPD-21517: Updated isWellPhaseActiveStatus and activePhases.
** 27.07.2012 makkkkam ECPD-21486: Updated updateEndDateOnChildObjects
** 25.09.2013 makkkkam ECPD-24225: Updated updatDateOnConnectedObjects and updateStartDateOnChildObjects
...
** 04.05.2014 deshpadi ECPD-26763: Modified function getFieldFromWebo to access RESV_BLOCK_FORM_ID, that has been moved to
...
**                                 modified the functions getPwelOnStreamHrs and getIwelOnStreamHrs with the function call of getDailyWellDownHrs intended for PD.0020 new deferment module,
**                                 modified the function IsDeferred and the procedure chkDeferEventWhenWellClosed to include the check with PD.0020 new deferment module
** 12.01.2015 Sohalran ECPD-27443: EcDp_Well.IsDeferred doesn't include deferment version PD.0006
...
** 05.02.2016 jainnraj ECPD-33314: Calculated On Stream Hours missing from Monthly Production Well Screen
** 29.02.2016 patinpra ECPD-31675: Stream Formulas: When using WELL_HOOKUP object types, TOTAL_THEOR_WELL_* methods return null if swing wells aren't configured
...
** 11.01.2017 jainnraj ECPD-41764: Modified checkClosedWellWithinPeriod to update cursor c_iwel to check for well_type
** 23.03.2017 dhavaalo ECPD-44003: Modified getDailyWellDownHrs.
** 23.06.2017 dhavaalo ECPD-46343: Modified getDailyWellDownHrs.
** 06-11-2017 leongwen ECPD-50437: Modified PROCEDURE chkDeferEventWhenWellClosed and FUNCTION IsDeferred, getDailyWellDownHrs to include the check on class_name for well_deferment table.
** 21.11.2017 dhavaalo ECPD-45043: Remove reference of well_equip_Downtime. Remove un-used variables.
** 18.12.2017 singishi ECPD-51137: Removed all the references of PD.0001.02 and PD.0002.02
...
-- Description    : Returns sleeve UOM for a well associated with well bore interval.
--
-- Preconditions  :
-- Postcondition  :
-- Using Tables   :
--
-- Using functions: EcDp_Well.getWellSleeveUOM
--
-- Configuration
-- required       :
--
-- Behaviour      : Retrieves the Sleeve UOM for well based on well bore interval selected under navigator.
--
---------------------------------------------------------------------------------------------------
FUNCTION getWellSleeveUOM(
    p_object_id well.object_id%TYPE,
...
--</EC-DOC>
 IS
  ...
  lv2_sleeve_uom VARCHAR2(32);

  CURSOR cur_sleeve_uom IS
    SELECT wv.object_id, wv.sleeve_uom sleeve_uom
      FROM webo_interval wi, well_version wv
     WHERE wi.well_id = wv.object_id
       AND wi.object_id = p_object_id
       AND wv.daytime <= p_daytime
       AND nvl(wv.end_date, p_daytime + 1) >= p_daytime
     ORDER BY wv.object_id;
BEGIN

  FOR mycur IN cur_sleeve_uom LOOP

    lv2_sleeve_uom := mycur.sleeve_uom;

    EXIT;

  END LOOP;

  RETURN lv2_sleeve_uom;
...
CURSOR field_cur(cp_wellBoreId VARCHAR2) IS
   ...
   WHERE
	 ...
	 piv.object_id = pi.object_id
     AND piv.resv_block_formation_id = rbf.object_id
...
ln_OffStreamSession    NUMBER := 0;
ln_num_of_hours        NUMBER := 0;
ld_pwel_period_prev_daytime DATE;
ld_def_start_day       DATE := NULL;
ld_def_end_day         DATE := NULL;
ln_dailyWellDownHrs    NUMBER := 0;

BEGIN
  lv2_on_strm_method := Nvl(p_on_strm_method, nvl(ec_well_version.on_strm_method(p_object_id, p_daytime, '<='), 'MEASURED'));

  IF lv2_on_strm_method = 'MEASURED' THEN
    ln_onStreamHrs := ec_pwel_day_status.on_stream_hrs(p_object_id, p_daytime);
  ...
  ELSIF lv2_on_strm_method = 'WELL_ACTIVE_STATUS' THEN
    ...
    ln_num_of_hours := Ecdp_Timestamp.getNumHours('WELL', p_object_id,p_daytime);
    ld_pwel_period_prev_daytime := ec_pwel_period_status.prev_equal_daytime(p_object_id, ld_start_day, 'EVENT');
    ln_onStreamHrs := 0;

    FOR c IN cur_pwelPeriodSample(ld_start_day, ld_end_day, ln_num_of_hours, ld_pwel_period_prev_daytime) LOOP
      ln_onStreamHrs := Nvl(c.uptime,0);
    END LOOP;
  ...
  ELSIF lv2_on_strm_method = 'DEFERMENT_STATUS' THEN
    ...
    IF lv2_def_version = 'PD.0020' THEN
      ln_onStreamHrs := Ecdp_Timestamp.getNumHours('WELL', p_object_id,p_daytime); --assume maximum on stream hours for a day
      ln_dailyWellDownHrs := EcDp_Well.getDailyWellDownHrs(p_object_id,p_daytime);
      ln_onStreamHrs := ln_onStreamHrs - ln_dailyWellDownHrs;
      IF ln_onStreamHrs < 0 THEN
        ln_onStreamHrs := 0;
      END IF;
    ELSIF (lv2_def_version = 'PD.0001' or lv2_def_version = 'PD.0002') THEN
      ln_onStreamHrs := Ecdp_Timestamp.getNumHours('WELL', p_object_id,p_daytime); --assume maximum on stream hours for a day
      ld_pwel_period_prev_daytime := ec_pwel_period_status.prev_equal_daytime(p_object_id, ld_start_day, 'EVENT');
        ...
        FOR c IN cur_objectsDefermentEvent (ld_start_day, ld_end_day, p_object_id, ld_pwel_period_prev_daytime) LOOP
          ...
          ELSE
            ln_OffStreamSession := (LEAST(ld_def_end_day,ld_end_day) - GREATEST(ld_def_start_day,ld_start_day))* Ecdp_Timestamp.getNumHours('WELL', p_object_id,p_daytime);
            ln_onStreamHrs := ln_onStreamHrs - ln_OffStreamSession;
            ld_def_start_day := c.ChangeDate;
            ld_def_end_day   := c.endDay;
            ln_OffStreamSession := 0;
          END IF;
        END LOOP;
  ...
  ELSIF (substr(lv2_on_strm_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ln_onStreamHrs := Ue_Well.getPwelOnStreamHrs(p_object_id, p_daytime);
  END IF;
...
BEGIN
  ...
  IF lv2_on_strm_method = 'MEASURED' THEN
    ln_onStreamHrs := ec_iwel_day_status.on_stream_hrs(p_object_id, p_daytime,p_inj_type);
  ELSIF lv2_on_strm_method = 'WELL_ACTIVE_STATUS' THEN
    ln_prod_day_offset := EcDp_ProductionDay.getProductionDayOffset('WELL',p_object_id, p_daytime)/24;
    ld_start_day := TRUNC(p_daytime) + ln_prod_day_offset;
    ld_end_day := ld_start_day + 1;
  ...
  ELSIF lv2_on_strm_method = 'DEFERMENT_STATUS' THEN
    ...
    IF lv2_def_version = 'PD.0020' THEN
      ln_onStreamHrs := Ecdp_Timestamp.getNumHours('WELL', p_object_id,p_daytime); --assume maximum on stream hours for a day
      ln_dailyWellDownHrs := EcDp_Well.getDailyWellDownHrs(p_object_id,p_daytime,p_inj_type);
      ln_onStreamHrs := ln_onStreamHrs - ln_dailyWellDownHrs;
      IF ln_onStreamHrs < 0 THEN
        ln_onStreamHrs := 0;
      END IF;
    ...
    END IF;
  ELSIF (substr(lv2_on_strm_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN
    ln_onStreamHrs := Ue_Well.getIwelOnStreamHrs(p_object_id, p_inj_type, p_daytime);
  END IF;
  RETURN ln_onStreamHrs;
...
BEGIN
   ...
   ln_num_of_hours := Ecdp_Timestamp.getNumHours('WELL', p_object_id,TRUNC(p_start_daytime,'DD'));
...
BEGIN
     ...
     FOR c IN c_verify_well_bore LOOP
         Raise_Application_Error(-20641,getErrorMsg(c.object_id,c.daytime,'END_DATE',null));
     END LOOP;

     FOR c IN c_verify_well_bore_split LOOP
         Raise_Application_Error(-20642,getErrorMsg(c.well_bore_id,c.daytime,'END_DATE','Well Bore Split'));
     END LOOP;

     FOR c IN c_verify_well_bore_interval LOOP
         Raise_Application_Error(-20643,getErrorMsg(c.object_id,c.daytime,'END_DATE',null));
     END LOOP;

     FOR c IN c_verify_well_bore_int_split LOOP
         Raise_Application_Error(-20644,getErrorMsg(c.object_id,c.daytime,'END_DATE','Well Bore Interval Split'));
     END LOOP;

     FOR c IN c_verify_perf_interval LOOP
         Raise_Application_Error(-20645,getErrorMsg(c.object_id,c.daytime,'END_DATE',null));
     END LOOP;

     FOR c IN c_verify_perf_int_split LOOP
         Raise_Application_Error(-20646,getErrorMsg(c.object_id,c.daytime,'END_DATE','Perforation Interval Split'));
     END LOOP;
...
BEGIN
    ...
    FOR cur_daysmth IN c_getdaysmth(p_daytime) LOOP
      ...
      ELSIF (p_inj_type IS NULL AND EcDp_Well.isWellProducer(p_object_id, cur_daysmth.daytime)='Y') THEN
		    IF lv2_on_strm_method = 'MEASURED' THEN
               ln_sumdaily_onStreamHrs := ec_pwel_mth_status.on_stream_hrs(p_object_id,p_daytime);
            ELSE
...
CURSOR well_pd0020 IS
SELECT 1 FROM deferment_event
WHERE object_id=p_object_id
...
BEGIN
  ...
  ELSIF lv2_def_version = 'PD.0020' THEN
    FOR onerow IN well_pd0020 LOOP
      ln_counter :=1;
      EXIT;
    END LOOP;
  END IF;
...
-- Description    : To check if another side of the well is Open, then follows the following conditions.
--
-- Preconditions  : Upon inserting or updating record in iwel_period_status and pwel_period_status
-- Postconditions :
--
...
begin
   --check for the same well on the same date
   SELECT count(*) INTO ln_cnt
   FROM pwel_period_status
   WHERE object_id = p_object_id
   AND daytime = p_daytime;
   ...
   --row does not exist, insert new
    ...
    IF p_new_status = 'OPEN' THEN
    --insert new row with status SHUT_IN
      lv_sql := 'INSERT INTO pwel_period_status (object_id,daytime,time_span,summer_time,well_status)'||
          'VALUES ('''|| p_object_id ||''',to_date('''||to_char(p_daytime,'yyyy-mm-dd hh24:mi:ss')||''',''yyyy-mm-dd hh24:mi:ss''),''EVENT'',''N'',''SHUT_IN'')';

    ELSIF p_new_status = 'CLOSED' THEN
    --insert new row with the previous status of the same well.
      ...
      lv_sql := 'INSERT INTO pwel_period_status (object_id,daytime,time_span,summer_time,well_status)'||
          'VALUES ('''|| p_object_id ||''',to_date('''||to_char(p_daytime,'yyyy-mm-dd hh24:mi:ss')||''',''yyyy-mm-dd hh24:mi:ss''),''EVENT'',''N'','''|| lv_prev_status ||''')';
    END IF;
   ELSIF ln_cnt = 1 THEN
   --row already exist, update status
    ...
    IF p_new_status = 'OPEN' THEN
    --update status of the other side as SHUT_IN
      lv_sql := 'UPDATE pwel_period_status SET well_status=''SHUT_IN'''||
           'WHERE object_id= '''|| p_object_id ||''''||
           'AND daytime = to_date('''||to_char(p_daytime,'yyyy-mm-dd hh24:mi:ss')||''',''yyyy-mm-dd hh24:mi:ss'')';
    --ELSE DO NOTHING
    END IF;
   ELSE
    RAISE_APPLICATION_ERROR(-20000,'There are more than 1 row for well with the same type and daytime');
   END IF;
...
begin
   ...
   select cd.code2 INTO lv_inj_type
   from ctrl_code_dependency cd
	where cd.code_type2 = 'INJ_TYPE'
   and cd.code_type1 = 'WELL_TYPE'
   and cd.code1 = p_well_type;
...
BEGIN
      SELECT COUNT(*) INTO ln_count FROM pwel_period_status
      WHERE object_id = p_object_id
      AND daytime = p_daytime;

      IF ln_count > 0 THEN
       lv_sql := 'DELETE FROM pwel_period_status '||
                 'WHERE object_id = '''|| p_object_id ||''''||
                 'AND daytime = to_date('''||to_char(p_daytime,'yyyy-mm-dd hh24:mi:ss')||''',''yyyy-mm-dd hh24:mi:ss'')';
...
-- Description    : To check if another side of the well exist for the same date/time, then follows the following conditions.
--
-- Preconditions  : Upon deleting record in pwel_period_status
-- Postconditions :
--
-- Using tables   :iwel_period_status
-- Using functions:
...
select daytime,inj_type,least(nvl(ec_iwel_period_status.next_daytime(object_id, daytime,inj_type,'EVENT'),p_end_date),p_end_date) end_date
          from iwel_period_status
         where object_id = p_object_id
           and daytime >= nvl(ec_iwel_period_status.prev_equal_daytime(p_object_id,p_daytime,ec_well_version.well_type(p_object_id,p_daytime,'<='),'EVENT'),p_daytime)
           and daytime < p_end_date
           and Nvl(active_well_status,'OPEN') <> 'OPEN'
         order by daytime;
...
IS
  ...
  -- To find any opened event in PD.0020 Well Deferment
  CURSOR c_OpenedPD0020(cp_object_id varchar2) IS
  SELECT wd1.daytime
  FROM deferment_event wd1
  WHERE wd1.object_id = cp_object_id
  AND wd1.daytime IS NOT NULL
  AND wd1.end_date IS NULL  -- NULL End date is an Opened Event.
  AND wd1.class_name IN ('WELL_DEFERMENT','WELL_DEFERMENT_CHILD');

  -- To find the start daytime of previous row in PD.0020 Well Deferment
  CURSOR c_getPrevRowPD0020(cp_object_id varchar2, cp_OpenedEventStDate DATE) IS
  SELECT max(wd2.daytime) as maxDaytime
  FROM deferment_event wd2
  WHERE wd2.object_id = cp_object_id
  AND wd2.daytime < cp_OpenedEventStDate
  AND wd2.class_name IN ('WELL_DEFERMENT','WELL_DEFERMENT_CHILD');

  CURSOR c_getPrevRowEndDtPD0020(cp_object_id varchar2, cp_OpenedEventStDate DATE) IS
  SELECT max(wd3.end_date) as maxEnd_date
  FROM deferment_event wd3
  WHERE wd3.object_id = cp_object_id
  AND wd3.daytime = cp_OpenedEventStDate
  AND wd3.class_name IN ('WELL_DEFERMENT','WELL_DEFERMENT_CHILD');

  CURSOR c_getDatesBtwPD0020(cp_object_id varchar2, cp_daytime DATE) IS
  SELECT wd4.daytime, wd4.end_date
  FROM deferment_event wd4
  WHERE wd4.object_id = cp_object_id
  AND cp_daytime BETWEEN wd4.daytime AND (wd4.end_date - 1/86400)
  AND wd4.class_name IN ('WELL_DEFERMENT','WELL_DEFERMENT_CHILD');
...
BEGIN
  ...
  IF lv2_deferment_version LIKE 'PD.0001%' THEN
    NULL;
  ELSIF (lv2_deferment_version = 'PD.0020') THEN
    ...
    IF NVL(lv2_CurrWellStatus,'!@#') <> p_well_status and p_action IN ('INSERT', 'UPDATE') THEN -- Only perform the following if there is a change on Well Status
      ...
      IF lv2_activeWellStatus IN ('CLOSED', 'CLOSED_LT') THEN
        ...
        IF lv2_deferment_version = 'PD.0020' THEN
          FOR cur4a IN c_OpenedPD0020(p_object_id) LOOP
            ld_OpenedEventStDate  := cur4a.daytime;
          END LOOP;
        END IF;

        IF ld_OpenedEventStDate IS NOT NULL THEN
          IF lv2_deferment_version = 'PD.0020' THEN
            FOR cur5a IN c_getPrevRowPD0020(p_object_id, ld_OpenedEventStDate) LOOP
              ld_OpenedEventPrevDate := cur5a.maxDaytime;
            END LOOP;
          END IF;
          IF ld_OpenedEventPrevDate IS NULL THEN
            ...
            IF ld_nextstatusDate IS NOT NULL THEN
              ...
              IF lv2_WellStatus2 NOT IN ('OPEN') THEN
                RAISE_APPLICATION_ERROR(-20662, 'Inserting or updating a closed well status record is invalid as there is a deferment event registered for that period.');
              END IF;
            ELSE
              RAISE_APPLICATION_ERROR(-20662, 'Inserting or updating a closed well status record is invalid as there is a deferment event registered for that period.');
            END IF;
          ELSE
            IF lv2_deferment_version = 'PD.0020' THEN
              FOR cur7a IN c_getPrevRowEndDtPD0020(p_object_id, ld_OpenedEventPrevDate) LOOP
                ld_OpenedEventPrevEndDt := cur7a.maxEnd_date;
              END LOOP;
            END IF;
        ...
        IF lv2_deferment_version = 'PD.0020' THEN
          FOR cur8a IN c_getDatesBtwPD0020(p_object_id, p_daytime) LOOP
            ld_chk_stDate  := cur8a.daytime;
            ld_chk_endDate := cur8a.end_date;
          END LOOP;
        END IF;
    ...
    ELSIF p_action IN ('DELETE') THEN
      ...
      IF lv2_WellStatus3 IN ('CLOSED', 'CLOSED_LT') THEN -- check with the previous row is a closed event.
        IF lv2_deferment_version = 'PD.0020' THEN
          FOR cur10a IN c_OpenedPD0020(p_object_id) LOOP
            ld_OpenedEventStDate  := cur10a.daytime;
          END LOOP;
        END IF;
        IF ld_OpenedEventStDate IS NOT NULL THEN
          RAISE_APPLICATION_ERROR(-20663, 'Deleting an open well status record is invalid as there is a deferment event registered for that period.');
        END IF;
      END IF;
    END IF;
  ELSIF lv2_deferment_version IS NULL THEN
    RAISE_APPLICATION_ERROR(-20664, 'Deferment version was configured with null value in System Attributes screen.');
  END IF;
...
-- Description    : Returns the daily well DOWN deferment event time in hours                      --
-- Preconditions  :                                                                                --
-- Postconditions :                                                                                --
--                                                                                                 --
...
-- required       :                                                                                --
--                                                                                                 --
-- Behaviour      : It checks on 'DOWN' Event_type and the overlapped 'DOWN' event in 24 hrs for   --
--                  daily on strm hrs use.                                                         --
-----------------------------------------------------------------------------------------------------
FUNCTION getDailyWellDownHrs(
      p_object_id VARCHAR2,
      p_daytime   DATE,
      p_inj_type  VARCHAR2 DEFAULT NULL
      )
RETURN NUMBER
--</EC-DOC>
IS
...
BEGIN
  ...
  ELSIF lv2_on_strm_method = 'DEFERMENT_STATUS' THEN
    ...
    ld_end_day := ld_start_day + 1;
        ...
        IF (lv2_def_version = 'PD.0001' or lv2_def_version = 'PD.0002') THEN
           ll_onStreamHrs := Ecdp_Timestamp.getNumHours('WELL', p_object_id,p_daytime); --assume maximum on stream hours for a day
           ld_pwel_period_prev_daytime := ec_pwel_period_status.prev_equal_daytime(p_object_id,
                                                                 ld_start_day,
                                                                 'EVENT');
           ...
           FOR c IN cur_objectsDefermentEvent (ld_start_day, ld_end_day, p_object_id, ld_pwel_period_prev_daytime) LOOP
              ...
              ELSE
                OffStreamSession := (LEAST(ld_def_end_day,ld_end_day) - GREATEST(ld_def_start_day,ld_start_day))* Ecdp_Timestamp.getNumHours('WELL', p_object_id,p_daytime);
...
-- Description    :  This procedure will update the op_fcty_class_1_id of child objects
--
-- Preconditions  :
-- Postconditions :
--
...
PROCEDURE updateopfctyidOnConnObjects(p_op_fcty_class_1_id VARCHAR2, p_daytime DATE, p_object_id VARCHAR2)
IS

BEGIN
     -- Update version table on Perf Interval, with new op_fcty_class_1_id
     UPDATE perf_interval_version piv
        SET piv.op_fcty_class_1_id = p_op_fcty_class_1_id
      WHERE piv.daytime >= p_daytime
        AND piv.object_id IN
            (SELECT object_id FROM perf_interval WHERE well_id IN p_object_id);

END updateopfctyidOnConnObjects;

--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- Function       : wellOnTest                                                                   --
-- Description    : Checks if well is on test.                                                   --
--                  Returns 'Y' if wells on test.                                                --
--                                                                                               --
-- Preconditions  :                                                                              --
-- Postcondition  :                                                                              --
...
IS
       CURSOR c_well IS
       SELECT o.object_id
       FROM ptst_object o, ptst_definition d
       WHERE o.class_name = 'WELL'
       AND o.test_no = d.test_no
       AND p_daytime BETWEEN d.daytime AND NVL(d.end_date,p_daytime+1)
       AND o.object_id = p_well_id;
BEGIN
