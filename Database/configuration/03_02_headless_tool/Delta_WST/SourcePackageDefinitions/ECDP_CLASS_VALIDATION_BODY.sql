...
SELECT dm.class_name, dm.property_name
 ...
 WHERE dm.class_name = p_class_name
   ...
   AND Nvl(dm.viewhidden, 'x') != 'true'
   AND dm.property_name not in (SELECT cr.role_name FROM class_relation_cnfg cr where cr.to_class_name=dm.class_name)
 ORDER BY sort_order;
...
CURSOR c_isExistClassAttrEditable(p_class_name VARCHAR2) IS
  SELECT COUNT(*) cnt
  FROM class_attribute_cnfg ca, class_attr_editable cae
...
CURSOR c_isExistObjectAttrValidation(p_class_name VARCHAR2, p_object_id VARCHAR2) IS
  SELECT COUNT(*) cnt
  FROM class_attribute_cnfg ca, object_attr_validation oav
  WHERE ca.class_name = p_class_name
   ...
   BEGIN
    ...
    FETCH c_attr_editable BULK COLLECT INTO l_editable LIMIT 400;
      ...
      FOR i IN 1..l_editable.COUNT LOOP
        ...
        l_editable(i).rev_text := 'Created by Copy Validation at '||to_char(Ecdp_Timestamp.getCurrentSysdate,'yyyy.mm.dd hh24:mi:ss');
    ...
    FETCH c_attr_validation BULK COLLECT INTO l_validation LIMIT 400;
      ...
      FOR i IN 1..l_validation.COUNT LOOP
        ...
        l_validation(i).rev_text := 'Created by Copy Validation at '||to_char(Ecdp_Timestamp.getCurrentSysdate,'yyyy.mm.dd hh24:mi:ss');
...
---------------------------------------------------------------------------------------------------
FUNCTION getClassAttributeViewLabelHead(
         p_class_name class_attr_property_cnfg.class_name%TYPE,
         p_attribute_name class_attr_property_cnfg.attribute_name%TYPE)
RETURN VARCHAR2
--</EC-DOC>
IS
...
BEGIN
   lv2_viewlabelhead := EcDp_ClassMeta_Cnfg.getStaticViewlabelhead(p_class_name, p_attribute_name);
