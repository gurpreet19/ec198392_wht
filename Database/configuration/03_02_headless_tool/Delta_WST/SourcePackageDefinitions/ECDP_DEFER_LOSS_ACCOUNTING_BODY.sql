    ...
    *****************************************************************/
    ...
    BEGIN
       ...
       IF ecdp_objects.GetObjClassName(p_object_id) IN ('FCTY_CLASS_1','FCTY_CLASS_2') THEN
         ln_num_of_hours     := Ecdp_Timestamp.getNumHours('FACILITY', p_object_id, TRUNC(p_daytime, 'DD'));
       ELSE
         ln_num_of_hours     := Ecdp_Timestamp.getNumHours('', p_object_id, TRUNC(p_daytime, 'DD'));
       END IF;
       RETURN (Nvl(p_end_date, Ecdp_Timestamp.getCurrentSysdate) - p_daytime) * ln_num_of_hours;
    ...
    BEGIN
       -- handle that start date and end date can be empty
        ...
        ld_end   := nvl(nvl(p_end_date, ec_DEFER_LOSS_ACC_EVENT.end_date(p_event_no)),Ecdp_Timestamp.getCurrentSysdate);
    ...
    BEGIN
        ...
        ln_num_of_hours     := Ecdp_Timestamp.getNumHours('STREAM', p_object_id, TRUNC(p_daytime, 'DD'));
        ld_day              := p_daytime +
