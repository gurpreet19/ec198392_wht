...
** 23.05.2013	muhammah	ECPD-23062: add lv_la_id := null; to function:aiuStorPcForecast
...
** 27-12-2016 	asareswi	ECPD-42104: Added exception block and passed correct value to company_id column in aiuStorPcForecast procedure.
** 30-12-2016 	royyypur	ECPD-42284: Modified apportionStorPcDay to retain the already existing plan values
******************************************************************/
...
BEGIN
	...
	lv_plan := nvl(trim(p_plan),'PLAN');

	-- get column name
	IF lv_plan = 'PLAN' THEN
		lv_column := 'GRS_VOL';
	ELSIF lv_plan = 'A' THEN
		lv_column := 'PLAN_A';
	ELSIF lv_plan = 'B' THEN
		lv_column := 'PLAN_B';
	ELSIF lv_plan = 'C' THEN
		lv_column := 'PLAN_C';
	END IF;
	...
	LOOP
        ...
        MERGE INTO stor_day_pc_forecast USING DUAL ON (object_id = p_storage_id AND profit_centre_id = p_pc_id AND daytime = ln_date)
            WHEN MATCHED THEN UPDATE SET
                grs_vol = CASE WHEN lv_plan = 'PLAN' THEN ln_current ELSE grs_vol END,
                grs_vol_plan_a  = CASE WHEN lv_plan = 'A' THEN ln_current ELSE grs_vol_plan_a END,
                grs_vol_plan_b  = CASE WHEN lv_plan = 'B' THEN ln_current ELSE grs_vol_plan_b END,
                grs_vol_plan_c  = CASE WHEN lv_plan = 'C' THEN ln_current ELSE grs_vol_plan_c END,
                last_updated_by = p_user
            WHEN NOT MATCHED THEN INSERT (object_id, profit_centre_id, daytime, grs_vol, grs_vol_plan_a, grs_vol_plan_b, grs_vol_plan_c, created_by)
                VALUES (p_storage_id, p_pc_id, ln_date,
                    CASE WHEN lv_plan = 'PLAN' THEN ln_current END,
                    CASE WHEN lv_plan = 'A' THEN ln_current END,
                    CASE WHEN lv_plan = 'B' THEN ln_current END,
                    CASE WHEN lv_plan = 'C' THEN ln_current END,
                    p_user);

		-- insert/update account forecast
		IF lv_plan = 'PLAN' THEN
			ecdp_storage_forecast.aiuStorPcForecast(p_storage_id, p_pc_id, ln_date, NULL, ln_current, p_user, lv_table, lv_attribute);
		END IF;
...
BEGIN
	...
	IF Nvl(p_old_qty,-1) <> Nvl(p_new_qty,-1) THEN
	          ...
	          LOOP
			        ...
			        EXIT WHEN cv%NOTFOUND;
	          		...
	          		IF lv_la_id IS NOT NULL THEN
                        MERGE INTO lift_acc_day_forecast USING DUAL ON (object_id = lv_la_id AND daytime = p_daytime)
                            WHEN MATCHED THEN UPDATE SET forecast_qty = p_new_qty * (lv_share/100), last_updated_by = p_user
                            WHEN NOT MATCHED THEN INSERT (object_id, daytime, forecast_qty, created_by)
	          		...
	          		--  all profit centre values should be aggregated to the account
						...
						lv_sql := lv_sql || ' FROM stor_day_pc_forecast s, '||lv_from||', equity_share e, prosty_codes c ';
						lv_sql := lv_sql || ' WHERE '||lv_from||'.commercial_entity_id = e.object_id';
						...
						lv_sql := lv_sql || ' AND e.company_id = :cpy_id';
	          			...
	          			EXECUTE IMMEDIATE lv_sql INTO ln_qty USING p_storage_id, 'EQUITY_PHASE', p_storage_id, p_daytime, p_daytime, p_daytime, lv_cpy_id;
		...
		EXCEPTION
			WHEN OTHERS THEN
	            RAISE_APPLICATION_ERROR(-20583,'Company is not mapped with lifting account as per equity share config.');
        END;
	END IF;
