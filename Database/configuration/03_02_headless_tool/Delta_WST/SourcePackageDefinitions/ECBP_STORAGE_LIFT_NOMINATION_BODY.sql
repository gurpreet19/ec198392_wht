CREATE OR REPLACE PACKAGE BODY EcBP_Storage_Lift_Nomination IS
  ...
  **      18.10.2006 rajarsar Tracker 4635 - Updated deleteNomination Procedure with user exit function
          ...
          12.08.2010 lauuufus ECPD-12065 Add new procedure updateStorageLifting() and insertCargo()
  ...
  **	  31.03.2015 asareswi ECPD-19757: Update the function CalcNomSplitQty : Call Ue_Storage_Lift_Nomination.CalcSplitQty function instead of calling local
  **									  		CalcSplitQty function directly.
  **	  31.03.2015 asareswi ECPD-19757: Update the function CalcActualSplitQty : Call Ue_Storage_Lift_Nomination.CalcSplitQty function instead of calling local
  **									  		CalcSplitQty function directly.
  **	  31.03.2015 asareswi ECPD-19757: Update the function CalcAggrSubDaySplitQty : Call Ue_Storage_Lift_Nomination.CalcSplitQty function instead of calling
  **									  		local CalcSplitQty function directly.
  **	  15.04.2015 muhammah ECPD-29411: Updated PROCEDURE bdStorageLiftNomination to remove EcBp_Cargo_Transport.cleanLonesomeCargoes
  ...
  **      10.02.2017 farhaann ECPD-42403: Updated procedure bdStorageLiftNomination to include delete from STORAGE_LIFT_NOM_SPLIT table
  **      15.02.2017 farhaann ECPD-42403: Updated procedure deleteNomination to include delete from STORAGE_LIFT_NOM_SPLIT table
  **      30.03.2017 farhaann ECPD-41890: Modified getDefSplit
  **      30.05.2017 thotesan ECPD-44413: Modified bdStorageLiftNomination for restricting nominations with cargo status R/C/A
  ...
  ********************************************************************/
    ...
    CURSOR c_product_meas_setup(cp_storage_id VARCHAR2, cp_xtra_qty NUMBER, cp_lifting_event VARCHAR2) IS
      SELECT distinct (e.unit)
       ...
       WHERE b.object_id = cp_storage_id
         AND ec_stor_version.product_id(b.object_id, Ecdp_Timestamp.getCurrentSysdate, '<=') =
             c.object_id
         AND -- obs Ecdp_Timestamp.getCurrentSysdate(but better than object start date)
    ...
    CURSOR c_fcty_storage(cp_storage_id VARCHAR2, cp_xtra_qty NUMBER) IS
      SELECT distinct (e.unit)
        FROM storage                  b,
             ...
             LIFTING_MEASUREMENT_ITEM e,
             stor_version             g
       WHERE ec_stor_version.product_id(b.object_id, Ecdp_Timestamp.getCurrentSysdate, '<=') =
             c.object_id
         AND -- obs Ecdp_Timestamp.getCurrentSysdate(but better than object start date)
             c.object_id = d.object_id
         AND d.item_code = e.item_code
  ...
  -- Postconditions : Uncommited changes
  --
  -- Using tables   : storage_lifting, lift_doc_instruction, lifting_doc_receiver, lifting_doc_set, storage_lift_nom_split
  -- Using functions:
  --
  -- Configuration
  -- required       :
  ...
  --</EC-DOC>
   ...
   CURSOR c_cargo_status(cp_parcel_no VARCHAR2) IS
         SELECT ct.cargo_status
          FROM STORAGE_LIFT_NOMINATION N,
               CARGO_TRANSPORT         CT,
     ...
     lv_cargo_status VARCHAR2(1);

  BEGIN

   	FOR curstatus IN c_cargo_status(p_parcel_no) LOOP
      lv_cargo_status := curstatus.cargo_status;
    END LOOP;

    IF NVL(lv_cargo_status,'UNKNOWN') NOT IN ('R', 'C', 'A') THEN
		-- delete parcel data
		...
		delete storage_lift_nom_split where parcel_no = p_parcel_no;
    ELSE
      Raise_Application_Error(-20586,'Deletes are not allowed when cargo status is Ready for harbour/Closed/Approved.');
    END IF;
  ...
  -- Postconditions :
  --
  -- Using tables   : storage_lift_nomination, storage_lift_nom_split
  ...
  --</EC-DOC>
   ...
   CURSOR c_parcel(cp_storage_id VARCHAR2, cp_from_date DATE, cp_to_date DATE) IS
    SELECT parcel_no from storage_lift_nomination
     WHERE (fixed_ind is null or fixed_ind = 'N')
       AND cargo_no is NULL
       AND nom_firm_date >= cp_from_date
       AND nom_firm_date < cp_to_date
       AND object_id = Nvl(cp_storage_id, object_id);

    CURSOR c_parcel_cargo(cp_storage_id VARCHAR2, cp_from_date DATE, cp_to_date DATE) IS
    SELECT parcel_no from storage_lift_nomination
     WHERE parcel_no IN
           (SELECT n.parcel_no
             ...
             WHERE (fixed_ind is null or fixed_ind = 'N')
               ...
               AND EcBp_Cargo_Status.getEcCargoStatus(c.cargo_status) in
                   ('T', 'O')
               AND n.nom_firm_date >= cp_from_date
               AND n.nom_firm_date < cp_to_date
  ...
  BEGIN
    ...
    FOR curParcel IN c_parcel(p_storage_id, p_from_date, p_to_date) LOOP
        DELETE storage_lift_nom_split where parcel_no = curParcel.parcel_no;
    END LOOP;
    ...
    FOR curParcelCargo IN c_parcel_cargo(p_storage_id, p_from_date, p_to_date) LOOP
        DELETE storage_lift_nom_split where parcel_no = curParcelCargo.parcel_no;
    END LOOP;
  ...
  -- Description    : Returns default split depending on contract and date
  --
  -- Preconditions  :
  -- Postconditions :
  --
  -- Using tables   : cntr_lift_acc_share, storage_lift_nomination, lift_account_version
  -- Using functions:
  --
  -- Configuration
  -- required       :
  ...
  FUNCTION getDefSplit(p_parcel_no NUMBER, p_lifting_account_id VARCHAR2)
    ...
    cursor c_get_account_share(c_parcel_no          NUMBER,
                          c_lifting_account_id VARCHAR2) is
      select clas.account_share
        from cntr_lift_acc_share clas
       where clas.lifting_account_id = c_lifting_account_id
      and exists
       (select 1
                from storage_lift_nomination sln, lift_account_version lav
               where clas.object_id = lav.contract_id
                 ...
                 and sln.nom_firm_date <
                     COALESCE(clas.end_date, sln.nom_firm_date+1))
 ...
 --<EC-DOC>
  ...
  BEGIN
	...
	ln_split_qty := Ue_Storage_Lift_Nomination.CalcSplitQty(p_parcel_no, p_company_id, p_lifting_account_id, ld_date, ln_grs_vol_nominated);
 ...
 --<EC-DOC>
  ...
  BEGIN
	...
	ld_date 		:= nvl(ec_storage_lift_nomination.bl_date(p_Parcel_No),	ec_storage_lift_nomination.nom_firm_date(p_parcel_no));
	ln_lifted_qty 	:= getLiftedVol(p_parcel_no, p_xtra_qty);
	ln_split_qty 	:= Ue_Storage_Lift_Nomination.CalcSplitQty(p_parcel_no, p_company_id, p_lifting_account_id, ld_date, ln_lifted_qty);
 ...
 --<EC-DOC>
  ...
  BEGIN
	...
	ln_split_qty 	:= Ue_Storage_Lift_Nomination.CalcSplitQty(p_parcel_no, p_company_id, p_lifting_account_id, p_daytime, ln_lifted_qty);
	...
	ln_result NUMBER;
   	...
   	IS
   		SELECT object_id,
   		       ...
   		       DECODE(cp_xtra_qty, 0, n.grs_vol_schedule, 1, n.grs_vol_scheduled2, 2, n.grs_vol_scheduled3) grs_vol_scheduled,
   		       DECODE(cp_xtra_qty, 0, n.lifted_qty, 1, n.lifted_qty2, 2, n.lifted_qty3) lifted_qty,
