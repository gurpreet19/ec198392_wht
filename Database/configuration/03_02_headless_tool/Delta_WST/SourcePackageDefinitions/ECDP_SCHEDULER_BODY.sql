...
IS
	ret VARCHAR2(100);
	lv_sname VARCHAR2(100) := sNAME;
	i NUMBER;
	CURSOR c_col_val(p_sname VARCHAR2) IS
...
BEGIN
	...
	i := Instr(sNAME, '--!##RERUN##!');
	IF i > 0 THEN
		lv_sname := substr(sNAME, 1,i-1);
	END IF;
	FOR cur_row IN c_col_val(lv_sname) LOOP
...
BEGIN
   ...
   FROM action_instance ai, schedule s
...
--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- Function       : SchedulesCleanup
-- Description    : Limit the size of table schedule_history
--
-- Preconditions  :
-- Postconditions :
--
...
IS
	...
	IS
		SELECT schedule_no
		FROM schedule
		WHERE name like '%.AUTOGEN'
		AND created_date <= (Ecdp_Timestamp.getCurrentSysdate - p_retain_days);

BEGIN
	FOR cur_job IN c_auto_jobs LOOP
		DELETE
		FROM action_instance_history
		WHERE action_instance_no in (
		SELECT action_instance_no
		FROM action_instance
		WHERE schedule_no = cur_job.schedule_no);
