...
** 03-12-12   kumarsur   ECPD-22570 - Generate missing rows in subdaily injection screen is to slow. Modified populate() function.
** 22-07-13   musthram   ECPD-23999 - Modified checkAccessLevel
** 20-08-13   rsmy       Merged P07 with override for ECPD-16797. Still waiting for ECSR-7626 => ECPD-22739 => ECPD-16797.
** 12-01-16   kashisag   ECPD-32774 EcDp_SubDaily_Utilities.generateMissingRows does not work for DST switch at 00:00
** 31-08-16   dhavaalo   ECPD-38607: Remove usage of EcDp_Utilities.executeSinglerowString
** 04-04-17   singishi   ECPD-41316: Corrected generateMissingRecords Cursor to generate records when column type is function.
**************************************************************/
...
BEGIN
   -- Add one hour to day start if it does not exist. (DST shift at 00:00)
  ld_prod_day_start := Nvl(Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day+p_offset/24),
                           Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day+(p_offset+1/24)));

  ld_day_start_next := Nvl(Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day+1+p_next_offset/24),
                           Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day+1+(p_next_offset+1)/24));
  ...
  lv2_sql := 'SELECT ' || joinList(p_colnames) || ', Ecdp_Timestamp.local2utc(NULL, :p_object_id, daytime) as daytime, production_day '
...
BEGIN
  ld_start := Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day + p_offset/24);
  ld_end := Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day + 1 + p_offset/24);
...
BEGIN
  ...
  ld_day_start := nvl(Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day+p_offset/24), Ecdp_Timestamp.local2utc(NULL, p_object_id, p_day+(p_offset+1)/24));

  IF lv2_insert IS NOT NULL THEN

    ld_current_date := Ecdp_Timestamp.getCurrentSysdate;
  ...
  FOR one IN 1..p_status.count LOOP
    ...
    IF c_ref IS NOT NULL THEN
      ...
      DBMS_SQL.bind_variable(c_ref, ':p_daytime', Ecdp_Timestamp.utc2local(NULL, p_object_id, ld_daytime));
...
IS
  CURSOR c_number_columns (p_class_name IN VARCHAR2) IS
  with class_attr_property_max as(
  select p1.class_name, p1.attribute_name, p1.property_code, p1.property_value
  from class_attr_property_cnfg p1
  where p1.property_code = 'DISABLED_IND'
  and p1.owner_cntx = (
    ...
    SELECT c.class_name, c.class_type, a.attribute_name
      FROM class_attribute_cnfg a
      INNER JOIN class_cnfg c ON c.class_name = a.class_name
      INNER JOIN class_attr_property_max m on m.class_name = a.class_name and m.attribute_name = a.attribute_name
      WHERE a.class_name = p_class_name
        AND a.data_type = 'NUMBER'
        AND nvl(m.property_value,'N') = 'N'
      ORDER BY EcDp_ClassMeta_Cnfg.getScreenSortOrder(a.class_name, a.attribute_name);
...
BEGIN
  ...
  FOR one IN c_number_columns( p_subday_class) LOOP
    IF EcDp_ClassMeta_Cnfg.isReportOnly(one.class_name, one.attribute_name)='N' AND
       EcDp_ClassMeta_Cnfg.getStaticViewhidden(one.class_name, one.attribute_name)!='true' THEN
      IF lv_view_name IS NULL THEN
        lv_view_name := EcDp_ClassMeta_Cnfg.getClassViewName(one.class_name, one.class_type);
      END IF;
      ll_colnames(ln_count) := one.attribute_name;
      ln_count := ln_count + 1;
    END IF;
  END LOOP;
...
BEGIN
   ...
   lv_record_status := executeSingleRowString(lv2_sql);
...
-- Description    : Checks Access Level for Logged in User
--
--
-- Preconditions  :
--
...
BEGIN
   ...
   IF lv_access > 0 THEN
       ...
       RETURN lv_access;
   ELSE
      RAISE_APPLICATION_ERROR(-20235,'You do not have access to perform this operation');
