...
** -------  ----------  ------- --------------------------------------
** 1.3      30.04.2014  xxnarlo Added new session variables l_new_bcbf_list / l_old_bcbf_list
**                              Updated AddBCList to use the new variables.
***************************************************************************/
...
BEGIN
  ...
  IF l_new_bc_list IS NULL THEN
    ...
    l_new_bcbf_list  :=  t_bf_list();
    l_old_bcbf_list  :=  t_bf_list();
    l_new_bc_list  :=  t_bc_list();
    l_old_bc_list  :=  t_bc_list();

  END IF;

  l_new_bcbf_list.EXTEND;
  l_old_bcbf_list.EXTEND;
  l_new_bc_list.EXTEND;
  l_old_bc_list.EXTEND;

  l_new_bcbf_list(l_new_bcbf_list.LAST) := p_new_bf_code;
  l_old_bcbf_list(l_old_bcbf_list.LAST) := p_old_bf_code;
...
--<EC-DOC>
---------------------------------------------------------------------------------------------------
-- function       : updateBFList
-- Description    : Updates the functions in the temporary table
--
-- Preconditions  :
-- Postcondition  :
--
...
--
---------------------------------------------------------------------------------------------------
PROCEDURE updateBFList
--</EC-DOC>
IS

BEGIN
  IF l_new_bf_list IS NOT NULL THEN

     -- Some BF codes has chaneged , need to update children for this

     FOR i in 1..l_new_bf_list.count LOOP

        UPDATE BF_COMPONENT SET BF_CODE = l_new_bf_list(i) WHERE BF_CODE = l_old_bf_list(i);
        UPDATE CNTX_MENU_ITEM SET BF_CODE =  l_new_bf_list(i) WHERE BF_CODE = l_old_bf_list(i);
        UPDATE CNTX_MENU_ITEM_PARAM SET BF_CODE = l_new_bf_list(i) WHERE BF_CODE = l_old_bf_list(i);
        UPDATE BF_PROFILE_SETUP SET BF_CODE = l_new_bf_list(i) WHERE BF_CODE = l_old_bf_list(i);
    END LOOP;

    l_new_bf_list.delete;
    l_old_bf_list.delete;
...
-- Description    : Updates the components based on the temporary table
--
-- Preconditions  :
-- Postcondition  :
--
...
--
---------------------------------------------------------------------------------------------------
PROCEDURE updateBCList
--</EC-DOC>
IS

BEGIN
  IF l_new_bc_list IS NOT NULL THEN
   -- Syncronization under will update CNTX_MENU_ITEM and CNTX_MENU_ITEM_PARAM for rows refering to
   -- BF Components that has changed

   -- Loop all rows that we need to do syncronisation for

   FOR i in 1..l_new_bc_list.count LOOP

        UPDATE CNTX_MENU_ITEM
        SET BF_CODE =  l_new_bcbf_list(i),
            COMP_CODE = l_new_bc_list(i)
        WHERE BF_CODE = l_old_bcbf_list(i)
        AND   COMP_CODE = l_old_bc_list(i);

        UPDATE CNTX_MENU_ITEM_PARAM
        SET BF_CODE =  l_new_bcbf_list(i),
            COMP_CODE = l_new_bc_list(i)
        WHERE BF_CODE = l_old_bcbf_list(i)
        AND   COMP_CODE = l_old_bc_list(i);
   ...
   END LOOP;
    ...
    l_new_bcbf_list.delete;
    l_old_bcbf_list.delete;
    l_new_bc_list.delete;
    l_old_bc_list.delete;
  END IF;
END;
