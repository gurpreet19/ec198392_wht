...
--cursor for looping through account_template_list
CURSOR c_account_template_list(cp_tmpl_code VARCHAR2) IS
SELECT
  atl.code,
  atl.account_code,
  atl.sort_order
  FROM account_template_list atl
WHERE
  atl.code = cp_tmpl_code;

--cursor get account_list
CURSOR c_account_list (cp_acc_code VARCHAR2) IS
SELECT
  ac.account_code,
  ac.account_name,
  ac.product_id,
  ac.price_concept_code,
  ac.sort_order,
  ac.description
  FROM account_list ac
  WHERE ac.account_code = cp_acc_code;

--cursor to check for existing contract_account
CURSOR c_contract_account (cp_contract_id VARCHAR2, cp_acc_code VARCHAR2)IS

  SELECT 1
  FROM contract_account ca
  WHERE ca.account_code = cp_acc_code
  AND ca.object_id = cp_contract_id;
...
BEGIN
	...
	FOR curAccTemplList IN c_account_template_list(p_tmpl_code) LOOP
		--reset the lb_exist switch
		lb_exist := FALSE;

		FOR  curCntrAcct IN c_contract_account (p_contract_id, curAccTemplList.account_code) LOOP
			lb_exist := TRUE;
             EXIT;
          END LOOP;

	IF NOT lb_exist THEN
		FOR curAcctList IN c_account_list (curAccTemplList.account_code) LOOP
			INSERT INTO contract_account(object_id, account_code, name, daytime, price_concept_code, product_id, sort_order, description, created_by)
			VALUES (p_contract_id, curAcctList.account_code, curAcctList.account_name, p_daytime, curAcctList.price_concept_code, curAcctList.product_id, curAcctList.sort_order, curAcctList.description, ecdp_context.getAppUser);
		END LOOP;
	END IF;
END LOOP;
