...
** ------      -----       -----------------------------------------------------------------------------------
** 27.09.2013  leeeewei		ECPD-19504: Added getSumFracCompRecFac, getSumFracProductCompRecFac and getFcstSumFracCompRecFac
** 21.08.2014  leeeewei		ECPD-28347: Modified getFcstSumFracCompRecFac to either sum by either nomination location or both nomination location and product
**************************************************************************************************************/
...
FUNCTION getFcstSumFracCompRecFac(p_class_name VARCHAR2,
								  ...
								  p_dim1_key VARCHAR2 DEFAULT NULL)
--</EC-DOC>
 RETURN NUMBER IS
   ...
   CURSOR c_sum_dim1 IS
    SELECT SUM(f.frac) sumfrac
     ...
     WHERE f.class_name = p_class_name
       ...
       AND f.daytime = p_daytime
       AND f.forecast_id = p_forecast_id
       AND f.dim1_key = p_dim1_key;

BEGIN

  IF p_dim1_key is not null THEN
    FOR cur_sum_dim1 IN c_sum_dim1 LOOP
      ln_sum := cur_sum_dim1.sumfrac;
    END LOOP;
  ELSE
    FOR cur_sum IN c_sum LOOP
      ln_sum := cur_sum.sumfrac;
