...
**  18-May-2015  deshpadi Modified lockedMonthAffected(), as it can also check the 1st date of the locked month.
**  10-July-2017 dhavaalo  ECPD-45211-Function overloading support added for localLockedMonthAffected
****************************************************************/
...
--
-- Preconditions  : Checks whether the period provided by the input parameters affects a local locked month.
--                  Above function is used in Production Allocation screen to check whether allocation network is locked locally.
-- Postconditions :
--
-- Using tables   : operational_lock
--
...
--
---------------------------------------------------------------------------------------------------
FUNCTION localLockedMonthAffected(p_alloc_network_id VARCHAR2, p_from_daytime DATE, p_to_daytime DATE) RETURN DATE
--</EC-DOC>
IS

   CURSOR c_local_locked(cp_element_id_class VARCHAR2, cp_element_id VARCHAR2) IS
   SELECT daytime
   FROM operational_lock
   WHERE object_class = cp_element_id_class
   AND object_id = cp_element_id
   AND lock_ind = 'Y'
   AND p_from_daytime < LAST_DAY(daytime) + 1 AND NVL(p_to_daytime, daytime + 1) >= daytime
   ORDER BY daytime;

   ld_lock DATE := NULL;

   CURSOR c_alloc_network_object IS
   SELECT element_id,ecdp_objects.GetObjClassName(element_id) element_id_class
   FROM CALC_COLLECTION_ELEMENT
   WHERE object_id=p_alloc_network_id;

BEGIN

FOR cur_alloc_network IN c_alloc_network_object LOOP

   FOR cur_rec IN c_local_locked(cur_alloc_network.element_id_class, cur_alloc_network.element_id) LOOP
