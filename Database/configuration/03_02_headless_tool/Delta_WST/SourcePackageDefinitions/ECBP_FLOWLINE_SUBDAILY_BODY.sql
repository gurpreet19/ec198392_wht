...
** 11.12.2012    musthram         ECPD-22552 Modified aggregateSubDaily and aggregateAllSubDaily
** 25.07.2013    musthram         ECPD-23999: Updated aggregateAllSubDaily
** 25.07.2013    musthram         ECPD-25927: Updated aggregateSubDaily and aggregateAllSubDaily to include flowline_type = GP
*********************************************************************************************************************************/
...
BEGIN
   ...
   IF lv_record_status = 'A' THEN
     -- must have access to modify 'A'
      ...
      ELSE
         RAISE_APPLICATION_ERROR(-20235,
                              'You do not have enough access to run aggregate current for Approved record.');
      END IF;
    ...
    ELSIF lv_record_status = 'V' THEN
      ...
      ELSE
         RAISE_APPLICATION_ERROR(-20235,
                              'You do not have enough access to run aggregate current for Verified record.');
      END IF;
     ...
     ELSIF (lv_record_status = 'P') OR (lv_record_status IS NULL) THEN
      ...
      ELSE
         RAISE_APPLICATION_ERROR(-20235,
                              'You do not have enough access to run aggregate current for Provisional record.');
      END IF;
   END IF;
