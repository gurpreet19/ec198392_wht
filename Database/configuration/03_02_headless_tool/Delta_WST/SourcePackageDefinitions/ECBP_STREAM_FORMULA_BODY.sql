...
** 09.05.2012 limmmchu ECPD-19937: Added new method FLWL_THEOR_WAT_INJ_VOL,FLWL_THEOR_GAS_INJ_VOL,'FLWL_THEOR_OIL_MASS,FLWL_THEOR_GAS_MASS,FLWL_THEOR_WATER_MASS,FLWL_THEOR_COND_MASS for p_object_type = FLOWLINE.
** 17.07.2012 musthram ECPD-21375: Added new method under Facility Objects - TOTAL_THEOR_WELL_COND_MASS_PROD and SUM_OPER_WELL_COND_MASS_PROD
** 01.11.2012 abdulmaw ECPD-22037: Enhance evaluateMethod to support DILUENT_VOL_DAY_DIFF
** 21.03.2013 kumarsur ECPD-22598: Modified evaluateMethod().
** 29.11.2013 abdulmaw ECPD-25999: Modified evaluateMethod to handle monthly storage funtions.
** 02.12.2014 kumarsur ECPD-29550: Modified evaluateMethod changed Equipment to EQPM.
** 31.12.2014 dhavaalo ECPD-29571: Modified evaluateMethod, changed Equipment to EQPM and added support for the new deferment module (PD.0020) to get Equipment downtime hours in evaluateMethod Function.
** 11.04.2016 mishrdha ECPD-33474: Modified evaluateMethod, changed system_days cursor to pick up date values in sorted order
** 19.07.2016 jainngou ECPD-35734: Modified evaluateMethod() to calculate monthly gross/net volumne/mass for STREAM object.
** 08.08.2016 jainngou ECPD-35728: Modified evaluateMethod() to calculate month inventory change for Tanks object.
** 23.09.2016 xxbrattr ECPD-35731: Modified evaluateMethod() with new methods for Flowline objects
** 23.02.2017 johamlei ECPD-42769: Modified evaluateMethod() with new methods for Production Separator Objects.
                                   Added support for object_type PRODSEPARATOR.
                                   Methods added are:
									                 NET_OIL_STD, NET_GAS_STD, TOT_WAT_STD, NET_OIL_FLC, NET_GAS_FLC, TOT_WAT_FLC,
                                   OIL_OUT_FLC, GAS_OUT_FLC, WAT_OUT_FLC
** 19.09.2017 shindani ECPD-44259: Modified evaluateMethod() with new method for Equipment Downtime(PD.0022).
** 06-11-2017 leongwen ECPD-50437: Modified FUNCTION evaluateMethod to include the check on class_name for well_deferment table.
...
--              PRODSEPARATOR - Singe Prodseparator
--
-- Supported methods:
--              NET_VOL                        (STREAM)
...
--              FLWL_RISER_GL_VOL              (FLOWLINE)
--              FLWL_HC_MASS                   (FLOWLINE)
--
--              NET_OIL_STD                    (PRODSEPARATOR)
--              NET_GAS_STD                    (PRODSEPARATOR)
--              TOT_WAT_STD                    (PRODSEPARATOR)
--              NET_OIL_FLC                    (PRODSEPARATOR)
--              NET_GAS_FLC                    (PRODSEPARATOR)
--              TOT_WAT_FLC                    (PRODSEPARATOR)
--              GAS_OUT_FLC                    (PRODSEPARATOR)
--              WAT_OUT_FLC                    (PRODSEPARATOR)
--
-- Preconditions:
...
FUNCTION evaluateMethod(
 ...
 CURSOR c_days IS
  SELECT *
  FROM system_days
  WHERE daytime BETWEEN p_daytime AND Nvl(p_to_date,p_daytime) order by daytime;

 CURSOR c_deferment_event(cp_daytime DATE) IS
  SELECT *
...
BEGIN
  ...
  ELSIF p_object_type IN ('STREAM') THEN
	...
	ELSIF p_method = 'MTHSUM_GRSVOL' THEN   -- Month sum grs_vol for stream.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(TO_DATE(p_daytime),'MM') THEN
        ln_return_val := ecbp_stream_fluid.findgrsstdvol(p_object_id, p_daytime, LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;

    ELSIF  p_method = 'MTHSUM_NETVOL' THEN   -- Month sum net_vol for stream.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(TO_DATE(p_daytime),'MM') THEN
        ln_return_val := ecbp_stream_fluid.findnetstdvol(p_object_id, p_daytime, LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;

    ELSIF  p_method = 'MTHSUM_GRSMASS' THEN   -- Month sum grs_mass for stream.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(TO_DATE(p_daytime),'MM') THEN
        ln_return_val := ecbp_stream_fluid.findgrsstdmass(p_object_id, p_daytime, LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;

    ELSIF  p_method = 'MTHSUM_NETMASS' THEN   -- Month sum net_mass for stream.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(TO_DATE(p_daytime),'MM') THEN
        ln_return_val := ecbp_stream_fluid.findnetstdmass(p_object_id, p_daytime, LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;
  ...
  ELSIF p_object_type = 'TANK' THEN
    ...
    ELSIF p_method = 'MTHOPEN_GRSVOL' THEN   -- Month opening gross volume for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findOpeningGrsVol(p_object_id,'DAY_CLOSING',p_daytime);
       ELSE
         ln_return_val := NULL;
       END IF;

    ELSIF  p_method = 'MTHOPEN_NETVOL' THEN   -- Month opening net volume for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findOpeningNetVol(p_object_id,'DAY_CLOSING', p_daytime);
       ELSE
         ln_return_val := NULL;
    ...
    ELSIF  p_method = 'MTHOPEN_GRSMASS' THEN   -- Month opening gross mass for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findOpeningGrsMass(p_object_id,'DAY_CLOSING', p_daytime);
       ELSE
         ln_return_val := NULL;
    ...
    ELSIF  p_method = 'MTHOPEN_NETMASS' THEN   -- Month opening net mass for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findOpeningNetMass(p_object_id,'DAY_CLOSING', p_daytime);
       ELSE
         ln_return_val := NULL;
    ...
    ELSIF p_method = 'MTHCLOSE_GRSVOL' THEN   -- Month closing gross volume for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findClosingGrsVol(p_object_id,'DAY_CLOSING',LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;

    ELSIF  p_method = 'MTHCLOSE_NETVOL' THEN   -- Month closing net volume for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findClosingNetVol(p_object_id,'DAY_CLOSING',LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;

    ELSIF  p_method = 'MTHCLOSE_GRSMASS' THEN   -- Month closing gross mass for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findClosingGrsMass(p_object_id,'DAY_CLOSING',LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;

    ELSIF  p_method = 'MTHCLOSE_NETMASS' THEN   -- Month closing net mass for tank.  Return NULL if daytime <> first of month
       IF p_daytime = TRUNC(p_daytime,'MM') THEN
        ln_return_val := ecbp_tank.findClosingNetMass(p_object_id,'DAY_CLOSING',LAST_DAY(p_daytime));
       ELSE
         ln_return_val := NULL;
       END IF;
  ...
  ELSIF p_object_type = 'FLOWLINE' THEN
    ...
    ELSIF p_method = 'FLWL_RISER_GL_VOL' THEN
      ln_return_val := ec_pflw_day_status.math_riser_gl_rate(p_object_id, p_daytime,p_to_date);

    ELSIF p_method = 'FLWL_HC_MASS' THEN
      ln_return_val := ecbp_flowline_theoretical.findHCMassDay(p_object_id, p_daytime, NULL);
  ...
  ELSIF p_object_type = 'EQPM' THEN
    ...
    ELSIF p_method = 'RUN_TIME' THEN
      ...
      ELSIF ec_eqpm_version.on_time_method(p_object_id,p_daytime,'<=') = 'EQUIP_DOWNTIME' THEN
        ...
        ln_return_val := Ecdp_Timestamp.getNumHours('EQPM',p_object_id, p_daytime);
        ...
        IF  lv2_deferment_version = 'PD.0020' OR lv2_deferment_version IS NULL THEN
          FOR curDowntime IN c_deferment_event(ld_daytime) LOOP
            ln_num_hours := Ecdp_Timestamp.getNumHours('EQPM',p_object_id, p_daytime);
      ...
      ELSIF ec_eqpm_version.on_time_method(p_object_id,p_daytime,'<=') = 'EQUIP_DOWNTIME_PD0022' THEN
        ln_prod_day_offset := (EcDp_ProductionDay.getProductionDayOffset('EQPM',p_object_id,p_daytime)/24);
        ld_daytime := p_daytime + ln_prod_day_offset;
        ln_return_val := Ecdp_Timestamp.getNumHours('EQPM',p_object_id, p_daytime);
  ...
  -- Production Separator Objects
  ELSIF p_object_type = 'PRODSEPARATOR' THEN

    IF p_method = 'NET_OIL_STD' THEN   -- Net produced oil from separator at standard conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_NET_OIL_RATE_ADJ(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'NET_GAS_STD' THEN   -- Net produced gas from separator at standard conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_NET_GAS_RATE_ADJ(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'TOT_WAT_STD' THEN   -- Total produced water from separator at standard conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_TOT_WATER_RATE_ADJ(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'NET_OIL_FLC' THEN   -- Net produced oil from separator at flowing conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_NET_OIL_RATE_FLC(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'NET_GAS_FLC' THEN   -- Net produced gas from separator at flowing conditions.
       ln_return_val := EC_PSEP_DAY_STATUS.MATH_NET_GAS_RATE_FLC(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'TOT_WAT_FLC' THEN   -- Total produced water from separator at flowing conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_TOT_WATER_RATE_FLC(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'OIL_OUT_FLC' THEN   -- Measured oil from separator outlet at flowing conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_OIL_OUT_RATE_FLC(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'GAS_OUT_FLC' THEN   -- Measured gas from separator outlet at flowing conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_GAS_OUT_RATE_FLC(p_object_id, p_daytime, p_to_date);

    ELSIF p_method = 'WAT_OUT_FLC' THEN   -- Measured water from separator outlet at flowing conditions.
      ln_return_val := EC_PSEP_DAY_STATUS.MATH_WATER_OUT_RATE_FLC(p_object_id, p_daytime, p_to_date);
