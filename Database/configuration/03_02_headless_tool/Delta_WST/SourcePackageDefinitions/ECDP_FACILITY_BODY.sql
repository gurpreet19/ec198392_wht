...
** 			27-06-2014 abdulmaw	ECDP-26928 Modified getParentFacility: Update EQPM to support merging all equipment into one class
**          31-12-2014 dhavaalo ECPD-29571: Removed cursor c_class_dependency in Function getParentFacility.
**          2016-08-05 xxbrattr ECPD-36546 Added new function daysSinceLastIncident for calculating days since last incident .
*****************************************************************/
...
BEGIN
   ...
   CASE lv2_class_name
     ...
     WHEN 'TEST_DEVICE'
        THEN lv_facility_id := Nvl(ec_test_device_version.op_fcty_class_1_id(p_object_id, p_daytime, '<='),ec_test_device_version.op_fcty_class_2_id(p_object_id, p_daytime, '<='));
...
-- Description    : Gives the number of days since the last incident
--
-- Preconditions  :
-- Postconditions :
--
-- Using tables   : FCTY_DAY_HSE
--
-- Using functions:
--
-- Configuration
...
FUNCTION daysSinceLastIncident(
  p_object_id production_facility.object_id%TYPE,
  p_hse_type fcty_day_hse.hse_type%TYPE,
  p_daytime DATE)
RETURN NUMBER
IS
...
BEGIN
SELECT MAX(daytime) INTO ld_last_date FROM fcty_day_hse WHERE OBJECT_ID = p_object_id AND hse_type= p_hse_type AND quantity<>0 AND daytime<= p_daytime;
IF ld_last_date IS NULL
THEN
 ln_ret_val :=0 ;
ELSE
SELECT TRUNC(p_daytime) - TRUNC(ld_last_date) INTO ln_ret_val FROM dual;
END IF;
