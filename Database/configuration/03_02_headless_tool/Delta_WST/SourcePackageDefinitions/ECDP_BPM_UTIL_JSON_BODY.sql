CREATE OR REPLACE PACKAGE BODY EcDp_BPM_util_json IS
    ...
    BEGIN
        ...
        IF (n_idx = 0) THEN
            n_idx := INSTRC(p_json, ']') - 1;
            n_element_json := ',' || p_element_json;
        ELSE
            n_element_json := p_element_json || ',';
        END IF;

        RETURN SUBSTRC(p_json, 0, n_idx) || n_element_json || SUBSTRC(p_json, n_idx + 1);
    END;
    ...
    BEGIN
        n_value_start_idx := INSTRC(p_json, '".value":') + LENGTH('".value":');
        n_value_end_idx := INSTRC(p_json, ',', n_value_start_idx);
        n_value := SUBSTRC(p_json, n_value_start_idx, n_value_end_idx - n_value_start_idx);
    ...
    BEGIN
        ...
        WHILE n_level <> 0
        LOOP
            n_start_idx := INSTRC(p_json, '{', n_idx + 1);
            n_end_idx := INSTRC(p_json, '}', n_idx + 1);
    ...
    BEGIN
        ...
        WHILE NOT n_found
        LOOP
            n_escaped_idx := INSTRC(p_json, '\"', n_idx + 1);
            n_end_idx := INSTRC(p_json, '"', n_idx + 1);
    ...
    IS
        ...
        n_end_idx_3                      NUMBER;
    BEGIN
        n_end_idx_1 := INSTRC(p_json, ',', p_start_idx + 1);
        n_end_idx_2 := INSTRC(p_json, ']', p_start_idx + 1);
        n_end_idx_3 := INSTRC(p_json, '}', p_start_idx + 1);
    ...
    BEGIN
        n_start_symbol := SUBSTRC(p_json, p_start_idx, 1);
    ...
    BEGIN
        n_value_sep_idx := INSTRC(p_json, ':', p_start_idx);
    ...
    IS
        CURSOR n_path(cp_path_str VARCHAR2) IS (
            SELECT regexp_substr(cp_path_str,'[^/]+', 1, LEVEL) FROM dual
    ...
    BEGIN
        ...
        WHILE n_idx < n_end_idx LOOP
            IF INSTRC(p_json, '"' || p_element_name || '"', n_idx + 1) = n_idx + 1 THEN
                RETURN n_idx + 1;
            ELSE
    ...
    IS
        ...
        n_end_idx                        NUMBER;
    BEGIN
        n_idx := start_of_path(p_json, p_json_path, 1);
        IF n_idx = 0 THEN
            RETURN NULL;
        END IF;
        ...
        RETURN SUBSTRC(p_json, n_idx, n_end_idx - n_idx + 1);
    END;
    ...
    BEGIN
        ...
        RETURN SUBSTRC(p_json, 1, n_idx - 1) || p_value_json || SUBSTRC(p_json, n_end_idx + 1);
    END;
    ...
    IS
        ...
        n_end_idx                          NUMBER;
    BEGIN
        n_value := value_of(p_json, p_json_path);
        n_value := insert_array_element(n_value, p_element_json, p_position_idx);
        RETURN replace_value(p_json, p_json_path, n_value);
    END;
    ...
    IS
    BEGIN
        IF INSTRC(p_value, '"') = 1 AND INSTRC(p_value, '"', -1) = LENGTH(p_value)
        THEN
            RETURN SUBSTRC(p_value, 2, LENGTH(p_value) - 2);
        ELSE
            RETURN p_value;
        END IF;
    END;
