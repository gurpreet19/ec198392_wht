CREATE OR REPLACE PACKAGE BODY ue_Capacity_Release IS
/****************************************************************
...
BEGIN
    ...
    IF ln_seq_no IS NULL THEN
      ...
      EcDp_System_Key.assignNextNumber('CAPACITY_REL_LOC_EVENT'||to_char(Ecdp_Timestamp.getCurrentSysdate(),'yyyy'), ln_seq_no);
      ...
      lv_request_id :=  'AH-'||to_char(Ecdp_Timestamp.getCurrentSysdate(),'yyyy')||'-'||lv_seq_string;
