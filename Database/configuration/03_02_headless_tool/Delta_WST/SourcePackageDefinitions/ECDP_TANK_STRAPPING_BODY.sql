...
**			25.08.2014	hismahas ECPD-28159: Add a user exit for Object Tank, Strapping Method.
*****************************************************************/
...
BEGIN
  ...
  -- USER EXIT
  ELSIF (substr(lv2_strapping_method,1,9) = EcDp_Calc_Method.USER_EXIT) THEN

    ln_return_val := ue_tank_strapping.findValueFromDip(
                     p_tank_object_id,
                     p_dip_level,
                     p_daytime ,
                     p_strap_value_type);
...
BEGIN
  ...
  FOR curTankStrapping IN c_tank_strapping LOOP
    IF p_daytime IS NOT NULL THEN
      ...
      curTankStrapping.created_date := Ecdp_Timestamp.getCurrentSysdate;
...
IS
  ...
  n_last_updated_date DATE := Ecdp_Timestamp.getCurrentSysdate;
