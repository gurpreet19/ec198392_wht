...
** 20161202 farhaann ECPD-39440: Modified GetUnitLabel to use ctrl_unit.label%TYPE instead of VARCHAR2(16) for lv2_label
****************************************************************************************
*/
...
BEGIN
    ...
    IF ld_daytime IS NULL THEN
        ld_daytime := Ecdp_Timestamp.getCurrentSysdate;
    END IF;
...
IS
	...
	lv2_label ctrl_unit.label%TYPE;
...
-- Description    : Returns formatmask for the given unit, or empty string if formatmask is null.
--
-- Preconditions  :
-- Postconditions :
--
...
--
---------------------------------------------------------------------------------------------------
FUNCTION GetRevnViewFormatMaskValue(p_measurement_type VARCHAR2,p_uom_code	VARCHAR2)
RETURN VARCHAR2
IS
...
-- Description    : Returns viewformatmask for the given unit, or empty string if formatmask is null.
--                  The returned string is formatted as a key-value pair: 'viewformatmask=###,##0.00'
--
-- Preconditions  :
-- Postconditions :
--
...
RETURN VARCHAR2
IS
  lv2_viewformatmask VARCHAR2(32) := GetRevnViewFormatMaskValue(p_measurement_type, p_uom_code);
BEGIN
  IF lv2_viewformatmask <> '' THEN
      lv2_viewformatmask := 'viewformatmask='|| lv2_viewformatmask || ';';
  END IF;
  RETURN lv2_viewformatmask;
