...
BEGIN
   ...
   ELSIF lv_record_exists = 'N'THEN --create new record
      ...
      VALUES( p_contract_id, p_delpt_id, p_daytime,ln_sum_nominated_qty, p_user, Ecdp_Timestamp.getCurrentSysdate);
   END IF;
...
BEGIN
   ...
   ELSIF lv_record_exists = 'N' THEN --create new record
      ...
      VALUES( p_contract_id, p_delpt_id, p_daytime,ln_sum_req_qty, p_user, Ecdp_Timestamp.getCurrentSysdate);
   END IF;
...
BEGIN
   ...
   ELSIF lv_record_exists = 'N' THEN --create new record
      ...
      VALUES( p_contract_id, p_delpt_id, p_daytime,ln_sum_sent_qty, p_user, Ecdp_Timestamp.getCurrentSysdate);
   END IF;
...
BEGIN
   ...
   ELSIF lv_record_exists = 'N' THEN --create new record
      ...
      VALUES( p_contract_id, p_delpt_id, p_daytime,ln_sum_adj_qty, p_user, Ecdp_Timestamp.getCurrentSysdate);
   END IF;
...
-- < EC - DOC >
  ---------------------------------------------------------------------------------------------------
  -- Pro       : validateNominations
  -- Description    : Validates Renominations if there are no any Nominations for the date selected.
  --
  -- Preconditions  : p_daytime should be a logical contract day (zero hours/minutes/seconds)
  -- Postconditions :
  --
  -- Using tables   : DV_SCTR_DAY_NOM
  --
  --
  -- Configuration
  -- required       :
  --
  -- Behaviour      :Validates Renominations if there are no any Nominations for the date selected and restricts
  --                 Renominations for that day.
  --
  ---------------------------------------------------------------------------------------------------
PROCEDURE validateNominations(p_object_id VARCHAR2, p_daytime DATE) IS lv_nom_count NUMBER;
BEGIN
  SELECT COUNT(*)
    INTO lv_nom_count
    FROM DV_SCTR_DAY_NOM
   WHERE object_id = p_object_id
     and trunc(daytime) = trunc(p_daytime);

  IF lv_nom_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20580,
                            'Not allowed to enter Renominations if there are no any Nominations for the date selected.');
  END IF;
END validateNominations;
