CREATE OR REPLACE PACKAGE BODY EcDp_NOMINATION_CONTRACT_BAL IS
/******************************************************************************
** Package        :  EcDp_NOMINATION_CONTRACT_BAL, body part
**
** $Revision: 1.3 $
**
...
**22.10.2012   chooysie    ECPD-21108 Add function for Monthly Contract Balance screen
**************************************************************************************************************/
...
IS
          ...
          WHERE contract_id = p_contract_id AND nom_cycle_code = p_nom_cycle AND daytime = p_date
        AND nomination_type=p_nom_type AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;

          CURSOR c_contract_nom_cycle_oper IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_nomination
          WHERE contract_id = p_contract_id AND nom_cycle_code = p_nom_cycle AND daytime = p_date
        and nomination_type=p_nom_type and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;

          CURSOR c_contract_nom IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_nomination
          WHERE contract_id = p_contract_id AND nom_cycle_code is null AND daytime = p_date
        and nomination_type=p_nom_type AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
        ...
        AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
BEGIN
 --Find the day total
...
IS
          ...
          --Nomination Output
          CURSOR c_contract_nom_cycle_out(cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
      AND daytime = p_date and nomination_type=cp_nom_type_out AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;

          CURSOR c_contract_nom_cycle_oper_out(cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
        AND daytime = p_date and nomination_type=cp_nom_type_out and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;

          CURSOR c_contract_nom_out(cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null
        AND daytime = p_date and nomination_type=cp_nom_type_out AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;


          CURSOR c_contract_nom_oper_out(cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null AND daytime = p_date
        and nomination_type=cp_nom_type_out and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;


          --Nomination Input
          CURSOR c_contract_nom_cycle_in(cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
      AND daytime = p_date and nomination_type=cp_nom_type_in AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;


          CURSOR c_contract_nom_cycle_oper_in(cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
        AND daytime = p_date and nomination_type=cp_nom_type_in and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;

          CURSOR c_contract_nom_in(cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null
        AND daytime = p_date and nomination_type=cp_nom_type_in AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          CURSOR c_contract_nom_oper_in(cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null AND daytime = p_date
        and nomination_type=cp_nom_type_in and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
...
BEGIN
  ...
  ln_input  := ecdp_contract_attribute.getAttributeString (p_contract_id,'TRAN_INPUT',p_date);
  ln_output := ecdp_contract_attribute.getAttributeString (p_contract_id,'TRAN_OUTPUT',p_date);
 --Find the day total
 --Nomination Output and Nom cycle null
 IF p_nom_cycle is null THEN
    IF p_oper_nom_ind ='Y' THEN
        FOR curREQQtySum IN c_contract_nom_out(p_contract_id, lv_nom_type_out) LOOP
           ln_aggregated_output_qty:= curREQQtySum.result;
       END LOOP;
    ELSE IF p_oper_nom_ind ='N' THEN
       FOR curREQQtySum IN c_contract_nom_oper_out(p_contract_id, lv_nom_type_out) LOOP
           ln_aggregated_output_qty:= curREQQtySum.result;
       END LOOP;
           END IF;
       END IF;
  ...
  --Nomination Input and nom cycle not null
   IF p_nom_cycle is null THEN
    IF p_oper_nom_ind ='Y' THEN
        FOR curREQQtySum IN c_contract_nom_in(p_contract_id, lv_nom_type_in) LOOP
           ln_aggregated_input_qty:= curREQQtySum.result;
       END LOOP;
    ELSE IF p_oper_nom_ind ='N' THEN
       FOR curREQQtySum IN c_contract_nom_oper_in(p_contract_id, lv_nom_type_in) LOOP
           ln_aggregated_input_qty:= curREQQtySum.result;
       END LOOP;
           END IF;
       END IF;
...
--Nimination Output nom cycle null
 IF p_nom_cycle is not null THEN
    IF p_oper_nom_ind = 'Y' then
    FOR curREQQtySum IN c_contract_nom_cycle_out(p_contract_id, lv_nom_type_out) LOOP
    ln_aggregated_output_qty:= curREQQtySum.result;
    END LOOP;
    ELSE IF p_oper_nom_ind = 'N' THEN
    FOR curREQQtySum IN c_contract_nom_cycle_oper_out(p_contract_id, lv_nom_type_out) LOOP
    ln_aggregated_output_qty:= curREQQtySum.result;
    END LOOP;
        END IF;
     END IF;
 ...
 IF p_nom_cycle is not null THEN
    IF p_oper_nom_ind = 'Y' then
    FOR curREQQtySum IN c_contract_nom_cycle_in(p_contract_id, lv_nom_type_in) LOOP
    ln_aggregated_input_qty:= curREQQtySum.result;
    END LOOP;
    ELSE IF p_oper_nom_ind = 'N' THEN
    FOR curREQQtySum IN c_contract_nom_cycle_oper_in(p_contract_id, lv_nom_type_in) LOOP
    ln_aggregated_input_qty:= curREQQtySum.result;
    END LOOP;
        END IF;
     END IF;
...
IS
      ...
      AND daytime = p_date and nomination_type=cp_nom_type_out AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          SELECT SUM(ACCEPTED_QTY) result
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle AND daytime = p_date
        and nomination_type=cp_nom_type_out and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null AND daytime = p_date
        and nomination_type=cp_nom_type_out AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
      ...
      AND daytime = p_date and nomination_type=cp_nom_type_in AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
        ...
        and nomination_type=cp_nom_type_in and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          FROM nompnt_day_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null AND daytime = p_date
        and nomination_type=cp_nom_type_in AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
...
-- Description    : Returns aggregated qty for a contract on a given month for a given cycle
--
-- Preconditions  :
-- Postconditions :
--
-- Using tables   : nompnt_mth_nom
--
-- Using functions:
--
-- Configuration
...
IS
          ...
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_mth_nom
          WHERE contract_id = p_contract_id AND nom_cycle_code = p_nom_cycle AND (daytime between p_date_from and p_date_to)
        AND nomination_type=p_nom_type AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
        ...
        and nomination_type=p_nom_type and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_mth_nom
          WHERE contract_id = p_contract_id AND nom_cycle_code is null AND (daytime between p_date_from and p_date_to)
        and nomination_type=p_nom_type AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
...
-- Postconditions :
--
-- Using tables   : nompnt_mth_transfer
--
-- Using functions:
--
-- Configuration
...
IS
          ...
          CURSOR c_contract_nom_cycle_out(cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_mth_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
      AND (daytime between p_date_from and p_date_to) and nomination_type=cp_nom_type_out AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;

          CURSOR c_contract_nom_cycle_oper_out(cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2) IS
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_mth_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
        AND (daytime between p_date_from and p_date_to) and nomination_type=cp_nom_type_out and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
        ...
        AND (daytime between p_date_from and p_date_to) and nomination_type=cp_nom_type_out AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          SELECT SUM(REQUESTED_QTY) result
          FROM nompnt_mth_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null AND (daytime between p_date_from and p_date_to)
        and nomination_type=cp_nom_type_out and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          FROM nompnt_mth_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
      AND (daytime between p_date_from and p_date_to) and nomination_type=cp_nom_type_in AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          FROM nompnt_mth_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
        AND (daytime between p_date_from and p_date_to) and nomination_type=cp_nom_type_in and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
...
BEGIN
  ...
  ln_input  := ecdp_contract_attribute.getAttributeString (p_contract_id,'TRAN_INPUT',p_date_from);
  ln_output := ecdp_contract_attribute.getAttributeString (p_contract_id,'TRAN_OUTPUT',p_date_from);
 --Find the day total
 --Nomination Output and Nom cycle null
 IF p_nom_cycle is null THEN
    IF p_oper_nom_ind ='Y' THEN
...
-- Description    : Returns aggregated transfer accepted qty for a contract on a given month for a given cycle
--
-- Preconditions  :
-- Postconditions :
--
...
IS
      ...
      AND (daytime between p_date_from and p_date_to) and nomination_type=cp_nom_type_out AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
          ...
          SELECT SUM(ACCEPTED_QTY) result
          FROM nompnt_mth_transfer t
          WHERE ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle AND (daytime between p_date_from and p_date_to)
        and nomination_type=cp_nom_type_out and nvl(oper_nom_ind,'N')<>'Y' AND nom_status != 'REJ' and supplier_nompnt_id IS NULL;
...
-- Description    : Returns aggregated transfer adjusted qty for a contract on a given month for a given cycle
--
-- Preconditions  :
-- Postconditions :
--
...
-- Description    : Returns aggregated transfer schedule qty for a contract on a given month for a given cycle
--
-- Preconditions  :
-- Postconditions :
--
