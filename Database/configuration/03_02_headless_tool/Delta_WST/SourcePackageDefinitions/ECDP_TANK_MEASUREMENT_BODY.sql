...
-- Description    : ccreate Seal Location
-- Preconditions  :
-- Postconditions :                                                                                --
--                                                                                                 --
-- Using tables   :
...
--                                                                                                 --
-----------------------------------------------------------------------------------------------------
Procedure createSealLocation(p_object_id VARCHAR2, p_daytime DATE, p_user VARCHAR2)


--</EC-DOC>
IS

CURSOR c_enddaytime IS
  SELECT daytime
    FROM system_days
    WHERE daytime BETWEEN p_daytime + 1
    AND  TRUNC(nvl(p_daytime,Ecdp_Timestamp.getCurrentSysdate), 'DD');


BEGIN

	 INSERT INTO tank_seal
	  (object_id, daytime, created_by)
	   VALUES
	  (p_object_id, p_daytime, p_user);


END createSealLocation;
...
--                                                                                                 --
-----------------------------------------------------------------------------------------------------
Procedure delSealLocation(p_object_id VARCHAR2, p_daytime DATE)
--</EC-DOC>
IS
...
--<EC-DOC>
-----------------------------------------------------------------
-- Function     : delMeasurementSet
-- Description  : deletes Ticket Details
--
-- Preconditions:
-- Postcondition:
-- Using Tables:  TANK_MEASUREMENT
...
BEGIN
	...
	DELETE FROM TANK_MEASUREMENT
	WHERE DAYTIME = p_daytime AND
		  OBJECT_ID = p_tank_object_id AND
		  MEASUREMENT_EVENT_TYPE IN ('DUAL_DIP_OPENING','DUAL_DIP_CLOSING');

END delMeasurementSet;
