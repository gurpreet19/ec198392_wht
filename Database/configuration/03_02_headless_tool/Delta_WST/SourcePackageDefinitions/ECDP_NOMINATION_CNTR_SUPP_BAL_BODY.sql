CREATE OR REPLACE PACKAGE BODY EcDp_Nomination_Cntr_Supp_Bal IS
/******************************************************************************
** Package        :  EcDp_Nomination_Cntr_Supp_Bal, body part
**
** $Revision: 1.4 $
**
...
** --------  ---------   	-----------------------------------------------------------------------------------
**16.04.2013	muhammah	ECPD-23877:
								-Update supplier_nompnt_id = p_supplier_nompnt_id in query to nompnt_day_nomination in functions:
									getTotalReqQtyPrContract
									getTotalAccQtyPrContract
									getTotalExtAccQtyPrContract
									getTotalAdjQtyPrContract
									getTotalExtAdjQtyPrContract
									getTotalConfQtyPrContract
									getTotalExtConfQtyPrContract
									getTotalSchedQtyPrContract
								-Added p_location_id, p_transfer_service to functions: added p_location_id, p_transfer_service to functions:
									getTotalReqTransfQtyPrContract
									getTotalAccTransfQtyPrContract
									getTotalAdjTransfQtyPrContract
									getTotalSchTransfQtyPrContract
**************************************************************************************************************/
...
IS
          ...
          CURSOR c_contract_nom_cycle IS
			  ...
			  and nomination_type=p_nom_type and nom_status != 'REJ' and supplier_nompnt_id = p_supplier_nompnt_id
			  and p_location_id IN (ec_nomination_point.entry_location_id(object_id),ec_nomination_point.exit_location_id(object_id))
          ...
          CURSOR c_contract_nom_cycle_oper IS
			  ...
			  and nom_status != 'REJ' and supplier_nompnt_id = p_supplier_nompnt_id
			  and p_location_id IN (ec_nomination_point.entry_location_id(object_id),ec_nomination_point.exit_location_id(object_id))
			  and ((ec_nomination_point.entry_location_id(object_id) is not null and ec_nomination_point.exit_location_id(object_id) is null) or (ec_nomination_point.exit_location_id(object_id) is not null and ec_nomination_point.entry_location_id(object_id) is null));
...
IS
          ...
          CURSOR c_contract_nom_cycle_oper IS
		      ...
		      FROM nompnt_day_nomination
		      WHERE contract_id = p_contract_id AND nom_cycle_code = p_nom_cycle AND daytime = p_date
			  and nomination_type=p_nom_type and nvl(oper_nom_ind,'N')<>'Y' and nom_status != 'REJ' and supplier_nompnt_id = p_supplier_nompnt_id
			  and p_location_id IN (ec_nomination_point.entry_location_id(object_id),ec_nomination_point.exit_location_id(object_id))
			  and ((ec_nomination_point.entry_location_id(object_id) is not null and ec_nomination_point.exit_location_id(object_id) is null) or (ec_nomination_point.exit_location_id(object_id) is not null and ec_nomination_point.entry_location_id(object_id) is null));
...
BEGIN
 --Find the day total
FOR c_comp_cont in c_company_contracts (p_company_id) LOOP
    --ln_aggregated_qty:= Nvl(ln_aggregated_qty, 0) + Nvl(getTotalAccQtyPrContract(c_comp_cont.object_id,p_nom_cycle,p_nom_type,p_oper_nom_ind,p_date,null),0);
	null;
END LOOP;
...
BEGIN
 --Find the day total
FOR c_comp_cont in c_company_contracts (p_company_id) LOOP
    --ln_aggregated_qty:= Nvl(ln_aggregated_qty, 0) + NVL(getTotalExtAccQtyPrContract(c_comp_cont.object_id,p_nom_cycle,p_nom_type,p_oper_nom_ind,p_date,null),0);
	null;
END LOOP;
...
BEGIN
 --Find the day total
FOR c_comp_cont in c_company_contracts (p_company_id) LOOP
    --ln_aggregated_qty:= Nvl(ln_aggregated_qty, 0) + Nvl(getTotalAdjQtyPrContract(c_comp_cont.object_id,p_nom_cycle,p_nom_type,p_oper_nom_ind,p_date,null),0);
	null;
END LOOP;
...
BEGIN
 --Find the day total
FOR c_comp_cont in c_company_contracts (p_company_id) LOOP
    --ln_aggregated_qty:= Nvl(ln_aggregated_qty, 0) + Nvl(getTotalExtAdjQtyPrContract(c_comp_cont.object_id,p_nom_cycle,p_nom_type,p_oper_nom_ind,p_date,null),0);
	null;
END LOOP;
...
BEGIN
 --Find the day total
FOR c_comp_cont in c_company_contracts (p_company_id) LOOP
    --ln_aggregated_qty:= Nvl(ln_aggregated_qty, 0) + Nvl(getTotalConfQtyPrContract(c_comp_cont.object_id,p_nom_cycle,p_nom_type,p_oper_nom_ind,p_date,null),0);
	null;
END LOOP;
...
BEGIN
 --Find the day total
FOR c_comp_cont in c_company_contracts (p_company_id) LOOP
    --ln_aggregated_qty:= Nvl(ln_aggregated_qty, 0) + Nvl(getTotalExtConfQtyPrContract(c_comp_cont.object_id,p_nom_cycle,p_nom_type,p_oper_nom_ind,p_date,null),0);
	null;
END LOOP;
...
BEGIN
 --Find the day total
FOR c_comp_cont in c_company_contracts (p_company_id) LOOP
    --ln_aggregated_qty:= Nvl(ln_aggregated_qty, 0) + Nvl(getTotalSchedQtyPrContract(c_comp_cont.object_id,p_nom_cycle,p_nom_type,p_oper_nom_ind,p_date,null),0);
	null;
END LOOP;
...
FUNCTION getTotalReqTransfQtyPrContract(
  ...
  p_date                   DATE,
  p_supplier_nompnt_id     VARCHAR2,
  p_transfer_service       VARCHAR2 DEFAULT NULL
  )
RETURN NUMBER
--</EC-DOC>
IS
  ...
  --Nomination Output
  CURSOR c_contract_nom_cycle_out(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
    AND daytime = p_date AND nomination_type=cp_nom_type_out
    AND transfer_service = cp_transfer_service
    AND nom_status != 'REJ' AND supplier_nompnt_id = p_supplier_nompnt_id;

  CURSOR c_contract_nom_cycle_oper_out(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
  ...
  CURSOR c_contract_nom_out(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code IS NULL
    AND daytime = p_date AND nomination_type=cp_nom_type_out
    AND transfer_service = cp_transfer_service
    AND nom_status != 'REJ' AND supplier_nompnt_id = p_supplier_nompnt_id;

  CURSOR c_contract_nom_oper_out(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_out VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code IS NULL
  ...
  --Nomination Input
  CURSOR c_contract_nom_cycle_in(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
  ...
  CURSOR c_contract_nom_cycle_oper_in(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code = p_nom_cycle
  ...
  CURSOR c_contract_nom_in(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code is null
  ...
  CURSOR c_contract_nom_oper_in(cp_location_id VARCHAR2, cp_contract_id VARCHAR2, cp_nom_type_in VARCHAR2, cp_transfer_service VARCHAR2) IS
  SELECT SUM(REQUESTED_QTY) RESULT
  FROM nompnt_day_transfer t
  WHERE NVL(ec_nomination_point.entry_location_id(t.object_id),ec_nomination_point.exit_location_id(t.object_id)) = cp_location_id
    AND ec_nomination_point.contract_id(t.object_id) = p_contract_id AND nom_cycle_code IS NULL
...
BEGIN
 ...
 IF p_nom_cycle is null THEN
    IF p_oper_nom_ind ='Y' THEN
 	     FOR curREQQtySum IN c_contract_nom_out(p_location_id, p_contract_id, lv_nom_type_out, p_transfer_service) LOOP
		       ln_aggregated_output_qty:= curREQQtySum.result;
       END LOOP;
    ELSE IF p_oper_nom_ind ='N' THEN
       FOR curREQQtySum IN c_contract_nom_oper_out(p_location_id, p_contract_id, lv_nom_type_out, p_transfer_service) LOOP
		       ln_aggregated_output_qty:= curREQQtySum.result;
       END LOOP;
           END IF;
       END IF;
  ...
  IF p_nom_cycle is null THEN
    IF p_oper_nom_ind ='Y' THEN
 	     FOR curREQQtySum IN c_contract_nom_in(p_location_id, p_contract_id, lv_nom_type_in, p_transfer_service) LOOP
		       ln_aggregated_input_qty:= curREQQtySum.result;
       END LOOP;
    ELSE IF p_oper_nom_ind ='N' THEN
       FOR curREQQtySum IN c_contract_nom_oper_in(p_location_id, p_contract_id, lv_nom_type_in, p_transfer_service) LOOP
		       ln_aggregated_input_qty:= curREQQtySum.result;
       END LOOP;
           END IF;
       END IF;
 ...
 -- Nomination Output nom cycle null
 IF p_nom_cycle is not null THEN
  	IF p_oper_nom_ind = 'Y' then
    FOR curREQQtySum IN c_contract_nom_cycle_out(p_location_id, p_contract_id, lv_nom_type_out, p_transfer_service) LOOP
		ln_aggregated_output_qty:= curREQQtySum.result;
    END LOOP;
    ELSE IF p_oper_nom_ind = 'N' THEN
    FOR curREQQtySum IN c_contract_nom_cycle_oper_out(p_location_id, p_contract_id, lv_nom_type_out, p_transfer_service) LOOP
		ln_aggregated_output_qty:= curREQQtySum.result;
    END LOOP;
        END IF;
     END IF;
 END IF;
  ...
  --Nomination Input nom cycle not null
 IF p_nom_cycle is not null THEN
  	IF p_oper_nom_ind = 'Y' then
    FOR curREQQtySum IN c_contract_nom_cycle_in(p_location_id, p_contract_id, lv_nom_type_in, p_transfer_service) LOOP
		ln_aggregated_input_qty:= curREQQtySum.result;
    END LOOP;
    ELSE IF p_oper_nom_ind = 'N' THEN
    FOR curREQQtySum IN c_contract_nom_cycle_oper_in(p_location_id, p_contract_id, lv_nom_type_in, p_transfer_service) LOOP
		ln_aggregated_input_qty:= curREQQtySum.result;
    END LOOP;
        END IF;
     END IF;
