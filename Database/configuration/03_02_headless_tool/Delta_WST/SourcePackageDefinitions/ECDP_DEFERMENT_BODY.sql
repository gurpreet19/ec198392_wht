...
BEGIN
  ...
  BEGIN
    ...
    INTO lv2_object_id,ld_daytime,
         ...
         lv2_deferment_type,ld_end_date
    FROM deferment_event
    WHERE event_no  = p_event_no
    AND class_name IN ('WELL_DEFERMENT','WELL_DEFERMENT_CHILD');
  EXCEPTION
  ...
  IF lv2_deferment_type IN ('SINGLE','GROUP_CHILD') THEN
    ...
    SET oil_loss_rate = nvl(ln_oil_loss_rate, decode(ecdp_well.isWellPhaseActiveStatus(lv2_object_id,null,'OPEN',ld_daytime), 'Y', ecbp_well_potential.findOilProductionPotential(lv2_object_id,ld_daytime),null)),
        ...
        oil_loss_mass_rate = nvl(ln_oil_loss_mass_rate, decode(ecdp_well.isWellPhaseActiveStatus(lv2_object_id,null,'OPEN',ld_daytime), 'Y', ecbp_well_potential.findOilMassProdPotential(lv2_object_id,ld_daytime),null)),
        gas_loss_mass_rate = nvl(ln_gas_loss_mass_rate, decode(ecdp_well.isWellPhaseActiveStatus(lv2_object_id,null,'OPEN',ld_daytime), 'Y', ecbp_well_potential.findGasMassProdPotential(lv2_object_id,ld_daytime), null)),
        cond_loss_mass_rate = nvl(ln_cond_loss_mass_rate, decode(ecdp_well.isWellPhaseActiveStatus(lv2_object_id,null,'OPEN',ld_daytime), 'Y', ecbp_well_potential.findCondMassProdPotential(lv2_object_id,ld_daytime), null)),
        water_loss_mass_rate = nvl(ln_wat_loss_mass_rate, decode(ecdp_well.isWellPhaseActiveStatus(lv2_object_id,null,'OPEN',ld_daytime), 'Y', ecbp_well_potential.findWaterMassProdPotential(lv2_object_id,ld_daytime), null)),
