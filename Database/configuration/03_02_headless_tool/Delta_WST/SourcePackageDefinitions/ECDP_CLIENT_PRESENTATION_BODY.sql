...
** 28-10-2014	shindani	ECPD-29104: Added function getComponentExtName.
** 31-10-2014   dhavaalo	ECPD-29135: GetPresentationSyntax and GetName function updated to support new parameter_sub_type as ASSET_OBJECTS
******************************************************************/
...
IF(PARAMETER_TYPE = 'EC_OBJECT_TYPE') THEN
	RETURN ecdp_objects.GetObjName(PARAMETER_VALUE, Ecdp_Timestamp.getCurrentSysdate);
END IF;
...
IF(PARAMETER_TYPE = 'EC_TABLE_TYPE') THEN
  ...
  IF(parameter_sub_type='ASSET_OBJECTS') THEN
    v_tableret:=ecdp_objects.GetObjName(PARAMETER_VALUE, Ecdp_Timestamp.getCurrentSysdate);
   	IF(v_tableret IS NOT null) THEN
		RETURN v_tableret;
	END IF;
  END IF;
...
IF(PARAMETER_TYPE = 'EC_OBJECT_TYPE') THEN
	RETURN EcDp_ClassMeta_Cnfg.getLabel(parameter_sub_type);
END IF;
...
IF(PARAMETER_TYPE = 'BASIC_TYPE') THEN
	IF(PARAMETER_SUB_TYPE = 'DATE') THEN
		RETURN 'viewwidth=81;viewformatmask=yyyy-MM-dd;pickerurl=/FrontController/com.ec.pf.screenlet.common/filter_date_picker;pickerheight=310;pickerwidth=208';
	END IF;
	...
	IF(PARAMETER_SUB_TYPE = 'TIME') THEN
		RETURN 'viewwidth=52;viewformatmask=HH:mm;vieweditable=false;pickerurl=/FrontController/com.ec.pf.screenlet.common/filter_time_picker;pickerwidth=150;pickerheight=100';
	END IF;
	...
	IF(PARAMETER_SUB_TYPE = 'DATE_YEAR') THEN
		RETURN 'viewwidth=50;viewformatmask=yyyy;pickerurl=/FrontController/com.ec.pf.screenlet.common/filter_year_picker;pickerheight=218;pickerwidth=201';
	END IF;
	IF(PARAMETER_SUB_TYPE = 'DATE_MONTH') THEN
		RETURN 'viewformatmask=yyyy-MM;viewwidth=65;pickerurl=/FrontController/com.ec.pf.screenlet.common/filter_mth_picker;pickerheight=120;pickerwidth=140';
	END IF;
	IF(PARAMETER_SUB_TYPE = 'DATETIME') THEN
		RETURN 'viewwidth=112;viewformatmask=yyyy-MM-dd HH:mm;pickerurl=/FrontController/com.ec.pf.screenlet.common/filter_date_time_picker;pickerwidth=208;pickerheight=310';
	END IF;
...
IF(PARAMETER_TYPE = 'EC_OBJECT_TYPE') THEN
    ...
    IF(popup_dependency IS NOT NULL) THEN
        IF(navigator_value IS NOT NULL) THEN
            v_popup_depend := 'PopupDependency=Screen.this.currentRow.' || popup_dependency || '=ReturnField.OBJECT_ID$'|| navigator_value || ';';
        ELSE
...
IF(PARAMETER_TYPE = 'EC_TABLE_TYPE') THEN
  ...
  */
	...
	ELSIF parameter_sub_type = 'ASSET_OBJECTS' THEN
		IF(popup_dependency IS NOT NULL AND navigator_value IS NOT NULL) THEN
			RETURN 'viewwidth=120;PopupQueryURL=/com.ec.prod.co.screens/query/asset_object_popup.xml;PopupLayout=/com.ec.prod.co.screens/layout/asset_object_popup.xml;' ||
			'PopupReturnColumn=NAME;PopupDependency=Screen.this.currentRow.' || popup_dependency ||
			'=ReturnField.OBJECT_ID$RetrieveArg.FUNC_AREA=' || navigator_value ||
			';PopupWidth=250;PopupHeight=300';
		ELSE
			RETURN '';
     END IF;
...
BEGIN
    ...
    lv_interval := NUMTODSINTERVAL(TO_DATE - FROM_DATE, 'DAY');
    ...
    RETURN lv_hour_str || ':' || TO_CHAR(EXTRACT(MINUTE FROM lv_interval), 'FM00') || ':' || TO_CHAR(EXTRACT(SECOND FROM lv_interval), 'FM00');
...
START WITH  component_id = p_component_id
...
-- Description    : Returns full component name
--
-- Preconditions  :
-- Postconditions :
--
