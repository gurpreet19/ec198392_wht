...
** ----------  -----   -------------------------------------------
** 14-Jan-2009 oonnnng ECPD-9827: Update the Ecdp_Timestamp.getCurrentSysdate with p_daytime for lv_attribute_type = 'OBJECT' in getAttributeValueAsString function.
...
** 28.02.2013 sharawan ECPD-23041: Add support in getAttributeString, getAttributeDate and getAttributeNumber function for retrieving values for DIMENSIONED attributes
**                                 other than USER_EXIT.
** 16.08.2013 leeeewei ECPD-19773: Added new function getSumCntrQty
** 21.01.2014 chooysie ECPD-17104: Modified getAttributeValueAsString to return dimension name
...
BEGIN
  ...
  ELSIF lv_data_type = 'DATE' THEN
	...
	IF lv_dimension_type is null then
		lv_value := TO_CHAR(ecdp_contract_attribute.getAttributeDate(p_contract_id, p_attribute_name, p_daytime), 'YYYY-MM-DD"T"HH24:MI:SS');
    ELSE
		IF lv_attribute_type = 'USER_EXIT' THEN
			lv_value := TO_CHAR(ue_cntr_attr_dimension.getAttributeDate(p_contract_id, p_attribute_name, p_daytime, p_object_ec_code),  'YYYY-MM-DD"T"HH24:MI:SS');
		ELSE
			lv_value := TO_CHAR(ec_cntr_attr_dimension.attribute_date(p_contract_id, p_attribute_name, p_object_ec_code, p_daytime, '<='),  'YYYY-MM-DD"T"HH24:MI:SS');
		END IF;
    END IF;
  ...
  IF lv_dimension_type is not null AND p_object_ec_code is null THEN
    lv_value := ec_cntr_attr_dim_type.name(lv_dimension_type) ;
  END IF;
...
-- Description    : Check if the Dimension has child record in CNTR_TEMPLATE_ATTRIBUTE and CNTR_ATTR_DIMENSION
--
-- Preconditions  :
-- Postconditions :
--
...
-- required       :
--
-- Behaviour      : Validates the Dimension has no child dependencies
--
---------------------------------------------------------------------------------------------------
PROCEDURE validateDimension(p_dim_code VARCHAR2)
 ...
 IS
  ...
  --cursor to check for existing CNTR_ATTR_DIMENSION
  CURSOR c_cntr_attr_dim(cp_dim_code VARCHAR2) IS

    select 1
      from CNTR_ATTR_DIMENSION cad, CNTR_TEMPLATE_ATTRIBUTE cta
     where cad.attribute_name = cta.attribute_name
       and cta.dimension_type = cp_dim_code;

  --cursor to check for existing CNTR_TEMPLATE_ATTRIBUTE
  CURSOR c_cntr_temp_attr(cp_dim_code VARCHAR2) IS

    select 1
      from CNTR_TEMPLATE_ATTRIBUTE cta
     where cta.dimension_type = cp_dim_code;

BEGIN

   FOR curCntrAttrDim IN c_cntr_attr_dim(p_dim_code) LOOP
      RAISE_APPLICATION_ERROR(-20406, 'Not allowed to update or delete the record due to child record found.');
      EXIT;
    END LOOP;

   FOR curTempAtrr IN c_cntr_temp_attr(p_dim_code) LOOP
      RAISE_APPLICATION_ERROR(-20406, 'Not allowed to update or delete the record due to child record found.');
      EXIT;
    END LOOP;
...
--<EC-DOC>
  ---------------------------------------------------------------------------------------------------
  -- Procedure      : SetContractVersion
  -- Description    : Set the end_date according to latest version entered
  --
  -- Preconditions  :
  -- Postconditions :
  --
  ...
  -- required       :
  --
  -- Behaviour      : Set the end_date according to latest version entered
  --
  ---------------------------------------------------------------------------------------------------
  PROCEDURE SetContractAttrVersion(p_object_id      VARCHAR2,
                               p_attribute_name VARCHAR2,
                               p_daytime        DATE,
                               p_dim_code       VARCHAR2)
  ...
  BEGIN
    ...
    IF p_daytime > lv_max_daytime THEN
