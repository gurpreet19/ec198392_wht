...
** 15.11.2017  sharawan ECPD-50028: Modified calcEstClosingBalanceDay to check if there is production day setting for calculating adjustments
******************************************************************/
...
--
---------------------------------------------------------------------------------------------------
FUNCTION calcEstClosingBalanceDay(p_object_id  VARCHAR2, p_forecast_id VARCHAR2, p_daytime DATE, p_xtra_qty NUMBER DEFAULT 0, p_ignore_cache VARCHAR2 DEFAULT 'N')
RETURN NUMBER
--<EC-DOC>
IS
	...
	SELECT
		SUM(decode(cp_cargo_off_qty_ind,'N',grs_vol_nominated, nvl(ecbp_storage_lift_nomination.getLiftedVol(parcel_no,null),grs_vol_nominated))) NOMINATED_QTY,
		SUM(decode(cp_cargo_off_qty_ind,'N',grs_vol_nominated2,nvl(ecbp_storage_lift_nomination.getLiftedVol(parcel_no,1),   grs_vol_nominated2))) NOMINATED_QTY2,
		SUM(decode(cp_cargo_off_qty_ind,'N',grs_vol_nominated3,nvl(ecbp_storage_lift_nomination.getLiftedVol(parcel_no,2),   grs_vol_nominated3))) NOMINATED_QTY3,
        SUM(decode(cp_cargo_off_qty_ind,'N',balance_delta_qty,nvl(ecbp_storage_lift_nomination.getLoadBalDeltaVol(parcel_no),   balance_delta_qty))) balance_delta_qty,
        SUM(decode(cp_cargo_off_qty_ind,'N',balance_delta_qty2,nvl(ecbp_storage_lift_nomination.getLoadBalDeltaVol(parcel_no,1),balance_delta_qty2))) balance_delta_qty2,
        SUM(decode(cp_cargo_off_qty_ind,'N',balance_delta_qty3,nvl(ecbp_storage_lift_nomination.getLoadBalDeltaVol(parcel_no,2),balance_delta_qty3))) balance_delta_qty3
	FROM
		(SELECT f.parcel_no, f.grs_vol_nominated, f.grs_vol_nominated2, f.grs_vol_nominated3, f.balance_delta_qty, f.balance_delta_qty2, f.balance_delta_qty3
		...
		WHERE f.lifting_account_id = cp_object_id
			...
			AND nvl(f.bl_date, f.NOM_FIRM_DATE) < cp_to
			AND nvl(f.DELETED_IND, 'N') <> 'Y'
			AND f.cargo_no IS NULL) a;

	CURSOR curNom_sub_day(cp_object_id VARCHAR2, cp_forecast_id VARCHAR2,cp_from DATE, cp_to DATE, cp_xtra_qty NUMBER) IS
	SELECT
        SUM(ue_Stor_Fcst_Lift_Nom.aggrSubDayLifting(forecast_id, PARCEL_NO, production_day, NULL, cp_xtra_qty)) NOMINATED_QTY,
        SUM(ue_Stor_Fcst_Lift_Nom.aggrSubDayLifting(forecast_id, PARCEL_NO, production_day, 'BALANCE_DELTA', cp_xtra_qty)) balance_delta_qty
	FROM
		(SELECT f.forecast_id, f.PARCEL_NO, sn.production_day
		FROM  stor_fcst_lift_nom f,
...
BEGIN
	...
	ELSE
      ...
      ELSE
        ...
        ln_firstday_opening_bal := Nvl(EcBp_Lift_Acc_Balance.calcEstClosingBalanceDay(p_object_id, ld_forecast_start_day-1,p_xtra_qty,p_ignore_cache),0);
      END IF;
		END IF;
	END IF;
    ...
    --Check if the production day setting is set to Y before calculating adjustments
    IF nvl(lv_read_prod_day, 'N') = 'Y' THEN
      ld_from_date := EcDp_ProductionDay.getProductionDayStart('STORAGE',ec_lifting_account.storage_id(p_object_id),ld_forecast_start_day);
      ld_to_date := EcDp_ProductionDay.getProductionDayStart('STORAGE',ec_lifting_account.storage_id(p_object_id),p_daytime+1);
    ELSE
      ld_from_date := ld_forecast_start_day;
      ld_to_date := p_daytime+1;
    END IF;
    ln_adjustment := Nvl(getAdjustments(p_object_id, p_forecast_id, ld_from_date, ld_to_date, p_xtra_qty),0);
...
--
---------------------------------------------------------------------------------------------------
FUNCTION calcEstClosingBalanceSubDay(p_object_id  VARCHAR2, p_forecast_id VARCHAR2, p_daytime DATE, p_summer_time VARCHAR2, p_xtra_qty NUMBER DEFAULT 0, p_ignore_cache VARCHAR2 DEFAULT 'N')
RETURN NUMBER
--<EC-DOC>
IS
...
BEGIN
	...
	ELSE
		...
		ln_closing_balance := nvl(calcEstClosingBalanceDay(p_object_id, p_forecast_id, ld_production_day - 1, p_xtra_qty, p_ignore_cache), 0);
	END IF;
...
--
---------------------------------------------------------------------------------------------------
FUNCTION calcAggrEstClosingBalanceDay(p_object_id VARCHAR2, p_forecast_id VARCHAR2, p_daytime DATE, p_xtra_qty NUMBER DEFAULT 0)
RETURN NUMBER
--<EC-DOC>
...
BEGIN
  ...
  FOR curLiftAccount IN c_lift_account (lv_contract_id)  LOOP
    ln_aggr_closing_bal := ln_aggr_closing_bal + calcEstClosingBalanceDay(curLiftAccount.lifting_account_id, p_forecast_id, p_daytime, p_xtra_qty);
  END LOOP;
