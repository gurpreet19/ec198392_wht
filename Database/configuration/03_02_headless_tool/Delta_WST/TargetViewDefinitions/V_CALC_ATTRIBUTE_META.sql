CREATE OR REPLACE FORCE VIEW "V_CALC_ATTRIBUTE_META" ("CLASS_OBJECT_TYPE", "CLASS_NAME", "SQL_SYNTAX", "OBJECT_TYPE", "DATA_TYPE", "NAME", "IS_KEY", "IS_CALC_TYPE", "IS_SEQ_NO", "FILTER", "CALC_CONTEXT_ID", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  (
------------------------------------------------------------------------------------
...
select
   ...
   cdm.REV_NO,
   cdm.REV_TEXT
from calc_obj_attr_db_mapping cdm, calc_object_attribute coa, calc_object_filter cof, class_attribute ca
...
and cof.object_type_code = cdm.object_type_code
and cof.sql_syntax = cdm.sql_syntax
))  left join class_attribute ca on
