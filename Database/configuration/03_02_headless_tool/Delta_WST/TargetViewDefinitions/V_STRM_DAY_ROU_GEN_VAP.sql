CREATE OR REPLACE FORCE VIEW "V_STRM_DAY_ROU_GEN_VAP" ("OBJECT_ID", "EQPM_ID", "DAYTIME", "SEQUENCE", "GRS_VOL", "FLASH_FACTOR", "AVAIL_VAPOURS", "PRIM_REC_SYS", "RECORD_STATUS", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "REV_NO", "REV_TEXT") AS 
  (
--------------------------------------------------------------------------------------------------
...
decode(ec_strm_version.stream_phase(ogc.child_obj_id,sds.DAYTIME, '<='),'OIL',ecbp_stream_fluid.findGrsStdVol(ogc.child_obj_id,sds.DAYTIME,sds.DAYTIME),NULL) GRS_VOL,
decode(ec_strm_version.stream_phase(ogc.child_obj_id,sds.DAYTIME, '<='),'OIL',ec_strm_reference_value.flash_factor(ogc.child_obj_id,sds.DAYTIME, '<='),NULL) FLASH_FACTOR,
