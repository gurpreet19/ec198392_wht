...
FUNCTION getRepOwnerJoinStartColumn(p_data_class_name VARCHAR2,p_table_name VARCHAR2)
RETURN VARCHAR2
IS
  CURSOR c_data_class_start_date_column(cp_class_name IN VARCHAR2) IS
    SELECT * FROM (
      SELECT a.class_name, a.attribute_name, a.db_sql_syntax, a.db_mapping_type,
             CASE WHEN a.attribute_name = 'PRODUCTION_DAY' AND (c.time_scope_code='EVENT' OR c.time_scope_code LIKE '%HR') THEN 1
                  WHEN a.attribute_name = 'DAYTIME' THEN 20
                  WHEN a.attribute_name = 'FROM_DATE' THEN 30
                  WHEN a.attribute_name = 'START_DATE' THEN 40
    ...
    lv2_joinStart varchar2(1000) := NULL;
    
BEGIN
  FOR cur IN c_data_class_start_date_column(p_data_class_name) LOOP

    lv2_joinStart := cur.db_sql_syntax;

    IF cur.db_mapping_type <> 'FUNCTION' THEN
  
       lv2_joinStart := p_table_name ||'.'|| lv2_joinStart;
    
    END IF; 
    -- IF it's a function, the class has to prefix columns used if needed
    EXIT;  -- take the fists priority

  END LOOP;

  RETURN lv2_joinStart;
END getRepOwnerJoinStartColumn;
