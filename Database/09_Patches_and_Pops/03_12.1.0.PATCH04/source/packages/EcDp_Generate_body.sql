...
BEGIN
   ...
   IF lb_is_object_table THEN
      buildTrigger(lv2_trigger_name,
          ...
          ':NEW.object_id, :NEW.object_code, :NEW.start_date, :NEW.end_date, :NEW.created_by, :NEW.created_date);' || CHR(10) ||
          ...
          '    ELSE' || CHR(10) ||
          '      ecdp_trigger_utils.iudObject(' || CASE WHEN lb_has_class_name_column THEN ' :OLD.class_name, ' ELSE '''' || tbl.table_name || ''', ' END ||
          ':OLD.object_id, :OLD.object_code, :OLD.start_date, :OLD.end_date, :OLD.created_by, :OLD.created_date, FALSE);' || CHR(10) ||
          '    END IF;' || CHR(10) ||
          'END;' || CHR(10));
   ELSIF lb_is_object_version_table THEN
      buildTrigger(lv2_trigger_name,
          ...
          ':NEW.object_id, :NEW.name, :NEW.daytime, :NEW.end_date, :NEW.created_by, :NEW.created_date);' || CHR(10) ||
          '    ELSIF Updating THEN' || CHR(10) ||
          '      ecdp_trigger_utils.iudObjectVersion(''' || tbl.table_name || ''', ' ||
          ':NEW.object_id, :NEW.name, :NEW.daytime, :NEW.end_date, :NEW.created_by, :NEW.created_date);' || CHR(10) ||
          '    ELSE' || CHR(10) ||
          '      ecdp_trigger_utils.iudObjectVersion(''' || tbl.table_name || ''', ' ||
          ':OLD.object_id, :OLD.name, :OLD.daytime, :OLD.end_date, :OLD.created_by, :OLD.created_date, FALSE);' || CHR(10) ||
          '    END IF;' || CHR(10) ||
          'END;' || CHR(10));
   END IF;
