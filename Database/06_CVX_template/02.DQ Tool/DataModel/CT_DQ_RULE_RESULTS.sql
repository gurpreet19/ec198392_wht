create table CT_DQ_RULE_RESULTS
( RESULT_ID     INTEGER NOT NULL,
  ORIG_RUN_DATE       DATE NOT NULL,
  LAST_RUN_DATE       DATE NOT NULL,
  LAST_RUN_ID    INTEGER NOT NULL,
  RULE_ID       INTEGER NOT NULL,
  OBJECT_TYPE  VARCHAR2(32),
  OBJECT_ID    VARCHAR2(32) NOT NULL,
  DAYTIME      DATE,
  ALT_UNIQUE_KEY VARCHAR2(200),
  ADDNL_INFO VARCHAR2(1000),
  SQL_FLD_1  VARCHAR2(100),
  SQL_FLD_2  VARCHAR2(100),
  SQL_FLD_3  VARCHAR2(100),
  SQL_FLD_4  VARCHAR2(100),
  SQL_FLD_5  VARCHAR2(100),
  OP_PU_ID          VARCHAR2(32),
  OP_SUB_PU_ID          VARCHAR2(32),
  OP_AREA_ID          VARCHAR2(32),
  OP_SUB_AREA_ID          VARCHAR2(32),
  OP_FCTY_CLASS_2_ID          VARCHAR2(32),
  OP_FCTY_CLASS_1_ID          VARCHAR2(32),
  OP_WELL_HOOKUP_ID       VARCHAR2(32),
  CP_PU_ID          VARCHAR2(32),
  CP_SUB_PU_ID          VARCHAR2(32),
  CP_AREA_ID          VARCHAR2(32),
  CP_SUB_AREA_ID          VARCHAR2(32),
  CP_OPERATOR_ROUTE_ID          VARCHAR2(32),
  CP_COL_POINT_ID          VARCHAR2(32),
  GEO_AREA_ID          VARCHAR2(32),
  GEO_SUB_AREA_ID     VARCHAR2(32),
  GEO_FIELD_ID        VARCHAR2(32),
  GEO_SUB_FIELD_ID    VARCHAR2(32),
  GROUP_REF_ID_1    VARCHAR2(32),
  GROUP_REF_ID_2    VARCHAR2(32),
  GROUP_REF_ID_3    VARCHAR2(32),
  GROUP_REF_ID_4    VARCHAR2(32),
  GROUP_REF_ID_5    VARCHAR2(32),
  GROUP_REF_ID_6    VARCHAR2(32),
  GROUP_REF_ID_7    VARCHAR2(32),
  GROUP_REF_ID_8    VARCHAR2(32),
  GROUP_REF_ID_9    VARCHAR2(32),
  GROUP_REF_ID_10    VARCHAR2(32),
  RECORD_STATUS VARCHAR2(1),
  CREATED_BY     VARCHAR2(30),
  CREATED_DATE   DATE,
  LAST_UPDATED_BY VARCHAR2(30),
  LAST_UPDATED_DATE DATE,
  REV_NO    NUMBER,
  REV_TEXT   VARCHAR2(240)
)
 INITRANS 1
 MAXTRANS 255
 PCTUSED 40
 PCTFREE 40
 STORAGE
 (
   PCTINCREASE 20
   INITIAL 1048576
   NEXT 1048576
   MAXEXTENTS 240
 )
 TABLESPACE &&ts_data
 NOCACHE
/


ALTER TABLE CT_DQ_RULE_RESULTS ADD (
  CONSTRAINT CT_DQ_RULE_RESULTS_PK
 PRIMARY KEY
 (RESULT_ID)
    USING INDEX 
    TABLESPACE &&ts_index
    PCTFREE    10
    INITRANS   2
    MAXTRANS   255
    STORAGE    (
                INITIAL          1M
                NEXT             1M
                MINEXTENTS       1
                MAXEXTENTS       UNLIMITED
                PCTINCREASE      0
               ));

CREATE INDEX CT_DQ_RULE_RESULT_1 ON CT_DQ_RULE_RESULTS
(RULE_ID, LAST_RUN_ID)
LOGGING
TABLESPACE &&ts_index
PCTFREE    10
INITRANS   2
MAXTRANS   255
STORAGE    (
            INITIAL          1M
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
NOPARALLEL;


CREATE UNIQUE INDEX CT_DQ_RULE_RESULT_2 ON CT_DQ_RULE_RESULTS
(RULE_ID, OBJECT_ID, DAYTIME, ALT_UNIQUE_KEY)
LOGGING
TABLESPACE &&ts_index
PCTFREE    10
INITRANS   2
MAXTRANS   255
STORAGE    (
            INITIAL          1M
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
NOPARALLEL;