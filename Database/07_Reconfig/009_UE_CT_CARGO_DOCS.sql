...
** 21-Sep-2016   cvmk             111091 - Modified cargo_rct_func(), cargo_bol_func()
*****************************************************************/
FUNCTION cargo_statement_cooling_func (n_parcel_no NUMBER)

      RETURN ue_ct_stmnt_cool
      PIPELINED;
...
FUNCTION cargo_parcels_func (n_parcel_no NUMBER, v_lifting_account_id VARCHAR2)
      ...
      RETURN ue_ct_cargo_vals
      PIPELINED;

FUNCTION cargo_all_parcels_func (n_parcel_no NUMBER, v_lifting_account_id VARCHAR2)
...
-- Begin
/*
FUNCTION cargo_bol_func (v_original VARCHAR2,n_parcel_no NUMBER)
*/
FUNCTION cargo_bol_func (v_original VARCHAR2,n_parcel_no NUMBER, p_document_code VARCHAR2 DEFAULT NULL)
-- End
      RETURN ue_ct_cargo_bol
      PIPELINED;

FUNCTION cargo_rct_docs_func (n_parcel_no NUMBER)

      RETURN ue_ct_cargo_rct_docs
      PIPELINED;

FUNCTION cargo_rct_smpl_func (n_parcel_no NUMBER)

      RETURN ue_ct_cargo_rct_smpl
      PIPELINED;

FUNCTION cargo_auth_neg (n_parcel_no NUMBER)

      RETURN ue_ct_cargo_auth_neg
      PIPELINED;
...
EXCEPTION
   WHEN OTHERS   THEN
      ...
      OPEN myCursor FOR
         SELECT NULL,NULL,'Internal EC System Error "'||v_errm||'"',NULL,NULL,NULL,NULL, NULL FROM dual;
